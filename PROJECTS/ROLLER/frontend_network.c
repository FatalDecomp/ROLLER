...
void NetworkWait() {
  ...
  iSavedScrSize = scr_size; // Initialize network wait screen - save screen size
                            // and setup display
  front_fade = 0;
  tick_on = -1;
  frontend_on = -1;
  ...
  front_vga[0] = (tBlockHeader *)load_picture(
      "result.bm"); // Load UI graphics: result screen, fonts, car sprites, and
                    // text tables
  ...
  if (network_on) {
    while (1) {
      ...
      if (!iContinueLoop)
        goto LABEL_83;
      if (switch_types) // Handle game type switching (championship/single
                        // race/team game)
      {
      ...
      for (iPlayerLoop2 = 0; iPlayerLoop2 < players; iPlayerLoop2++) {
        Players_Cars[iPlayerLoop2] =
            switch_same - 666; // Convert switch value to car type
      }
      // iPlayerLoop2 = 0;
      // if (players > 0) {
      //   iArrayOffset2 = 0;                      // Set same-car mode for all
      //   players (switch_same - 666 magic number) do {
      //     iArrayOffset2 += 4;
      //     ++iPlayerLoop2;
      //     *(int *)((char *)&infinite_laps + iArrayOffset2) = switch_same -
      //     666;
      ...
      if (Players_Cars[player1_car] < 0) // Check if player1 car is invalid
                                         // (negative) - disconnect from network
      {
        StartPressed = 0;
        time_to_start = 0;
        broadcast_mode = -670;
    ...
    LABEL_26:
      ...
      { // RENDER FRAME (GPU)
        ...
        menu_render_background(
            mr, 0); // Main network lobby display loop - show waiting screen
        sprintf(buffer, "%s: %i", &language_buffer[64], players);
        menu_render_text(mr, 1, buffer, font2_ascii, font2_offsets, 16, 4,
                         0x8Fu, 0, pal_addr);
        sprintf(buffer, "%s: %i", &language_buffer[256], TrackLoad);
        menu_render_text(mr, 1, buffer, font2_ascii, font2_offsets, 16, 24,
                         0x8Fu, 0, pal_addr);
        ...
        menu_render_text(mr, 1, buffer, font2_ascii, font2_offsets, 200, 4,
                         0x8Fu, 1u, pal_addr);
        if (players_waiting == network_on) // Show flashing "Ready to start"
                                           // message when all players connected
        {
          if ((frames & 0xFu) < 8)
            menu_render_text(mr, 1, &language_buffer[4800], font2_ascii,
                             font2_offsets, 200, 22, 0x8Fu, 1u, pal_addr);
          if (time_to_start)
            iContinueLoop = 0;
        }
        iPlayerDisplayLoop = 0;
        if (players > 0) {
          iPlayerIndex = 0; // Display player list with names, car selections,
          ...
          do { // Show flashing indicator for player1, solid for others
            if (player_started[iPlayerIndex] &&
                (!iPlayerDisplayLoop && (frames & 0xFu) < 8 ||
                 iPlayerDisplayLoop > 0))
              menu_render_sprite(mr, 2, 0, 13, iY, 0, pal_addr);
            sprintf(buffer, "%i", iPlayerDisplayLoop + 1);
            iCarSpriteYOffset = 22 * iPlayerDisplayLoop;
            menu_render_text(mr, 1, buffer, font2_ascii, font2_offsets, 33,
                             iTextYPos, 0x8Fu, 0, pal_addr);
            sprintf(buffer, "%s", szCurrentPlayerName);
            menu_render_text(mr, 1, buffer, font2_ascii, font2_offsets, 85,
                             iTextYPos, 0x8Fu, 0, pal_addr);
            iCarType = Players_Cars[iPlayerIndex];
            if (iCarType >=
                0) // Display car company name and sprite if valid car selected
            {
              sprintf(buffer, "%s", CompanyNames[iCarType]);
              menu_render_text(mr, 1, buffer, font2_ascii, font2_offsets, 218,
                               iTextYPos, 0x8Fu, 0, pal_addr);
              iCarTypeForSprite = Players_Cars[iPlayerIndex];
              if (iCarTypeForSprite < 8) {
                if ((textures_off & TEX_OFF_ADVANCED_CARS) != 0)
                  menu_render_sprite(mr, 2, smallcars[1][iCarTypeForSprite],
                                     165, iCarSpriteYOffset + 46, 0, pal_addr);
                else
                  menu_render_sprite(mr, 2, smallcars[0][iCarTypeForSprite],
                                     165, iCarSpriteYOffset + 46, 0, pal_addr);
              } else {
                menu_render_text(mr, 1, "CHEAT", font2_ascii, font2_offsets,
                                 165, iTextYPos, 0x8Fu, 0, pal_addr);
              }
            } else {
              menu_render_text(mr, 1, &language_buffer[4160], font2_ascii,
                               font2_offsets, 218, iTextYPos, 0x8Fu, 0,
                               pal_addr);
            }
            ++iPlayerIndex;
            iTextYPos += 22;
        ...
        if (iContinueLoop) {
          ...
          if (!front_fade) {
            front_fade =
                -1; // Initialize screen fade and network synchronization
      ...
      while (fatkbhit()) // Handle keyboard input for network lobby
      {
        ...
        if (uiKeyPressed < 0xD) {
          if (!uiKeyPressed)
            fatgetch();
        } else if (uiKeyPressed <=
                   0xD) { // Enter key pressed - start race if all players ready
        ...
        } else if (uiKeyPressed == 27 && !time_to_start &&
                   !restart_net) // Escape key pressed - leave network game
        {
          StartPressed = 0;
          time_to_start = 0;
          broadcast_mode = -670;
    ...
    // iPlayerLoop1 = 0;                           // Reset same-car mode -
    // clear player settings if (players > 0) {
    //   iArrayOffset1 = 0;
    //   do {
    //     iArrayOffset1 += 4;
    //     ++iPlayerLoop1;
...
LABEL_83:
  ...
  if (time_to_start) // Final network synchronization before race starts
  {
    ...
    if (wConsoleNode == master) // Master node waits for all client records,
                                // then broadcasts seed
    {
      ...
      while (received_records < network_on) {
        if (frames - iLastRecordWaitLog >= 36) {
          ...
          SDL_Log(
              "[NET-START] master waiting for records received=%d expected=%d",
      ...
      while (!received_seed) {
        if (frames - iLastRecordResend >= 36) {
          ...
          SDL_Log("[NET-START] slave waiting for seed; resending record to "
                  "master=%d",
                  master);
  ...
  check_cars(); // Cleanup: fade screen, free graphics memory, restore screen
                // size
  // GPU fade-out
  {
    MenuRenderer *mr = GetMenuRenderer();
...
void restart_net_game() {
  ...
  if (game_type == 1 && !Race) {
    memset(championship_points, 0,
           sizeof(championship_points)); // Reset championship statistics for
                                         // new championship
  ...
  if (competitors ==
      2) // Determine actual number of competitors based on game type
  {
  ...
  if (competitors == 1)
    iActualCompetitors = players;
  iGridIndex = 0;
  if (iActualCompetitors >
      0) { // Fill grid with competing cars (skip non-competitors)
    for (iCompetitorLoop = 0; iCompetitorLoop < iActualCompetitors;
         ++iCompetitorLoop) {
  ...
  if (iActualCompetitors < numcars) {
    iGridOffset =
        4 * iActualCompetitors; // Fill remaining grid positions with AI cars
    iMaxCarOffset = 4 * numcars;
    do {
      for (iNonCompSearch = iCarIndex; !non_competitors[iNonCompSearch];
           ++iNonCompSearch)
        ++iCarIndex;
      ++iTotalCars;
  ...
  if (game_type == 1 && Race > 0) {
    if (iActualCompetitors > 0) {
      ...
      // for (j = 0; j < iActualCompetitors; finished_car[j + 15] = teamorder[j
      // + 7])
      //   ++j;
    }
  } else {
    ...
    for (iAISearch = 0; iAISearch < 6 * iActualCompetitors;
         grid[iSecondSwapPos] = iTempCarId) // Shuffle grid positions randomly
                                            // for non-championship races
    {
      iRandRange = network_on ? 0 : ROLLERrandRaw();
      // iFirstSwapPos = iRandRange % iActualCompetitors;  // Get random
      // position within grid bounds
      iFirstSwapPos =
          network_on
              ? NetworkGridRandRange(iActualCompetitors, &iNetworkGridSeed)
              : GetHighOrderRand(iActualCompetitors, iRandRange);
      // iFirstSwapPos = (iActualCompetitors * iRandRange -
      // (__CFSHL__((iActualCompetitors * iRandRange) >> 31, 15) +
      // ((iActualCompetitors * iRandRange) >> 31 << 15))) >> 15;
      iSecondRand = network_on ? 0 : ROLLERrandRaw();
      // iSecondSwapPos = iSecondRand % iActualCompetitors;  // Get second
      // random position within grid bounds
      iSecondSwapPos =
          network_on
              ? NetworkGridRandRange(iActualCompetitors, &iNetworkGridSeed)
              : GetHighOrderRand(iActualCompetitors, iSecondRand);
      // iSecondSwapPos = (iActualCompetitors * iSecondRand -
      // (__CFSHL__((iActualCompetitors * iSecondRand) >> 31, 15) +
      // ((iActualCompetitors * iSecondRand) >> 31 << 15))) >> 15;
    ...
    for (k = 0; k < players;
         ++k) // Position human players based on difficulty level
    {         // Calculate starting position handicap based on level
      ...
      if (iHandicapPos < 0)
        iHandicapPos = 0;
      iHumanPos = 0;
      for (iOrderLoop = 0; !human_control[grid[iOrderLoop]];
           ++iOrderLoop) // Find human player in grid and ensure proper
                         // positioning
        ++iHumanPos;
