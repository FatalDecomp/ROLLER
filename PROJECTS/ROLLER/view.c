...
void calculateview(int iViewMode, int iCarIdx, int iChaseCamIdx) {
  ...
  if (iViewMode < -1) // Handle different view modes - set camera offset values
  {
    if (iViewMode != -2)
      goto LABEL_8;
  LABEL_6:
    ext_x = 11.0; // Rear external view (mode -2) or cockpit view (mode 1) -
                  // behind car
    ext_y = 0.0;
    ext_z = 9.0;
    goto LABEL_9;
...
LABEL_9:
  ...
  {                  // Standard car camera views (driver, cockpit, chase)
    if (iViewMode >= 0) {
      ...
      { // Calculate pitch for cockpit view - check if car is stunned
        if (Car[iCarIdx].iStunned)
          iPitchCalculated =
              Car[iCarIdx].iPitchCameraOffset + Car[iCarIdx].nPitch;
        else
          iPitchCalculated = Car[iCarIdx].iPitchDynamicOffset +
                             Car[iCarIdx].iPitchCameraOffset +
                             Car[iCarIdx].nPitch;
        iElevation2 = ((int16)iPitchCalculated + 0x2000) & 0x3FFF;
        iRollCalculated = Car[iCarIdx].iRollDynamicOffset +
                          Car[iCarIdx].iRollCameraOffset + Car[iCarIdx].nRoll +
                          0x2000;
        ...
        if (Car[iCarIdx].iStunned)
          ...
          iPitchCalculated2 = Car[iCarIdx].iPitchDynamicOffset +
                              Car[iCarIdx].iPitchCameraOffset +
                              Car[iCarIdx].nPitch;
      ...
      iElevation2 = ((uint16)(Car[iCarIdx].iPitchCameraOffset) +
                     Car[iCarIdx].nPitch + 0x2000) &
                    0x3FFF;
      iRollCalculated =
          Car[iCarIdx].iRollCameraOffset + Car[iCarIdx].nRoll + 0x2000;
    }
    ...
    if (iViewMode == 2) // Check if chase view mode
    {
      newchaseview(
          iCarIdx,
          iChaseCamIdx); // Handle chase view - call specialized function
      return;
    }
    fCameraOffsetY =
        ext_y * 64.0f; // Scale camera offsets and apply to display position
    ...
    {
      fZ = Car[iCarIdx]
               .pos.fZ; // Cockpit view - get car position and orientation
      fY = Car[iCarIdx].pos.fY;
      fX = Car[iCarIdx].pos.fX;
      iYaw = Car[iCarIdx].nYaw;
      VIEWDIST = 120; // Set closer view distance for cockpit
      calculatetransform(
          Car[iCarIndex2].nCurrChunk, iYaw, iElevation2, iTilt, fX, fY, fZ,
          fPosX, fPosY,
          fPosZ); // Calculate transformation matrix for cockpit view
      iCurrChunk = Car[iCarIndex2].nCurrChunk;
      if (iCurrChunk == -1) // Check if car is off-track (chunk = -1)
      {
        worlddirn =
            Car[iCarIndex2].nYaw; // Car off-track - use raw orientation values
      ...
      calculatetransform(
          Car[iCarIdx].nCurrChunk, Car[iCarIdx].nYaw, iElevation2, iTilt,
          Car[iCarIdx].pos.fX, Car[iCarIdx].pos.fY, Car[iCarIdx].pos.fZ, fPosX,
          fPosY, fPosZ); // Driver view - calculate transformation matrix
    ...
    getworldangles(Car[iCarIndex2].nYaw, iElevation2, iTilt, iCurrChunk,
                   &worlddirn, &worldelev,
                   &worldtilt); // Convert car angles to world coordinates
  } else if (iViewMode == 3)    // Tower/spectator view mode (mode 3)
  {
    ...
    if (iChunkIndex == -1) // Check if car is off-track
    {
      ...
      pTransformMatrix = &localdata[iChunkIndex]; // Transform car position to
                                                  // world coordinates
    ...
    {
      dWorldZOffset =
          fWorldPosZ + 16384.0; // No towers - use default elevated view
      worldx = fWorldPosX2;
      worldy = fWorldPosX;
    LABEL_39:
      ...
      if (NumTowers > 0) {
        ...
        do {
          fChunkIdx =
              (float)TowerBase[iTowerArrayIdx]
                  .iChunkIdx; // Calculate distance considering track wraparound
          ...
          if (dMinDistance <
              fMinTowerDistance) // Update closest tower if this one is nearer
          {
      ...
      iTowerType = TowerBase[iClosestTowerIdx].iTowerType -
                   1; // Set view distance based on tower type
      ...
      case 0xFFFFFFFB:
        dWorldZOffset =
            (double)(TowerBase[iClosestTowerIdx].iVOffset << 7) +
            fWorldPosZ; // Mode -5: Elevated view with vertical offset
        worldx = fWorldPosX2;
        worldy = fWorldPosX;
        goto LABEL_39;
      case 0xFFFFFFFC:
        iChunkIdxMinus2 = TowerBase[iClosestTowerIdx].iChunkIdx -
                          2; // Mode -4: Look back 2 chunks
      ...
      case 2:
        dDeltaX =
            worldx - fWorldPosX2; // Mode -3/2: Directional offset from tower
        dDeltaY = worldy - fWorldPosX;
        dDeltaZ = worldz - fWorldPosZ;
        dInvMagnitude = 1.0 / sqrt(dDeltaX * dDeltaX + dDeltaY * dDeltaY +
                                   dDeltaZ * dDeltaZ);
      ...
      case 0xFFFFFFFE:
      case 1:
        worldx = (worldx - fWorldPosX2) * 0.25f +
                 fWorldPosX2; // Mode -2/1: Interpolated position
    ...
    TowerGy = worldy;
    TowerGz = worldz;
    fTowerDeltaX = fWorldPosX2 -
                   worldx; // Calculate direction and elevation to look at car
    fTowerDeltaY = fWorldPosX - worldy;
    NearTow = iClosestTowerIdx;
    fTowerVerticalDistance = fWorldPosZ - worldz;
    // if ((LODWORD(fTowerDeltaX) & 0x7FFFFFFF) != 0 || fabs(fWorldPosX -
    // worldy))
    if (fabs(fTowerDeltaX) > FLT_EPSILON || fabs(fWorldPosX - worldy))
      vdirection = getangle(
          fTowerDeltaX, fTowerDeltaY); // Calculate horizontal direction angle
    else
      vdirection = 0;
    fTowerHorizontalDistance =
        (float)sqrt(fTowerDeltaX * fTowerDeltaX + fTowerDeltaY * fTowerDeltaY);
    // if ((LODWORD(fTowerHorizontalDistance) & 0x7FFFFFFF) != 0 ||
    // (LODWORD(fTowerVerticalDistance) & 0x7FFFFFFF) != 0)
    if (fabs(fTowerHorizontalDistance) > FLT_EPSILON ||
        fabs(fTowerVerticalDistance) > FLT_EPSILON)
      iCalculatedElevation =
          getangle(fTowerHorizontalDistance,
                   fTowerVerticalDistance); // Calculate elevation angle
    else
      iCalculatedElevation = 0;
    velevation = iCalculatedElevation;
    vtilt = 0;
    calculatetransform(-1, vdirection, iCalculatedElevation, 0, worldx, worldy,
                       worldz, DDX, DDY,
                       DDZ); // Set up tower view transformation
    ...
    if (iViewMode == 6) {
      worldx = -localdata[2]
                    .pointAy[3]
                    .fX; // Set map view position from chunk 2 data
    ...
    fDeltaX =
        fTransformedPosX - worldx; // Calculate viewing angles for top/map view
    fDeltaY = fTransformedPosY - worldy;
    NearTow = -1;
    fVerticalDistance = fTransformedPosZ - worldz;
    // if ((LODWORD(fDeltaX) & 0x7FFFFFFF) != 0 || fabs(fTransformedPosY -
    // worldy))
    ...
    // if ((LODWORD(fHorizontalDistance) & 0x7FFFFFFF) != 0 ||
    // (LODWORD(fVerticalDistance) & 0x7FFFFFFF) != 0)
    if (fabs(fHorizontalDistance) > FLT_EPSILON ||
        fabs(fVerticalDistance) > FLT_EPSILON)
      velevation = getangle(fHorizontalDistance, fVerticalDistance);
    else
      velevation = 0;
    vtilt = 0;
    calculatetransform(-1, vdirection, velevation, 0, worldx, worldy, worldz,
                       DDX, DDY,
                       DDZ); // Set up final transformation and world angles
    worlddirn = vdirection;
    worldelev = velevation;
    worldtilt = vtilt;
  }
...
void initcarview(int iCarIdx, int iPlayer) {
  ...
  iSelectedView = SelectedView[iPlayer]; // Get the selected view type for the
                                         // current player
  if (iSelectedView != 1 && iSelectedView != 3 && iCarIdx >= 0)
    return; // Only process if view is cockpit (1) or chase (3) and car index is
            // valid
  if (iCarIdx >= 0) {
    iChaseViewIdx = chaseview[iPlayer]; // Initialize chase camera parameters
                                        // from viewdata for valid car
    ...
    CHASE_DIST[0] =
        viewdata[1].fChaseDistance; // Special case: use default chase view for
                                    // invalid car index
  ...
  iControlType =
      Car[iCarIdx].iControlType; // Get car control type (human player, AI, etc)
  if (!iControlType) {
    fChasePullDefault =
        viewdata[chaseview[iPlayer]]
            .fChasePullDefault; // Human player: use default chase pull factor
  SET_PULL_FACTOR:
    PULLZ[iPlayer] = fChasePullDefault;
    goto CALCULATE_CAMERA_POSITION;
  }
  if (iControlType >= 2 &&
      iControlType <= 3) { // AI cars: choose pull factor based on crash state
    if (Car[iCarIdx].iStunned)
...
CALCULATE_CAMERA_POSITION:
  if (Car[iCarIdx].nCurrChunk ==
      -1) // Get car orientation: use actual angles if not in a chunk
  {
    iYaw = Car[iCarIdx].nYaw;
  ...
  if (replaytype == 2 &&
      replaydirection ==
          -1) // Reverse chase distance if in replay mode with reverse direction
    dChaseDist = -CHASE_DIST[iPlayer];
  else
    dChaseDist = CHASE_DIST[iPlayer];
  dChaseDistMul = -(dChaseDist * 2.0);
  fDirX = -tcos[iYaw] * fSinPitch * tcos[iRoll] -
          tsin[iYaw] * tsin[iRoll]; // Calculate camera direction vector X
                                    // component using rotation matrix
  fDirY = tcos[iYaw] *
          tcos[iPitch]; // Calculate camera direction vector Y component
  fCameraX = (float)(dChaseDistMul * fDirY + PULLZ[iPlayer] * fDirX +
                     Car[iCarIdx].pos.fX); // Calculate final camera X position
                                           // with distance and pull factors
  fDirZ = tsin[iYaw] * tcos[iPitch];
  fRightX = -tsin[iYaw] * fSinPitch * tcos[iRoll] + tcos[iYaw] * tsin[iRoll];
  fCameraY = (float)(dChaseDistMul * fDirZ + PULLZ[iPlayer] * fRightX +
                     Car[iCarIdx].pos.fY); // Calculate final camera Y position
  fUpZ = tcos[iPitch] * tcos[iRoll];
  fCameraZ = (float)(dChaseDistMul * fSinPitch + PULLZ[iPlayer] * fUpZ +
                     Car[iCarIdx].pos.fZ); // Calculate final camera Z position
  iCurrChunk = Car[iCarIdx].nCurrChunk;
  if (iCurrChunk == -1) // Branch: handle camera position based on chunk state
  {
    chase_x = fCameraX; // No chunk: directly set global chase position and
                        // lastpos array
    chase_y = fCameraY;
    ...
    pData = &localdata[iCurrChunk]; // In chunk: transform camera position using
                                    // chunk transformation matrix
    chase_x = pData->pointAy[0].fY * fCameraY +
              pData->pointAy[0].fX * fCameraX +
              pData->pointAy[0].fZ * fCameraZ - pData->pointAy[3].fX;
    chase_y = pData->pointAy[1].fX * fCameraX +
              pData->pointAy[1].fY * fCameraY +
              pData->pointAy[1].fZ * fCameraZ - pData->pointAy[3].fY;
    chase_z = pData->pointAy[2].fX * fCameraX +
              pData->pointAy[2].fY * fCameraY +
              pData->pointAy[2].fZ * fCameraZ - pData->pointAy[3].fZ;
    lastpos[iPlayer][0].pos.fX = chase_x;
    lastpos[iPlayer][0].pos.fY =
        pData->pointAy[1].fX * fCameraX + pData->pointAy[1].fY * fCameraY +
        pData->pointAy[1].fZ * fCameraZ - pData->pointAy[3].fY;
    dTransformZ = fCameraY * pData->pointAy[2].fY +
                  fCameraX * pData->pointAy[2].fX +
                  fCameraZ * pData->pointAy[2].fZ - pData->pointAy[3].fZ;
    lastpos[iPlayer][0].fDistance = 0.0;
  ...
  // iPlayerShift = iPlayer << 10;                 // Copy current position to
  // backup slots for interpolation
  //*(float *)((char *)&lastpos[0][1].pos.fX + iPlayerShift) =
  // lastpos[iPlayer][0].pos.fX;
  //*(float *)((char *)&lastpos[0][1].pos.fY + iPlayerShift) =
  // lastpos[iPlayer][0].pos.fY;
  //*(float *)((char *)&lastpos[0][1].pos.fZ + iPlayerShift) =
  // lastpos[iPlayer][0].pos.fZ;

  dLastPosW = lastpos[iPlayer][0].fDistance;
  nextpoint[iPlayer] =
      1; // Set nextpoint flag to indicate new camera position is ready
...
void newchaseview(int iCarIdx, int iChaseCamIdx) {
  ...
  dBackwardX =
      -tcos[nYaw] * tsin[nPitch] * tcos[nRoll] -
      tsin[nYaw] *
          tsin[nRoll]; // Calculate backward direction vector from car
                       // orientation Uses trigonometric transformation to get
                       // direction opposite to car's forward vector
  dBackwardY =
      -tsin[nYaw] * tsin[nPitch] * tcos[nRoll] + tcos[nYaw] * tsin[nRoll];
  dBackwardZ = tcos[nPitch] * tcos[nRoll];
  fCameraPosX =
      PULLZ[iChaseCamIdx] * (float)dBackwardX +
      fX; // Calculate camera position (behind car) using PULLZ distance
  fCameraPosY = PULLZ[iChaseCamIdx] * (float)dBackwardY + fY;
  fCameraPosZ = PULLZ[iChaseCamIdx] * (float)dBackwardZ + fZ;
  fLookAtX =
      (float)dBackwardX * LOOKZ[iChaseCamIdx] +
      fX; // Calculate look-at position (in front of car) using LOOKZ distance
  ...
  if (iCurrChunk !=
      -1) // Transform positions to world coordinates if car is on track
  {
    pTransformMatrix =
        (float
             *)&localdata[iCurrChunk]; // Apply transformation matrix to convert
                                       // from car-local to world coordinates
    fLookTempX = (float)dBackwardX * LOOKZ[iChaseCamIdx] + fX;
    fLookAtX = pTransformMatrix[1] * fLookAtY + *pTransformMatrix * fLookAtX +
               pTransformMatrix[2] * fLookAtZ - pTransformMatrix[9];
    fLookTempX2 = fLookTempX;
    dLookTransformedY = pTransformMatrix[3] * fLookTempX +
                        pTransformMatrix[4] * fLookAtY +
                        pTransformMatrix[5] * fLookAtZ - pTransformMatrix[10];
    fLookTempY = (float)dBackwardY * LOOKZ[iChaseCamIdx] + fY;
    fLookAtY = (float)dLookTransformedY;
    fLookAtZ = pTransformMatrix[6] * fLookTempX2 +
               pTransformMatrix[7] * fLookTempY +
               pTransformMatrix[8] * fLookAtZ - pTransformMatrix[11];
    fPullTempX = fCameraPosX;
    fCameraPosX = pTransformMatrix[1] * fCameraPosY +
                  *pTransformMatrix * fCameraPosX +
                  pTransformMatrix[2] * fCameraPosZ - pTransformMatrix[9];
    fPullTempX2 = fPullTempX;
    dPullTransformedY =
        pTransformMatrix[3] * fPullTempX + pTransformMatrix[4] * fCameraPosY +
        pTransformMatrix[5] * fCameraPosZ - pTransformMatrix[10];
    fPullTempY = PULLZ[iChaseCamIdx] * (float)dBackwardY + fY;
    fCameraPosY = (float)dPullTransformedY;
    fCameraPosZ = pTransformMatrix[6] * fPullTempX2 +
                  pTransformMatrix[7] * fPullTempY +
                  pTransformMatrix[8] * fCameraPosZ - pTransformMatrix[11];
  }
  iPrevIndex = nextpoint[iChaseCamIdx] -
               1; // Get previous position index in circular buffer
  if (iPrevIndex < 0)
    iPrevIndex = 63;
  ...
  // iCurrArrayOffset = 16 * nextpoint[iChaseCamIdx] + (iChaseCamIdx << 10);//
  // Get current position data from position history buffer fPrevPosX = *(float
  // *)((char *)&lastpos[0][0].pos.fX + iCurrArrayOffset); fPrevPosY = *(float
  // *)((char *)&lastpos[0][0].pos.fY + iCurrArrayOffset); fPrevPosZ = *(float
  // *)((char *)&lastpos[0][0].pos.fZ + iCurrArrayOffset);

  dDistanceFromPrev = sqrt(
      (fCameraPosX - lastpos[iChaseCamIdx][iPrevArrayIndex].pos.fX) *
          (fCameraPosX - lastpos[iChaseCamIdx][iPrevArrayIndex].pos.fX) +
      (fCameraPosY - lastpos[iChaseCamIdx][iPrevArrayIndex].pos.fY) *
          (fCameraPosY - lastpos[iChaseCamIdx][iPrevArrayIndex].pos.fY) +
      (fCameraPosZ - lastpos[iChaseCamIdx][iPrevArrayIndex].pos.fZ) *
          (fCameraPosZ -
           lastpos[iChaseCamIdx][iPrevArrayIndex]
               .pos.fZ)); // Calculate distance from previous recorded position
  if (dDistanceFromPrev >= 3200.0) // Check if car has moved far enough to
                                   // record new position (3200 units)
  {
    iNextIndex =
        nextpoint[iChaseCamIdx] +
        1; // Car moved enough - advance to next position in circular buffer
    ...
    lastpos[iChaseCamIdx][iNewPositionIndex].fDistance =
        (float)sqrt((fPrevPosX - fCameraPosX) * (fPrevPosX - fCameraPosX) +
                    (fPrevPosY - fCameraPosY) * (fPrevPosY - fCameraPosY) +
                    (fPrevPosZ - fCameraPosZ) * (fPrevPosZ - fCameraPosZ));
    // iNewArrayOffset = 16 * nextpoint[iChaseCamIdx] + (iChaseCamIdx << 10);//
    // Store new position and distance in history buffer
    ...
    //                                                                    -
    //                                                                    fCameraPosX)
    //                                                                    * (fPrevPosX - fCameraPosX)
    //                                                                  +
    //                                                                  (fPrevPosY
    //                                                                  -
    //                                                                  fCameraPosY)
    //                                                                  * (fPrevPosY - fCameraPosY)
    //                                                                  +
    //                                                                  (fPrevPosZ
    //                                                                  -
    //                                                                  fCameraPosZ)
    //                                                                  * (fPrevPosZ - fCameraPosZ));
  } else {
    ...
    //*(float *)((char *)&lastpos[0][0].pos.fX + iCurrArrayOffset) =
    // fCameraPosX;// Car hasn't moved enough - update current position in place
    //*(float *)((char *)&lastpos[0][0].pos.fY + iCurrArrayOffset) =
    // fCameraPosY;
    //*(float *)((char *)&lastpos[0][0].pos.fZ + iCurrArrayOffset) =
    // fCameraPosZ; fCurrentDistance = dDistanceFromPrev;
    //*(float *)((char *)&lastpos[0][0].fDistance + iCurrArrayOffset) =
    // fCurrentDistance;
  }
  iLoopCounter =
      62; // Search for chase camera position at specified distance behind car
  ...
  while (1) {
    dAccumulatedDistance =
        fAccumulatedDist +
        lastpos[iChaseCamIdx][iSearchIndex]
            .fDistance; // Accumulate distance from position history
  ...
  if (iLoopCounter <= 0) {
    iNextPosIndex = nextpoint[iChaseCamIdx] +
                    1; // Not enough history - use position ahead of current
    ...
    // fViewX = *(float *)((char *)&lastpos[0][0].pos.fX + iCarArrayOffset2 +
    // iNextArrayOffset); fViewY = *(float *)((char *)&lastpos[0][0].pos.fY +
    // iCarArrayOffset2 + iNextArrayOffset); fViewZ = *(float *)((char
    // *)&lastpos[0][0].pos.fZ + iCarArrayOffset2 + iNextArrayOffset);
  } else {
    iPrevSearchIndex =
        iSearchIndex - 1; // Interpolate camera position between two history
                          // points Calculate exact position at target distance
                          // using linear interpolation
    fInterpolationFactor = (lastpos[iChaseCamIdx][iSearchIndex].fDistance +
                            fAccumulatedDist - CHASE_DIST[iChaseCamIdx]) /
                           lastpos[iChaseCamIdx][iSearchIndex].fDistance;

    // Handle circular buffer wraparound for previous index
    int iPrevIndex =
        (iPrevSearchIndex < 0) ? (iPrevSearchIndex + 64) : iPrevSearchIndex;
    fViewX = (lastpos[iChaseCamIdx][iSearchIndex].pos.fX -
              lastpos[iChaseCamIdx][iPrevIndex].pos.fX) *
                 fInterpolationFactor +
             lastpos[iChaseCamIdx][iPrevIndex].pos.fX;
    fViewY = (lastpos[iChaseCamIdx][iSearchIndex].pos.fY -
              lastpos[iChaseCamIdx][iPrevIndex].pos.fY) *
                 fInterpolationFactor +
             lastpos[iChaseCamIdx][iPrevIndex].pos.fY;
    fViewZ =
        fInterpolationFactor * (lastpos[iChaseCamIdx][iSearchIndex].pos.fZ -
                                lastpos[iChaseCamIdx][iPrevIndex].pos.fZ) +
        lastpos[iChaseCamIdx][iPrevIndex].pos.fZ;
    // iCarArrayOffset = iChaseCamIdx << 10;
    // iSearchArrayIndex = iSearchIndex;
    // iPrevArrayOffset = 16 * (iPrevSearchIndex + (iPrevSearchIndex < 0 ? 64 :
    // 0)); fViewX = (*(float *)((char *)&lastpos[0][iSearchArrayIndex].pos.fX +
    // iCarArrayOffset) - *(float *)((char *)&lastpos[0][0].pos.fX +
    // iCarArrayOffset + iPrevArrayOffset))
    //   * fInterpolationFactor
    //   + *(float *)((char *)&lastpos[0][0].pos.fX + iCarArrayOffset +
    //   iPrevArrayOffset);
    // fViewY = (*(float *)((char *)&lastpos[0][iSearchArrayIndex].pos.fY +
    // iCarArrayOffset) - *(float *)((char *)&lastpos[0][0].pos.fY +
    // iCarArrayOffset + iPrevArrayOffset))
    //   * fInterpolationFactor
    //   + *(float *)((char *)&lastpos[0][0].pos.fY + iCarArrayOffset +
    //   iPrevArrayOffset);
    // fViewZ = fInterpolationFactor
    //   * (*(float *)((char *)&lastpos[0][iSearchArrayIndex].pos.fZ +
    //   iCarArrayOffset) - *(float *)((char *)&lastpos[0][0].pos.fZ +
    //   iCarArrayOffset + iPrevArrayOffset))
    //   + *(float *)((char *)&lastpos[0][0].pos.fZ + iCarArrayOffset +
    //   iPrevArrayOffset);
  }
  fHorizontalDeltaX =
      fLookAtX - fViewX; // Calculate direction from camera to look-at point
  fHorizontalDeltaY = fLookAtY - fViewY;
  // if ((LODWORD(fHorizontalDeltaX) & 0x7FFFFFFF) != 0 || fabs(fLookAtY -
  // fViewY))// Calculate horizontal direction angle (yaw)
  if (fabs(fHorizontalDeltaX) > FLT_EPSILON ||
      fabs(fLookAtY - fViewY)) // Calculate horizontal direction angle (yaw)
    iCalculatedDirection = getangle(fHorizontalDeltaX, fHorizontalDeltaY);
  else
    iCalculatedDirection =
        lastcamdirection; // Use last camera direction if no movement detected
  vdirection = iCalculatedDirection;
  fHorizontalDistance = (float)sqrt(
      fHorizontalDeltaX * fHorizontalDeltaX +
      fHorizontalDeltaY *
          fHorizontalDeltaY); // Calculate elevation angle (pitch) from
                              // horizontal distance and height difference
  // if ((LODWORD(fHorizontalDistance) & 0x7FFFFFFF) != 0 || fabs(fLookAtZ -
  // fViewZ)) {
  if (fabs(fHorizontalDistance) > FLT_EPSILON || fabs(fLookAtZ - fViewZ)) {
    ...
    iCalculatedElevation =
        lastcamelevation; // Use last camera elevation if no height difference
  }
  velevation = iCalculatedElevation;
  vtilt = 0;
  calculatetransform(-1, vdirection, iCalculatedElevation, 0, fViewX, fViewY,
                     fViewZ, DDX, DDY,
                     DDZ); // Set up camera transformation matrix with
                           // calculated position and orientation
  lastcamdirection =
      vdirection; // Save camera angles for next frame to avoid sudden changes
