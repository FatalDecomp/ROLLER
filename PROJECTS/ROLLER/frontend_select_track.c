...
void select_track() {
  ...
  iAnimationTimer = 36;
  fade_palette(
      0); // Initialize track selection screen - fade out and reset state
  iSoundFlag = 0;
  front_fade = 0;
  iExitFlag = 0;
  iFrontendSpeechPending = 0;
  if (TrackLoad >
      0) // Set initial track selection based on current TrackLoad and game type
  {
  ...
  fre((void **)&front_vga[3]); // Load track selection UI graphics and 3D track
                               // preview
  if (TrackLoad >= 0)
  ...
  do { // Main track selection display loop
    ...
    { // RENDER FRAME (GPU)
      ...
      menu_render_sprite(
          mr, 5, player_type, -4, 247, 0,
          pal_addr); // Display player type and game type indicators
      menu_render_sprite(mr, 5, game_type + 5, 135, 247, 0, pal_addr);
      menu_render_sprite(mr, 4, 1, 76, 257, -1, pal_addr);
      if (iSelectedTrack >= 8) // Draw track selector: championship view or
                               // individual track selector
      {
        ...
        menu_render_text(mr, 2, "~", font2_ascii, font2_offsets,
                         sel_posns[iSelectedTrack].x,
                         sel_posns[iSelectedTrack].y, 0x8Fu, 0, pal_addr);
      }
      menu_render_text(mr, 2, "AUTO ARIEL", font2_ascii, font2_offsets,
                       sel_posns[0].x + 132, sel_posns[0].y + 7, 0x8Fu, 2u,
                       pal_addr); // Display hardcoded track names (should use
                                  // track name data)
      ...
      menu_render_text(mr, 2, "REISE WAGON", font2_ascii, font2_offsets,
                       ...
                       pal_addr);
      iBlockIdx =
          (TrackLoad - 1) / 8; // Calculate cup icon index (8 tracks per cup)
      // iBlockIdx = (TrackLoad - 1 - (__CFSHL__((TrackLoad - 1) >> 31, 3) + 8 *
      // ((TrackLoad - 1) >> 31))) >> 3;// Calculate cup icon index for display
      if (TrackLoad >= 0) { // Render 3D track preview
        menu_render_load_track_mesh(mr, palette);
        if (iAnimationTimer)
          menu_render_draw_track_preview(mr, fZ, 1280, iYaw, PREVIEW_X,
                                         TRACK_PREVIEW_Y, PREVIEW_W, PREVIEW_H);
        else
          menu_render_draw_track_preview(mr, fZ_1, 1280, iYaw, PREVIEW_X,
                                         TRACK_PREVIEW_Y, PREVIEW_W, PREVIEW_H);
        menu_render_set_layer(mr, MENU_LAYER_FOREGROUND);
        if (game_type >=
            2) // Set number of laps based on game type and difficulty level
        {
          NoOfLaps = 5;
        } else {
        ...
        if (RecordCars[TrackLoad] <
            0) // Display track record holder and lap time
        {
          ...
          sprintf(buffer, "%s - %s - %02i:%02i:%02i", RecordNames[TrackLoad],
                  CompanyNames[RecordCars[TrackLoad] & 0xF], iRecordMinutes,
                  iRecordSeconds, iRecordCentiseconds);
          // dRecordTimeFloat = RecordLaps[TrackLoad] * 100.0;// Format lap time
          // as MM:SS:CC (minutes:seconds:centiseconds) _CHP();
      ...
      menu_render_sprite(mr, 14, iBlockIdx, 500, 300, 0,
                         pal_addr); // Display cup icon and track name/type
      if (TrackLoad <= 0) {
        if (TrackLoad)
          ...
          menu_render_text(mr, 2, "TRACK ZERO", font2_ascii, font2_offsets, 190,
                           350, 0x8Fu, 0, pal_addr);
      } else if (TrackLoad >= 17) // Display track name: bonus tracks (17+),
                                  // regular tracks (1-16), or special tracks
      {
    ...
    if (switch_same > 0) // Handle same-car mode activation/deactivation
    {
      ...
      // iPlayerLoop2 = 0;                         // Activate same-car mode:
      // set all players to same car type if (players > 0) {
      //   iArrayOffset2 = 0;
      //   do {
      //     iArrayOffset2 += 4;
      //     ++iPlayerLoop2;
      ...
      // iPlayerLoop1 = 0;                         // Deactivate same-car mode:
      // reset all player car selections switch_same = 0; if (players > 0) {
      //   iArrayOffset1 = 0;
      //   do {
      //     iArrayOffset1 += 4;
      //     ++iPlayerLoop1;
    ...
    if (!iAnimationTimer) // Handle 3D camera zoom animation
    {
      ...
      if (dZoomInterpolation <= 10000000.0) {
        ...
        fZ_1 = 10000000.0; // Camera reached max zoom - load new track and reset
                           // animation
    ...
    if (bStartedFadeIn) // Initialize screen fade and track voice sample on
                        // first display
    {
      loadtracksample(TrackLoad);
      iFrontendSpeechPending = 0;
    ...
    if (TrackLoad != iPrevTrackLoad) // Track changed - play selection sound and
                                     // reset animation
    {
      if (!iSoundFlag) {
    ...
    iCupOffset =
        8 * iBlockIdx; // Calculate track selection values for input handling
    ...
    while (fatkbhit()) // Process keyboard input for track selection
    {
      ...
      if (byKey < 0xDu) {
        ...
        if (iSelectedTrack != 8 && iCurrentTrack != iCalculatedTrack ||
            iSelectedTrack == 8) {
          remove_frontendspeech();
          iFrontendSpeechPending = 0;
          sfxsample(SOUND_SAMPLE_BUTTON, 0x8000);
        }
        // if (iCurrentTrack == iCalculatedTrack && SoundCard &&
        // frontendspeechhandle != -1 && sosDIGISampleDone(*(int *)&DIGIHandle,
        ...
        if (byKey <= 0x1Bu) {
          ...
          sfxsample(SOUND_SAMPLE_BUTTON, 0x8000);
          iExitFlag = -1;
        } else if (byKey == 32 && game_type != 1 &&
                   TrackLoad >
                       0) // Space key pressed - cycle through cup categories
        {
          iCurrentTrack += 8;
          sfxsample(SOUND_SAMPLE_TRACK, 0x8000);
          iFrontendSpeechPending = 0;
          if (iCurrentTrack > 8 && iCurrentTrack < 17 &&
              (cup_won & 1) == 0) // Skip locked cup categories based on
