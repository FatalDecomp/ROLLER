...
int team_col[16] = // 000A6118
    {255, 207, 231, 171, 219, 195, 143, 243,
     159, 159, 159, 159, 159, 112, 112, 112};
char default_names[16][9] = { // 000A6158
    "SAL", "HAL",     "SLAVE", "ZEN",    "ASH",    "BISHOP", "VOYAGER", "NOMAD",
    "BOB", "VINCENT", "EDDIE", "MARVIN", "KRYTEN", "HOLLY",  "ROBBY",   "GORT"};
  ...
  for (i = 0; i < 14; ++i) {
    ...
    if (iNumGears > 0) {
      ...
      do {
        ...
        eng_chg_revs[iChgIdx + iRevsOffset] = (float)calc_revs(
            CarEngines.engines[carDesignIndex3].pRevs, iCurrGear, fChg);
        fNextChg =
            (float)CarEngines.engines[carDesignIndex3].pChgs[iChgIdx + 1];
        iChgIdx += 2;
        iNextGear = iCurrGear++;
        eng_chg_revs[iChgIdx + iRevsOffset - 1] = (float)calc_revs(
            CarEngines.engines[carDesignIndex3].pRevs, iNextGear, fNextChg);
...
void CalcCarSizes() {
  ...
  for (nCarDesignsIdx = 0; nCarDesignsIdx < 14; ++nCarDesignsIdx) {
    ...
    nCoordsIdx = 0;
    // Go through everything listed in CarDesigns and get the max and min values
    // to generate a hit box for each car
    ...
    while (nCoordsIdx < CarDesigns[nCarDesignsIdx].byNumCoords) {
      ...
      if (*p_fZ > (double)fZHigh)
        fZHigh = *p_fZ;
      pCoords =
          (tVec3 *)(p_fZ + 1); // funny way to get to the next set of coords
      ++nCoordsIdx;
    }
  ...
  CarBaseX = 0.0;
  CarBaseY = 0.0;
  for (nAutoCoordsIdx = 0; nAutoCoordsIdx < CarDesigns[0].byNumCoords;
       ++nAutoCoordsIdx) {
...
void placecars() {
  ...
  surfaceTypeAy[0] =
      car_c_reference_1[0]; // Copy car reference array (car type lookup table)
  surfaceTypeAy[1] = car_c_reference_1[1];
  surfaceTypeAy[2] = car_c_reference_1[2];
  surfaceTypeAy[3] = car_c_reference_1[3];
  memset(Car, 0,
         sizeof(tCar) * numcars); // Initialize all car data structures to zero
  uiStartingLane = 1;
  memset(CarSpray, 0,
         1408 * (numcars +
                 2)); // Initialize spray effect data for all cars + 2 extra
  ...
  if (numcars > 0) {
    ...
    do {
      iDriverIdx =
          grid[uiGridOffset / 4]; // Get driver index from grid position
      iCarIdx = iDriverIdx;
      byDriversCarDesign =
          Drivers_Car[iDriverIdx];          // Get car design for this driver
      ...
      if ((byCarIdx & 1) !=
          0) // Alternate Y position based on car index (staggered grid)
        Car[iDriverIdx].pos.fY = 500.0f;
      else
        Car[iDriverIdx].pos.fY = -500.0f;
      iCarIdx2 = iDriverIdx;
      Car[iCarIdx2].pos.fZ = 0.0;
      Car[iCarIdx2].fBestLapTime =
          9999.9902f;          // Initialize best lap time to maximum value
      Car[iCarIdx2].nRoll = 0; // Initialize car orientation angles
      Car[iCarIdx2].nYaw = 0;
      Car[iCarIdx2].fFinalSpeed = 0.0f;
      Car[iCarIdx2].byGearAyMax = -1;
      iChampMode =
          champ_mode; // Set starting track chunk based on championship mode
      Car[iCarIdx2].nPitch = 0;
      if (iChampMode)
        nStartChunk =
            TRAK_LEN - 56 -
            iGridOffsetWords; // start further back (56 chunks + offset)
      else
        nStartChunk =
            TRAK_LEN - 2 -
            iGridOffsetDWords; // start close to end (2 chunks + offset)
      Car[iDriverIdx].nCurrChunk =
          nStartChunk; // Set current track chunk for car
      iCarIdx3 = iDriverIdx;
      Car[iCarIdx3].iLastValidChunk =
          Car[iDriverIdx]
              .nCurrChunk; // Initialize track chunk tracking variables
      ...
      Car[iCarIdx3].fDurability =
          CarEngines.engines[Car[iDriverIdx].byCarDesignIdx].fDurability;
      ...
      Car[iDriverIdx].iAICurrentLine =
          GetHighOrderRand(4, iRandomVal); // iRandomVal >> 13;
      // Car[iDriverIdx].iUnk37 = (iRandomVal % 8192) / 8192;
      // Car[iDriverIdx].iUnk37 = (iRandomVal - (__CFSHL__(iRandomVal >> 31, 13)
      // + (iRandomVal >> 31 << 13))) >> 13;
      ...
      Car[iDriverIdx].byRacePosition =
          iCarCounter; // Set race position and other race parameters
      ...
      *(int *)((char *)carorder + uiOrderOffset) =
          iDriverIdx; // Add car to race order array
      Car[iDriverIdx].byAccelerating =
          0; // Initialize various car flags and counters
      ...
      iCarIdx4 =
          non_competitors[iDriverIdx]; // Check if this is a non-competitor car
      Car[iDriverIdx].byCheatCooldown = 0;
      if (iCarIdx4) // Handle non-competitor cars (spectators/disabled)
      {
        Car[iDriverIdx].byLives =
            -1; // Non-competitors have unlimited lives (-1)
        finished_car[iDriverIdx] = -1; // Mark as finished (disabled)
        pTrackData = localdata[Car[iDriverIdx].nCurrChunk]
                         .pointAy;        // Get track data for positioning
        Car[iDriverIdx].iControlType = 0; // Set control type to inactive (0)
        Car[iDriverIdx].pos.fX =
            0.0f * pTrackData->fY + 0.0f * pTrackData->fX +
            0.0f * pTrackData->fZ -
            pTrackData[3].fX; // Transform and set position using track
                              // transformation matrix
        Car[iDriverIdx].pos.fY = 0.0f * pTrackData[1].fX +
                                 0.0f * pTrackData[1].fY +
                                 0.0f * pTrackData[1].fZ - pTrackData[3].fY;
        dZCmp = 0.0 * pTrackData[2].fX + 0.0 * pTrackData[2].fY +
                0.0 * pTrackData[2].fZ - pTrackData[3].fZ +
                1000.0; // Calculate Z position with offset
        Car[iDriverIdx].fFinalSpeed = 0.0;
        // LOWORD(pTrackData) = Car[iDriverIdx].nCurrChunk;
        Car[iDriverIdx].fSpeedOverflow = 0;
        Car[iDriverIdx].nReferenceChunk =
            Car[iDriverIdx].nCurrChunk;  // (__int16)pTrackData;
        Car[iDriverIdx].nCurrChunk = -1; // Set chunk to -1 (disabled car)
        Car[iDriverIdx].pos.fZ = (float)dZCmp;
      }
      if (champ_mode &&
          iDriverIdx == ViewType[0]) // Special handling for player car in
                                     // championship mode
      {
        iViewCarIdx = ViewType[0];
        pEngine =
            &CarEngines
                 .engines[Car[iViewCarIdx].byCarDesignIdx]; // Get engine data
                                                            // for player car
        Car[iViewCarIdx].fRPMRatio = 1.0;
        byGearAyMax =
            (uint8)(pEngine->iNumGears) - 1; // Set maximum gear for this car
        Car[iViewCarIdx].byGearAyMax = byGearAyMax;
        Car[iViewCarIdx].fFinalSpeed =
            pEngine->pSpds[byGearAyMax]; // Set max speed from engine data
        Car[iViewCarIdx].fBaseSpeed = Car[iViewCarIdx].fFinalSpeed;
        dPower = calc_pow(
            Car[iViewCarIdx].byCarDesignIdx, (char)Car[iViewCarIdx].byGearAyMax,
            Car[iViewCarIdx]
                .fRPMRatio); // Calculate power for this car configuration
      ...
      nCurrChunk =
          Car[iDriverIdx]
              .nCurrChunk; // Position cars on track based on available lanes
      if (nCurrChunk >= 0 && nCurrChunk < TRAK_LEN) { // added by ROLLER
        pTrackLocalData =
            &localdata[nCurrChunk]; // Get track data for current chunk
        pTrackChunk =
            &TrackInfo[nCurrChunk]; // Get track info for surface types]
        while ((TrakColour[Car[iDriverIdx].nCurrChunk]
                          [surfaceTypeAy[uiStartingLane]] &
                SURFACE_FLAG_NO_SPAWN) != 0)
        // while ((TrakColour[Car[iDriverIdx].nCurrChunk][*(int *)((char
        // *)surfaceTypeAy + Car[iDriverIdx].nCurrChunk)] & 0x8000000) != 0)//
        // Find valid lane (avoid surfaces with special flags)
        {
          if (++uiStartingLane == 4)
            uiStartingLane = 0;
        }
        if (uiStartingLane <=
            3) { // Calculate Y position based on selected lane
          switch (uiStartingLane) {
          case 0u:
            dTrackYPos =
                pTrackChunk->fLShoulderWidth * 0.5 +
                pTrackLocalData
                    ->fTrackHalfWidth; // Lane 0: left shoulder position
            break;
          case 1u:
            dTrackYPos = pTrackLocalData->fTrackHalfWidth *
                         0.5; // Lane 1: left side of track
            break;
          case 2u:
            dTrackYPos = -pTrackLocalData->fTrackHalfWidth *
                         0.5; // Lane 2: right side of track
            break;
          case 3u:
            dTrackYPos = -pTrackLocalData->fTrackHalfWidth -
                         pTrackChunk->fRShoulderWidth *
                             0.5; // Lane 3: right shoulder position
            break;
          }
          Car[iDriverIdx].pos.fY =
              (float)dTrackYPos; // Set calculated Y position for car
        }
      }
      ++uiStartingLane;
  ...
  if (local_players > 0) {
    ...
    do {
      ...
      SLight[0][iSLightOffset / 0x30].currentPos.fX =
          SLight[iPlayers][0].targetPos.fX + 10000.0f;
      SLight[0][iSLightOffset / 0x30].currentPos.fY =
          SLight[iPlayers][0].targetPos.fY;
      SLight[0][iSLightOffset / 0x30].targetPos.fZ = fLightPosZ;
      SLight[0][iSLightOffset / 0x30].currentPos.fZ =
          SLight[iPlayers][0].targetPos.fZ + 5000.0f;
      SLight[0][iSLightOffset / 0x30 + 1].targetPos.fY =
          Car[iPlayerCarIdx].pos.fY + -500.0f;
      ...
      SLight[0][iSLightOffset / 0x30 + 1].currentPos.fX =
          SLight[iPlayers][1].targetPos.fX + 10000.0f;
      SLight[0][iSLightOffset / 0x30 + 1].currentPos.fY =
          SLight[iPlayers][1].targetPos.fY + -1000.0f;
      SLight[0][iSLightOffset / 0x30 + 1].targetPos.fZ = fLightPosZ;
      SLight[0][iSLightOffset / 0x30 + 1].currentPos.fZ =
          SLight[iPlayers][1].targetPos.fZ + 5000.0f;
      SLight[0][iSLightOffset / 0x30 + 2].targetPos.fY =
          Car[iPlayerCarIdx].pos.fY + 500.0f;
      SLight[0][iSLightOffset / 0x30 + 2].targetPos.fX = fLightPosX;
      SLight[0][iSLightOffset / 0x30 + 2].currentPos.fX =
          SLight[iPlayers][2].targetPos.fX + 10000.0f;
      SLight[0][iSLightOffset / 0x30 + 2].currentPos.fY =
          SLight[iPlayers][2].targetPos.fY + 1000.0f;
      ...
      do {
        ...
        SLight[0][iSLightOffset / 0x30].currentPos.fX =
            (float)(dTransform2 + dTransform3 * fTempZ - pLightTrackData[9]);
        SLight[0][iSLightOffset / 0x30].currentPos.fY =
            pLightTrackData[3] * fX + pLightTrackData[4] * fY +
            pLightTrackData[5] * fTempZ - pLightTrackData[10];
        SLight[0][iSLightOffset / 0x30].currentPos.fZ =
            pLightTrackData[6] * fX + pLightTrackData[7] * fY +
            pLightTrackData[8] * fTempZ - pLightTrackData[11];
        ...
        SLight[0][iSLightOffset / 0x30].targetPos.fX =
            (float)(dTransform5 + dTransform6 * fTempZ - pLightTrackData[9]);
        SLight[0][iSLightOffset / 0x30].targetPos.fY =
            pLightTrackData[3] * fX + pLightTrackData[4] * fY +
            pLightTrackData[5] * fTempZ - pLightTrackData[10];
        SLight[0][iSLightOffset / 0x30].targetPos.fZ =
            pLightTrackData[6] * fX + pLightTrackData[7] * fY +
            pLightTrackData[8] * fTempZ - pLightTrackData[11];
        SLight[0][iSLightOffset / 0x30].speed.fX =
            (float)(fabs(SLight[0][iSLightOffset / 0x30].targetPos.fX -
                         SLight[0][iSLightOffset / 0x30].currentPos.fX) *
                    0.01388888888888889);
        SLight[0][iSLightOffset / 0x30].speed.fY =
            (float)(fabs(SLight[0][iSLightOffset / 0x30].targetPos.fY -
                         SLight[0][iSLightOffset / 0x30].currentPos.fY) *
                    0.01388888888888889);
        dLightSpeedZ = fabs(SLight[0][iSLightOffset / 0x30].targetPos.fZ -
                            SLight[0][iSLightOffset / 0x30].currentPos.fZ) *
                       0.02777777777777778;
        SLight[0][iSLightOffset / 0x30].speed.fZ = (float)dLightSpeedZ;
        iSLightOffset += 48;
        // StoreEngines[iSLightOffset / 0x18 + 13].speeds[2] =
        // (float)dLightSpeedZ;
...
void DrawCars(int iCarIdx, int iViewMode) {
  ...
  if (numcars > 0) {
    ...
    do {
      ...
      fZ = pCar->pos.fZ;
      if (iCurrChunk_1 ==
          -1) // Use last valid chunk if current chunk is invalid (-1)
        iCurrChunk = pCar->iLastValidChunk;
      if (backwards) // Visibility culling - check if car is in visible track
                     // sections
      {
    ...
    LABEL_42:
      if (iVisibilityFlag && (pCar->byLives & 0x80u) == 0) {
        iRoll = (pCar->iRollDynamicOffset + pCar->nRoll) &
                0x3FFF; // Calculate car rotation matrix from yaw, pitch, roll
        iYaw = pCar->nYaw;
        iPitch = ((uint16)pCar->iPitchDynamicOffset + pCar->nPitch) & 0x3FFF;
        fTransformM00 =
            tcos[iYaw] *
            tcos[iPitch]; // Build 3x3 rotation matrix for car orientation
        fTransformM10 = tsin[iYaw] * tcos[iPitch];
        fTransformM20 = tsin[iPitch];
        fTransformM01 =
            tcos[iYaw] * fTransformM20 * tsin[iRoll] - tsin[iYaw] * tcos[iRoll];
        fTransformM11 =
            tsin[iYaw] * fTransformM20 * tsin[iRoll] + tcos[iYaw] * tcos[iRoll];
        fTransformM12 = -tsin[iRoll] * tcos[iPitch];
        fTransformM02 = -tcos[iYaw] * fTransformM20 * tcos[iRoll] -
                        tsin[iYaw] * tsin[iRoll];
        fTransformM21 = -tsin[iYaw] * fTransformM20 * tcos[iRoll] +
                        tcos[iYaw] * tsin[iRoll];
        fTransformM22 = tcos[iPitch] * tcos[iRoll];
        iPointIdx = 0;
        fClosestZ =
            1073676300.0f; // Initialize closest Z distance for depth sorting

        if (iCurrChunk_1 >= 0 &&
            iCurrChunk_1 < TRAK_LEN) // check added by ROLLER
          pointAy = localdata[iCurrChunk_1].pointAy;
        else
          pointAy = NULL;

        pHitboxPt =
            CarBox.hitboxAy[pCar->byCarDesignIdx]; // Get car hitbox points and
                                                   // chunk transform matrix
        do {
          fHitboxX =
              pHitboxPt->fX; // Transform car hitbox points loop (8 points)
          fHitboxY = pHitboxPt->fY;
          fHitboxZ = pHitboxPt->fZ;
          CarPt[iPointIdx].world.fX =
              pHitboxPt->fX * fTransformM00 + fHitboxY * fTransformM01 +
              fHitboxZ * fTransformM02 +
              fX; // Apply car rotation matrix to hitbox point
          CarPt[iPointIdx].world.fY = fHitboxX * fTransformM10 +
                                      fHitboxY * fTransformM11 +
                                      fHitboxZ * fTransformM21 + fY;
          ++pHitboxPt;
          CarPt[iPointIdx].world.fZ = fHitboxX * fTransformM20 +
                                      fHitboxY * fTransformM12 +
                                      fHitboxZ * fTransformM22 + fZ;
          if (iCurrChunk_1 != -1 && pointAy) // Transform from car space to
                                             // track chunk space if valid chunk
          {
            fWorldX = CarPt[iPointIdx].world.fX;
            fWorldY = CarPt[iPointIdx].world.fY;
            fWorldZ = CarPt[iPointIdx].world.fZ;
            CarPt[iPointIdx].world.fX = pointAy->fY * fWorldY +
                                        pointAy->fX * fWorldX +
                                        pointAy->fZ * fWorldZ - pointAy[3].fX;
            CarPt[iPointIdx].world.fY = pointAy[1].fX * fWorldX +
                                        pointAy[1].fY * fWorldY +
                                        pointAy[1].fZ * fWorldZ - pointAy[3].fY;
            CarPt[iPointIdx].world.fZ = pointAy[2].fX * fWorldX +
                                        pointAy[2].fY * fWorldY +
                                        pointAy[2].fZ * fWorldZ - pointAy[3].fZ;
          }
          k1 = CarPt[iPointIdx].world.fX -
               viewx; // Transform to camera view space for rendering
        ...
        car_draw_order[iCarDrawIdx].fMinZDepth =
            fClosestZ; // Store car in draw order array for Z-sorting
        car_draw_order[iCarDrawIdx].iChunkIdx = iCurrChunk;
        if (iCarIterator == iCarIdx &&
            (!iViewMode ||
             iViewMode == -1)) // Hide player car in certain view modes
          car_draw_order[iCarDrawIdx].iChunkIdx = -2;
      } else {
        car_draw_order[iCarDrawIdx].iChunkIdx =
            -2; // Mark invisible cars with special values
      ...
      iCarDrawIdx++;
      // CarZOrder[++iCarDrawIdx + 499].iPolygonIndex = iCarIterator;//
      // references car_draw_order
      ++pCar;
      ++iCarIterator;
...
void DisplayCarWithPose(int iCarIndex, uint8 *pScreenBuffer,
  ...
  nRoll = pCar->iRollCameraOffset + iMotionX + pose->roll +
          pCar->iRollDynamicOffset;
  iYawAngle = ((int16)iMotionZ + pose->yaw) & 0x3FFF;
  iPitch = ((uint16)pCar->iPitchCameraOffset + (int16)iMotionY +
            (uint16)pCar->iPitchDynamicOffset + pose->pitch) &
           0x3FFF;

  fRotMat00 =
      tcos[iYawAngle] *
      tcos[iPitch]; // Calculate 3x3 rotation matrix from car orientation angles
  ...
  iRoll = nRoll & 0x3FFF;
  fRotMat10 =
      (float)dCosYaw * fRotMat02 * tsin[iRoll] - tsin[iYawAngle] * tcos[iRoll];
  fRotMat12 =
      tsin[iYawAngle] * fRotMat02 * tsin[iRoll] + tcos[iYawAngle] * tcos[iRoll];
  fRotMat21 = -tsin[iRoll] * tcos[iPitch];
  fRotMat20 = -tcos[iYawAngle] * fRotMat02 * tcos[iRoll] -
              tsin[iYawAngle] * tsin[iRoll];
  fRotMat11 = -tsin[iYawAngle] * fRotMat02 * tcos[iRoll] +
              tcos[iYawAngle] * tsin[iRoll];
  ...
  uiColorFrom =
      color_remap ? color_remap[0] : car_flat_remap[carDesignIndex].uiColorFrom;
  uiColorTo =
      color_remap ? color_remap[1] : car_flat_remap[carDesignIndex].uiColorTo;
  pAnms = CarDesigns[carDesignIndex].pAnms;
  if (fDistanceToCar >= 8000.0 && VisibleCars >= 4 ||
      (textures_off & 0x100) != 0 || fDistanceToCar <= 0.0 ||
      (pCar->byStatusFlags & 2) != 0)
    goto LABEL_117; // Early exit if car is too far, invisible, or textures
                    // disabled
  ...
  if (pCar->nCurrChunk ==
      -1) // Setup car bounding box corners based on banking angle
  {
    if (iBank > 4096) {
      if (iBank <= 0x2000) {
...
LABEL_28:
  if (pCar->nCurrChunk == -1) {
    ...
    do {
      carworld[uiCarWorldOffset / 3].fX =
          (float)(carpoint[uiCarWorldOffset / 3].fY * fRotMat10 +
                  carpoint[uiCarWorldOffset / 3].fX * fRotMat00 +
                  carpoint[uiCarWorldOffset / 3].fZ * fRotMat20 +
                  *p_fX); // Transform car corner points from local to world
                          // coordinates
      carworld[uiCarWorldOffset / 3].fY =
          (float)(carpoint[uiCarWorldOffset / 3].fY * fRotMat12 +
      ...
      uiCarWorldOffset += 3;
      // wConsoleNode_variable_1[uiCarWorldOffset] = dTransformMat21;// offset
      // into carworld
    } while (uiCarWorldOffset != 12);
    for (i = 0; i != 4;
         ++i) // carpoint[i + 3].fZ = dLocalZ)// offset into carlocal
    {
      carlocal[i].fX = (carworld[i].fY + pTrackData->pointAy[3].fY) *
                           pTrackData->pointAy[1].fX +
                       (carworld[i].fX + pTrackData->pointAy[3].fX) *
                           pTrackData->pointAy[0].fX +
                       (carworld[i].fZ + pTrackData->pointAy[3].fZ) *
                           pTrackData->pointAy[2].fX;
      carlocal[i].fY = (carworld[i].fY + pTrackData->pointAy[3].fY) *
                           pTrackData->pointAy[1].fY +
                       (carworld[i].fX + pTrackData->pointAy[3].fX) *
                           pTrackData->pointAy[0].fY +
                       (carworld[i].fZ + pTrackData->pointAy[3].fZ) *
                           pTrackData->pointAy[2].fY;
      dLocalZ = (carworld[i].fY + pTrackData->pointAy[3].fY) *
                    pTrackData->pointAy[1].fZ +
                (carworld[i].fX + pTrackData->pointAy[3].fX) *
                    pTrackData->pointAy[0].fZ +
                (carworld[i].fZ + pTrackData->pointAy[3].fZ) *
                    pTrackData->pointAy[2].fZ;
      carlocal[i].fZ = (float)dLocalZ;
      //++i;                                      // replace carpoint assignment
      // with this in for loop
    }
  } else {
    ...
    do {
      carlocal[iCornerIndex].fX = fRotMat10 * carpoint[iCornerIndex].fY +
                                  fRotMat00 * carpoint[iCornerIndex].fX +
                                  fRotMat20 * carpoint[iCornerIndex].fZ +
                                  *pCarPosPtr;
      carlocal[iCornerIndex].fY =
          (float)(fRotMat12 * carpoint[iCornerIndex].fY +
      ...
      ++iCornerIndex;
      // carpoint[++iCornerIndex + 3].fZ = dTransformMat21_2;// offset into
      // carlocal
  ...
  fTrackRelativeY = (float)(pTrackData->pointAy[0].fY * dRelativeX +
                            pTrackData->pointAy[1].fY * dRelativeY +
                            pTrackData->pointAy[2].fY * dRelativeZ);
  fTrackRelativeZ = (float)(dRelativeX * pTrackData->pointAy[0].fZ +
                            dRelativeY * pTrackData->pointAy[1].fZ +
                            dRelativeZ * pTrackData->pointAy[2].fZ);
  iTunnelFlag = 0;
  if (fTrackRelativeY <= 0.0) {
    if (-pTrackData->fTrackHalfWidth < fTrackRelativeY) {
      ...
      if ((iRightSurfaceType == 5 || iRightSurfaceType == 6 ||
           iRightSurfaceType == 9) &&
          fTrackRelativeZ < (double)pTrackInfo->fRoofHeight)
        goto LABEL_51;
    }
  ...
  if ((TrakColour[iChunk][iLaneType] & SURFACE_FLAG_SKIP_RENDER) != 0 &&
      GroundColour[iChunk][2] < 0)
    // if ((((HIDWORD(ullTrackColor) ^ (unsigned int)ullTrackColor) -
    // HIDWORD(ullTrackColor)) & 0x20000) != 0 && GroundColour[iChunk][2] < 0)
    iShadowRenderFlag = 0;
  ...
  if (iGroundColorType == -2) {
    ...
    iUndergroundIdx = 0;
    while (1) {
      if (pTrackData->fTrackHalfWidth + pTrackInfo->fLShoulderWidth <
              carlocal[iUndergroundIdx].fY ||
          -pTrackData->fTrackHalfWidth - pTrackInfo->fRShoulderWidth >
              carlocal[iUndergroundIdx].fY) {
        if (!iUndergroundCheck) {
          dUndergroundHeight =
              getbankz(carlocal[iUndergroundIdx].fY, iChunk, 0);
          goto LABEL_101;
        }
      } else if (!iTunnelFlag) {
        dUndergroundHeight = getgroundz(carlocal[iUndergroundIdx].fX,
                                        carlocal[iUndergroundIdx].fY, iChunk);
        goto LABEL_101;
      }
      dUndergroundHeight = getroadz(carlocal[iUndergroundIdx].fX,
                                    carlocal[iUndergroundIdx].fY, iChunk);
    LABEL_101:
  ...
  do {
    ...
    if (pTrackData->fTrackHalfWidth + pTrackInfo->fLShoulderWidth >=
      ...
      if ((TrakColour[iChunk][pCar->iLaneType] & SURFACE_FLAG_SKIP_RENDER) ==
              0 &&
          roadheight[iHeightIdx] + -400.0 <= carlocal[iCornerIdx].fZ)
        iOnTrackFlag = -1;
    }
    ++iHeightIdx;
    ++iCornerIdx;
...
LABEL_105:
  ...
  if (iShadowRenderFlag) {
    for (iShadowOffset = 0; iShadowOffset != 48;
         iShadowOffset += 0xC) // wConsoleNode_variable_1[iShadowOffset / 4u] =
                               // dWorldToLocalZ)// offset into carworld
    {
      carworld[iShadowOffset / 0xCu].fX =
          pTrackData->pointAy[0].fY * carlocal[iShadowOffset / 0xCu].fY +
      ...
      carworld[iShadowOffset / 0xCu].fY =
          pTrackData->pointAy[1].fY * carlocal[iShadowOffset / 0xCu].fY +
      ...
      dWorldToLocalZ =
          pTrackData->pointAy[2].fY * carlocal[iShadowOffset / 0xCu].fY +
      ...
      // iShadowOffset += 0xC;                     // replace consolenode assign
      // with this in for loop
    }
    iScreenSize = scr_size;
    for (ii = 0; ii != 4;
         ++ii) // carworld[ii + 3].fY = (float)iTemp)// offset into carpoint
    {
      dViewDeltaX = carworld[ii].fX -
                    viewx; // Render car shadow as flat polygon on ground
      dViewDeltaY = carworld[ii].fY - viewy;
      dViewDeltaZ = carworld[ii].fZ - viewz;
      fShadowViewX =
          (float)(dViewDeltaX * vk1 + dViewDeltaY * vk4 + dViewDeltaZ * vk7);
      fShadowViewY =
          (float)(dViewDeltaX * vk2 + dViewDeltaY * vk5 + dViewDeltaZ * vk8);
      ...
      //++ii;                                     // replace reference to
      // carworld in for loop with this
    }
    ...
    game_render_quad_screen(g_pGameRenderer, &CarPol, TEXTURE_HANDLE_INVALID,
                            NULL);
  }
LABEL_117:
  ...
  fWarpDistortion =
      tsin[((uint16)warp_angle + ((uint16)iCarIndexCopy << 11)) & 0x3FFF] *
      0.5f;
  if (iNumCoords >
      0) // Process 3D car model coordinates and transform to screen space
  {
    iCoordOffset = 0;
    ...
    do {
      ...
      if ((cheat_mode & CHEAT_MODE_TINY_CARS) !=
          0) // Apply CHEAT_MODE_TINY_CARS scaling if enabled
      {
        fModelCoordXLocal = fModelCoordXLocal * 0.25f;
        fModelCoordYLocal = 0.25f * fModelCoordYLocal;
      }
      // CHEAT_MODE_WARP
      if ((cheat_mode & CHEAT_MODE_WARP) !=
          0) // Apply CHEAT_MODE_WARP distortion effect if enabled
      {
        fModelCoordXLocal =
            fModelCoordXLocal * fWarpDistortion + fModelCoordXLocal;
        fModelCoordYLocal =
            fModelCoordYLocal * fWarpDistortion + fModelCoordYLocal;
        fModelCoordZLocal =
            fWarpDistortion * fModelCoordZLocal + fModelCoordZLocal;
      }
      dTransformedX = fModelCoordXLocal;
      dTransformedY = fModelCoordYLocal;
      dTransformedZ = fModelCoordZLocal;
      CarPt[iCoordOffset / 0x20].world.fX =
          fModelCoordXLocal * fRotMat00 + fModelCoordYLocal * fRotMat10 +
          fModelCoordZLocal * fRotMat20 + fCarPosX;
      CarPt[iCoordOffset / 0x20].world.fY =
          (float)(dTransformedX * fRotMat01 + dTransformedY * fRotMat12 +
                  dTransformedZ * fRotMat11 + fCarPosY);
      iCurrentChunk = iCurrChunk;
      CarPt[iCoordOffset / 0x20].world.fZ =
          (float)(dTransformedX * fRotMat02 + dTransformedY * fRotMat21 +
                  dTransformedZ * fRotMat22Copy + fCarPosZ);
      if (iCurrentChunk != -1) {
        ...
        dChunkTransformX = dChunkTransformY * fModelWorldY +
                           pChunkData->pointAy[0].fX * fModelWorldX;
        dChunkMatrixZ = pChunkData->pointAy[0].fZ;
        fModelWorldZ = CarPt[iCoordOffset / 0x20].world.fZ;
        CarPt[iCoordOffset / 0x20].world.fX =
            (float)(dChunkTransformX + dChunkMatrixZ * fModelWorldZ -
                    pChunkData->pointAy[3].fX);
        CarPt[iCoordOffset / 0x20].world.fY =
            pChunkData->pointAy[1].fX * fModelWorldX +
      ...
      fModelViewX = (float)(dViewRelativeX * vk1 + dViewRelativeY * vk4 +
                            dViewRelativeZ * vk7);
      fModelViewY = (float)(dViewRelativeX * vk2 + dViewRelativeY * vk5 +
                            dViewRelativeZ * vk8);
      dCameraSpaceZ =
          dViewRelativeX * vk3 + dViewRelativeY * vk6 + dViewRelativeZ * vk9;
      ...
      dProjScreenX =
          dProjViewDistance * fModelViewX * dProjInverseZ + (double)xbase;
      //_CHP();
      xp = d2i(dProjScreenX);
      dProjScreenY =
          dProjInverseZ * (dProjViewDistance * fModelViewY) + (double)ybase;
      //_CHP();
      yp = d2i(dProjScreenY);
      ...
      //*(_DWORD *)((char *)&carlocal[3].fX + iCoordOffset) = (scr_size * (199 -
      // yp)) >> 6; roadheight[iCoordOffset / 4 + 1] = fModelViewX;
  ...
  if (pCarDesign && (int)pCarDesign->byNumPols > 0) {
    ...
    do {
      ...
      // for (jj = 0; jj != 16; *(_DWORD *)((char *)&tempData.fAIMaxSpeed + jj)
      // = pCarVertex)// reference into polygonVertices
      //{
      //   jj += 4;
      ...
      dCrossProduct1X =
          ...
          polygonVertices[0]->view.fX; // Test polygon visibility using cross
                                       // product (backface culling)
      fCrossProductY =
          polygonVertices[2]->view.fY - polygonVertices[0]->view.fY;
      dCrossProduct1Z =
          polygonVertices[2]->view.fZ - polygonVertices[0]->view.fZ;
      ...
      fBackfaceDotProduct = (float)((fCrossProductY * dCrossProduct2Z -
                                     dCrossProduct1Z * dCrossProduct2Y) *
                                        fPolygonVertex1X +
                                    (dCrossProduct1Z * dCrossProduct2X -
                                     dCrossProduct2Z * dCrossProduct1X) *
                                        fPolygonVertex1Y +
                                    (dCrossProduct2Y * dCrossProduct1X -
                                     dCrossProduct2X * fCrossProductY) *
                                        fPolygonVertex1Z);
      // SURFACE_FLAG_FLIP_BACKFACE
      if ((pPols->uiTex & SURFACE_FLAG_FLIP_BACKFACE) != 0 ||
          fBackfaceDotProduct <= 0.0) {
        // if ((byTextureFlags & 0x20) != 0 || fBackfaceDotProduct <= 0.0) {
        //  SURFACE_FLAG_BACK
        if (fBackfaceDotProduct > 0.0 &&
            (pPols->uiTex & SURFACE_FLAG_BACK) != 0)
      ...
      if (nNextPolIdx >= 0) {
        ...
        do {
          ...
          if (iLinkedPolygonIdx > 0 &&
              iDepthSortIdx > 0) // depth sort check added by ROLLER
            CarZOrder[iLinkedPolygonIdx].fZDepth =
                CarZOrder[iDepthSortIdx].fZDepth + -1.0f;
  ...
  if (iVisiblePolygons > 0 || (Car[iCarIndexCopy].byStatusFlags & 2) != 0) {
    ...
    if (iCurrChunk >= 0 &&
        iCurrChunk < TRAK_LEN) // bounds check added by ROLLER
    ...
    do { // Process and render car smoke/spray particle effects
      if (pSmokeSpray->iLifeTime > 0) {
        ...
        SmokePt[0][iSmokeIndex].world.fX = fPosX * fRotMat00 +
                                           fPosY * fRotMat10 +
                                           fPosZ * fRotMat20 + fCarPosX;
        SmokePt[0][iSmokeIndex].world.fY = fPosX * fRotMat01 +
                                           fPosY * fRotMat12 +
                                           fPosZ * fRotMat11 + fCarPosY;
        iSmokeChunkIdx = iCurrChunk;
        SmokePt[0][iSmokeIndex].world.fZ = fPosX * fRotMat02 +
                                           fPosY * fRotMat21 +
                                           fPosZ * fRotMat22Copy + fCarPosZ;
        if (iSmokeChunkIdx != -1 && pTrackChunkPtr) {
          ...
          dSmokeChunkTransX =
              dSmokeChunkTransY * fSmokeWorldY + *pTrackChunkPtr * fSmokeWorldX;
          ...
          SmokePt[0][iSmokeIndex].world.fX =
              (float)(dSmokeChunkTransX + dSmokeChunkMatZ * fSmokeWorldZ -
                      pTrackChunkPtr[9]);
          SmokePt[0][iSmokeIndex].world.fY = pTrackChunkPtr[3] * fSmokeWorldX +
                                             pTrackChunkPtr[4] * fSmokeWorldY +
                                             pTrackChunkPtr[5] * fSmokeWorldZ -
                                             pTrackChunkPtr[10];
          dSmokeChunkTransZ = pTrackChunkPtr[6] * fSmokeWorldX +
                              pTrackChunkPtr[7] * fSmokeWorldY +
                              pTrackChunkPtr[8] * fSmokeWorldZ -
                              pTrackChunkPtr[11];
        ...
        fSmokeViewX = (float)(dSmokeViewDeltaX * vk1 + dSmokeViewDeltaY * vk4 +
                              dSmokeViewDeltaZ * vk7);
        fSmokeViewY = (float)(dSmokeViewDeltaX * vk2 + dSmokeViewDeltaY * vk5 +
                              dSmokeViewDeltaZ * vk8);
        dSmokeCameraZ = dSmokeViewDeltaX * vk3 + dSmokeViewDeltaY * vk6 +
                        dSmokeViewDeltaZ * vk9;
        ...
        dSmokeProjScreenX =
            dSmokeProjViewDist * fSmokeViewX * dSmokeProjInverseZ +
            (double)xbase;
        //_CHP();
        xp = d2i(dSmokeProjScreenX);
        dSmokeProjScreenY =
            dSmokeProjInverseZ * (dSmokeProjViewDist * fSmokeViewY) +
            (double)ybase;
        //_CHP();
        ...
        if ((uint8)(pSmokeSpray->iType) == 1) {
          ...
          SmokePt[1][iSmokeIndex].world.fX = fPosX * fRotMat00 +
                                             fPosY * fRotMat10 +
                                             fPosZ * fRotMat20 + fCarPosX;
          SmokePt[1][iSmokeIndex].world.fY = fPosX * fRotMat01 +
                                             fPosY * fRotMat12 +
                                             fPosZ * fRotMat11 + fCarPosY;
          iSmokeChunkIdx2 = iCurrChunk;
          SmokePt[1][iSmokeIndex].world.fZ = fPosX * fRotMat02 +
                                             fPosY * fRotMat21 +
                                             fPosZ * fRotMat22Copy + fCarPosZ;
          if (iSmokeChunkIdx2 != -1 && pTrackChunkPtr) {
            ...
            dSmoke2ChunkTransX = dSmoke2ChunkTransY * fSmokeWorldY +
                                 *pTrackChunkPtr * fSmokeWorldX;
            ...
            SmokePt[1][iSmokeIndex].world.fX =
                (float)(dSmoke2ChunkTransX + dSmoke2ChunkMatZ * fSmokeZ -
                        pTrackChunkPtr[9]);
            SmokePt[1][iSmokeIndex].world.fY =
                pTrackChunkPtr[3] * fSmokeWorldX +
                pTrackChunkPtr[4] * fSmokeWorldY + pTrackChunkPtr[5] * fSmokeZ -
                pTrackChunkPtr[10];
            dSmoke2ChunkTransZ = pTrackChunkPtr[6] * fSmokeWorldX +
                                 pTrackChunkPtr[7] * fSmokeWorldY +
                                 pTrackChunkPtr[8] * fSmokeZ -
                                 pTrackChunkPtr[11];
            pChunkDataPtr = pTrackChunkPtr;
          ...
          fSmoke2ViewX =
              (float)(dSmoke2ViewDeltaX * vk1 + dSmoke2ViewDeltaY * vk4 +
                      dSmoke2ViewDeltaZ * vk7);
          fSmoke2ViewY =
              (float)(dSmoke2ViewDeltaX * vk2 + dSmoke2ViewDeltaY * vk5 +
                      dSmoke2ViewDeltaZ * vk8);
          dSmoke2CameraZ = dSmoke2ViewDeltaX * vk3 + dSmoke2ViewDeltaY * vk6 +
                           dSmoke2ViewDeltaZ * vk9;
          ...
          dSmoke2ProjScreenX =
              dSmoke2ProjViewDist * fSmoke2ViewX * dSmoke2ProjInverseZ +
              (double)xbase;
          //_CHP();
          xp = d2i(dSmoke2ProjScreenX);
          dSmoke2ProjScreenY =
              dSmoke2ProjInverseZ * (dSmoke2ProjViewDist * fSmoke2ViewY) +
              (double)ybase;
          //_CHP();
        ...
        if ((uint8)(pSmokeSpray->iType) == 2)
          CarZOrder[iSmokeZOrderIdx].fZDepth =
              SmokePt[0][iSmokeIndex].view.fZ + -32768.0f;
        else
    ...
    qsort(CarZOrder, iVisiblePolygons, 0xCu,
          carZcmp); // Sort polygons by Z-depth for proper rendering order
    ...
    if (iVisiblePolygons > 0) {
      ...
      do { // Main polygon rendering loop - draw sorted polygons
        if (CarZOrder[uiRenderLoopOffset / 0xC].iPolygonLink >= -1) {
          ...
          pRenderPolygon =
              &CarDesigns[dwCarDesignOffset / 0x1C].pPols[iPolygonIdx];
          uiTextureSurface = pRenderPolygon->uiTex;

          for (mm = 0; mm < 4; ++mm) {
            polygonVertices[mm] = &CarPt[pRenderPolygon->verts[mm]];
          }
          // for (mm = 0; mm != 16; *(_DWORD *)((char *)&tempData.fAIMaxSpeed +
          // mm) = pPolygonVertex)// offset into polygonVertices
          //{
          //   mm += 4;
          ...
          if (polygonVertices[0]->view.fZ >=
              (double)polygonVertices[1]->view.fZ)
          ...
          if (fMinDepthVertex01 >= (double)fMinDepthVertex23) {
            if (polygonVertices[2]->view.fZ >=
                (double)polygonVertices[3]->view.fZ)
          ...
          if (iBackfaceFlag) {
            uiTextureSurface =
                CarDesigns[dwCarDesignOffset / 0x1C].pBacks[iPolygonIdx];
            ...
            if (byTextureIndex >= 4)
              ...
              uiTextureSurface =
                  pAnimation
                      ->framesAy[(char)(options ? options->anim_frame : 0)];
          }
          // SURFACE_FLAG_BACK
          ...
          if ((uiTextureSurface & SURFACE_FLAG_APPLY_TEXTURE) != 0 &&
              !iTextureDisabled) // Apply texture mapping if
                                 // SURFACE_FLAG_APPLY_TEXTURE set and not
                                 // disabled
          {
            iAnimationFrame = (uint8)uiTextureSurface;
            SET_LOWORD(uiTextureSurface, uiTextureSurface & 0xFE00);
            uiTextureSurface +=
                car_remap[256 * car_texmap[uiTextureMapOffset / 4] - 256 +
                          iAnimationFrame];
          }
          // TEX_OFF_ADVANCED_CARS SURFACE_FLAG_APPLY_TEXTURE
          if ((textures_off & TEX_OFF_ADVANCED_CARS) != 0 &&
              (uiTextureSurface & SURFACE_FLAG_APPLY_TEXTURE) == 0 &&
              (uint8)uiTextureSurface == uiColorFrom)
          ...
          if (renderer) {
            ...
            if ((uiTextureSurface & SURFACE_FLAG_APPLY_TEXTURE) != 0 &&
                iTextureDisabled) {
              carPolyTexture = game_render_get_texture_handle(
                  renderer, car_texmap[uiTextureMapOffset / 4]);
            }
            game_render_quad_world_subdivide_type(
                renderer, verts, carPolyTexture, uiTextureSurface,
                iSubdivisionParam, bCloseToCamera ? 0.0f : 1.0f);
          }
        } else {
          CarZOrder[uiRenderLoopOffset / 0xC].iPolygonLink = -1;
          ...
          if (fSmokeDepthCheck > 80.0) {
            ...
            if ((uint8)(pCurrentSprite->iType) == 1) {
              fSpriteSize =
                  (double)VIEWDIST * pCurrentSprite->fSize / fSmokeDepthCheck;
              //_CHP();
              ...
              LABEL_267:
                game_render_quad_screen(g_pGameRenderer, &CarPol,
                                        TEXTURE_HANDLE_INVALID, NULL);
                goto LABEL_268;
              }
            }
            game_render_quad_screen(
                g_pGameRenderer, &CarPol,
                game_render_get_texture_handle(g_pGameRenderer, 18), NULL);
          }
        }
      LABEL_268:
  ...
  if (NamesLeft < 5 && NamesLeft >= -2 &&
      ...
      replaytype != 2 && (pCar->byStatusFlags & 2) == 0 &&
      !winner_mode) // Render driver name above car if enabled and visible
  {
    ...
    if (iCurrChunk == -1) {
      ...
      if (pChunkDataPtr) { // added by ROLLER
        CarPt[0].world.fX = pChunkDataPtr[1] * fNameTagWorldY +
                            *pChunkDataPtr * fNameTagWorldX +
                            pChunkDataPtr[2] * fNameTagWorldZ -
                            pChunkDataPtr[9];
        CarPt[0].world.fY = pChunkDataPtr[3] * fNameTagWorldX +
                            pChunkDataPtr[4] * fNameTagWorldY +
                            pChunkDataPtr[5] * fNameTagWorldZ -
                            pChunkDataPtr[10];
        CarPt[0].world.fZ = fNameTagWorldY * pChunkDataPtr[7] +
                            fNameTagWorldX * pChunkDataPtr[6] +
                            fNameTagWorldZ * pChunkDataPtr[8] -
                            pChunkDataPtr[11];
      }
    }
    ...
    fNameTagViewX = (float)(dNameTagDeltaX * vk1 + dNameTagDeltaY * vk4 +
                            dNameTagDeltaZ * vk7);
    fNameTagViewY = (float)(dNameTagDeltaX * vk2 + dNameTagDeltaY * vk5 +
                            dNameTagDeltaZ * vk8);
    dNameTagCameraZ =
        dNameTagDeltaX * vk3 + dNameTagDeltaY * vk6 + dNameTagDeltaZ * vk9;
    ...
    dNameTagScreenX =
        dNameTagViewDist * fNameTagViewX * dNameTagInverseZ + (double)xbase;
    //_CHP();
    xp = d2i(dNameTagScreenX);
    dNameTagScreenY =
        dNameTagInverseZ * (dNameTagViewDist * fNameTagViewY) + (double)ybase;
    ...
    CarPt[0].screen.y =
        (iNameTagScreenSize * (199 - d2i(dNameTagScreenY))) >> 6;
    ...
    if (pCar->byRacePosition < racers - 1 || racers == 1)
      sprintf(buffer, "%s (%s)", driver_names[iCarIndexCopy],
    ...
    if (iNameDisplayX >= 0 && iNameDisplayX < winw - iNameWidth &&
        iNameDisplayY > 0 && iNameDisplayY < winh - 16 && iNameWidth > 0 &&
        !intro) {
      iPrevScrSize = scr_size;
      scr_size = 64;
      if (mirror)
        mini_prt_string_rev(rev_vga[0], buffer, iNameDisplayX,
                            CarPt[0].screen.y - 16);
      else
        mini_prt_string(rev_vga[0], buffer, iNameDisplayX,
                        CarPt[0].screen.y - 16);
