...
//-------------------------------------------------------------------------------------------------
// 00075DC0
int select_comport(int iIconIdx) { // BIOS interrupt 11h - Equipment
                                   // determination (detect available COM ports)
  ...
  //__asm { int     11h; BIOS interrupt 11h - Equipment determination(detect
  // available COM ports) }
  iExitFlag = 0; // Initialize exit flag (0 = continue loop, -1 = exit)

  // Set this to 4 for modern systems to always show 4 comm ports
  iAvailableComPorts =
      4; // ((int)(unsigned __int16)iIconIdx >> 9) & 7;// Extract COM port count
         // from BIOS equipment word (AX bits 9-11) - DECOMPILER ERROR: uses AX
         // from INT 11h, not parameter a1

  if ((unsigned int)iAvailableComPorts <
      2) // Ensure minimum of 2 COM ports are available
    iAvailableComPorts = 2;
  if (iAvailableComPorts < serial_port) // Validate current serial port
                                        // selection against available ports
    serial_port = 2;
  check16550(
      serial_port); // Test 16550 UART availability on selected serial port
  ...
  do {
    ...
    display_block(scrbuf, front_vga[6], 0, 36, 2,
                  0); // Display UI element at position (36, 2)
    display_block(scrbuf, front_vga[5], iIconIdx, -4, 247,
                  0); // Display gear selection indicator at bottom left
    display_block(scrbuf, front_vga[5], game_type + 5, 135, 247,
                  0); // Display game type indicator (5 + game_type offset)
    display_block(
        scrbuf, front_vga[4], 4, 76, 257,
        -1); // Display UI element at position (76, 257) with special color
    display_block(
        scrbuf, front_vga[6], 4, 62, 336,
        -1); // Display UI element at position (62, 336) with special color
    scale_text(front_vga[15], &language_buffer[5312], font1_ascii,
               font1_offsets, 400, 75, 143, 1u, 200,
               640); // Display main menu title text
    scale_text(front_vga[15], &language_buffer[3008], font1_ascii,
               font1_offsets, 400, 93, 143, 1u, 200,
               640);      // Display COM port selection instructions
    if (serial_port == 1) // Set color for COM 1 text (highlighted if selected)
      byCom1Color = 0xAB;
    else
      byCom1Color = 0x8F;
    scale_text(front_vga[15], "COM 1", font1_ascii, font1_offsets, 400, 135,
               byCom1Color, 1u, 200, 640); // Display 'COM 1' option text
    if (serial_port == 2) // Set color for COM 2 text (highlighted if selected)
      byCom2Color = 0xAB;
    else
      byCom2Color = 0x8F;
    scale_text(front_vga[15], "COM 2", font1_ascii, font1_offsets, 400, 153,
               byCom2Color, 1u, 200, 640); // Display 'COM 2' option text
    ...
    { // Set color for COM 3 text (highlighted if selected)
      if (serial_port == 3)
        ...
        byCom3Color = 0x8F;
      scale_text(front_vga[15], "COM 3", font1_ascii, font1_offsets, 400, 171,
                 byCom3Color, 1u, 200,
                 640); // Display 'COM 3' option text (if available)
    }
    ...
    { // Set color for COM 4 text (highlighted if selected)
      if (serial_port == 4)
        ...
        byCom4Color = 0x8F;
      scale_text(front_vga[15], "COM 4", font1_ascii, font1_offsets, 400, 189,
                 byCom4Color, 1u, 200,
                 640); // Display 'COM 4' option text (if available)
    }
    if (no_16550) // Check if 16550 UART error occurred
      scale_text(front_vga[15], &language_buffer[6976], font1_ascii,
                 font1_offsets, 400, 351, 231, 1u, 200,
                 640);       // Display 16550 error message at bottom of screen
    ...
    while (fatkbhit())       // Process all pending keyboard input
    {
      ...
      { // Check for extended key code (0 = function/arrow keys)
        if (!uiTempKey) {
          ...
          uiKey = uiExtendedKey;
          if (uiExtendedKey >=
              0x48) // Check if key code is 0x48 or higher (Up arrow = 0x48)
          {         // Check for Up arrow key (0x48)
            if (uiExtendedKey <=
                0x48) { // Move to previous COM port (minimum COM 1)
              if (serial_port != 1)
                --serial_port;
            LABEL_38:
              check16550(
                  serial_port); // Test 16550 UART on newly selected COM port
      ...
      } else if (uiTempKey <= 0xD ||
                 uiTempKey == 0x1B) // Check for Enter (13) or Escape (27) keys
      {
        iExitFlag = -1; // Set exit flag to exit menu loop
      }
    }
  } while (!iExitFlag); // Continue loop until user presses Enter or Escape
  ROLLERCommsSetComPort(
      serial_port); // Configure communications system with selected COM port
...
int select_modemstuff(int iIconIdx) {
  ...
  // TODO
  //__asm { int     11h; BIOS equipment determination to detect COM ports }//
  // BIOS equipment determination to detect COM ports
  iTemp = modem_port;
  iMaxCOMPorts = ((int)(uint16)iResult >> 9) &
                 7;   // Extract number of COM ports from BIOS data
  ...
  do {
    // szTemp2 = (char *)unInputMode;              // Main UI loop - display
    // menu and handle input
    display_essentials(iIconIdx, iMenuPos, unInputMode);
    if (iDisplayMessage == 93 || iDisplayMessage == 94 ||
        iDisplayMessage == 95 || iDisplayMessage == 96 ||
        iDisplayMessage == 104) // Display flashing status messages
    {
      if ((frames & 0xFu) < 8) {
        // szTemp2 = font1_ascii;
        // iTemp = (int)font1_offsets;
        scale_text(front_vga[15], &language_buffer[64 * iDisplayMessage],
                   font1_ascii, font1_offsets, 400, 279, 143, 1u, 200, 640);
      }
    } else if (iDisplayMessage >= 0) {
      // szTemp2 = font1_ascii;
      // iTemp = (int)font1_offsets;
      scale_text(front_vga[15], &language_buffer[64 * iDisplayMessage],
                 font1_ascii, font1_offsets, 400, 279, 231, 1u, 200, 640);
    }
    ...
    while (fatkbhit()) {
      ...
      if (unInputMode) {
        if (unInputMode <=
            1) { // String input mode - edit init string or phone number
          if (iMenuPos == 3)
          ...
          LABEL_63:
            if (keys[WHIP_SCANCODE_LSHIFT] ||
                keys[WHIP_SCANCODE_RSHIFT]) // Handle shifted characters for
                                            // special symbols
            {
            ...
            if (keypair.iKeycode < 0xDu ||
                keypair.iKeycode > 0xDu && keypair.iKeycode != 27)
              goto LABEL_63;
            szTemp2 = (char *)-1;
            unInputMode = 0;
            iMessageTimeout = 0;
          ...
          if (keypair.iKeycode == 27) {
            ...
            // iTemp = (int)font1_offsets;
            // szTemp2 = font1_ascii;
            scale_text(front_vga[15], &language_buffer[6912], font1_ascii,
                       font1_offsets, 400, 279, 143, 1u, 200, 640);
            // keypair.iCharacter = (int)screen;
            copypic(scrbuf, screen);
            close_network();
          }
        ...
        if (keypair.iKeycode < 0xDu) {
          ...
          case 0:
            ...
            break;
          case 1:
            // szTemp2 = (char *)(modem_port + 1);// Menu 1: Cycle through COM
            // ports
            iTemp = iMaxCOMPorts;
          ...
          case 6:
            ...
            scale_text(front_vga[15], &language_buffer[6656], font1_ascii,
                       font1_offsets, 400, 279, 143, 1u, 200, 640);
            copypic(scrbuf, screen);
    ...
    if (unInputMode == 2 &&
        !iMessageTimeout) // Handle modem responses during call
    {
      // TODO
      iModemResponse = 0; // gssModemCheckResponse(0, keypair.iCharacter,
                          // (int)szTemp2, iTemp);
      ...
      if (iModemResponse >= 0) {
        ...
        case 0:
          ...
          if (fabs(fDialTime) != 0) {
            // TODO
            // gssModemInit((int)modem_initstring, iModemRespCopy, (int)szTemp2,
            // iTemp);
            fDialTime = 0.0;
        ...
        case 1:
          ...
          break;
        case 2:
          if (iWaitingForAnswer &&
              !modem_call) // Response 2: Incoming call detected
          {
            // TODO
            // gssModemAnswer();
            iDisplayMessage = 96;
        ...
        case 7:
          iDisplayMessage =
              iModemResponse + 94; // Response 3-7: Connection errors
          close_network();
          unInputMode = 0;
          break;
        default:
    ...
    // TODO
    if (fabs(fInitTime) !=
        0) // && ((double)(unsigned int)clock() - fInitTime) * 0.0099999998
           // > 5.0)// Check for dial timeout (5 seconds)
    {
      close_network();
      iTemp = 105;
      fInitTime = 0.0;
...
void loadmodemnames() {
  ...
  pFileModem = fopen(
      "MODEM.INI", "r"); // Open MODEM.INI file for reading modem configurations
  fgets(szBuffer, 512,
        pFileModem); // Read first line containing number of modems
  pszToken = strtok(szBuffer, " ,\n\t\r"); // Parse modem count from first token
  modems = strtol(pszToken, 0,
                  10); // Convert string to integer and store global modem count
  modembuffer = (char *)getbuffer(
      56 * modems); // Allocate buffer for modem data (56 bytes per modem)
  ...
  if (modems > 0) {
    ...
    do {
      fgets(szBuffer, 512,
            pFileModem); // Read next line from MODEM.INI for each modem entry
      pszModemData =
          strtok(szBuffer, " "); // Parse modem type ID from first token
      *(int *)&modembuffer[iModemOffset + 52] =
          strtol(pszModemData, 0, 10) -
          1; // Store modem type (subtract 1 for 0-based index) at offset +52
      pszSrc = strtok(0, "\r\n"); // Get remainder of line (modem name string)
      pszDest = &modembuffer[iModemOffset]; // Set destination pointer to
                                            // current modem buffer slot
      do {
        byChar1 =
            *pszSrc; // Copy modem name string 2 bytes at a time for efficiency
        *pszDest = *pszSrc;
        if (!byChar1)
          break;
        byChar2 = pszSrc[1];
  ...
  pFileModemStr =
      fopen("MODEMSTR.INI",
            "r"); // Open MODEMSTR.INI file for reading modem command strings
  fgets(szBuffer, 512,
        pFileModemStr); // Read first line containing number of modem strings
  pszStrToken = strtok(
      szBuffer, pszDelimiters); // Parse modem string count from first token
  modemstrs =
      strtol(pszStrToken, 0,
             10); // Convert string to integer and store global string count
  modembuffer2.bufPtr = (char *)getbuffer(
      51 *
      modemstrs); // Allocate buffer for modem strings (51 bytes per string)
  ...
  if (modemstrs > 0) {
    iStrOffsetCurrent = 0;
    do {
      fgets(szBuffer, 512, pFileModemStr); // Read next line from MODEMSTR.INI
                                           // for each string entry
      pszStrData = strtok(
          szBuffer, "\t"); // Parse modem command string using tab delimiter
      iStrOffset = iStrOffsetCurrent;
      pszStrSrc = pszStrData;
      pszStrDest =
          &modembuffer2.bufPtr[iStrOffsetCurrent]; // Set destination pointer to
                                                   // current string buffer slot
      ++iStrCount;
      do {
      ...
      iStrOffsetCurrent =
          iStrOffset + 51; // Advance to next string slot (51 bytes per entry)
    } while (iStrCount < modemstrs);
  }
  fclose(pFileModemStr); // Close MODEMSTR.INI file
  if (current_modem >=
      modems) // Validate current_modem index - clamp to valid range
    current_modem =
        modems - 1; // If current modem >= total modems, set to last valid index
...
void select_modem(int iIconIdx) {
  ...
  if (current_modem <
      0) // Handle negative modem index (convert to positive for display)
  ...
  while (!iExitFlag) // Main display and input loop
  {
    ...
    display_block(
        scrbuf, front_vga[6], 4, 62, 336,
        -1); // Display UI element at position (62, 336) with special color
    scale_text(front_vga[15], &language_buffer[6784], font1_ascii,
               font1_offsets, 400, 75, 143, 1u, 200,
               640); // Display 'Select Modem' title text
    iStartIdx = iSelectedModem -
                7; // Calculate display window start (7 items before current)
    if (iSelectedModem - 7 < 0) // Ensure start index is not negative
      iStartIdx = 0;
    iEndIdx = iSelectedModem +
              7; // Calculate display window end (7 items after current)
    if (iSelectedModem + 7 >=
        modems) // Ensure end index does not exceed available modems
      iEndIdx = modems - 1;
    ...
    {
      iModemOffset = 56 * iStartIdx; // Calculate byte offset into modem buffer
                                     // (56 bytes per modem)
      iYPosition = -18 * iSelectedModem + 18 * iStartIdx +
                   237; // Calculate Y position for first visible modem (18
                        // pixels per line)
      do {              // Set highlight color for currently selected modem
        ...
        scale_text(front_vga[15], &modembuffer[iModemOffset], font1_ascii,
                   font1_offsets, 400, iYPosition, byTextColor, 1u, 200,
                   640);  // Display modem name from buffer (56 bytes per entry)
        iYPosition += 18; // Move to next line (18 pixels down)
        iModemOffset += 56; // Move to next modem data (56 bytes forward)
      } while (iCurrentIdx <=
               iEndIdx); // Continue until all visible modems are displayed
    }
    copypic(scrbuf, screen); // Copy completed frame buffer to screen
    while (fatkbhit())       // Process all pending keyboard input
    {
      ...
      { // Check for extended key code (0 = function/arrow keys)
        if (!uiKeyCode) {
          iExtendedKey =
              fatgetch() -
              71; // Get extended key and subtract 71 (base for navigation keys)
          iMaxIdx = modems - 1;
          switch (iExtendedKey) {
          case 0:
          ...
          LABEL_26:
            iSelectedModem = iMaxIdx; // LEFT key (75) and END key (79) - Move
                                      // to previous modem
            break;
          case 2:
          ...
          case 8:
            goto LABEL_26;
          case 9:
            if (iMaxIdx > iSelectedModem) // RIGHT key (77) and DOWN key (80) -
                                          // Move to next modem
              ++iSelectedModem;
            continue;
          case 10:
      ...
      } else if (uiKeyCode <= 0xD) // Check for Enter key (13) to select modem
      {
        ...
        pszDestInit = modem_initstring; // Set destination pointer for modem
                                        // init string copy
        pszSrcInit =
            &modembuffer2
                 .bufPtr[51 *
                         *(int *)&modembuffer[56 * iSelectedModem +
                                              52]]; // Calculate source pointer
                                                    // to modem init string (51
                                                    // bytes per init string)
        iExitFlag = -1; // Set exit flag to leave menu
        do {
          byChar1 = *pszSrcInit; // Copy modem initialization string (2 bytes
                                 // per iteration)
          *pszDestInit = *pszSrcInit;
          ...
          pszDestInit += 2;
                 ...
                 27) // Check for Escape key (27) to cancel selection
      {
...
char *getprintstring(const char *szStr, int iEditMode) {
  ...
  pszDest = modembuffer2.buffer; // Initialize destination pointer to
                                 // modembuffer2[1] (skip first byte)
  ...
  {
    uiCursorPos = (int)strlen(modembuffer2.buffer) +
                  1; // Get string length and add underscore cursor at end
    uiLastCharIdx = uiCursorPos - 1;
    modembuffer2.buffer[uiLastCharIdx] =
        '_'; // Place underscore cursor at end of string (ASCII 95 = '_')
    modembuffer2.buffer[uiLastCharIdx + 1] = '\0';
    if (stringwidth(modembuffer2.buffer) <=
        240) // Check if string width fits in 240 pixel limit
    {
      uiFinalLen = uiCursorPos;
    } else {
      pszTruncDest =
          &modembuffer2
               .buffer[3]; // String too wide - truncate with ellipsis (...)
      pszTruncSrc = szStr;
      memset(modembuffer2.buffer, 46,
             3); // Set first 3 chars to dots (0x2E2E2E = "...")
      do {
        byTruncChar1 =
            *pszTruncSrc; // Copy remainder of source string after ellipsis
      ...
      while (stringwidth(modembuffer2.buffer) >
             240) // Shrink string from beginning until it fits width limit
  ...
  } else if (stringwidth((char *)szStr) >
             240) // Not in edit mode - just check if string fits
  {
    uiStringLen = (int)strlen(modembuffer2.buffer);
    do {
      modembuffer2.buffer[uiStringLen - 1] =
          '.'; // Truncate from end with "..." until string fits
  ...
  return modembuffer2
      .buffer; // Return pointer to formatted string in modembuffer2[1]
}

//-------------------------------------------------------------------------------------------------
// 00077060
void display_essentials(int iSelectedItem, int iHighlightedItem,
                        int iEditMode) {
  ...
  iBlockIdx =
      iSelectedItem; // Store selected block index for gear selection display
  display_picture(
      scrbuf,
      front_vga[0]); // Display main background picture (modem config screen)
  display_block(scrbuf, front_vga[1], 3, head_x, head_y,
                0); // Display driver head icon at current position
  display_block(scrbuf, front_vga[6], 0, 36, 2,
                0); // Display UI element at position (36, 2)
  display_block(scrbuf, front_vga[5], iBlockIdx, -4, 247,
                0); // Display selected gear indicator at bottom left
  display_block(scrbuf, front_vga[5], game_type + 5, 135, 247,
                0); // Display game type indicator (5 + game_type offset)
  display_block(
      scrbuf, front_vga[4], 4, 76, 257,
      -1); // Display UI element at position (76, 257) with special color (-1)
  display_block(
      scrbuf, front_vga[6], 4, 62, 336,
      -1); // Display UI element at position (62, 336) with special color (-1)
  scale_text(front_vga[15], &language_buffer[5376], font1_ascii, font1_offsets,
             400, 75, 143, 1u, 200, 640); // Display main menu title text
  if (modembuffer)                        // Check if modem data is available
  { // Check if no modem is currently selected (negative index)
    if (current_modem <
        0) { // Set color for 'no modem' text based on highlight state
      if (iHighlightedItem)
        byTextColor2 = 0x8F;
      else
        byTextColor2 = 0xAB;
      scale_text(front_vga[15], &language_buffer[6848], font1_ascii,
                 font1_offsets, 400, 117, byTextColor2, 1u, 200,
                 640); // Display 'no modem selected' message
    } else {           // Set color for modem name based on highlight state
      if (iHighlightedItem)
        byTextColor1 = 0x8F;
      else
        byTextColor1 = 0xAB;
      scale_text(front_vga[15], &modembuffer[56 * current_modem], font1_ascii,
                 font1_offsets, 400, 117, byTextColor1, 1u, 200,
                 640); // Display selected modem name (56 bytes per modem entry)
    }
  }
  sprintf(szText, "COM %d",
          modem_port);       // Format COM port number into text buffer
  if (iHighlightedItem == 1) // Set colors for COM port label (item 1)
    byTextColor3 = 0xAB;
  else
    byTextColor3 = 0x8F;
  scale_text(front_vga[15], &language_buffer[5440], font1_ascii, font1_offsets,
             400, 135, byTextColor3, 2u, 200,
             640);           // Display COM port label text
  if (iHighlightedItem == 1) // Set colors for COM port value (item 1)
    byTextColor4 = 0xAB;
  else
    byTextColor4 = 0x8F;
  scale_text(front_vga[15], szText, font1_ascii, font1_offsets, 400, 135,
             byTextColor4, 0, 200,
             640);           // Display COM port number (e.g. 'COM 1')
  if (iHighlightedItem == 2) // Set colors for dial mode label (item 2)
    byTextColor5 = 0xAB;
  else
    byTextColor5 = 0x8F;
  scale_text(front_vga[15], &language_buffer[5504], font1_ascii, font1_offsets,
             400, 153, byTextColor5, 2u, 200,
             640); // Display dial mode label text
  ...
  {                // Set colors for 'TONE' mode display
    if (iHighlightedItem == 2)
      ...
      byTextColor6 = 0x8F;
    scale_text(front_vga[15], &language_buffer[5568], font1_ascii,
               font1_offsets, 400, 153, byTextColor6, 0, 200,
               640); // Display 'TONE' dialing mode text
  } else {           // Set colors for 'PULSE' mode display
    if (iHighlightedItem == 2)
      byTextColor7 = 0xAB;
    else
      byTextColor7 = 0x8F;
    scale_text(front_vga[15], &language_buffer[5632], font1_ascii,
               font1_offsets, 400, 153, byTextColor7, 0, 200,
               640); // Display 'PULSE' dialing mode text
  }
  if (iHighlightedItem == 3) // Set colors for modem init string label (item 3)
    byTextColor8 = 0xAB;
  else
    byTextColor8 = 0x8F;
  scale_text(front_vga[15], &language_buffer[5696], font1_ascii, font1_offsets,
             400, 171, byTextColor8, 2u, 200,
             640);           // Display modem initialization string label
  ...
  bIsInitEditMode =
      iHighlightedItem == 3 &&
      iEditMode ==
          1; // Check if init string is in edit mode (item 3 + edit flag)
  pszInitString = (char *)getprintstring(
      modem_initstring, bIsInitEditMode); // Get printable version of init
                                          // string (handles edit cursor)
  scale_text(front_vga[15], pszInitString, font1_ascii, font1_offsets, 400, 171,
             byInitTextColor, 0, 200,
             640);           // Display modem initialization string
  if (iHighlightedItem == 4) // Set colors for phone number label (item 4)
    byTextColor10 = 0xAB;
  else
    byTextColor10 = 0x8F;
  scale_text(front_vga[15], &language_buffer[5760], font1_ascii, font1_offsets,
             400, 189, byTextColor10, 2u, 200,
             640);           // Display phone number label text
  ...
  bIsPhoneEditMode =
      iHighlightedItem == 4 &&
      iEditMode ==
          1; // Check if phone number is in edit mode (item 4 + edit flag)
  pszPhoneString = (char *)getprintstring(
      modem_phone, bIsPhoneEditMode); // Get printable version of phone number
                                      // (handles edit cursor)
  scale_text(front_vga[15], pszPhoneString, font1_ascii, font1_offsets, 400,
             189, byPhoneTextColor, 0, 200, 640); // Display phone number string
  if (iHighlightedItem == 5) // Set colors for 'Accept' button (item 5)
    byTextColor12 = 0xAB;
  else
    byTextColor12 = 0x8F;
  scale_text(front_vga[15], &language_buffer[5824], font1_ascii, font1_offsets,
             400, 225, byTextColor12, 1u, 200,
             640);           // Display 'Accept' button text
  if (iHighlightedItem == 6) // Set colors for 'Cancel' button (item 6)
    byTextColor13 = 0xAB;
  else
    byTextColor13 = 0x8F;
  scale_text(front_vga[15], &language_buffer[5888], font1_ascii, font1_offsets,
             400, 243, byTextColor13, 1u, 200,
             640); // Display 'Cancel' button text
  if (no_16550)    // Check if 16550 UART error occurred
    scale_text(front_vga[15], &language_buffer[64 * no_16550], font1_ascii,
               font1_offsets, 400, 351, 231, 1u, 200,
               640); // Display 16550 error message (64 bytes per error code)
}

//-------------------------------------------------------------------------------------------------
// 00077560
void check16550(
    int iPort) { // Test if 16550 UART is available at the specified port
  // TODO
  // if (gss16550(iPort)) {
  if (true) {
  ...
  {
    no_16550 =
        110; // CD music available - set error code 110 (CD audio fallback)
  } else {
    no_16550 =
        109; // No 16550 and no CD music - set error code 109 (no audio support)
  }
