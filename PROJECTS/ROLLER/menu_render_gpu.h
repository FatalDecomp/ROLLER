...
// Fade system
void menu_render_gpu_begin_fade(MenuRendererGPU *renderer, int direction,
                                int durationFrames);
...
// Text rendering
void menu_render_gpu_text(MenuRendererGPU *renderer, int fontSlot,
...
// 3D mesh previews
void menu_render_gpu_load_car_mesh(MenuRendererGPU *renderer, int carIdx,
                                   const tColor *palette);
void menu_render_gpu_free_car_mesh(MenuRendererGPU *renderer);
void menu_render_gpu_draw_car_preview(MenuRendererGPU *renderer, float angle,
                                      float distance, int carYaw, int destX,
                                      int destY, int destW, int destH);

void menu_render_gpu_load_track_mesh(MenuRendererGPU *renderer,
                                     const tColor *palette);
void menu_render_gpu_free_track_mesh(MenuRendererGPU *renderer);
void menu_render_gpu_draw_track_preview(MenuRendererGPU *renderer,
                                        float cameraZ, int elevation, int yaw,
