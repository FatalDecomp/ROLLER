...
//-------------------------------------------------------------------------------------------------
// 000257E0
static void dodivide(float fX0_3D, float fY0_3D, float fZ0_3D, float fX1_3D,
                     float fY1_3D, float fZ1_3D, float fX2_3D, float fY2_3D,
                     float fZ2_3D, float fX3_3D, float fY3_3D, float fZ3_3D,
                     int iScreenX0, int iScreenY0, int iScreenX1, int iScreenY1,
                     int iScreenX2, int iScreenY2, int iScreenX3, int iScreenY3,
  ...
  while (2) {
    ...
    if (!iCullFlag) {
      ...
      if (!divtype && !flatpol) {
        // Calculate perimeter in screen space to determine if polygon is large
        // enough to subdivide
        polyxsize = abs(iScreenY3 - iScreenY2) + abs(iScreenY0 - iScreenY1) +
                    abs(iScreenX3 - iScreenX2) + abs(iScreenX0 - iScreenX1);
        polyysize = abs(iScreenY1 - iScreenY2) + abs(iScreenY0 - iScreenY3) +
                    abs(iScreenX1 - iScreenX2) + abs(iScreenX0 - iScreenX3);
      ...
      case 0: // No subdivision - render the polygon
        ...
        if (iPolyType < 1) {
          ...
          LABEL_111:
            if ((subpoly->iSurfaceType & SURFACE_FLAG_APPLY_TEXTURE) !=
                0) // SURFACE_FLAG_APPLY_TEXTURE
            {
              // Render textured pol with car texture
              // TODO is this correct?
              game_render_quad_screen(
                  g_pGameRenderer, subpoly,
                  game_render_get_texture_handle(g_pGameRenderer,
                                                 car_texmap[subpolytype - 3]),
                  NULL);
              // POLYTEX(
              //   (&horizon_vga)[*(&car_draw_order[15].iChunkIdx +
              //   subpolytype)],// offset into car_texmap subptr, subpoly,
              //   *(&car_draw_order[15].iChunkIdx + subpolytype),// offset into
              //   car_texmap gfx_size);
              goto LABEL_115;
            }
            pFrameBuf = subptr;
            pPolyParams = subpoly;
          LABEL_114:
            game_render_quad_screen(g_pGameRenderer, pPolyParams,
                                    TEXTURE_HANDLE_INVALID,
                                    NULL); // render flat pol
          LABEL_115:
            // Debug: draw pol outline if showsub is enabled
            if (showsub) {
              compout(subptr, subpoly->vertices[0].x, subpoly->vertices[0].y,
                      subpoly->vertices[1].x, subpoly->vertices[1].y,
                      0x9Fu); // 0x9F is light blue in PALETTE.PAL
              compout(subptr, subpoly->vertices[1].x, subpoly->vertices[1].y,
                      subpoly->vertices[2].x, subpoly->vertices[2].y, 0x9Fu);
              compout(subptr, subpoly->vertices[2].x, subpoly->vertices[2].y,
                      subpoly->vertices[3].x, subpoly->vertices[3].y, 0x9Fu);
              compout(subptr, subpoly->vertices[0].x, subpoly->vertices[0].y,
                      subpoly->vertices[1].x, subpoly->vertices[1].y, 0x9Fu);
            }
            return;
          }
          if ((pPolyParams->iSurfaceType & SURFACE_FLAG_APPLY_TEXTURE) ==
              0) // SURFACE_FLAG_APPLY_TEXTURE
            goto LABEL_106;
        } else if (iPolyType > 1) {
          if (iPolyType <= 2) {
          ...
          {
            game_render_quad_screen(
                g_pGameRenderer, pPolyParamsLocal,
        ...
        game_render_quad_screen(
            g_pGameRenderer, pPolyParamsLocal,
            game_render_get_texture_handle(g_pGameRenderer, 0), NULL);
        goto LABEL_115;
      case 1: // Horiz subdivision only
        ...
        if (fY_mid01 < 1.0 || isnan(fY_mid01)) {
          ...
          dProjX = (dViewDist * fX_mid01 * dInvZ + (double)xbase) *
                   dScreenScale * 0.015625;
          dProjX = round(dProjX); //_CHP();
          iScreenX_mid12 = (int)dProjX;
          dProjY = dScreenScale *
                   (199.0 - dInvZ * (dViewDist * fMidZ_quad) - (double)ybase) *
                   0.015625;
        ...
        if (fZ_mid12 < 1.0 || isnan(fZ_mid12)) {
          ...
          dProjX_1 = (dViewDist_1 * fX_mid12 * dInvZ_1 + (double)xbase) *
                     dScreenScale_1 * 0.015625;
          dProjX_1 = round(dProjX_1); //_CHP();
          iScreenX_mid23 = (int)dProjX_1;
          dProjY_1 =
              dScreenScale_1 *
              (199.0 - dInvZ_1 * (dViewDist_1 * fY_mid12) - (double)ybase) *
              0.015625;
        ...
        if ((fliptype & 1) != 0) {
          ...
          dodivide(fX_mid01, fMidZ_quad, fY_mid01, fX1_3D, fY1_3D, fZ1_3D,
                   fX2_3D, fY2_3D, fZ2_3D, fX_mid12, fY_mid12, fZ_mid12,
                   iScreenX_mid12, iScreenX_subdiv1, iScreenCenterY, iScreenY1,
                   iScreenX2, iScreenY2, iScreenX_mid23, iScreenY_mid12,
                   iTexWid + iTexU, iTexV, iTexWid, iTexHgt);
          ...
          dodivide(fX_mid01, fMidZ_quad, fY_mid01, fX1_3D, fY1_3D, fZ1_3D,
                   fX2_3D, fY2_3D, fZ2_3D, fX_mid12, fY_mid12, fZ_mid12,
                   iScreenX_mid12, iScreenX_subdiv1, iScreenX1, iScreenY1,
                   iScreenX2, iScreenY2, iScreenX_mid23, iScreenY_mid12, iTexU,
                   iTexV, iTexWid, iTexHgt);
      ...
      case 2: // Vertical flip
        ...
        if (fZ_mid12_1 < 1.0 || isnan(fZ_mid12_1)) {
          ...
          dProjX_2 = (dViewDist_2 * fX_mid12_1 * dInvZ_2 + (double)xbase) *
                     dScreenScale_2 * 0.015625;
          dProjX_2 = round(dProjX_2); //_CHP();
          iScreenX_mid12_1 = (int)dProjX_2;
          dProjY_2 =
              dScreenScale_2 *
              (199.0 - dInvZ_2 * (dViewDist_2 * fY_mid12_1) - (double)ybase) *
              0.015625;
        ...
        if (fZ_mid30 < 1.0 || isnan(fZ_mid30)) {
          ...
          dProjX_3 = (dViewDist_3 * fX_mid30 * dInvZ_3 + (double)xbase) *
                     dScreenScale_3 * 0.015625;
          dProjX_3 = round(dProjX_3); //_CHP();
          iScreenX_mid30 = (int)dProjX_3;
          dProjY_3 =
              dScreenScale_3 *
              (199.0 - dInvZ_3 * (dViewDist_3 * fY_mid30) - (double)ybase) *
              0.015625;
        ...
        if ((fliptype & 2) != 0) {
          ...
          dodivide(fX0_3D, fY0_3D, fZ0_3D, fX1_3D, fY1_3D, fZ1_3D, fX_mid12_1,
                   fY_mid12_1, fZ_mid12_1, fX_mid30, fY_mid30, fZ_mid30,
                   iScreenX0, iScreenY0, iScreenX1, iScreenY1, iScreenX_mid12_1,
                   iScreenY_mid12_1, iScreenX_mid30, iScreenY_mid30, iTexU,
      ...
      case 3:     // Both horiz and vert subdivision
        ...
        if (fY_mid01 < 1.0 || isnan(fY_mid01)) {
          ...
          dProjX_4 = (dViewDist_4 * fX_mid01 * dInvZ_4 + (double)xbase) *
                     dScreenScale_4 * 0.015625;
          dProjX_4 = round(dProjX_4); //_CHP();
          iScreenX_mid01 = (int)dProjX_4;
          dProjY_4 =
              dScreenScale_4 *
              (199.0 - dInvZ_4 * (dViewDist_4 * fMidZ_quad) - (double)ybase) *
              0.015625;
        ...
        if (fZ_mid12_2 < 1.0 || isnan(fZ_mid12_2)) {
          ...
          dProjX_5 = (dViewDist_5 * fX_mid12_2 * dInvZ_5 + (double)xbase) *
                     dScreenScale_5 * 0.015625;
          dProjX_5 = round(dProjX_5); //_CHP();
          iScreenX_mid12_2 = (int)dProjX_5;
          dProjY_5 =
              dScreenScale_5 *
              (199.0 - dInvZ_5 * (dViewDist_5 * fY_mid12_2) - (double)ybase) *
              0.015625;
        ...
        if (fZ_mid23 < 1.0 || isnan(fZ_mid23)) {
          ...
          dProjX_6 = (dViewDist_6 * fX_mid23 * dInvZ_6 + (double)xbase) *
                     dScreenScale_6 * 0.015625;
          dProjX_6 = round(dProjX_6); //_CHP();
          iScreenX_mid23_1 = (int)dProjX_6;
          dProjY_6 =
              dScreenScale_6 *
              (199.0 - dInvZ_6 * (dViewDist_6 * fY_mid23) - (double)ybase) *
              0.015625;
        ...
        if (fZ_mid30_1 < 1.0 || isnan(fZ_mid30_1)) {
          ...
          dProjX_7 = (dViewDist_7 * fX_mid30_1 * dInvZ_7 + (double)xbase) *
                     dScreenScale_7 * 0.015625;
          dProjX_7 = round(dProjX_7); //_CHP();
          iScreenX_mid30_1 = (int)dProjX_7;
          dProjY_7 =
              dScreenScale_7 *
              (199.0 - dInvZ_7 * (dViewDist_7 * fY_mid30_1) - (double)ybase) *
              0.015625;
        ...
        if (fZ_center < 1.0 || isnan(fZ_center)) {
          ...
          dProjX_8 = (dViewDist_8 * fX_center * dInvZ_8 + (double)xbase) *
                     dScreenScale_8 * 0.015625;
          dProjX_8 = round(dProjX_8); //_CHP();
          iScreenX_center = (int)dProjX_8;
          dProjY_8 =
              dScreenScale_8 *
              (199.0 - dInvZ_8 * (dViewDist_8 * fY_center) - (double)ybase) *
              0.015625;
        ...
        case 0: // no flip
          // Top-left quadrant
          dodivide(fX_mid01, fMidZ_quad, fY_mid01, fX1_3D, fY1_3D, fZ1_3D,
                   fX_mid12_2, fY_mid12_2, fZ_mid12_2, fX_center, fY_center,
                   fZ_center, iScreenX_mid01, iScreenY_mid01, iScreenX1,
                   iScreenY1, iScreenX_mid12_2, iScreenY_mid23, iScreenX_center,
                   iScreenY_center, iTexU, iTexV, iTexWid, iTexHgt);

          // Top-right quadrant
          dodivide(fX0_3D, fY0_3D, fZ0_3D, fX_mid01, fMidZ_quad, fY_mid01,
                   fX_center, fY_center, fZ_center, fX_mid30_1, fY_mid30_1,
                   fZ_mid30_1, iScreenX0, iScreenY0, iScreenX_mid01,
                   iScreenY_mid01, iScreenX_center, iScreenY_center,
                   iScreenX_mid30_1, iScreenY_mid30_2, iTexU_right, iTexV,
                   iTexWid, iTexHgt);

          iScreenY1 = iScreenY_center;

          // bottom-right quadrant
          dodivide(fX_center, fY_center, fZ_center, fX_mid12_2, fY_mid12_2,
                   fZ_mid12_2, fX2_3D, fY2_3D, fZ2_3D, fX_mid23, fY_mid23,
                   fZ_mid23, iScreenX_center, iScreenY_center, iScreenX_mid12_2,
                   iScreenY_mid23, iScreenX2, iScreenY2, iScreenX_mid23_1,
                   iScreenY_mid30_1, iTexU, iTexV_bottom, iTexWid, iTexHgt);
        ...
        case 1: // horiz flip
          ...
          dodivide(fX0_3D, fY0_3D, fZ0_3D, fX_mid01, fMidZ_quad, fY_mid01,
                   ...
                   fZ_mid30_1, iScreenX0, iScreenY0, iScreenX_mid01,
                   iScreenY_mid01, iScreenX_center, iScreenY_center,
                   iScreenX_mid30_1, iScreenY_mid30_2,
                   iTexU, // left side tex
                   iTexV, iTexWid, iTexHgt);
          iScreenX1 = iScreenX_center;
          ...
          dodivide(fX_center, fY_center, fZ_center, fX_mid12_2, fY_mid12_2,
                   ...
                   fZ_mid23, iScreenX_center, iScreenY_center, iScreenX_mid12_2,
                   iScreenY_mid23, iScreenX2, iScreenY2, iScreenX_mid23_1,
                   iScreenY_mid30_1, iTexU_right, iTexV_bottom, iTexWid,
                   iTexHgt);
        ...
        case 2: // vertical flip
          ...
          dodivide(fX0_3D, fY0_3D, fZ0_3D, fX_mid01, fMidZ_quad, fY_mid01,
                   ...
                   fZ_mid30_1, iScreenX0, iScreenY0, iScreenX_mid01,
                   iScreenY_mid01, iScreenX_center, iScreenY_center,
                   iScreenX_mid30_1, iScreenY_mid30_2, iTexU_right,
...
static void subdivide(uint8 *pDest, tPolyParams *polyParams, float fX0_3D,
                      float fY0_3D, float fZ0_3D, float fX1_3D, float fY1_3D,
                      float fZ1_3D, float fX2_3D, float fY2_3D, float fZ2_3D,
  ...
  if ((polyParams->iSurfaceType & SURFACE_FLAG_SKIP_RENDER) !=
  ...
  fliptype = (polyParams->iSurfaceType & SURFACE_FLAG_FLIP_HORIZ) !=
             0; // SURFACE_FLAG_FLIP_HORIZ
  if ((polyParams->iSurfaceType & SURFACE_FLAG_FLIP_VERT) !=
      0)           // SURFACE_FLAG_FLIP_VERT
  ...
  flatpol = ((subpoly->iSurfaceType & SURFACE_FLAG_APPLY_TEXTURE) != 0) -
            1; // SURFACE_FLAG_APPLY_TEXTURE
...
LABEL_10:
  ...
  dodivide(fX0_3D, fY0_3D, fZ0_3D, fX1_3D, fY1_3D, fZ1_3D, fX2_3D, fY2_3D,
           fZ2_3D, fX3_3D, fY3_3D, fZ3_3D, iX0, iY0, iX1, iY1, iX2, iY2, iX3,
           iY3, 0, 0, tex_wid, tex_hgt);
...
static void scene_render_sw_bind_target(SceneRendererSoftware *sw) {
  if (!sw || !sw->targetBuffer)
    return;
  screen_pointer =
      sw->targetBuffer + sw->viewportY * sw->targetStride + sw->viewportX;
}
...
void scene_render_sw_quad_world_legacy(SceneRendererSoftware *sw,
  ...
  int useCloudProjection =
      options.subdivideType == SCENE_RENDER_SUBDIVIDE_TYPE_CLOUD;
  int subpolyType;
  ...
  if (subpolyType == SUBPOLY_BUILDING) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      iVx[i] = (int)(dx * proj->view[0][0] + dy * proj->view[1][0] +
                     dz * proj->view[2][0]);
      iVy[i] = (int)(dx * proj->view[0][1] + dy * proj->view[1][1] +
                     dz * proj->view[2][1]);
      iVz[i] = (int)(dx * proj->view[0][2] + dy * proj->view[1][2] +
                     dz * proj->view[2][2]);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      float fVx = (float)(dx * proj->view[0][0] + dy * proj->view[1][0] +
                          dz * proj->view[2][0]);
      float fVy = (float)(dx * proj->view[0][1] + dy * proj->view[1][1] +
                          dz * proj->view[2][1]);
      double dCameraZ =
          dx * proj->view[0][2] + dy * proj->view[1][2] + dz * proj->view[2][2];
      float fVz = (float)dCameraZ;
      float fProjectedZ = fVz;
      if (fProjectedZ < 80.0f)
        fProjectedZ = 80.0f;
      ...
      if (useCarProjection || useCloudProjection) {
        ...
        yp = (int)round(dInvZ * (viewDist * (double)fVy) +
                        (double)proj->centerY);
      }
      ...
      subVx[i] = fVx;
      subVy[i] = fVy;
      subVz[i] = (useCarProjection || useCloudProjection)
                     ? fVz
                     : (float)((int)round(dCameraZ));
      directVz[i] = subVz[i];
    }
  }

  if (subpolyType == SUBPOLY_WALL)
    set_starts(1u);
  ...
  if (useDirect && handle == SCENE_TEXTURE_HANDLE_INVALID) {
    ...
    POLYTEX(slot->pixels, screen_pointer, &poly, slot->tex_idx,
            slot->texHalfRes);
  } else {
    subdivide(screen_pointer, &poly, subVx[0], subVy[0], subVz[0], subVx[1],
              subVy[1], subVz[1], subVx[2], subVy[2], subVz[2], subVx[3],
              subVy[3], subVz[3], subpolyType, proj->texHalfRes);
  }
