...
static void LogDiscoveryAddress(const char *szMessage,
                                const int32 pAddress[4]) {
  char szAddr[32];
  ROLLERCommsFormatAddr((const tROLLERNetAddr *)pAddress, szAddr,
                        sizeof(szAddr));
...
void send_resync(int iFrameNumber) {
  if (network_on) {
    ...
    ROLLERCommsSendData(&p_header, sizeof(tSyncHeader), &resync, sizeof(int32),
                        master);
  }
}
...
void send_quit() {
  ...
  if (network_on) {
    ...
    if (wConsoleNode == master) {
      for (i = 0; i < network_on; ++i) {
        if (i != wConsoleNode) {
          while (!ROLLERCommsSendData(&p_header, sizeof(tSyncHeader), p_data, 0,
                                      i))
            UpdateSDL(); // added by ROLLER
        }
      }
...
void send_ready() {
  ...
  if (network_on) {
    ...
    if (wConsoleNode != master || player_ready[master]) {
      do {
        //_disable();
        iDataSent = ROLLERCommsSendData(&p_header, sizeof(tSyncHeader), p_data,
                                        0, master);
        //_enable();
        UpdateSDL(); // added by ROLLER
      } while (!iDataSent);
...
void send_record_to_master(int iRecordIdx) {
  if (network_on) {
    ...
    strncpy(p_record.szRecordName, RecordNames[iRecordIdx],
            sizeof(p_record.szRecordName));
    ...
    while (!ROLLERCommsSendData(&p_header, sizeof(tSyncHeader), &p_record,
                                sizeof(tRecordPacket), master))
      UpdateSDL(); // added by ROLLER
  }
}
  ...
  if (network_on) {
    ...
    if (*szNetworkMessage) {
      ...
      if (iNode == -1) {
        ...
        if (network_on > 0) {
          ...
          do {
            if (iNodeIdx != wConsoleNode && net_players[iNetPlayers]) {
              while (!ROLLERCommsSendData(&p_header, sizeof(tSyncHeader),
                                          p_data, sizeof(p_data), iNodeIdx))
                UpdateSDL(); // added by ROLLER
            }
            ++iNetPlayers;
            ++iNodeIdx;
        ...
        while (!ROLLERCommsSendData(&p_header, sizeof(tSyncHeader), p_data,
                                    sizeof(p_data), iNode))
          UpdateSDL(); // added by ROLLER
      }
    } else {
...
void send_seed(int iRandomSeed) {
  if (network_on) {
    ...
    p_header.byConsoleNode = (uint8)wConsoleNode;
    SDL_Log("[NET-START] send seed=%d from node=%d to %d nodes", iRandomSeed,
            (int)wConsoleNode, network_on);
    for (int i = 0; i < network_on; ++i) {
      if (i != wConsoleNode) {
...
void send_single(uint32 uiData) {
  ...
  // TODO network
  ROLLERCommsSendData(&p_header, sizeof(p_header), &slave_data,
                      sizeof(tDataPacket), master);
...
void send_pause() {
  ...
  if (network_on) {
    ...
    bPaused = paused == 0;
    while (!ROLLERCommsSendData(&syncHeader, sizeof(tSyncHeader), &bPaused,
                                sizeof(bool), master))
      UpdateSDL(); // added by ROLLER
  }
}
...
void send_slot() {
  ...
  if (network_on) {
    ...
    while (!ROLLERCommsSendData(&syncHeader, sizeof(tSyncHeader), &iSlot,
                                sizeof(int), 21))
      UpdateSDL(); // added by ROLLER
  }
}
    ...
    if (network_on > 0) {
      ...
      do {
        if (iNode != master && net_players[iNetPlayerIdx]) {
          while (!ROLLERCommsSendData(&syncHeader, sizeof(tSyncHeader),
                                      &bPaused, sizeof(bool), iNode))
            UpdateSDL(); // added by ROLLER
        }
        ++iNetPlayerIdx;
        ++iNode;
...
void send_multiple() {
  ...
  if (network_on) {
    ...
    if (net_type) {
      iSendCopyMultiple = -1;
      if (memcmp((const char *)&copy_multiple[((int16)writeptr - 1) & 0x1FF]
                                             [player_to_car[0]],
                 (const char *)&copy_multiple[writeptr][player_to_car[0]], 4))
        iSendCopyMultiple = 0;
      pCopyMultiple = copy_multiple[((int16)writeptr - 1) & 0x1FF];
      if (memcmp((const char *)&pCopyMultiple[player_to_car[1]],
                 (const char *)&copy_multiple[writeptr][player_to_car[1]], 4))
        iSendCopyMultiple = 0;
      if (!frame_number)
        iSendCopyMultiple = 0;
        ...
        if (iNode >= 0) {
          ...
          do {
            ...
            if (iNode == master || !net_players[iNode4])
              ...
              iNodeIsValid = ROLLERCommsSendData(&p_header, sizeof(tSyncHeader),
                                                 pCopyMultiple, 1, iNode);
            }
            if (iNodeIsValid) {
              --iNode4;
              --iNode;
      ...
      if (iNode >= 0) {
        ...
        do {
          if (!iNodeIsValid)
            break;
          iNodeIsValid =
              iNode == master || !net_players[iNode2]
                  ? -1
                  : ROLLERCommsSendData(&p_header, sizeof(tSyncHeader),
                                        test_multiple, 64, iNode);
          if (iNodeIsValid) {
            --iNode2;
            --iNode;
          }
  ...
  if (network_on && !net_quit) {
    ...
    while (ROLLERCommsGetHeader(&in_header, 12, &pPacket)) {
      ...
      case PACKET_ID_PLAYER_INFO:
        ROLLERCommsGetBlock(pPacket, &playerInfoPacket,
                            sizeof(tPlayerInfoPacket));
        Players_Cars[in_header.byConsoleNode] = playerInfoPacket.iPlayerCar;
        check_cars();
        name_copy(player_names[in_header.byConsoleNode],
                  playerInfoPacket.szPlayerName);
        ...
        manual_control[in_header.byConsoleNode] =
            playerInfoPacket.iManualControl;
        ROLLERCommsPostListen();
        continue;
  ...
  if (network_on) {
    while (ROLLERCommsGetHeader(&in_header, sizeof(tSyncHeader), &pData)) {
      ...
      case PACKET_ID_TRANSMIT_INIT:
        ROLLERCommsGetBlock(pData, &transmitInitPacket,
      ...
      case PACKET_ID_SINGLE:
        if (byConsoleNode < 0 || byConsoleNode >= MAX_CARS)
          goto LABEL_31;
        if (car_to_player[byConsoleNode] < 0 ||
            car_to_player[byConsoleNode] >= network_on)
          goto LABEL_31;
        if (!net_players[car_to_player[byConsoleNode]])
          goto LABEL_31;
        ROLLERCommsGetBlock(pData, &slave_data, 6);
        copy_multiple[writeptr][in_header.byConsoleNode].uiFullData =
            slave_data.uiData;
      ...
      case PACKET_ID_PAUSE:
        ...
        continue;
      case PACKET_ID_PLAYER_INFO:
        ROLLERCommsGetBlock(pData, &playerInfoPacket,
                            sizeof(tPlayerInfoPacket));
        Players_Cars[in_header.byConsoleNode] = playerInfoPacket.iPlayerCar;
        check_cars();
        name_copy(player_names[in_header.byConsoleNode],
...
void do_sync_stuff() {
  ...
  if (net_type) {
    do {
      ...
      // Send data
      iSendDataSuccess =
          ROLLERCommsSendData(&in_header, 12, test_mini, 8, syncnode);

      // Advance state
      iSyncPtr = ((uint16)syncptr + 1) &
                 0x1FF; // ensure this index doesn't go beyond 511
      --syncleft;
      ++syncframe;
    ...
    while (syncleft) {
      ...
      // Send data
      iSendDataSuccess2 =
          ROLLERCommsSendData(&in_header, 12, test_multiple, 64, syncnode);
...
int TransmitInit() {
  ...
  if (network_on) {
    ...
    int iKnownPeerSuccess =
        ROLLERCommsSendData(&header, sizeof(tSyncHeader), &initPacket,
                            sizeof(tTransmitInitPacket), 21);
    int iBroadcastSuccess = ROLLERCommsBroadcastData(
        &header, sizeof(tSyncHeader), &initPacket, sizeof(tTransmitInitPacket),
        ROLLER_DEFAULT_PORT);
...
void CheckNewNodes() {
  ...
  pTestSeed = &test_seed; // Initialize pointer to test seed for random number
                          // generation

  while (ROLLERCommsGetHeader(
      &syncHeader, sizeof(tSyncHeader),
      &pPacket2)) // Main packet processing loop - handle incoming network
                  // packets from other nodes
  { // Handle PACKET_ID_TRANSMIT_INIT (0x686C6361) - player initialization
    // packets
    ...
    case PACKET_ID_TRANSMIT_INIT: // PACKET_ID_TRANSMIT_INIT
      ...
      ROLLERCommsGetBlock(pPacket2, &transmitInitPacket,
                          sizeof(tTransmitInitPacket));
      ...
      if (network_slot >= 0) // Process init packet when we are already
                             // connected (network_slot >= 0)
      {
        ...
        test = 4;
        iFoundNodeOffset = 0;
        for (iNetworkNodeIndex = 0; iNetworkNodeIndex < network_on;
             ++iNetworkNodeIndex) // Search through existing network nodes to
                                  // find matching address
        {

          // This byte-by-byte comparison could be simplified to:
          iAddressMatchFlag2 =
              memcmp(&address[iNetworkNodeIndex * 4],
                     &transmitInitPacket.address, sizeof(_NETNOW_NODE_ADDR));
          ...
          // do {                                   // Compare network addresses
          // byte by byte to detect duplicate nodes
        ...
        if (!iFoundNodeOffset) {
          ...
          memcpy(&address[network_on * 4], &transmitInitPacket.address,
                 sizeof(_NETNOW_NODE_ADDR));
          // pNetNowNodeAddr3 = &address[network_on * 4];// Manual address
          // copying - could be simplified to: memcpy(&address[network_on * 4],
          // &transmitInitPacket.address, sizeof(_NETNOW_NODE_ADDR));
          //*(_DWORD *)pNetNowNodeAddr3->sIPX.sInternetAddr.bNetwork = *(_DWORD
          //*)transmitInitPacket.address.sIPX.sInternetAddr.bNetwork;
          // pNetNowNodeAddr3 = (_NETNOW_NODE_ADDR *)((char *)pNetNowNodeAddr3 +
          // 4);
          //*(_DWORD *)pNetNowNodeAddr3->sIPX.sInternetAddr.bNetwork = *(_DWORD
          //*)&transmitInitPacket.address.sNETBIOS.bNode[4];
          //*(_QWORD *)&pNetNowNodeAddr3->sNETBIOS.bNode[4] = *(_QWORD
          //*)&transmitInitPacket.address.sNETBIOS.bNode[8];
          ...
          if (my_age < transmitInitPacket.iMyAge) {
            ...
            // } while (szTxInitPacketNamesItr2 != (char *)&messagePacket);//
            // messagePacket is immediately after transmitInitPacket
            ...
            if ((transmitInitPacket.iLevelFlags & 0x100) !=
                0) // Process level flags and update cheat mode settings
                   // accordingly
            {
            ...
            if ((level & 0x1000) != 0) {
              if ((level & 0x4000) != 0) {
                if ((cheat_mode & 0x2000) == 0) {
                  cheat_mode |=
                      (CHEAT_MODE_50HZ_TIMER | CHEAT_MODE_100HZ_TIMER);
          ...
          players_waiting = 0;
          for (i = 0; i <= network_on;
               ++i) // Count how many players have pressed start and are waiting
          {
            if (player_started[i])
              ++players_waiting;
          }
        ...
        LABEL_24:
          if (!transmitInitPacket.iNetworkChampOn) {
            ...
            if (iNumGamersInSlot > 0) {
              // szGamerAddressPtr =
              // gamers_address[iTxInitPacketNetworkSlotMinus1];
              iMaxGamerIndex = iNumGamersInSlot + 678;
              do {

                iAddressMatchFlag = memcmp(
                    &gamers_address[iTxInitPacketNetworkSlotMinus1]
                                   [iGamerIndex],
                    &transmitInitPacket.address, sizeof(_NETNOW_NODE_ADDR));
            ...
            if (!transmitInitPacket.iNetworkChampOn) {
              ...
              if (iFoundGamerIndex) {
                name_copy(&szGamerNamesPtr[9 * iFoundGamerIndex - 6102],
                          transmitInitPacket.szPlayerName);
              } else {
                name_copy(&szGamerNamesPtr[9 * iNumGamersInSlot],
                          transmitInitPacket.szPlayerName);
                pNetNowNodeAddr4 =
                    &gamers_address[iSlotIndex][iNumGamersInSlot];
                iSlotIndex2 = iTxInitPacketNetworkSlotMinus1;

                memcpy(&gamers_address[iSlotIndex][iNumGamersInSlot],
                       &transmitInitPacket.address, sizeof(_NETNOW_NODE_ADDR));
                //*(_DWORD *)pNetNowNodeAddr4->sIPX.sInternetAddr.bNetwork =
                //*(_DWORD
                //*)transmitInitPacket.address.sIPX.sInternetAddr.bNetwork;
                // pNetNowNodeAddr4 = (_NETNOW_NODE_ADDR *)((char
                // *)pNetNowNodeAddr4 + 4);
                //*(_DWORD *)pNetNowNodeAddr4->sIPX.sInternetAddr.bNetwork =
                //*(_DWORD *)&transmitInitPacket.address.sNETBIOS.bNode[4];
                //*(_QWORD *)&pNetNowNodeAddr4->sNETBIOS.bNode[4] = *(_QWORD
                //*)&transmitInitPacket.address.sNETBIOS.bNode[8];
    ...
    case PACKET_ID_QUIT: // PACKET_ID_QUIT
      net_quit = -1;     // Handle PACKET_ID_QUIT (0x686C6364) - player
                         // disconnection notification
      ROLLERCommsPostListen();
      continue;
    case PACKET_ID_READY:                         // PACKET_ID_READY
      if (player_ready[syncHeader.byConsoleNode]) // Handle PACKET_ID_READY
                                                  // (0x686C6367) - player ready
                                                  // status
        goto LABEL_40;
      ++active_nodes;
    ...
    case PACKET_ID_SEED: // PACKET_ID_SEED
      ROLLERCommsGetBlock(pPacket2, &test_seed,
                          4); // Handle PACKET_ID_SEED (0x686C6368) -
                              // synchronize random seed across network
      ROLLERsrand(*pTestSeed);
      random_seed = *pTestSeed;
      received_seed = -1;
      SDL_Log("[NET-START] received seed=%d on node=%d", random_seed,
              (int)wConsoleNode);
      ROLLERCommsPostListen();
      continue;
    case PACKET_ID_PLAYER_INFO: // PACKET_ID_PLAYER_INFO
      ROLLERCommsGetBlock(pPacket2, &playerInfoPacket,
                          40); // Handle PACKET_ID_PLAYER_INFO (0x686C636A) -
      ...
      manual_control[syncHeader.byConsoleNode] =
          playerInfoPacket.iManualControl;
      goto LABEL_40;
    case PACKET_ID_RECORD: // PACKET_ID_RECORD
      ++received_records;  // Handle PACKET_ID_RECORD (0x686C636B) - update lap
                           // record if faster than current
      ...
      if (recordPacket.fRecordLap < (double)RecordLaps[TrackLoad]) {
        ...
        // for (j = 0; j < 9; *((_BYTE *)&fudge_wait + 9 * TrackLoad + j + 3) =
        // recordPacket.szRecordName[j - 1])// fudge_wait is offset into
        // RecordNames
        //   ++j;
      }
      goto LABEL_40;
    ...
    case PACKET_ID_SLOT: // PACKET_ID_SLOT
      ...
      //*(_DWORD *)&gamers_address[3][15].sNETBIOS.bNode[4 *
      // syncHeader.byConsoleNode + 12] = -2;
      ROLLERCommsPostListen();
      continue;
    ...
    case PACKET_ID_MESSAGE: // PACKET_ID_MESSAGE
      ROLLERCommsGetBlock(pPacket2, &messagePacket,
                          48); // Handle PACKET_ID_MESSAGE (0x686C6373) -
                               // receive and process chat messages
      iNode = syncHeader.byConsoleNode;
      if (iNode != ROLLERCommsNetAddrToNode(address) &&
        ...
        // for (iMsgCopyIndex = 0; iMsgCopyIndex < 32;
        // LoadCarTextures_variable_1[iMsgCopyIndex] =
        // transmitInitPacket.default_names[15][iMsgCopyIndex + 8])
        //   ++iMsgCopyIndex;
      }
      goto LABEL_40;
    default:
...
void FoundNodes() {
  if (!master)
    return; // Only process if we're the master
  ...
  // The lobby address table is discovery-ordered with our own address in slot
  // 0, while the comms node table is sorted.  Remap player configuration by
  // address so player indices match sorted node indices before the race car
  // mapping runs.
  for (int i = 0; i < network_on; i++) {
    Players_Cars[i] = -1;
    manual_control[i] = 0;
  ...
  for (int i = 0; i < network_on; i++) {
    SDL_Log("[NET]   player[%d]: car=%d manual=%d started=%d%s\n", i,
            Players_Cars[i], manual_control[i], player_started[i],
            i == wConsoleNode ? " <-- self" : "");
  }
...
void SendPlayerInfo() {
  ...
  if (network_on) {
    ...
    for (int i = 0; i < network_on; ++i) {
      if (wConsoleNode != i) {
        while (!ROLLERCommsSendData(&p_header, sizeof(tSyncHeader), &playerInfo,
                                    sizeof(tPlayerInfoPacket), i))
          UpdateSDL(); // added by ROLLER
      }
    }
  }
...
void SendAMessage() {
  ...
  if (!broadcast_mode && send_message_to >= 0) {
    ...
    strncpy(messagePacket.szMessage, rec_mes_buf,
            sizeof(messagePacket.szMessage));
    ...
    if (ROLLERCommsSendData(&syncHeader, sizeof(tSyncHeader), &messagePacket,
                            sizeof(tMessagePacket), iNode))
      send_status = 18;
    else
      send_status = -18;
    send_message_to = -1;
...
void remove_messages(int iClear) {
  ...
  do {
    ...
    UpdateSDL();             // added by ROLLER
      ...
      &in_header, sizeof(tSyncHeader),
      &pPacketData)); // continue until there are no more messages
...
void send_here(int iNode, int iFrame) {
  ...
  if (network_on) {
    ...
    if (iNode == master) {
      for (i = 0; i < network_on; ++i) {
        if (i != master) {
          ROLLERCommsSendData(&syncHeader, sizeof(tSyncHeader), &iData,
                              sizeof(int), i);
        }
      }
    } else {
      ROLLERCommsSendData(&syncHeader, sizeof(tSyncHeader), &iData, sizeof(int),
                          master);
    }
  }
}
