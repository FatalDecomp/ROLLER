...
RenderQueue3D *render_queue_3d_global(void) { return &g_RenderQueue3D; }

//-------------------------------------------------------------------------------------------------

void render_queue_3d_clear(RenderQueue3D *pQueue) { pQueue->count = 0; }
...
const RenderCommand3D *render_queue_3d_command_at(const RenderQueue3D *pQueue,
                                                  int iIndex) {
...
int render_queue_3d_count(const RenderQueue3D *pQueue) { return pQueue->count; }
...
tTrackZOrderEntry *render_queue_3d_add_building(RenderQueue3D *pQueue,
                                                int iBuildingIdx,
                                                float fZDepth) {
  tTrackZOrderEntry *pEntry = render_queue_3d_add(
      pQueue, RENDER_QUEUE_3D_BUILDING_LEGACY_PRIORITY, iBuildingIdx, fZDepth);
  if (pEntry != NULL) {
...
tTrackZOrderEntry *
render_queue_3d_add_car(RenderQueue3D *pQueue, int iCarIdx, float fZDepth,
                        const GameRenderCarPose *pPose,
                        const GameRenderCarOptions *pOptions) {
  tTrackZOrderEntry *pEntry = render_queue_3d_add(
      pQueue, RENDER_QUEUE_3D_CAR_LEGACY_PRIORITY, iCarIdx, fZDepth);
  if (pEntry != NULL) {
    RenderCommand3D *pCommand = &pQueue->commands[pQueue->count - 1];
...
tTrackZOrderEntry *render_queue_3d_add_start_light(RenderQueue3D *pQueue,
                                                   ...
                                                   float fZDepth) {
  tTrackZOrderEntry *pEntry = render_queue_3d_add(
      pQueue, RENDER_QUEUE_3D_START_LIGHT_LEGACY_PRIORITY, iLightIdx, fZDepth);
  if (pEntry != NULL) {
    RenderCommand3D *pCommand = &pQueue->commands[pQueue->count - 1];
...
int render_queue_3d_compare_legacy_z_order(const void *pCommand1,
                                           const void *pCommand2) {
...
static int render_queue_3d_compare_sort_entries(const void *pCommand1,
                                                const void *pCommand2) {
  const RenderQueue3DSortEntry *pSortEntry1 =
      (const RenderQueue3DSortEntry *)pCommand1;
  const RenderQueue3DSortEntry *pSortEntry2 =
      (const RenderQueue3DSortEntry *)pCommand2;
  return render_queue_3d_compare_legacy_z_order(&pSortEntry1->entry,
                                                &pSortEntry2->entry);
}
...
void render_queue_3d_sort(RenderQueue3D *pQueue) {
  ...
  pSortEntries =
      (RenderQueue3DSortEntry *)malloc(sizeof(*pSortEntries) * pQueue->count);
  if (pSortEntries == NULL) {
    qsort(pQueue->entries, pQueue->count, sizeof(pQueue->entries[0]),
          render_queue_3d_compare_legacy_z_order);
  ...
  for (iCommandIndex = 0; iCommandIndex < pQueue->count; ++iCommandIndex) {
    ...
    pSortEntries[iCommandIndex].has_typed_command =
        pQueue->has_typed_command[iCommandIndex];
  }

  qsort(pSortEntries, pQueue->count, sizeof(pSortEntries[0]),
        render_queue_3d_compare_sort_entries);
