...
void calculatetransform(int iTrackChunkIdx, int iDirection, int iElevation,
  ...
  if (iTrackChunkIdx != -1) {
    ...
    viewx = pData->pointAy[0].fX * fViewX - pData->pointAy[3].fX +
            pData->pointAy[0].fY * fViewY + pData->pointAy[0].fZ * fViewZ;
    viewy = pData->pointAy[1].fX * fViewX - pData->pointAy[3].fY +
            pData->pointAy[1].fY * fViewY + pData->pointAy[1].fZ * fViewZ;
    viewz = fViewY * pData->pointAy[2].fY + fViewX * pData->pointAy[2].fX -
            pData->pointAy[3].fZ + fViewZ * pData->pointAy[2].fZ;
    v21 = pData->pointAy[0].fY * vk4 + pData->pointAy[0].fX * vk1 +
          pData->pointAy[0].fZ * vk7;
    v24 = pData->pointAy[0].fY * vk5 + pData->pointAy[0].fX * vk2 +
          pData->pointAy[0].fZ * vk8;
    v16 = pData->pointAy[0].fY * vk6 + pData->pointAy[0].fX * vk3 +
          pData->pointAy[0].fZ * vk9;
    v20 = pData->pointAy[1].fX * vk1 + pData->pointAy[1].fY * vk4 +
          pData->pointAy[1].fZ * vk7;
    v18 = pData->pointAy[1].fX * vk2 + pData->pointAy[1].fY * vk5 +
          pData->pointAy[1].fZ * vk8;
    v19 = pData->pointAy[1].fX * vk3 + pData->pointAy[1].fY * vk6 +
          pData->pointAy[1].fZ * vk9;
    v17 = pData->pointAy[2].fX * vk1 + pData->pointAy[2].fY * vk4 +
          pData->pointAy[2].fZ * vk7;
    v22 = vk8 * pData->pointAy[2].fZ + pData->pointAy[2].fX * vk2 +
          pData->pointAy[2].fY * vk5;
...
//-------------------------------------------------------------------------------------------------
// 00060DE0
void initlocaltrack() { // Initialize each track segment's local data
  int i;                // edx

  for (i = 0; i < TRAK_LEN; ++i)
    initlocalsection(i);
  dopitchchanges(
      0, 2); // Calculate pitch angles for inner track lanes (left=0, right=2)
  dopitchchanges(
      3, 4); // Calculate pitch angles for outer track lanes (left=3, right=4)
}
...
void initlocalsection(int iChunkIdx) {
  ...
  pointArray[0] =
      TrakPt[iArrayIndex].pointAy[2].fX; // Point 0: Current left edge
  pointArray[1] = TrakPt[iArrayIndex].pointAy[2].fY;
  pointArray[2] = TrakPt[iArrayIndex].pointAy[2].fZ;
  pointArray[3] =
      TrakPt[iArrayIndex].pointAy[3].fX; // Point 1: Current right edge
  ...
  do {
    // Transform all points to local coordinate system (centered at track
    // section)
    ...
    uiOffset += 9u;
    // matrixWork[uiOffset + 29] = fTemp;          // pointArray[uiOffset + 8] =
    // if moved above uiOffset increment
  } while (uiOffset != 27);
  iLoop1 = 0;
  fAngleY = getdirection(
      pointArray[15] - pointArray[12],
      pointArray[16] -
          pointArray[13]); // Calculate Y rotation angle from track direction
  dAngleYSaved = fAngleY;
  do {
    dSinY = sin(
        -fAngleY); // Apply Y-axis rotation to align track with coordinate axes
    dCosY = cos(-fAngleY);
    ...
    iLoop1 += 3;
    // LODWORD(matrixWork[iLoop1 + 27]) = LODWORD(dRotatedX);//
    // pointArray[iLoop] = if moved above iLoop increment
  ...
  iLoop2 = 0;
  fAngleZ = getdirection(
      pointArray[15] - pointArray[12],
      pointArray[17] -
          pointArray[14]); // Calculate Z rotation angle (track slope/elevation)
  ...
  do {
    ...
    iLoop2 += 3;
    // LODWORD(matrixWork[iLoop2 + 27]) = LODWORD(dRotatedX);//
    // pointArray[iLoop2] = if moved above iLoop2 increment
  ...
  iLoop3 = 0;
  dAngleXBank = getdirection(
      pointArray[19] - pointArray[22],
      pointArray[23] -
          pointArray[20]); // Calculate X rotation angle (track banking)
  dAngleX = dAngleXBank;
  do {
    dCosX = cos(dAngleX);
    dCosY = sin(dAngleX);
    ...
    iLoop3 += 3;
    // LODWORD(matrixWork[iLoop3 + 28]) = LODWORD(dRotatedY);//
    // pointArray[iLoop3] = if moved above iLoop3 increment
  ...
  fBankAngle =
      atan2(pointArray[7] - pointArray[10], pointArray[11] - pointArray[8]);
  // fBankAngle = IF_DATAN2(nFpStatus1, pointArray[7] - pointArray[10],
  // pointArray[11] - pointArray[8]);
  fBankDelta = (fBankAngle - atan2(pointArray[1] - pointArray[4],
                                   pointArray[5] - pointArray[2])) *
                   16384.0 / 6.28318530718 +
               0.5;
  // fBankDelta = (fBankAngle - atan2(pointArray[5] - pointArray[2],
  // fBankAngle)) * 16384.0 / 6.28318530718 + 0.5; fBankDelta = (fBankAngle -
  // IF_DATAN2(nFpStatus2, pointArray[5] - pointArray[2], fBankAngle)) * 16384.0
  // / 6.28318530718 + 0.5;
  floor(fBankDelta);
  //_CHP();
  ...
  iLoop4 = 0;
  dNextAngleY = getdirection(matrixWork[18] - matrixWork[15],
                             matrixWork[19] - matrixWork[16]);
  ...
  do {
    ...
    dRotatedY = dSinNextY * matrixWork[iLoop4 + 3] +
                matrixWork[iLoop4 + 4] * dWorkAngle;
    iLoop4 += 3;
  ...
  iLoop5 = 0;
  dNextAngleZ = getdirection(matrixWork[18] - matrixWork[15],
                             matrixWork[20] - matrixWork[17]);
  ...
  do {
    ...
    dRotatedZ = matrixWork[iLoop5 + 5] * dWorkAngle +
                -dSinNextZ * matrixWork[iLoop5 + 3];
    iLoop5 += 3;
  ...
  dWorkAngle = getdirection(matrixWork[22] - matrixWork[25],
                            matrixWork[26] - matrixWork[23]);
  dCosY = 1.0 / 6.28318530718; // Convert angles to internal fixed-point format
                               // (16384 = 2pi radians)
  ...
  pData->gravity.fX =
      (float)(dSinZSaved *
              -3.0); // Store up vector scaled by -3 for physics calculations
...
void dopitchchanges(int iLLaneIdx, int iRLaneIdx) {
  ...
  // a1 = iLeftLaneIndex (0=inner left, 3=outer left), a2 = iRightLaneIndex
  // (2=inner right, 4=outer right)
  ...
  if (TRAK_LEN > 0) {
    ...
    do {
      ...
      if (iNextTrackIndex + 1 == TRAK_LEN)
        iNextTrackIndex2 = 0;
      laneCornerPoints[0] = TrakPt[0]
                                .pointAy[iLeftLaneOffset]
                                .fX; // Load current track segment lane corner
                                     // points into working array
      ...
      laneCornerPoints[6] =
          TrakPt[iNextTrackIndex].pointAy[uiLeftLaneOffset12 / 0xC].fX;
      laneCornerPoints[7] =
          TrakPt[iNextTrackIndex].pointAy[uiLeftLaneOffset12 / 0xC].fY;
      laneCornerPoints[8] =
          TrakPt[iNextTrackIndex].pointAy[uiLeftLaneOffset12 / 0xC].fZ;
      laneCornerPoints[9] =
          TrakPt[iNextTrackIndex].pointAy[uiRightLaneOffset12 / 0xC].fX;
      laneCornerPoints[10] =
          TrakPt[iNextTrackIndex].pointAy[uiRightLaneOffset12 / 0xC].fY;
      pCurrentTrackData = &localdata[iTrackIndex];
      uiLoopCounter1 = 0;
      laneCornerPoints[11] =
          TrakPt[iNextTrackIndex].pointAy[uiRightLaneOffset12 / 0xC].fZ;
      do {
        transformCoords[uiLoopCounter1] =
            (pCurrentTrackData->pointAy[3].fY +
             laneCornerPoints[uiLoopCounter1 + 1]) *
                pCurrentTrackData->pointAy[1].fX +
            (pCurrentTrackData->pointAy[3].fX +
             laneCornerPoints[uiLoopCounter1]) *
                pCurrentTrackData->pointAy[0].fX +
            (pCurrentTrackData->pointAy[3].fZ +
             laneCornerPoints[uiLoopCounter1 + 2]) *
                pCurrentTrackData->pointAy[2]
                    .fX; // Transform points using matrix multiplication
        transformCoords[uiLoopCounter1 + 1] =
            (pCurrentTrackData->pointAy[3].fY +
             laneCornerPoints[uiLoopCounter1 + 1]) *
                pCurrentTrackData->pointAy[1].fY +
            (pCurrentTrackData->pointAy[3].fX +
             laneCornerPoints[uiLoopCounter1]) *
                pCurrentTrackData->pointAy[0].fY +
            (pCurrentTrackData->pointAy[3].fZ +
             laneCornerPoints[uiLoopCounter1 + 2]) *
                pCurrentTrackData->pointAy[2].fY;
        dTransformZ1 = (pCurrentTrackData->pointAy[3].fY +
                        laneCornerPoints[uiLoopCounter1 + 1]) *
                           pCurrentTrackData->pointAy[1].fZ +
                       (pCurrentTrackData->pointAy[3].fX +
                        laneCornerPoints[uiLoopCounter1]) *
                           pCurrentTrackData->pointAy[0].fZ +
                       (pCurrentTrackData->pointAy[3].fZ +
                        laneCornerPoints[uiLoopCounter1 + 2]) *
                           pCurrentTrackData->pointAy[2].fZ;
        transformCoords[uiLoopCounter1 + 2] = dTransformZ1;
        uiLoopCounter1 += 3;
        // transformCoordsOffset[uiLoopCounter1 + 2] = dTransformZ1;// Store Z
        // coordinate into transformCoords[2+loop_index] via offset alias
      ...
      bankRotData[1] =
          transformCoords[1] * dCosBank1 -
          transformCoords[2] *
              dSinBank1; // Apply first bank rotation transformation
      bankRotData[2] =
          transformCoords[1] * dSinBank1 + transformCoords[2] * dCosBank1;
      bankRotData[4] =
          transformCoords[4] * dCosBank1 - transformCoords[5] * dSinBank1;
      bankRotData[5] =
          transformCoords[4] * dSinBank1 + transformCoords[5] * dCosBank1;
      bankRotData[0] = transformCoords[0];
      bankRotData[3] = transformCoords[3];
      iBankAngle2 = (pCurrentTrackData->iBankDelta / 2) &
                    0x3FFF; // Calculate bank angle for rotation (positive half)
      dCosBank2 = tcos[iBankAngle2];
      dSinBank2 = tsin[iBankAngle2];
      bankRotData[7] =
          transformCoords[7] * dCosBank2 -
          transformCoords[8] *
              dSinBank2; // Apply second bank rotation transformation
      bankRotData[8] =
          transformCoords[7] * dSinBank2 + transformCoords[8] * dCosBank2;
      bankRotData[10] =
          transformCoords[10] * dCosBank2 - transformCoords[11] * dSinBank2;
      bankRotData[6] = transformCoords[6];
      bankRotData[9] = transformCoords[9];
      uiLoopCounter2 = 0;
      bankRotData[11] =
          transformCoords[10] * dSinBank2 + transformCoords[11] * dCosBank2;
      do {
        laneCornerPoints[uiLoopCounter2] =
            pCurrentTrackData->pointAy[0].fY * bankRotData[uiLoopCounter2 + 1] +
            pCurrentTrackData->pointAy[0].fX * bankRotData[uiLoopCounter2] +
            pCurrentTrackData->pointAy[0].fZ * bankRotData[uiLoopCounter2 + 2] -
            pCurrentTrackData->pointAy[3]
                .fX; // Transform coordinates and subtract offset
        laneCornerPoints[uiLoopCounter2 + 1] =
            pCurrentTrackData->pointAy[1].fY * bankRotData[uiLoopCounter2 + 1] +
            pCurrentTrackData->pointAy[1].fX * bankRotData[uiLoopCounter2] +
            pCurrentTrackData->pointAy[1].fZ * bankRotData[uiLoopCounter2 + 2] -
            pCurrentTrackData->pointAy[3].fY;
        dTransformZ2 =
            pCurrentTrackData->pointAy[2].fY * bankRotData[uiLoopCounter2 + 1] +
        ...
        uiLoopCounter2 += 3;
        // laneCornerPointsOffset[uiLoopCounter2 + 2] = dTransformZ2;//
        // laneCornerPoints[uiLoopCounter2 + 2] when above loop inc
      ...
      laneCornerPoints[24] =
          (laneCornerPoints[12] + laneCornerPoints[15]) * 0.5;
      laneCornerPoints[25] =
          (laneCornerPoints[13] + laneCornerPoints[16]) * 0.5;
      laneCornerPoints[26] =
          (laneCornerPoints[14] + laneCornerPoints[17]) * 0.5;
      ...
      do {
        laneCornerPoints[iPointIndex] =
            laneCornerPoints[iPointIndex] + dLaneCenterOffsetX;
        laneCornerPoints[iPointIndex + 1] =
            laneCornerPoints[iPointIndex + 1] + dLaneCenterOffsetY;
        laneCornerPoints[iPointIndex + 2] =
            laneCornerPoints[iPointIndex + 2] + dLaneCenterOffsetZ;
        laneCornerPoints[iPointIndex + 3] =
            laneCornerPoints[iPointIndex + 3] + dLaneCenterOffsetX;
        laneCornerPoints[iPointIndex + 4] =
            laneCornerPoints[iPointIndex + 4] + dLaneCenterOffsetY;
        laneCornerPoints[iPointIndex + 5] =
            laneCornerPoints[iPointIndex + 5] + dLaneCenterOffsetZ;
        laneCornerPoints[iPointIndex + 6] =
            laneCornerPoints[iPointIndex + 6] + dLaneCenterOffsetX;
        laneCornerPoints[iPointIndex + 7] =
            laneCornerPoints[iPointIndex + 7] + dLaneCenterOffsetY;
        laneCornerPoints[iPointIndex + 8] =
            laneCornerPoints[iPointIndex + 8] + dLaneCenterOffsetZ;
        laneCornerPoints[iPointIndex + 9] =
            laneCornerPoints[iPointIndex + 9] + dLaneCenterOffsetX;
        laneCornerPoints[iPointIndex + 10] =
            laneCornerPoints[iPointIndex + 10] + dLaneCenterOffsetY;
        dTempValue = laneCornerPoints[iPointIndex + 11] + dLaneCenterOffsetZ;
        laneCornerPoints[iPointIndex + 11] = dTempValue;
        iPointIndex += 12;
        // laneCornerPointsOffset[iPointIndex + 2] = dTempValue;//
        // laneCornerPoints[iPointIndex + 11] when before loop inc
      } while (iPointIndex != 27);
      iRotationIndex1 = 0;
      dLaneDirection1 =
          getdirection(laneCornerPoints[15] - laneCornerPoints[12],
                       laneCornerPoints[16] -
                           laneCornerPoints[13]); // Get direction angle for
                                                  // first rotation axis
      ...
      do {
        ...
        dRotMatrix02 =
            laneCornerPoints[iRotationIndex1] * dTempCos - dCosValue1;
        dRotMatrix01 = dSinValue1 * laneCornerPoints[iRotationIndex1] +
                       laneCornerPoints[iRotationIndex1 + 1] * dTempCos;
        laneCornerPoints[iRotationIndex1] = dRotMatrix02;
        laneCornerPoints[iRotationIndex1 + 1] = dRotMatrix01;
        iRotationIndex1 += 3;
        // LODWORD(laneCornerPointsOffset[iRotationIndex1]) =
        // LODWORD(dRotMatrix02);// laneCornerPoints[iRotationIndex1] when
        // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex1]) =
        // HIDWORD(dRotMatrix02); LODWORD(laneCornerPointsOffset[iRotationIndex1
        // + 1]) = LODWORD(dRotMatrix01);// laneCornerPoints[iRotationIndex1 +
        // 1] when before loop inc
        // HIDWORD(laneCornerPointsOffset[iRotationIndex1 + 1]) =
        // HIDWORD(dRotMatrix01);
      } while (iRotationIndex1 != 27);
      iRotationIndex2 = 0;
      dLaneDirection2 =
          getdirection(laneCornerPoints[15] - laneCornerPoints[12],
                       laneCornerPoints[17] -
                           laneCornerPoints[14]); // Get direction angle for
      ...
      do {
        ...
        dRotMatrix21 = dTempCos;
        dRotMatrix02 =
            dCosValue2 + laneCornerPoints[iRotationIndex2] * dTempCos;
        dRotMatrix00 = laneCornerPoints[iRotationIndex2 + 2] * dTempCos +
                       -dSinValue2 * laneCornerPoints[iRotationIndex2];
        laneCornerPoints[iRotationIndex2] = dRotMatrix02;
        laneCornerPoints[iRotationIndex2 + 2] = dRotMatrix00;
        iRotationIndex2 += 3;
        // LODWORD(laneCornerPointsOffset[iRotationIndex2]) =
        // LODWORD(dRotMatrix02);// laneCornerPoints[iRotationIndex2] when
        // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex2]) =
        // HIDWORD(dRotMatrix02); LODWORD(laneCornerPointsOffset[iRotationIndex2
        // + 2]) = LODWORD(dRotMatrix00);// laneCornerPoints[iRotationIndex2 +
        // 2] when before loop inc
        // HIDWORD(laneCornerPointsOffset[iRotationIndex2 + 2]) =
        // HIDWORD(dRotMatrix00);
      } while (iRotationIndex2 != 27);
      iRotationIndex3 = 0;
      dLaneDirectionAngle = getdirection(
          laneCornerPoints[19] - laneCornerPoints[22],
          laneCornerPoints[23] -
              laneCornerPoints[20]); // Get direction for third rotation axis
      ...
      do {
        ...
        dRotMatrix01 =
            dSinValue3 - laneCornerPoints[iRotationIndex3 + 2] * dTempCos;
        dRotMatrix00 = laneCornerPoints[iRotationIndex3 + 1] * dTempCos +
                       laneCornerPoints[iRotationIndex3 + 2] * dCosValue3;
        laneCornerPoints[iRotationIndex3 + 1] = dRotMatrix01;
        laneCornerPoints[iRotationIndex3 + 2] = dRotMatrix00;
        iRotationIndex3 += 3;
        // LODWORD(laneCornerPointsOffset[iRotationIndex3 + 1]) =
        // LODWORD(dRotMatrix01);// laneCornerPoints[iRotationIndex3 + 1] when
        // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex3 + 1])
        // = HIDWORD(dRotMatrix01);
        // LODWORD(laneCornerPointsOffset[iRotationIndex3 + 2]) =
        // LODWORD(dRotMatrix00);// laneCornerPoints[iRotationIndex3 + 2] when
        // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex3 + 2])
        // = HIDWORD(dRotMatrix00);
      ...
      dNegSinMatrix20 =
          -dTempSin * dSinDirection3 - dTempRotValue1 * dCosValue3;
      dSinMatrix10 = dTempSin * dMatrixComp2;
      dCosMatrix11 =
          dTempSin * dRotMatrix22 * dSinDirection3 + dMatrixComp1 * dCosValue3;
      dRotMatrix11 =
          dMatrixComp1 * dSinDirection3 - dTempSin * dRotMatrix22 * dCosValue3;
      ...
      laneCornerPoints[5] =
          TrakPt[iNextTrackIndex].pointAy[uiRightLaneOffset12 / 0xC].fZ;
      laneCornerPoints[6] =
          TrakPt[iNextTrackIndex2].pointAy[uiLeftLaneOffset12 / 0xC].fX;
      laneCornerPoints[7] =
          TrakPt[iNextTrackIndex2].pointAy[uiLeftLaneOffset12 / 0xC].fY;
      laneCornerPoints[8] =
          TrakPt[iNextTrackIndex2].pointAy[uiLeftLaneOffset12 / 0xC].fZ;
      laneCornerPoints[9] =
          TrakPt[iNextTrackIndex2].pointAy[uiRightLaneOffset12 / 0xC].fX;
      laneCornerPoints[10] =
          TrakPt[iNextTrackIndex2].pointAy[uiRightLaneOffset12 / 0xC].fY;
      pNextTrackData = &localdata[iNextTrackIndex];
      uiTransformLoop = 0;
      laneCornerPoints[11] =
          TrakPt[iNextTrackIndex2].pointAy[uiRightLaneOffset12 / 0xC].fZ;
      do {
        transformCoords[uiTransformLoop] =
            (pNextTrackData->pointAy[3].fY +
             laneCornerPoints[uiTransformLoop + 1]) *
                pNextTrackData->pointAy[1].fX +
            (pNextTrackData->pointAy[3].fX +
             laneCornerPoints[uiTransformLoop]) *
                pNextTrackData->pointAy[0].fX +
            (pNextTrackData->pointAy[3].fZ +
             laneCornerPoints[uiTransformLoop + 2]) *
                pNextTrackData->pointAy[2].fX;
        transformCoords[uiTransformLoop + 1] =
            (pNextTrackData->pointAy[3].fY +
             laneCornerPoints[uiTransformLoop + 1]) *
                pNextTrackData->pointAy[1].fY +
            (pNextTrackData->pointAy[3].fX +
             laneCornerPoints[uiTransformLoop]) *
                pNextTrackData->pointAy[0].fY +
            (pNextTrackData->pointAy[3].fZ +
             laneCornerPoints[uiTransformLoop + 2]) *
                pNextTrackData->pointAy[2].fY;
        dNextTransformZ = (pNextTrackData->pointAy[3].fY +
                           laneCornerPoints[uiTransformLoop + 1]) *
                              pNextTrackData->pointAy[1].fZ +
                          (pNextTrackData->pointAy[3].fX +
                           laneCornerPoints[uiTransformLoop]) *
                              pNextTrackData->pointAy[0].fZ +
                          (pNextTrackData->pointAy[3].fZ +
                           laneCornerPoints[uiTransformLoop + 2]) *
                              pNextTrackData->pointAy[2].fZ;
        transformCoords[uiTransformLoop + 2] = dNextTransformZ;
        uiTransformLoop += 3;
        // transformCoordsOffset[uiTransformLoop + 2] = dNextTransformZ;//
        // transformCoords[uiTransformLoop + 2] when before loop inc
      ...
      bankRotData[1] = transformCoords[1] * dNextCosBank2 -
                       transformCoords[2] * dNextSinBank1;
      bankRotData[2] = transformCoords[1] * dNextSinBank1 +
                       transformCoords[2] * dNextCosBank2;
      bankRotData[4] = transformCoords[4] * dNextCosBank2 -
                       transformCoords[5] * dNextSinBank1;
      bankRotData[5] = transformCoords[4] * dNextSinBank1 +
                       transformCoords[5] * dNextCosBank2;
      bankRotData[0] = transformCoords[0];
      bankRotData[3] = transformCoords[3];
      iNextBankAngle2 = (pNextTrackData->iBankDelta / 2) & 0x3FFF;
      dNextCosBank1 = tcos[iNextBankAngle2];
      dNextSinBank2 = tsin[iNextBankAngle2];
      bankRotData[7] = transformCoords[7] * dNextCosBank1 -
                       transformCoords[8] * dNextSinBank2;
      bankRotData[8] = transformCoords[7] * dNextSinBank2 +
                       transformCoords[8] * dNextCosBank1;
      bankRotData[10] = transformCoords[10] * dNextCosBank1 -
                        transformCoords[11] * dNextSinBank2;
      bankRotData[6] = transformCoords[6];
      bankRotData[9] = transformCoords[9];
      uiNextTransformLoop = 0;
      bankRotData[11] = transformCoords[10] * dNextSinBank2 +
                        transformCoords[11] * dNextCosBank1;
      do {
        laneCornerPoints[uiNextTransformLoop] =
            pNextTrackData->pointAy[0].fY *
                bankRotData[uiNextTransformLoop + 1] +
            pNextTrackData->pointAy[0].fX * bankRotData[uiNextTransformLoop] +
            pNextTrackData->pointAy[0].fZ *
                bankRotData[uiNextTransformLoop + 2] -
            pNextTrackData->pointAy[3].fX;
        laneCornerPoints[uiNextTransformLoop + 1] =
            pNextTrackData->pointAy[1].fY *
                bankRotData[uiNextTransformLoop + 1] +
        ...
        // laneCornerPointsOffset[uiNextTransformLoop + 2] =
        // dNextSegmentTransformZ;// laneCornerPoints[uiNextTransformLoop + 2]
        // when before loop inc
      ...
      do {
        ...
        transformCoords[uiFinalRotationLoop] = dSinMatrix10 * dPointY +
                                               dCosMatrix00 * dPointX +
                                               dRotMatrix12 * dPointZ;
        transformCoords[uiFinalRotationLoop + 1] = dCosMatrix11 * dPointY +
                                                   dMatrixRotValue2 * dPointX +
                                                   dRotMatrix20 * dPointZ;
        transformCoords[uiFinalRotationLoop + 2] = dPointY * dRotMatrix11 +
                                                   dPointX * dMatrixRotValue1 +
                                                   dPointZ * dCosMatrix22;
        uiFinalRotationLoop += 3;
        // transformCoordsOffset[uiFinalRotationLoop + 2] = dPointY *
        // dRotMatrix11 + dPointX * dMatrixRotValue1 + dPointZ * dCosMatrix22;//
        // transformCoords[uiFinalRotationLoop + 1] when before loop inc
      } while (uiFinalRotationLoop != 27);
      iDirectionCalcLoop = 0;
      dFinalDirection = getdirection(
          transformCoords[15] - transformCoords[12],
          transformCoords[16] - transformCoords[13]); // Get final direction
                                                      // angle for last rotation
      do {
        ...
        dTempSin = dTempCos;
        dRotMatrix02 =
            transformCoords[iDirectionCalcLoop] * dTempCos - dCosFinal;
        dRotMatrix01 = dSinFinal * transformCoords[iDirectionCalcLoop] +
                       transformCoords[iDirectionCalcLoop + 1] * dTempCos;
        transformCoords[iDirectionCalcLoop] = dRotMatrix02;
        transformCoords[iDirectionCalcLoop + 1] = dRotMatrix01;
        iDirectionCalcLoop += 3;
        // LODWORD(transformCoordsOffset[iDirectionCalcLoop]) =
        // LODWORD(dRotMatrix02);// transformCoords[iDirectionCalcLoop] when
        // before loop inc HIDWORD(transformCoordsOffset[iDirectionCalcLoop]) =
        // HIDWORD(dRotMatrix02);
        // LODWORD(transformCoordsOffset[iDirectionCalcLoop + 1]) =
        // LODWORD(dRotMatrix01);// transformCoords[iDirectionCalcLoop + 1] when
        // before loop inc HIDWORD(transformCoordsOffset[iDirectionCalcLoop +
        // 1]) = HIDWORD(dRotMatrix01);
      } while (iDirectionCalcLoop != 27);

      // Calculate final pitch angle in degrees and store result
      dFinalLanePitchAngle =
          getdirection(transformCoords[15] - transformCoords[12],
                       transformCoords[17] - transformCoords[14]) *
              16384.0 / 6.28318530718 +
          0.5;
      pOutputTrackData = &localdata[iTrackIndex];
      dStoredPitchAngle = dFinalLanePitchAngle;

      // Store calculated pitch angle: iUnk18 for inner lanes, iUnk19 for outer
      // lanes
      if (iLeftLaneIndex) {
...
int getpitchchange(int iChunkIdx, int iLLaneIdx, int iRLaneIdx) {
  ...
  iNextTrackIndex =
      iChunkIdx + 1; // Calculate next track indices with wraparound
  ...
  iTrack72ByteOffset =
      72 * iChunkIdx; // Calculate byte offsets for track and lane data access
  ...
  // laneCornerPoints[0] = *(float *)((char *)&TrakPt[0].pointAy[0].fX +
  // iLaneAOffset);// Load current track segment corner points for lanes a2 and
  // a3 laneCornerPoints[1] = *(float *)((char *)&TrakPt[0].pointAy[0].fY +
  // iLaneAOffset); laneCornerPoints[2] = *(float *)((char
  // *)&TrakPt[0].pointAy[0].fZ + iLaneAOffset);
  ...
  // laneCornerPoints[3] = *(float *)((char *)&TrakPt[0].pointAy[0].fX +
  // iLaneBOffset); laneCornerPoints[4] = *(float *)((char
  // *)&TrakPt[0].pointAy[0].fY + iLaneBOffset); laneCornerPoints[5] = *(float
  // *)((char *)&TrakPt[0].pointAy[0].fZ + iLaneBOffset);
  ...
  laneCornerPoints[12] = (laneCornerPoints[0] + laneCornerPoints[3]) *
                         0.5; // Calculate midpoints between lane corner pairs
  laneCornerPoints[13] = (laneCornerPoints[1] + laneCornerPoints[4]) * 0.5;
  laneCornerPoints[14] = (laneCornerPoints[2] + laneCornerPoints[5]) * 0.5;
  laneCornerPoints[15] = (laneCornerPoints[6] + laneCornerPoints[9]) * 0.5;
  laneCornerPoints[16] = (laneCornerPoints[7] + laneCornerPoints[10]) * 0.5;
  ...
  dNegOffsetX = -pCurrentTrackData->pointAy[3]
                     .fX; // Calculate negative track offset coordinates
  dNegOffsetY = -pCurrentTrackData->pointAy[3].fY;
  dNegOffsetZ = -pCurrentTrackData->pointAy[3].fZ;
  laneCornerPoints[0] =
      laneCornerPoints[0] +
      dNegOffsetX; // Apply offset to all corner points to center at origin
  ...
  do {
    ...
    laneCornerPoints[iPointIndex + 1] =
        laneCornerPoints[iPointIndex + 1] + dOffsetY;
    laneCornerPoints[iPointIndex + 2] =
        laneCornerPoints[iPointIndex + 2] + dOffsetZ;
    laneCornerPoints[iPointIndex + 3] =
        laneCornerPoints[iPointIndex + 3] + dOffsetX;
    laneCornerPoints[iPointIndex + 4] =
        laneCornerPoints[iPointIndex + 4] + dOffsetY;
    laneCornerPoints[iPointIndex + 5] =
        laneCornerPoints[iPointIndex + 5] + dOffsetZ;
    laneCornerPoints[iPointIndex + 6] =
        laneCornerPoints[iPointIndex + 6] + dOffsetX;
    laneCornerPoints[iPointIndex + 7] =
        laneCornerPoints[iPointIndex + 7] + dOffsetY;
    laneCornerPoints[iPointIndex + 8] =
        laneCornerPoints[iPointIndex + 8] + dOffsetZ;
    laneCornerPoints[iPointIndex + 9] =
        laneCornerPoints[iPointIndex + 9] + dOffsetX;
    laneCornerPoints[iPointIndex + 10] =
        laneCornerPoints[iPointIndex + 10] + dOffsetY;
  ...
  dDirection1 =
      getdirection(laneCornerPoints[15] - laneCornerPoints[12],
                   laneCornerPoints[16] -
                       laneCornerPoints[13]); // First rotation pass around
  ...
  do {
    ...
    dRotMatrix01 = dSin1 * laneCornerPoints[iRotationIndex1] +
                   laneCornerPoints[iRotationIndex1 + 1] * dTempCos;
    ...
    iRotationIndex1 += 3;
    // LODWORD(laneCornerPointsOffset[iRotationIndex1]) =
    // LODWORD(dRotMatrix00);// laneCornerPoints[iRotationIndex1] when before
    // loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex1]) =
    // HIDWORD(dRotMatrix00); LODWORD(laneCornerPointsOffset[iRotationIndex1 +
    // 1]) = LODWORD(dRotMatrix01);// laneCornerPoints[iRotationIndex1 + 1] when
    // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex1 + 1]) =
    // HIDWORD(dRotMatrix01);
  } while (iRotationIndex1 != 27);
  iRotationIndex2 = 0;
  dDirection2 =
      getdirection(laneCornerPoints[15] - laneCornerPoints[12],
                   laneCornerPoints[17] -
                       laneCornerPoints[14]); // Second rotation pass around
                                              // different direction
  do {
    ...
    dRotMatrix02 = laneCornerPoints[iRotationIndex2 + 2] * dTempCos +
                   -dSin2 * laneCornerPoints[iRotationIndex2];
    ...
    iRotationIndex2 += 3;
    // LODWORD(laneCornerPointsOffset[iRotationIndex2]) =
    // LODWORD(dRotMatrix00);// laneCornerPoints[iRotationIndex2] when before
    // loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex2]) =
    // HIDWORD(dRotMatrix00); LODWORD(laneCornerPointsOffset[iRotationIndex2 +
    // 2]) = LODWORD(dRotMatrix02);// laneCornerPoints[iRotationIndex2 + 1] when
    // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex2 + 2]) =
    // HIDWORD(dRotMatrix02);
  } while (iRotationIndex2 != 27);
  iRotationIndex3 = 0;
  dDirection3Stored = getdirection(
      laneCornerPoints[19] - laneCornerPoints[22],
      laneCornerPoints[23] -
          laneCornerPoints[20]); // Third rotation pass around third direction
  ...
  do {
    ...
    dRotMatrix02 = laneCornerPoints[iRotationIndex3 + 1] * dTempSin +
                   laneCornerPoints[iRotationIndex3 + 2] * dCos3;
    ...
    // LODWORD(dRotMatrix01);// laneCornerPoints[iRotationIndex3 + 1] when
    // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex3 + 1]) =
    // HIDWORD(dRotMatrix01); LODWORD(laneCornerPointsOffset[iRotationIndex3 +
    // 2]) = LODWORD(dRotMatrix02);// laneCornerPoints[iRotationIndex3 + 2] when
    // before loop inc HIDWORD(laneCornerPointsOffset[iRotationIndex3 + 2]) =
    // HIDWORD(dRotMatrix02);
  } while (iRotationIndex3 != 27);
  dTempCos3 = dCos3;
  dPitchAngle1 = atan2(laneCornerPoints[7] - laneCornerPoints[10],
                       laneCornerPoints[11] -
                           laneCornerPoints[8]); // Calculate intermediate pitch
                                                 // angle using atan2
  // dPitchAngle1 = IF_DATAN2(v27, laneCornerPoints[7] - laneCornerPoints[10],
  // laneCornerPoints[11] - laneCornerPoints[8]);// Calculate intermediate pitch
  // angle using atan2
  dPitchAngle2 =
      (dPitchAngle1 -
       atan2(laneCornerPoints[5] - laneCornerPoints[2], dPitchAngle1)) *
          16384.0 / 6.28318530718 +
      0.5; // Calculate and store banking angle difference
  // dPitchAngle2 = (dPitchAngle1 - IF_DATAN2(v29, laneCornerPoints[5] -
  // laneCornerPoints[2], dPitchAngle1)) * 16384.0 / 6.28318530718 + 0.5;//
  // Calculate and store banking angle difference
  floor(dPitchAngle2);
  //_CHP();
  pCurrentTrackData->iBankAngleDelta = (int)-dPitchAngle2;
  dCurrentMatrixX =
      pCurrentTrackData->pointAy[0]
          .fX; // Load current track transformation matrix components
  ...
  // iNextLaneAOffset = 72 * iNextTrackIndex + 12 * iLLaneIdx;// Load next track
  // segment points for comparison laneCornerPoints[0] = *(float *)((char
  // *)&TrakPt[0].pointAy[0].fX + iNextLaneAOffset); laneCornerPoints[1] =
  // *(float *)((char *)&TrakPt[0].pointAy[0].fY + iNextLaneAOffset);
  // laneCornerPoints[2] = *(float *)((char *)&TrakPt[0].pointAy[0].fZ +
  // iNextLaneAOffset);
  ...
  do {
    dPointY = laneCornerPoints[iTransformIndex + 1] +
              dNegOffsetY; // Transform points using current track matrix
    dPointX = laneCornerPoints[iTransformIndex] + dNegOffsetX;
    dPointZ = laneCornerPoints[iTransformIndex + 2] + dNegOffsetZ;
    transformCoords[iTransformIndex] = dMatrixRotX * dPointY +
                                       dCurrentMatrixX * dPointX +
                                       dMatrixScaleX * dPointZ;
    transformCoords[iTransformIndex + 1] =
        dMatrixRotY * dPointY + dCurrentY * dPointX + dMatrixScaleY * dPointZ;

    // loop offset fix
    transformCoords[iTransformIndex + 2] =
        dPointY * dMatrixRotZ + dPointX * dCurrentZ + dPointZ * dMatrixScaleZ;

    iTransformIndex += 3;
    // laneCornerPoints[iTransformIndex + 26] = dPointY * dMatrixRotZ + dPointX
    // * dCurrentZ + dPointZ * dMatrixScaleZ;// transformCoords[iTransformIndex
    // + 2] when before loop inc
  ...
  dFinalDirection = getdirection(
      transformCoords[15] - transformCoords[12],
      transformCoords[16] -
          transformCoords[13]); // Final rotation pass for pitch calculation
  do {
    ...
    dRotMatrix00 =
        transformCoords[iFinalRotationIndex] * dFinalCosSin - dFinalCos;
    dRotMatrix01 = dFinalSin * transformCoords[iFinalRotationIndex] +
                   transformCoords[iFinalRotationIndex + 1] * dFinalCosSin;
    ...
    // LODWORD(laneCornerPoints[iFinalRotationIndex + 24]) =
    // LODWORD(dRotMatrix00);// transformCoords[iFinalRotationIndex] when before
    // loop inc HIDWORD(laneCornerPoints[iFinalRotationIndex + 24]) =
    // HIDWORD(dRotMatrix00); LODWORD(laneCornerPoints[iFinalRotationIndex +
    // 25]) = LODWORD(dRotMatrix01);// transformCoords[iFinalRotationIndex + 1]
    // when before loop inc HIDWORD(laneCornerPoints[iFinalRotationIndex + 25])
    // = HIDWORD(dRotMatrix01);
  ...
  dFinalPitchAngle =
      getdirection(transformCoords[15] - transformCoords[12],
                   transformCoords[17] - transformCoords[14]) *
          16384.0 / 6.28318530718 +
      0.5; // Calculate final pitch change angle and return as integer
...
void getworldangles(int iYaw, int iPitch, int iRoll, int iChunkIdx,
  ...
  fProjX = (float)(pData->pointAy[0].fY * dDirY + pData->pointAy[0].fX * dDirX +
                   pData->pointAy[0].fZ * dDirZ);
  fProjY = (float)(pData->pointAy[1].fX * dDirX + pData->pointAy[1].fY * dDirY +
                   pData->pointAy[1].fZ * dDirZ);
  fProjZ = (float)(dDirX * pData->pointAy[2].fX + dDirY * pData->pointAy[2].fY +
                   dDirZ * pData->pointAy[2].fZ);
  ...
  *piElevation = iNewElevation;
  ...
  fRollProjX = pData->pointAy[0].fY * fRollVecZ +
               pData->pointAy[0].fX * fRollVecY +
               pData->pointAy[0].fZ * fRollVecW;
  fRollProjY = pData->pointAy[1].fX * fRollVecY +
               pData->pointAy[1].fY * fRollVecZ +
               pData->pointAy[1].fZ * fRollVecW;
  fRollProjZ = pData->pointAy[2].fX * fRollVecY +
               pData->pointAy[2].fY * fRollVecZ +
               pData->pointAy[2].fZ * fRollVecW;

  // Rotate projections using azimuth
  fRollRotatedX =
      fRollProjX * tcos[iNewAzimuth_1] + fRollProjY * tsin[iNewAzimuth_1];
  fRollRotatedY =
      -fRollProjX * tsin[iNewAzimuth_1] + fRollProjY * tcos[iNewAzimuth_1];

  // Apply elevation rotation
  fFinalX = (float)(fRollRotatedX * tsin[iNewElevation] -
                    fRollProjZ * tcos[iNewElevation]);
...
void getlocalangles(int iYaw, int iPitch, int iRoll, int iChunkIdx,
  ...
  fProjX = (float)(pData->pointAy[1].fX * dDirY + pData->pointAy[0].fX * dDirX +
                   pData->pointAy[2].fX * dDirZ);
  fProjY = (float)(pData->pointAy[0].fY * dDirX + pData->pointAy[1].fY * dDirY +
                   pData->pointAy[2].fY * dDirZ);
  fProjZ = (float)(dDirX * pData->pointAy[0].fZ + dDirY * pData->pointAy[1].fZ +
                   dDirZ * pData->pointAy[2].fZ);
  ...
  *piElevation = iElevation;
  ...
  fRollVecX =
      (float)(fNegRollSinTimesPitchSin * tcos[iYaw2] - fRollCos * tsin[iYaw2]);
  fRollVecY =
      (float)(fNegRollSinTimesPitchSin2 * tsin[iYaw2] + fRollCos * tcos[iYaw2]);
  fRollVecZ = (float)(fRollSin2 * tcos[iPitch]);

  // Project reference points onto roll-adjusted vector
  fRollProjX = pData->pointAy[1].fX * fRollVecY +
               pData->pointAy[0].fX * fRollVecX +
               pData->pointAy[2].fX * fRollVecZ;
  fRollProjY = pData->pointAy[0].fY * fRollVecX +
               pData->pointAy[1].fY * fRollVecY +
               pData->pointAy[2].fY * fRollVecZ;
  v24 = pData->pointAy[0].fZ * fRollVecX + pData->pointAy[1].fZ * fRollVecY +
        pData->pointAy[2].fZ * fRollVecZ;
  ...
  // Apply elevation rotation
  fRollProjZ =
      (float)(fRollRotatedX * tsin[iElevation] - v24 * tcos[iElevation]);
