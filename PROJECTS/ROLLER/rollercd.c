...
static int CueCharLower(int c) { return tolower((unsigned char)c); }
...
static bool CueStringEndsWithIgnoreCase(const char *szText,
                                        const char *szSuffix) {
...
static void NormalizePathForCdio(const char *szPath, char *szNormPath,
                                 int nBufSize) {
...
static bool BuildTempCuePath(const char *szDir, char *szCuePath, int nBufSize) {
  for (int i = 0; i < 100; i++) {
    int iLen = SDL_snprintf(szCuePath, nBufSize, "%sroller_data_track_%02d.cue",
                            szDir, i);
...
void SaveDefaultFatalIni(const char *szWhipRoot) {
  ...
  if (!getcwd(szPreviousDir, sizeof(szPreviousDir))) {
    SDL_Log("ExtractFATDATA: failed to capture current directory before saving "
            "FATAL.INI: %s",
            strerror(errno));
    return;
  }
  ...
  if (ChangeWorkingDirectory(szPreviousDir) != 0)
    SDL_Log("ExtractFATDATA: failed to restore directory '%s': %s",
            szPreviousDir, strerror(errno));
}

//-------------------------------------------------------------------------------------------------

static void JoinCuePath(const char *szCueDir, const char *szFile, char *szOut,
                        int nBufSize) {
...
static bool ParseCueFileLine(const char *szLine, char *szFile, int nFileSize,
                             char *szType, int nTypeSize) {
  char szParsedFile[ROLLER_MAX_PATH];
  char szParsedType[32];

  if (sscanf(szLine, " FILE \"%259[^\"]\" %31s", szParsedFile, szParsedType) ==
      2) {
...
static bool ParseCueFile(const char *szCuePath, tCueInfo *pCueInfo) {
  ...
  while (fgets(szLine, sizeof(szLine), pCue)) {
    ...
    if (ParseCueFileLine(szLine, szFile, sizeof(szFile), szType,
                         sizeof(szType))) {
    ...
    if (sscanf(szLine, " TRACK %u %31s", &uiTrack, szMode) == 2) {
      ...
      SDL_strlcpy(pCurrentTrack->szFile, szCurrentFile,
                  sizeof(pCurrentTrack->szFile));
    ...
    char szTime[32];
    if (pCurrentTrack &&
        sscanf(szLine, " INDEX %u %31s", &uiIndex, szTime) == 2) {
      if (uiIndex == 1 &&
          ParseCueTimeToSector(szTime, &pCurrentTrack->iIndex01Sector))
...
void ExtractAudioTracks(CdIo_t *p_cdio, const char *szOutDir) {
  ...
  SDL_Log("ExtractAudioTracks: tracks %u-%u", (unsigned)uiFirst,
          (unsigned)uiLast);
  ...
  for (track_t t = 2; t <= uiLast; t++) {
    if (cdio_get_track_format(p_cdio, t) != TRACK_FORMAT_AUDIO) {
      SDL_Log("ExtractAudioTracks: track %u is not audio, skipping",
              (unsigned)t);
      continue;
    }
    ...
    if (lsnStart == CDIO_INVALID_LSN || lsnLast == CDIO_INVALID_LSN) {
      SDL_Log("ExtractAudioTracks: track %u has invalid LSN, skipping",
              (unsigned)t);
      continue;
    }
    ...
    SDL_snprintf(szWavPath, ROLLER_MAX_PATH, "%s/track%02u.wav", szAudioDir,
                 (unsigned)t);
    ...
    if (!pOut) {
      SDL_Log("ExtractAudioTracks: failed to open '%s': %s", szWavPath,
              strerror(errno));
      continue;
    }
...
#define AUDIO_CHUNK_SECTORS 32
    ...
    while (lsn <= lsnLast) {
      ...
      if (uiCount > AUDIO_CHUNK_SECTORS)
        uiCount = AUDIO_CHUNK_SECTORS;
      if (cdio_read_audio_sectors(p_cdio, szBuf, lsn, uiCount) !=
          DRIVER_OP_SUCCESS) {
...
#undef AUDIO_CHUNK_SECTORS
    ...
    fclose(pOut);
    SDL_Log("ExtractAudioTracks: wrote '%s' (%u bytes)", szWavPath,
            44 + uiDataBytes);
  }
}
...
static uint64 CueTrackEndByte(const tCueInfo *pCueInfo, int iTrackIndex,
                              uint64 uiFileSize) {
...
static void ExtractAudioTrackFromCue(const tCueInfo *pCueInfo, int iTrackIndex,
                                     const char *szAudioDir) {
  const tCueTrackInfo *pTrack = &pCueInfo->tracks[iTrackIndex];

  if (!pTrack->bBinaryFile) {
    SDL_Log(
        "ExtractAudioTracksFromCue: track %u is not a binary file, skipping",
        (unsigned)pTrack->uiTrack);
    return;
  }

  FILE *pIn = fopen(pTrack->szFile, "rb");
  if (!pIn) {
    SDL_Log("ExtractAudioTracksFromCue: failed to open '%s': %s",
            pTrack->szFile, strerror(errno));
    return;
  }

  uint64 uiFileSize;
  if (!GetOpenFileSize(pIn, &uiFileSize)) {
    SDL_Log("ExtractAudioTracksFromCue: failed to determine size of '%s'",
            pTrack->szFile);
    fclose(pIn);
    return;
  }
  ...
  if (uiStart >= uiFileSize || uiEnd <= uiStart) {
    SDL_Log(
        "ExtractAudioTracksFromCue: track %u has invalid byte range, skipping",
        (unsigned)pTrack->uiTrack);
    fclose(pIn);
    return;
  }
  ...
  if (uiDataBytes64 > 0xffffffffu) {
    SDL_Log("ExtractAudioTracksFromCue: track %u is too large for a WAV file, "
            "skipping",
            (unsigned)pTrack->uiTrack);
    fclose(pIn);
    return;
  ...
  FILE *pOut = fopen(szWavPath, "wb");
  if (!pOut) {
    SDL_Log("ExtractAudioTracksFromCue: failed to open '%s': %s", szWavPath,
            strerror(errno));
    fclose(pIn);
    return;
  }
...
#undef AUDIO_FILE_CHUNK_BYTES
  ...
  SDL_Log("ExtractAudioTracksFromCue: wrote '%s' (%llu bytes)", szWavPath,
          (unsigned long long)(44 + uiBytesWritten));
}

//-------------------------------------------------------------------------------------------------

static void ExtractAudioTracksFromCue(const tCueInfo *pCueInfo,
                                      const char *szOutDir) {
  char szAudioDir[ROLLER_MAX_PATH];
  SDL_snprintf(szAudioDir, ROLLER_MAX_PATH, "%s/audio", szOutDir);
  SDL_CreateDirectory(szAudioDir);
    ...
    if (pStat->type == _STAT_DIR) {
      ...
      if (pOut) {
        ...
        while (uiBytesLeft > 0) {
          ...
          uint32 uiToWrite =
              uiBytesLeft > ISO_BLOCK_SIZE ? ISO_BLOCK_SIZE : uiBytesLeft;
        ...
        SDL_Log("  -> FAILED to open '%s' for writing: %s", szOutPath,
                strerror(errno));
      }
    }
    iso9660_stat_free(pStat);
...
static bool WriteSingleTrackCue(const char *szSourceCuePath,
                                const tCueTrackInfo *pTrack, char *szCuePath,
                                int nBufSize) {
  ...
  if (!BuildTempCuePath(szTempCueDir, szCuePath, nBufSize)) {
    SDL_Log(
        "WriteSingleTrackCue: generated cue path is too long or unavailable");
    return false;
  }

  FILE *pCue = fopen(szCuePath, "w");
  if (!pCue) {
    SDL_Log("WriteSingleTrackCue: failed to open '%s': %s", szCuePath,
            strerror(errno));
    return false;
  }
  ...
  if (pTrack->bHasIndex01 && pTrack->iIndex01Sector != 0) {
    SDL_Log("WriteSingleTrackCue: data track INDEX 01 is not 00:00:00; reading "
            "from the file start");
  }
...
static bool ExtractFATDATAFromMultiFileCue(const char *szCuePath,
                                           const char *szOutDir,
                                           const tCueInfo *pCueInfo) {
  const tCueTrackInfo *pDataTrack = FindCueDataTrack(pCueInfo);
  if (!pDataTrack) {
    SDL_Log("ExtractFATDATAFromMultiFileCue: no data track found in '%s'",
            szCuePath);
    return false;
  }

  char szTempCuePath[ROLLER_MAX_PATH];
  if (!WriteSingleTrackCue(szCuePath, pDataTrack, szTempCuePath,
                           ROLLER_MAX_PATH))
    return false;
  ...
  if (!p_cdio) {
    SDL_Log("ExtractFATDATAFromMultiFileCue: cdio_open_am failed for '%s'",
            szNormTempCuePath);
...
void ExtractFATDATA(const char *szImagePath, const char *szOutDir) {
  ...
  SDL_Log("ExtractFATDATA: opening '%s', output dir '%s'", szImagePath,
          szOutDir);

  if (CueStringEndsWithIgnoreCase(szImagePath, ".cue")) {
    tCueInfo cueInfo;
    if (ParseCueFile(szImagePath, &cueInfo) && cueInfo.bMultiFile) {
      SDL_Log("ExtractFATDATA: detected multi-file CUE with %d tracks",
              cueInfo.iNumTracks);
