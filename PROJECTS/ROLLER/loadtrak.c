...
tSurface surface[14] = // 000A5FA8
    {{50.0, 100, 500.0, 75}, {50.0, 95, 500.0, 65}, {50.0, 90, 500.0, 55},
     {50.0, 85, 500.0, 50},  {50.0, 80, 500.0, 45}, {50.0, 75, 500.0, 40},
     {50.0, 70, 500.0, 35},  {50.0, 65, 500.0, 30}, {50.0, 60, 500.0, 25},
     {50.0, 55, 500.0, 20},  {50.0, 50, 500.0, 15}, {50.0, 40, 500.0, 10},
     {30.0, 30, 500.0, 5},   {10.0, 20, 500.0, 0}};
...
void loadtrack(int iTrackIdx, int iPreviewMode) {
  ...
  if ((unsigned int)iTrackIdx_1 <= 0x18) {
    pFile =
        ROLLERfopen(names[iTrackIdx_1], "r"); // Open and validate track file
    if (!pFile) {
    ...
    if ((int16)iCompactedFileLength ==
        8224) // Check if file is compacted (magic number 8224)
    {
  ...
  meof = 0;
  if (iTrackIdx_1 >= 0)
    readline2(&pCurrDataPtr, "iddd", &TRAK_LEN, &dWallCalc3, &dWallCalc2,
              &dWallCalc1); // Read track header: length and initial position
  iChunkIdx = 0;
  TrackFlags = 0;
  if (TRAK_LEN > 0) {
    p_iCenterSurfType =
        &TrakColour[0][TRAK_COLOUR_CENTER]; // Main track chunk processing loop
    ...
    do {
      if (iTrackIdx_1 >= 0) {
        ...
        readline2(&pCurrDataPtr, "ddddddddddddddiiidfsss", &dLeftShoulderWidth,
                  &dLeftLaneWidth, &dRightLaneWidth, &dRightShoulderWidth,
                  &dLeftShoulderHeight, &dRightShoulderHeight, &dLength, &dYaw,
                  &dPitch, &dRoll, &dAILine1, &dAILine2, &dAILine3, &dAILine4,
                  &iTrackGrip, &iLeftShoulderGrip, &iRightShoulderGrip,
                  &dAIMaxSpeed, p_fGroundLevel, p_nAudioAboveTrigger,
                  p_nAudioTriggerSpeed,
                  p_nAudioBelowTrigger); // Read track segment geometry data
        readline2(&pCurrDataPtr, "iiiiiiiiiiiiiiiiii",
                  &TrakColour[iTrakColourIdx], // LeftSurfType
                  p_iCenterSurfType, p_iRightSurfType, p_iLeftWallType,
                  p_iRightWallType, p_uiRoofType, p_uiLUOWallType,
                  p_iLLOWallType, p_iOFloorType, p_iRLOWallType, p_iRUOWallType,
                  p_iHorizonType, &iSignType, &iSignHOffset, &iSignVOffset,
                  &iSignYaw, &iSignPitch,
        ...
        readline2(&pCurrDataPtr, "dddddddddddddiiiuuuuuuuuuuuiii",
                  &dLUOWallHOffset, &dLLOWallHOffset, &dLOFloorHOffset,
                  &dROFloorHOffset, &dRLOWallHOffset, &dRUOWallHOffset,
                  &dLUOWallHeight, &dLLOWallHeight, &dLOFloorHeight,
                  &dROFloorHeight, &dRLOWallHeight, &dRUOWallHeight,
                  &dRoofHeight, &iDrawOrder1, &iForwardExtraStart, &iDrawOrder3,
                  ...
                  &Subdivide[iSubdivideIdx].subdivides[10], &iDrawOrderBackward,
                  &iBackwardExtraStart2,
                  &iBackwardExtraChunks); // Read wall heights, offsets and view
                                          // distance data
        if (bMinimalMode) {
        ...
        if ((cheat_mode & 0x1000) != 0 &&
            !bMinimalMode) // Apply double track width cheat if enabled
        {
        ...
        do {
          ...
          int iElementIdx =
              iSubdivArrayIdx %
              11; // Which element within the chunk's subdivides array
          if (!Subdivide[iChunkIdx].subdivides[iElementIdx] &&
              (unsigned int)iSubdivLoopIdx <= 10) {
            ...
            case 7:
            case 9:
              Subdivide[iChunkIdx].subdivides[iElementIdx] =
                  20; // 0x14 - Default subdivision level
              break;
            case 6:
            case 8:
            case 10:
              Subdivide[iChunkIdx].subdivides[iElementIdx] =
                  4; // Special subdivision level for specific geometry
              break;
            default:
          ...
          // if (!Subdivide[0].subdivides[iSubdivArrayIdx] && (unsigned
          // int)iSubdivLoopIdx <= 10)
        ...
        if (iForwardExtraStart < 0 || iForwardExtraStart > TRAK_LEN ||
            iDrawOrder3 < 8 || iDrawOrder3 > 128) {
        ...
        *(&TrakView[0].byForwardExtraChunks + v22) = iDrawOrder3;
        ...
        TrakView[uiTrakViewOffset / 8].byBackwardMainChunks =
            iDrawOrderBackward;
        if (iBackwardExtraStart < 0 || iBackwardExtraStart > TRAK_LEN ||
            iBackwardExtraChunks < 8 || iBackwardExtraChunks > 128) {
          iBackwardExtraStart2 = -1;
          iBackwardExtraChunks = 0;
        }
        uiTrakViewOffset2 = uiTrakViewOffset;
        TrakView[uiTrakViewOffset / 8].nBackwardExtraStart =
            iBackwardExtraStart2;
        TrakView[uiTrakViewOffset2 / 8].byBackwardExtraChunks =
            iBackwardExtraChunks;
        //*(&TrakView[0].byBackwardExtraChunks + uiTrakViewOffset2) =
        // iBackwardExtraChunks;
      }
      ...
      if (iSignType != -1) { // Process signs/objects: buildings vs towers
        if (iSignType < 256) {
          ...
          *pTowerBasePtr = (iSignType - 256) / 16;
          //*pTowerBasePtr = (iSignType - 256 - (__CFSHL__((iSignType - 256) >>
          // 31, 4) + 16 * ((iSignType - 256) >> 31))) >> 4;
          pTowerBasePtr = pTowerBase3 + 1;
          ++NumTowers;
        }
      }
      rotatepoint(0.0, dLeftShoulderWidth + dLeftLaneWidth, dLeftShoulderHeight,
                  dClampedYaw, dClampedPitch, dRoll, &dX, &dY,
                  &dZ); // Calculate and set track geometry points
      setpoint(iChunkIdx, 0, dWallCalc3 + dX, dWallCalc2 + dY, dWallCalc1 + dZ);
      rotatepoint(0.0, dLeftShoulderWidth + dLeftLaneWidth, 0.0, dClampedYaw,
                  dClampedPitch, dRoll, &dX, &dY, &dZ);
      setpoint(iChunkIdx, 1, dWallCalc3 + dX, dWallCalc2 + dY, dWallCalc1 + dZ);
      rotatepoint(0.0, dLeftLaneWidth, 0.0, dClampedYaw, dClampedPitch, dRoll,
                  &dX, &dY, &dZ);
      setpoint(iChunkIdx, 2, dWallCalc3 + dX, dWallCalc2 + dY, dWallCalc1 + dZ);
      rotatepoint(0.0, -dRightLaneWidth, 0.0, dClampedYaw, dClampedPitch, dRoll,
                  &dX, &dY, &dZ);
      setpoint(iChunkIdx, 3, dWallCalc3 + dX, dWallCalc2 + dY, dWallCalc1 + dZ);
      rotatepoint(0.0, -dRightLaneWidth - dRightShoulderWidth,
                  dRightShoulderHeight, dClampedYaw, dClampedPitch, dRoll, &dX,
                  &dY, &dZ);
      setpoint(iChunkIdx, 4, dWallCalc3 + dX, dWallCalc2 + dY, dWallCalc1 + dZ);
      rotatepoint(0.0, -dRightLaneWidth - dRightShoulderWidth, 0.0, dClampedYaw,
                  dClampedPitch, dRoll, &dX, &dY, &dZ);
      ...
      if (!bMinimalMode) { // Calculate ground points based on floor type
        if (GroundColour[uiGroundColourOffset / 0x14][GROUND_COLOUR_OFLOOR] ==
            -2) {
        ...
        if (v34 == -2) {
          rotatepoint(0.0, dTempCalc1, dLeftWallTotalHeight - dLOFloorHeight,
                      dClampedYaw, dClampedPitch, dRoll, &dX, &dY, &dZ);
          setgpoint(iChunkIdx, 0, dWallCalc3 + dX, dWallCalc2 + dY,
                    dWallCalc1 + dZ);
          rotatepoint(0.0, dLLOWallHOffset + dLOFloorHOffset + dLeftLaneWidth,
                      dLLOWallHeight - dLOFloorHeight, dClampedYaw,
                      dClampedPitch, dRoll, &dX, &dY, &dZ);
          setgpoint(iChunkIdx, 1, dWallCalc3 + dX, dWallCalc2 + dY,
                    dWallCalc1 + dZ);
          rotatepoint(0.0, dLOFloorHOffset + dLeftLaneWidth, -dLOFloorHeight,
                      dClampedYaw, dClampedPitch, dRoll, &dX, &dY, &dZ);
          setgpoint(iChunkIdx, 2, dWallCalc3 + dX, dWallCalc2 + dY,
                    dWallCalc1 + dZ);
          rotatepoint(0.0, -dROFloorHOffset - dRightLaneWidth, -dROFloorHeight,
                      dClampedYaw, dClampedPitch, dRoll, &dX, &dY, &dZ);
          setgpoint(iChunkIdx, 3, dWallCalc3 + dX, dWallCalc2 + dY,
                    dWallCalc1 + dZ);
          rotatepoint(0.0, -dRLOWallHOffset - dROFloorHOffset - dRightLaneWidth,
                      dRLOWallHeight - dROFloorHeight, dClampedYaw,
                      dClampedPitch, dRoll, &dX, &dY, &dZ);
          setgpoint(iChunkIdx, 4, dWallCalc3 + dX, dWallCalc2 + dY,
                    dWallCalc1 + dZ);
          rotatepoint(0.0,
                      -dRUOWallHOffset - dRLOWallHOffset - dROFloorHOffset -
                          dRightLaneWidth,
                      dRUOWallHeight + dRLOWallHeight - dROFloorHeight,
                      dClampedYaw, dClampedPitch, dRoll, &dX, &dY, &dZ);
          setgpoint(iChunkIdx, 5, dWallCalc3 + dX, dWallCalc2 + dY,
                    dWallCalc1 + dZ);
        } else {
          rotatepoint(0.0, dTempCalc1, dLeftWallTotalHeight + dLOFloorHeight,
                      dClampedYaw, dClampedPitch, 0.0, &dX, &dY, &dZ);
          setgpoint(iChunkIdx, 0, dWallCalc3 + dX, dWallCalc2 + dY, dZ);
          rotatepoint(0.0, dLLOWallHOffset + dLOFloorHOffset + dLeftLaneWidth,
                      dLLOWallHeight + dLOFloorHeight, dClampedYaw,
                      dClampedPitch, 0.0, &dX, &dY, &dZ);
        ...
        if (TrakColour[iTrakColourIdx][TRAK_COLOUR_LEFT_WALL] >= 0) {
          rotatepoint(0.0, dLeftLaneWidth + dLeftShoulderWidth,
                      dRoofHeight + dLeftShoulderHeight, dClampedYaw,
                      dClampedPitch, dRoll, &dX, &dY, &dZ);
        } else {
          dLeftShoulderHeight = 0.0;
          rotatepoint(0.0, dLeftLaneWidth, dRoofHeight, dClampedYaw,
                      dClampedPitch, dRoll, &dX, &dY, &dZ);
        }
        setpoint(iChunkIdx, 1, dWallCalc3 + dX, dWallCalc2 + dY,
                 dWallCalc1 + dZ);
        if (TrakColour[iTrakColourIdx][TRAK_COLOUR_RIGHT_WALL] >= 0) {
          rotatepoint(0.0, -dRightLaneWidth - dRightShoulderWidth,
                      dRoofHeight + dRightShoulderHeight, dClampedYaw,
                      dClampedPitch, dRoll, &dX, &dY, &dZ);
        } else {
          dRightShoulderHeight = 0.0;
          rotatepoint(0.0, -dRightLaneWidth, dRoofHeight, dClampedYaw,
                      dClampedPitch, dRoll, &dX, &dY, &dZ);
        }
        setpoint(iChunkIdx, 5, dWallCalc3 + dX, dWallCalc2 + dY,
                 dWallCalc1 + dZ);
        TrackInfo[iTrackInfoIdx].fLShoulderWidth = (float)
            dLeftShoulderWidth; // Store track info data for current chunk
        ...
        dLeftAngle =
            dLeftShoulderWidth != 0.0
                ? atan2(dLeftShoulderHeight / dLeftShoulderWidth, 1.0) *
                      16384.0 * dRadiansPerDegree
                : 0.0;
        //_CHP();
        iTemp3 = (int)dLeftAngle;
        dRightAngle = dRightShoulderWidth != 0.0
                          ? dRightShoulderHeight / dRightShoulderWidth
                          : 0.0;
        ...
        //*(float *)((char *)&localdata[0].fAILine2 + uiLocalDataOffset1) =
        // dAILine2;
        //*(float *)((char *)&localdata[0].fAILine3 + uiLocalDataOffset1) =
        // dAILine3;
        //*(float *)((char *)&localdata[0].fAILine4 + uiLocalDataOffset1) =
        // dAILine4;
        uiLocalDataOffset2 = uiLocalDataOffset;
        localdata[uiLocalDataOffset / 0x80].iCenterGrip = iTrackGrip;
        dAIMaxSpeed_1 = dAIMaxSpeed;
        localdata[uiLocalDataOffset2 / 0x80].iLeftShoulderGrip =
            iLeftShoulderGrip;
        localdata[uiLocalDataOffset2 / 0x80].iRightShoulderGrip =
            iRightShoulderGrip;
        //*(int *)((char *)&localdata[0].iLeftShoulderGrip + uiLocalDataOffset2)
        //= iLeftShoulderGrip;
        //*(int *)((char *)&localdata[0].iRightShoulderGrip +
        // uiLocalDataOffset2) = iRightShoulderGrip;
        ...
        //*(float *)((char *)&localdata[0].fAIMaxSpeed + uiLocalDataOffset2) =
        // dAIMaxSpeed_1;
        iOFloorType =
            GroundColour[uiGroundColourOffset1 / 0x14][GROUND_COLOUR_OFLOOR];
        // iOFloorType = *(int *)((char *)&GroundColour[0][GROUND_COLOUR_OFLOOR]
        // + uiGroundColourOffset1);
        TrackInfo[iTrackInfoIdx].iRightSurfaceType =
            -100; // Determine surface grip types for physics
        if (iOFloorType != -2) {
          TrackInfo[iTrackInfoIdx].iLeftSurfaceType = 0;
          TrackInfo[iTrackInfoIdx].iRightSurfaceType = 0;
        }
        if (GroundPt[uiGroundPtOffset / 0x48].pointAy[1].fZ >=
                (double)GroundPt[uiGroundPtOffset / 0x48].pointAy[2].fZ &&
            !TrakColour[iTrakColourIdx][TRAK_COLOUR_LEFT_WALL] &&
            GroundColour[uiGroundColourOffset / 0x14][GROUND_COLOUR_OFLOOR] ==
                -2) {
          TrackInfo[iTrackInfoIdx].iLeftSurfaceType = 1;
        }
        if (GroundPt[uiGroundPtOffset / 0x48].pointAy[4].fZ >=
                (double)GroundPt[uiGroundPtOffset / 0x48].pointAy[3].fZ &&
        ...
        if (TrakColour[iTrakColourIdx][TRAK_COLOUR_LEFT_WALL] > 0 &&
            GroundPt[uiGroundPtOffset / 0x48].pointAy[1].fZ >=
                (double)GroundPt[uiGroundPtOffset / 0x48].pointAy[2].fZ &&
            GroundColour[uiGroundColourOffset / 0x14][GROUND_COLOUR_OFLOOR] ==
                -2) {
        ...
        if (TrakColour[iTrakColourIdx][TRAK_COLOUR_RIGHT_WALL] > 0 &&
            GroundPt[uiGroundPtOffset / 0x48].pointAy[4].fZ >=
                (double)GroundPt[uiGroundPtOffset / 0x48].pointAy[3].fZ &&
            GroundColour[uiGroundColourOffset / 0x14][GROUND_COLOUR_OFLOOR] ==
                -2) {
        ...
        if ((!iLeftSurfaceType || iLeftSurfaceType == 2) &&
            (abs(TrakColour[iTrakColourIdx][TRAK_COLOUR_LEFT_LANE]) &
             SURFACE_FLAG_BOUNCE_20) == 0)
          TrackInfo[iTrackInfoIdx].iLeftSurfaceType = 1;
        iRightSurfaceType = TrackInfo[iTrackInfoIdx].iRightSurfaceType;
        if ((!iRightSurfaceType || iRightSurfaceType == 2) &&
            (abs(TrakColour[iTrakColourIdx][TRAK_COLOUR_RIGHT_LANE]) &
             SURFACE_FLAG_BOUNCE_20) == 0)
          TrackInfo[iTrackInfoIdx].iRightSurfaceType = 1;
        iLeftSurfCheck = TrackInfo[iTrackInfoIdx].iLeftSurfaceType;
        if ((iLeftSurfCheck == 6 || iLeftSurfCheck == 9) &&
            (abs(TrakColour[iTrakColourIdx][TRAK_COLOUR_LEFT_LANE]) &
             SURFACE_FLAG_BOUNCE_20) == 0)
          TrackInfo[iTrackInfoIdx].iLeftSurfaceType = 5;
        iRightSurfCheck = TrackInfo[iTrackInfoIdx].iRightSurfaceType;
        if ((iRightSurfCheck == 6 || iRightSurfCheck == 9) &&
            (abs(TrakColour[iTrakColourIdx][TRAK_COLOUR_RIGHT_LANE]) &
             SURFACE_FLAG_BOUNCE_20) == 0)
          TrackInfo[iTrackInfoIdx].iRightSurfaceType = 5;
        if ((abs(TrakColour[iTrakColourIdx][TRAK_COLOUR_LEFT_WALL]) &
        ...
        if ((abs(TrakColour[iTrakColourIdx][TRAK_COLOUR_RIGHT_WALL]) &
             SURFACE_FLAG_SKIP_RENDER) != 0) {
      ...
      dCos = cos(0.0174532925199 *
                 dPitch); // Calculate cosine for length projection
      ++iTrakColourIdx;
      ++iTrackInfoIdx;
      ++iChunkIdx;
      p_iCenterSurfType += 6;
      dWallCalc3 = cos(0.0174532925199 * dYaw) * (dCos * dLength) +
                   dWallCalc3; // Update position coordinates using trigonometry
      p_iRightSurfType += 6;
      p_iLeftWallType += 6;
      p_uiRoofType += 6;
      p_iLLOWallType += 5;
  ...
  if (!bMinimalMode)
    ReadAnimData(pFile_2,
                 &pCurrDataPtr); // Load animation data if not in minimal mode
  if (bMinimalMode) {
    if (iTrackIdx_1 >= 0)
    ...
    placecars();
    if (iTrackIdx_1 >= 0) {
      start_f = pCurrDataPtr; // Read additional track data: stunts, textures,
                              // buildings
  ...
  if (iTrackIdx_1 >= 0) {
    readline2(
        &pCurrDataPtr, "i",
        &actualtrack); // Read actual track ID and validate against requested
    if (iCompactedFlag && replaytype != 2 && iTrackIdx_1 != actualtrack &&
        !bMinimalMode) {
    ...
    if (!meof) {
      readline2(&pCurrDataPtr, "iiiiii", cur_laps, &cur_laps[1], &cur_laps[2],
                &cur_laps[3], &cur_laps[4],
                &cur_laps[5]); // Read lap configuration and map settings
    ...
    if (meof) {
      ...
      do {
        ...
        ++iDifficulty;
        // iLapValue = track_laps[0][iTrackLapOffset++];
        //*(_DWORD *)&samplemin[2 * iDifficulty + 498] = iLapValue;// offset
        // into cur_laps
      } while (iDifficulty < 6);
  ...
  do {
    if (iFoundBacks) {
      ...
      if (pTrackData == *ppTrackData)
        return;
      if (fp_buf[0] == 'B' && fp_buf[1] == 'A' && fp_buf[2] == 'C' &&
          fp_buf[3] == 'K' && fp_buf[4] == 'S')
        iFoundBacks = -1;
    }
  } while (!iBacksEnd);
}
...
void read_texturemap(uint8 **ppTrackData) {
  ...
  do
    memgets(fp_buf, ppTrackData); // Read lines from track data until we find a
                                  // line starting with 'T' (texture map entry)
  while (fp_buf[0] != 84);
  pszBufPtr = fp_buf;
  do
    byCurrentChar =
        *++pszBufPtr; // Scan forward in the line to find the ':' separator
  ...
  do {
    ...
    if ((uint8)byChar >= 0x61u &&
        (uint8)byChar <=
            0x7Au) // Convert lowercase letters to uppercase (a-z -> A-Z)
...
void read_bldmap(uint8 **ppTrackData) {
  ...
  pbyOriginalTrackData =
      *ppTrackData; // Save original track data pointer to restore later
  iFoundBLD = 0;
  *ppTrackData =
      start_f; // Reset track data pointer to start of file for searching
  do {
    pbyPrevPos = *ppTrackData;
    memgets(fp_buf, ppTrackData);   // Read next line from track data
    if (pbyPrevPos == *ppTrackData) // Check if we reached end of file (pointer
                                    // didn't advance)
      iFoundBLD = -1;
    if (fp_buf[0] == 66 && fp_buf[1] == 76 &&
        fp_buf[2] == 68) // Check if line starts with "BLD" (building map entry)
      iFoundBLD = -1;
  } while (!iFoundBLD);
  if (fp_buf[0] == 66 && fp_buf[1] == 76 &&
      fp_buf[2] == 68) // Process the BLD line if found
  {
    pbyBufPtr = fp_buf;
    if (fp_buf[0] != 58) {
      do
    ...
    pszFilenamePtr =
        (char *)(pbyBufPtr + 1); // Start of filename is right after the ':'
    iIndex = 0;
    do {
      byChar = *pszFilenamePtr++; // Copy building texture filename character by
                                  // character
    ...
    strcpy(
        (char *)fp_buf,
        "building.drh"); // Use default building texture if no BLD entry found
  }
...
void readstuntdata(uint8 **pTrackData) {
  ...
  do {
    readline2(
        ...
        pTrackData, "iiiiiiiiii", &iGeometryIndex, &iChunkCount, &iNumTicks,
        &iTickStartIdx, &iTimingGroup, &iHeight, &iTimeBulging, &iTimeFlat,
        &iRampSideLength, &iFlags);
    if (iGeometryIndex != -1) {
...
void activatestunts() {
  ...
  do {
    ...
    if (iGeometryIdx == -1) {
      ...
      pRampBuf = initramp(iGeometryIdx, iChunkCount, iNumTicks, iTickStartIdx,
                          iTimingGroup, iHeight, iTimeBulging, iTimeFlat,
                          iRampSideLength, iFlags);
  ...
  if (pFile) {
    do {
      ...
      if ((iSignType & 0x8000u) == 0) {
        ...
        // VisibleBuildings_variable_1[-(__int16)iSignType] = 0;// offset into
        // advert_list
      }
...
void readline2(uint8 **ppFileData, const char *pszFormat, ...) {
  ...
  while (1) {
    ...
    if (pszToken) {
      ...
      for (uiFormatIndex = 0; uiFormatIndex < strlen(pszFormat);
           ++uiFormatIndex) {
...
void rotatepoint(double dX, double dY, double dZ, double dYaw, double dPitch,
  ...
  dYawRad = dYaw * 0.0174532925199; // Convert X angle from degrees to radians
                                    // (PI/180 = 0.0174532925199)
  dPitchRad =
      dPitch * 0.0174532925199;       // Convert Y angle from degrees to radians
  ...
  *pdOutX = (dblCosX * dblSinY * dblSinZ - dblSinX * dblCosZ) * dY +
            dX * dblCosX * dblCosY +
            (-dblCosX * dblSinY * dblCosZ - dblSinX * dblSinZ) *
                dZ; // Apply 3D rotation matrix to calculate new X coordinate
  *pdOutY = (dblSinX * dblSinY * dblSinZ + dblCosX * dblCosZ) * dY +
            dX * dblSinX * dblCosY +
            (-dblSinX * dblSinY * dblCosZ + dblCosX * dblSinZ) *
                dZ; // Apply 3D rotation matrix to calculate new Y coordinate
  *pdOutZ =
      dblSinY * dX + dblSinZ * (-dblCosY * dY) +
      dZ * dblCosY *
          dblCosZ; // Apply 3D rotation matrix to calculate new Z coordinate
}
...
void min_skip_stuff(uint8 **ppFileData) {
  ...
  do
    readline2(ppFileData, "iiiiiiiiii", &iData0, &iData1, &iData2, &iData3,
              &iData4, &iData5, &iData6, &iData7, &iData8, &iData9);
  while (iData0 != -1);
  do
