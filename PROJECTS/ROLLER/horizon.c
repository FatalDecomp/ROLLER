...
void DrawHorizon(uint8 *pScrBuf) {
  ...
  pScrBuf_1 = pScrBuf;
  fSinElevation =
      tsin[worldelev & 0x3FFF]; // Calculate sine of world elevation (masked to
                                // 14 bits for lookup table)
  pScrPtr = pScrBuf;
  bySkyColor = 0x91;
  byGroundColor = HorizonColour[front_sec];

  // 0x10 = TEX_OFF_HORIZON
  if (fSinElevation > 0.7 || fSinElevation < -0.7 ||
      (textures_off & TEX_OFF_HORIZON) !=
          0) // Check for simple fill cases: steep angle or horizon textures
             // disabled
  {
    ...
    if ((textures_off & TEX_OFF_HORIZON) != 0)
      byFillColor = bySkyColor;
    memset(pScrBuf, byFillColor,
           winh * winw); // Simple case: fill entire screen with single color
  } else {
    iWorldTiltMasked =
        worldtilt &
        0x3FFF; // Complex horizon rendering: calculate tilt trigonometry
    fNegSinTilt = -tsin[iWorldTiltMasked];
    fCosTilt = tcos[iWorldTiltMasked];
    upside_down = tcos[worldelev & 0x3FFF] < 0.0 !=
                  fCosTilt < 0.0; // Determine if world is upside-down
                                  // (elevation vs tilt sign comparison)
    ground_left =
        fNegSinTilt <
        0.0; // Determine which side has ground based on tilt direction
    if (tcos[worldelev & 0x3FFF] < 0.0)
      ground_left = fNegSinTilt >= 0.0;
    dViewDistTan = (double)VIEWDIST *
                   ptan[worldelev & 0x3FFF]; // Calculate horizon line position
    ...
    // if ((LODWORD(fNegSinTilt) & 0x7FFFFFFF) != 0)// Calculate horizon slope
    // (rise/run) with divide-by-zero protection
    if (fabs(fNegSinTilt) > FLT_EPSILON) // Calculate horizon slope (rise/run)
                                         // with divide-by-zero protection
      fSlope = fCosTilt / fNegSinTilt;
    else
      fSlope = 41.0;
    if (fSlope <= 40.0 && fSlope >= -40.0) // Check if slope is reasonable for
                                           // scanline rendering (not too steep)
    {
      fScale = (float)((double)scr_size *
                       0.015625); // Scanline rendering case: calculate per-row
                                  // horizon intersection
      if (tcos[worldelev & 0x3FFF] <
          0.0) // Handle upside-down case by flipping tilt values
      {
        fCosTilt = -fCosTilt;
      ...
      fScanValue = (float)(((double)iHorizonY * fSlope + (double)iHorizonX) *
                           fScale); // Pre-calculate horizon intersection points
                                    // for each scanline
      if (winh > 0) {
        ...
        //   iLoopCounter += 4;
        //   *(float *)((char *)&GroundPt[499].pointAy[5].fZ + iLoopCounter) =
        //   fScanValue;// reference to hor_scan fScanValue =
        //   (float)dPrevScanValue;
      ...
      if (ground_left) // Branch: ground on left side of screen
      {
        ...
        if (winh > 0) {
          iScanIdx = 0;
          do {
            dScanValue = hor_scan[iScanIdx]; // Get horizon intersection point
                                             // for current scanline from
                                             // pre-calculated array
            //_CHP();
            iGroundWidth = (int)dScanValue;
            if ((int)dScanValue <=
                winw) // Check if horizon intersection is within screen bounds
            { // Horizon intersection is on-screen: render ground portion
              if ((int)dScanValue >= 0) {
                iGroundWidthCopy = iGroundWidth;
                memset(pScrPtr, byGroundColor,
                       iGroundWidth); // Fill ground pixels from left edge to
                                      // horizon intersection
                pScrPtr += iGroundWidthCopy;
              } else {
                dNegGroundWidth =
                    (double)iGroundWidth; // Horizon intersection is off-screen
                                          // left: calculate texture coordinates
                dInvScale = 1.0 / fScale;
                fTempCalc = (float)(-fCosTilt * dNegGroundWidth * dInvScale);
                modf(dNegGroundWidth * fNegSinTilt * dInvScale,
                     &dModfResult1); // modf() splits floating point into
              ...
              if (winw - iGroundWidth >= 0) {
                memset(pScrPtr, bySkyColor,
                       winw - iGroundWidth); // Fill sky pixels from horizon
                                             // intersection to right edge
                pScrPtr += iSkyWidth;
              }
            } else {
        ...
        pScrPtrRight = &pScrPtr[winw - 1]; // Branch: ground on right side of
                                           // screen (render right-to-left)
        iRowCounter2 = 0;
        if (winh > 0) {
          iScanIdx2 = 0;
          do {
            ...
            if ((int)dScanValue2 >=
                0) // Right-side rendering: check horizon intersection bounds
            {
              if (iGroundWidthTemp <= winw) {
                pScrPtrRight -= winw - iGroundWidthTemp;
                memset(pScrPtrRight + 1, byGroundColor,
                       winw - iGroundWidthTemp); // Fill ground pixels from
                                                 // intersection to right edge
                                                 // (right-to-left)
              } else {
                iYBase = iGroundWidthTemp -
                         (winw - 1); // Horizon extends beyond screen right:
              ...
              if (iGroundWidth2 >= 0) {
                ...
                memset(pScrPtrRight + 1, bySkyColor,
                       iGroundWidth2); // Fill sky pixels from left edge to
    ...
    } else { // Steep slope case: fill screen in large blocks rather than
             // scanlines
      if (upside_down) {
        iGroundHeight1 = (iHorizonY * scr_size + 32) / 64;
        // iGroundHeight1 = (iHorizonY * scr_size + 32 - (__CFSHL__((iHorizonY *
        // scr_size + 32) >> 31, 6) + ((iHorizonY * scr_size + 32) >> 31 << 6)))
        // >> 6;
        ...
        // iGroundHeight1 = (iHorizonY * scr_size - (__CFSHL__((iHorizonY *
        // scr_size) >> 31, 6) + ((iHorizonY * scr_size) >> 31 << 6))) >> 6;
      ...
      if (upside_down) {
        ...
        // iSkyHeight1 = (iHorizonY * scr_size + 32 - (__CFSHL__((iHorizonY *
        // scr_size + 32) >> 31, 6) + ((iHorizonY * scr_size + 32) >> 31 << 6)))
        // >> 6;
        if (iSkyHeight1 > winh)
          iSkyHeight1 = winh;
  ...
  // 0x8 = TEX_OFF_CLOUDS
  if ((textures_off & TEX_OFF_CLOUDS) ==
      0) // Render clouds on top of horizon if cloud textures enabled
    displayclouds(pScrBuf_1);
}

//-------------------------------------------------------------------------------------------------
// 0006B1A0
void initclouds() { // Loop through all 40 cloud slots
  int iCloudIdx;    // edi
  // int iRandVal1; // eax
  ...
  for (iCloudIdx = 0; iCloudIdx < 40; ++iCloudIdx) {
    ...
    do {
      // iRandVal1 = ROLLERrandRaw();                       // Generate random
      // angle1 (theta) for spherical coordinates uiAngle1Calc = (uint32)3400 *
      // (uint32)GetHighOrderRand(iRandVal1, iRandVal1);
      ////iAngle1Calc = 3400 * (((iRandVal1 * iRandVal1) & 0x7FFF) >> 15);
      ////iAngle1Calc = 3400 * ((iRandVal1 * iRandVal1 - (__CFSHL__((iRandVal1 *
      /// iRandVal1) >> 31, 15) + ((iRandVal1 * iRandVal1) >> 31 << 15))) >>
      /// 15);
      // iAngle1 = ((uiAngle1Calc & 0x8000) >> 15) + 520;  // Will be either 520
      // or 521
      ////iAngle1 = ((iAngle1Calc - (__CFSHL__(iAngle1Calc >> 31, 15) +
      ///(iAngle1Calc >> 31 << 15))) >> 15) + 520;
      // iRandVal2 = ROLLERrandRaw();                       // Generate random
      // angle2 (phi) for spherical coordinates
      //
      ////TODO look at this
      // iAngle2 = GetHighOrderRand(2, iRandVal2);  // Will be 0 or 1
      ////iAngle2 = ((iRandVal2 << 14) - (__CFSHL__(iRandVal2 << 14 >> 31, 15) +
      ///(iRandVal2 << 14 >> 31 << 15))) >> 15;

      iAngle1 = (ROLLERrandRaw() & 0x0FFF) + 0x200;
      iAngle2 = ROLLERrandRaw() & 0x3FFF;

      fCos1Cos2 =
          tcos[iAngle2] * tcos[iAngle1]; // Calculate rotation matrix elements
      ...
      fCos1 = tcos[iAngle2];
      fRotComp1 =
          -fCos1 * fSin2; // Calculate rotation matrix components:
                          // -cos(phi)*sin(theta) and -sin(phi)*sin(theta)
      fRotComp2 = fNegSin1 * fSin2;
      fNegRadius = -fRadius; // Negative radius for translation calculations
      fBaseX = 10000000.0f * fCos1Cos2; // Calculate 3x4 transformation matrix
      ...
      cloud[iCloudIdx].world.fX = fRotComp1 * 0.0f + 0.0f * fNegSin1 +
                                  fBaseX; // World X coordinate: final cloud
                                          // position after transformation
      cloud[iCloudIdx].world.fY =
          0.0f * fCos1 + fBaseY +
          0.0f * fRotComp2; // World Y coordinate: final cloud position after
                            // transformation
      cloud[iCloudIdx].world.fZ =
          fBaseZ + 0.0f * fCos2; // World Z coordinate: final cloud position
                                 // after transformation
      iRandColorBase =
          ROLLERrandRaw(); // Generate random cloud color/texture index
      iColorIndex = GetHighOrderRand(5, iRandColorBase);
      // iColorIndex = (5 * iRandColorBase - (__CFSHL__((5 * iRandColorBase) >>
      // 31, 15) + ((5 * iRandColorBase) >> 31 << 15))) >> 15;
      cloud[iCloudIdx].iSurfaceType = iColorIndex;
      cloud[iCloudIdx].iSurfaceType = iColorIndex + 0x508;
      if (ROLLERrand() <
          0x4000) // Randomly modify color properties (50% chance each)
        cloud[iCloudIdx].iSurfaceType += 0x1000;
      if (ROLLERrand() < 0x4000)
        SET_HIWORD(cloud[iCloudIdx].iSurfaceType,
                   4 + GET_HIWORD(cloud[iCloudIdx].iSurfaceType));
      ...
      if (iCloudIdx > 0) { // Check collision with previously placed clouds
        for (iCheckIdx = 0; iCheckIdx < iCloudIdx; ++iCheckIdx) {
          if (iValidPlacement) {
            ...
            fDistance = (float)sqrt(
                fXYDistSq +
                fDeltaZ *
                    fDeltaZ); // Calculate 3D distance between cloud centers
          }
          fMinDistance = fRadius + cloud[iCloudIdx].fRadius;
          // TODO
          if (fDistance <
              fMinDistance * 1.3f) // Check if clouds overlap (distance <
                                   // combined radii * 1.3)
            iValidPlacement = 0;
        }
      }
      if (!iValidPlacement)
        fRadius =
            fRadius + -2000.0f; // Reduce radius and retry if collision detected
...
void displayclouds(uint8 *pScrBuf) {
  ...
  if ((textures_off & 8) ==
      0) // Skip cloud rendering if textures disabled (bit 3 of textures_off)
  {      // Loop through all 40 cloud objects
    for (iCloudIdx = 0; iCloudIdx != 40; ++iCloudIdx) {
      dCorner0X =
          cloud[iCloudIdx].matrix[0][0] -
          viewx; // Transform cloud corner 0 from world space to camera space
      dCorner0Y = cloud[iCloudIdx].matrix[0][1] - viewy;
      dCorner0Z = cloud[iCloudIdx].matrix[0][2] * 0.5 - viewz;
      fViewX0 = (float)(dCorner0X * vk1 + dCorner0Y * vk4 +
                        dCorner0Z * vk7); // Apply view matrix transformation to
      ...
      dViewDist = (double)VIEWDIST; // Project 3D coordinates to 2D screen
                                    // coordinates using perspective division
      dInvZ0 = 1.0 / fViewZ0;
      ...
      //_CHP();
      yp = (int)dProjY0;
      fScreenX0 = (float)((long long)iScrSizeTemp * xp >>
                          6); // Convert to screen space and check clipping
                              // bounds (-5000 to +5000)
      ...
      if (iBehindCamera || fScreenX0 >= -5000.0 && fScreenX0 <= 5000.0 &&
                               fScreenY0 >= -5000.0 && fScreenY0 <= 5000.0) {
        dScreenX0 =
            fScreenX0; // Store vertex 0 coordinates in polygon structure
        //_CHP();
      ...
      if (!iBehindCamera) {
        dCorner1X = cloud[iCloudIdx].matrix[1][0] -
                    viewx; // Transform cloud corner 1 (repeat process for
                           // second vertex)
        ...
        if (iBehindCamera || fScreenX1 >= -5000.0 && fScreenX1 <= 5000.0 &&
                                 fScreenY1 >= -5000.0 && fScreenY1 <= 5000.0) {
          dScreenX1 =
              fScreenX1; // Store vertex 1 coordinates in polygon structure
          //_CHP();
        ...
        if (!iBehindCamera) {
          dCorner2X = cloud[iCloudIdx].matrix[2][0] -
                      viewx; // Transform cloud corner 2 (third vertex of quad)
          dCorner2Y = cloud[iCloudIdx].matrix[2][1] - viewy;
          dCorner2Z = cloud[iCloudIdx].matrix[2][2] * 0.5 - viewz;
          fViewX2 =
              (float)(dCorner2X * vk1 + dCorner2Y * vk4 + dCorner2Z * vk7);
          fViewY2 =
              (float)(dCorner2X * vk2 + dCorner2Y * vk5 + dCorner2Z * vk8);
          ...
          if (iBehindCamera || fScreenX2 >= -5000.0 && fScreenX2 <= 5000.0 &&
                                   fScreenY2 >= -5000.0 &&
                                   fScreenY2 <= 5000.0) {
            dScreenX2 =
                fScreenX2; // Store vertex 2 coordinates in polygon structure
            //_CHP();
          ...
          if (!iBehindCamera) {
            dCorner3X =
                cloud[iCloudIdx].matrix[3][0] -
                viewx; // Transform cloud corner 3 (final vertex of quad)
            dCorner3Y = cloud[iCloudIdx].matrix[3][1] - viewy;
            dCorner3Z = cloud[iCloudIdx].matrix[3][2] * 0.5 - viewz;
            fViewX3 =
                (float)(dCorner3X * vk1 + dCorner3Y * vk4 + dCorner3Z * vk7);
            fViewY3 =
                (float)(dCorner3X * vk2 + dCorner3Y * vk5 + dCorner3Z * vk8);
            ...
            if (iBehindCamera || fScreenX3 >= -5000.0 && fScreenX3 <= 5000.0 &&
                                     fScreenY3 >= -5000.0 &&
                                     fScreenY3 <= 5000.0) {
              dScreenX3 =
                  fScreenX3; // Store vertex 3 coordinates in polygon structure
              //_CHP();
