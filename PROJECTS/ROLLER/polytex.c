...
fixed16_16 startsx[4] = {
    0x3FF000, 0x0, 0x0,
    0x3FF000}; // 000A7474 0x3FF000 = 64.0 in 16.16 fixed point
fixed16_16 startsy[4] = {
    0x0, 0x0, 0x3FF000,
    0x3FF000}; // 000A7484 0x3FF000 = 64.0 in 16.16 fixed point
...
uint8 *mapsel[4884];    // 0019EC28 changed to uint8* by ROLLER, original code
                        // these are 16-bit DOS selectors, [19][257]
...
void remove_mapsels() {
  ...
  //   iCarIdx = 1;
  //   iCarMapselOffset = 514;                     // starting offset: 514 =
  //   257*2 (skip types 0 and 1) do {
  //     iMapselOffset = iCarMapselOffset;
  //     iCarDataOffset = iCarIdx * 4;             // 4 bytes per car entry
  //
  //     // Process all textures for this car
  //     for (iTextureIdx = 0; iTextureIdx < bld_remap[iCarIdx + 255];
  //     iMapselOffset += 2)// reference into num_textures
  //     {
  //       // Check if selector is allocated
  //       if (*(int16 *)((char *)mapsel[0] + iMapselOffset)) {
  //         // Free the selector
  //         regs.w.bx = *(int16 *)((char *)mapsel[0] + iMapselOffset);
  //         regs.w.ax = 1;                        // DPMI function: free LDT
  //         descriptor int386(0x31, &regs, &regs);           // DPMI services
  ...
  //     *(int *)((char *)&bld_remap[255] + iCarDataOffset) = 0;// reference
  //     into num_textures
  //
      ...
      fIntersection < 1.0) // valid intersection exists
  {
    ...
    if (iCrossProduct1 <= 0) {
      ...
      *vertices = vertices[2];
      ...
      dIntersectionX1 =
          (double)nEdge23X * fIntersection + (double)vertices[2].x;
      //_CHP();
      vertices[2].x = (int)dIntersectionX1;
      dIntersectionY1 =
          (double)nEdge23Y * fIntersection + (double)vertices[2].y;
      //_CHP();
      ...
      //_CHP();
      startsx[2] =
          (int)((double)iDeltaTexX23 * fIntersection + (double)iOgStartsX2);
      ...
      startsy[2] =
          (int)(fIntersection * (double)iDeltaTexY23 + (double)iOgStartsY2);
    ...
    *pCurrVert = pVert1Copy;
    ...
    fCrossProduct2 =
        (float)(nDiagonal13Y * nDiagonal03X - nDiagonal03Y * nDiagonal13X);
    if (fabs(fCrossProduct2) != 0.0 // non-parallel
        ...
        (iCrossProduct2_1 = nEdge02X * nDiagonal03Y - nDiagonal03X * nEdge02Y,
         ...
         fIntersection_1 =
             (float)((double)iCrossProduct2_1 / (double)fCrossProduct2),
         fIntersection_1 > 0.0) &&
        fIntersection_1 < 1.0) // valid intersection
    {
      ...
      if (iCrossProduct2_1 <= 0) {
        ...
        *vertices = vertices[1];
        ...
        //_CHP();
        vertices[2].x = (int)((double)nDiagonal13X * fIntersection_1 +
                              (double)pIntersectionVert_1.x);
        dIntersectionY1_2 =
            (double)nDiagonal13Y * fIntersection_1 + (double)vertices[2].y;
        //_CHP();
        ...
        //_CHP();
        startsx[2] =
            (int)((double)(iOgStartsX0_1 - iOgStartsX2_1) * fIntersection_1 +
                  (double)iOgStartsX2_1);
        //_CHP();
        startsy[2] =
            (int)(fIntersection_1 * (double)(iOgStartsY0_1 - iOgStartsY2_2) +
                  (double)iOgStartsY2_2);
      ...
      } else // intersection from other side
      {
        ...
        //_CHP();
        startsx[2] =
            (int)((double)(iOgStartsX1_1 - iOgStartsX2_1) * fIntersection_1 +
                  (double)iOgStartsX2_1);
        //_CHP();
        startsy[2] =
            (int)(fIntersection_1 * (double)(iOgStartsY1_1 - iOgStartsY2_2) +
                  (double)iOgStartsY2_2);
...
//-------------------------------------------------------------------------------------------------
// 0006C620
void POLYTEX(uint8 *pTexture, uint8 *pScrBuf, tPolyParams *pPolyParams,
             int iTexIdx, int iGfxSize) {
  ...
  if ((uint8)pPolyParams->iSurfaceType >= NoOfTextures &&
          (uiSurfaceType & SURFACE_FLAG_SKIP_RENDER) == 0 && !iTexIdx ||
      (uint8)uiSurfaceType >= BldTextures &&
          (uiSurfaceType & SURFACE_FLAG_SKIP_RENDER) == 0 && iTexIdx == 17) {
  ...
  if ((uiSurfaceType & SURFACE_FLAG_SKIP_RENDER) == 0) {
    ...
    // SURFACE_FLAG_BACK added by ROLLER, TODO: why didn't the IDA decomp check
    // for this and how does it work?
    if ((uiSurfaceType & (SURFACE_FLAG_FLIP_BACKFACE | SURFACE_FLAG_BACK)) !=
        0) {
      ...
      if ((pPolyParams->vertices[0].x - pPolyParams->vertices[1].x) *
              iDeltaY02 >
          iDeltaY01 * iDeltaX02) {
    ...
    if ((uiSurfaceType & SURFACE_FLAG_PARTIAL_TRANS) != 0) {
      ...
      // iMapselOffset = 514 * iTexIdx + 2 * (uint8)uiSurfaceType; //offset
      // assumes array of int16s
...
void polyt(tPoint *vertices, int iNumVerts, uint8_t *pTex) {
  ...
  float fLeftTexX = 0.0f,
        fLeftTexY = 0.0f; // Current texture coords on left edge
  float fRightTexX = 0.0f,
        fRightTexY = 0.0f; // Current texture coords on right edge
  float fLeftTexXStep = 0.0f,
        fLeftTexYStep = 0.0f; // Texture coord steps for left edge
  float fRightTexXStep = 0.0f,
        fRightTexYStep = 0.0f; // Texture coord steps for right edge
  ...
  if (iScanlineY >= 0) {
    ...
    do {
      ...
      if (iLeftEdgeHeight > 0) {
        ...
        fLeftTexXStep = (FROM_FIXED(startsx[iLeftVertexIdx]) - fLeftTexX) /
                        (float)iLeftEdgeHeight;
        fLeftTexYStep = (FROM_FIXED(startsy[iLeftVertexIdx]) - fLeftTexY) /
                        (float)iLeftEdgeHeight;
    ...
    do {
      ...
      if (iRightEdgeHeight > 0) {
        ...
        // Calculate texture coordinate steps
        fRightTexXStep = (FROM_FIXED(startsx[iRightVertexIdx]) - fRightTexX) /
                         (float)iRightEdgeHeight;
        fRightTexYStep = (FROM_FIXED(startsy[iRightVertexIdx]) - fRightTexY) /
                         (float)iRightEdgeHeight;
    ...
    if (iLeftEdgeHeight > 0) {
      ...
      fLeftTexXStep = (FROM_FIXED(startsx[iLeftVertexIdx]) - fTexXStart) /
                      (float)iTotalHeight;
      fLeftTexYStep = (FROM_FIXED(startsy[iLeftVertexIdx]) - fTexYStart) /
                      (float)iTotalHeight;
    ...
    if (iRightEdgeHeight > 0) {
      ...
      fRightTexXStep = (FROM_FIXED(startsx[iRightVertexIdx]) - fTexXStart) /
                       (float)iTotalHeight;
      fRightTexYStep = (FROM_FIXED(startsy[iRightVertexIdx]) - fTexYStart) /
                       (float)iTotalHeight;
    ...
    if (iLeftX < iRightX && iRightX > 0 && iLeftX < winw) {
      ...
      if (iStartX < iEndX) {
        ...
        for (int x = iStartX; x < iEndX; x++) {
          // Extract texture coordinates (convert float to int for 64x64
          // texture)
          int iU = (int)fTexX;
          int iV = (int)fTexY;
