...
void menu_render_sw_sprite(MenuRendererSoftware *sw, int slot, int blockIdx,
  ...
  (void)sw;
  (void)palette;
  display_block(scrbuf, front_vga[slot], blockIdx, x, y, transparentColorIndex);
}
...
void menu_render_sw_scaled_text(MenuRendererSoftware *sw, int fontSlot,
  ...
  (void)sw;
  (void)palette;
  scale_text(front_vga[fontSlot], (char *)text, mappingTable, charVOffsets, x,
             y, (char)colorReplace, alignment, clipLeft, clipRight);
}
...
void menu_render_sw_load_car_mesh(MenuRendererSoftware *sw, int carIdx,
  ...
  if (texIdx > 0 && cartex_vga[texIdx - 1]) {
    scene_render_load_texture(sw->scene, cartex_vga[texIdx - 1], 256, 0, texIdx,
                              gfx_size);
  }
}
...
void menu_render_sw_draw_car_preview(MenuRendererSoftware *sw, float angle,
  ...
  (void)destX;
  (void)destY;
  if (sw->loadedCarIdx < 0 || sw->loadedCarIdx > CAR_DESIGN_DEATH)
    return;
  if (!CarDesigns[sw->loadedCarIdx].pCoords)
    return;
...
void menu_render_sw_draw_track_preview(MenuRendererSoftware *sw, float cameraZ,
  ...
  (void)destW;
  (void)destH;
  // Original code: show_3dmap uses xbase/ybase globals internally (set by
  // init_screen)
