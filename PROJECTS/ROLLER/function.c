...
float mapsize[25] = // 000A45C0
    {6000.0, 6000.0, 3500.0, 4500.0, 6000.0, 3000.0, 4000.0, 3000.0, 4000.0,
     6000.0, 6000.0, 6000.0, 6000.0, 6000.0, 6000.0, 6000.0, 6000.0, 6000.0,
...
void finish_race() {
  ...
  if (iLap <= NoOfLaps) // Check if race is still in progress (current lap <=
                        // total laps)
  {
        ...
        do { // Find human player and update finishing order
          if (human_control[iCarIndex] && !finished_car[iCarIndex]) {
            ...
            if (iTargetPosition > iRacePosition) {
              iPositionIndex =
                  iRacePosition; // Shift car positions in finishing order array
              do {
    ...
    } else if (game_type == 1 && Race > 0) // Championship mode: Order AI cars
                                           // by championship standings
    {
      iChampAiIndex = 0;
      iAiOrderIndex = 0;
      if (iAiCarCount > 0) {
      ...
      iRandomOrderCounter =
          0; // Random placement mode: Randomly order AI cars for finish
      if (iAiCarCount > 0) {
        iAiPlacementIndex = 0;
        while (1) {
          iRandomValue = ROLLERrandRaw();
          iRandomCarIndex = GetHighOrderRand(numcars, iRandomValue);
          // iRandomCarIndex = (iRandomValue * numcars -
          // (__CFSHL__((iRandomValue * numcars) >> 31, 15) + ((iRandomValue *
          // numcars) >> 31 << 15))) >> 15;
          if (iRandomCarIndex == numcars)
            iRandomCarIndex = numcars - 1;
          if (!human_control[iRandomCarIndex] &&
              !finished_car[iRandomCarIndex]) {
    ...
    for (iCarCountera = 0; iCarCountera < NoOfLaps;
         ++iCarCountera) // Generate lap times for each AI car for all laps
    {
      iLapTimeIndex = 0;
      if (iAiCarCount > 0) {
        iLapTimeCarIndex = 0;
        do {
          ...
          fRandomLapVariation =
              (float)((double)ROLLERrand() * 0.000030517578125 +
                      0.02); // Add random variation (0.02-0.05s) to base lap
                             // time
          if (iLapTimeIndex)
            dRunningLapTime =
                Car[carorder[iLapTimeCarIndex - 1]].fRunningLapTime;
          // dRunningLapTime = Car[nearcall[3][iLapTimeCarIndex +
          // 3]].fRunningLapTime;
          else
            dRunningLapTime = RecordLaps[game_track];
          Car[iLapTimeCarId].fRunningLapTime =
              (float)dRunningLapTime + fRandomLapVariation;
          iRaceTimeCarId = iLapTimeCarId;
          Car[iRaceTimeCarId].fTotalRaceTime =
              Car[iRaceTimeCarId].fRunningLapTime +
              Car[iRaceTimeCarId].fTotalRaceTime;
          if (Car[iRaceTimeCarId].fBestLapTime >
              (double)Car[iRaceTimeCarId].fRunningLapTime)
            Car[iRaceTimeCarId].fBestLapTime =
                Car[iRaceTimeCarId].fRunningLapTime;
          ++iLapTimeIndex;
          ++iLapTimeCarIndex;
    ...
    fTotalRaceTime =
        Car[carorder[0]].fTotalRaceTime; // Calculate final race positions based
                                         // on total race times
    ...
    if (iAiCarCount > 0) {
      ...
      do {
        ...
        if (dTimeDiff > fReferenceLapTime) // Calculate how many laps behind
    ...
    while (finishers < racers) {
      ...
      if (iLap_1 == iTotalLaps) // Check if car completed all laps
      {
        ...
        for (Car[iFinishingCarId_1].fTotalRaceTime = fWinnerTotalTime + 0.02f;
             ...
             Car[iFinishingCarId_1].fTotalRaceTime =
                 (float)((double)ROLLERrand() * 0.1 * 0.000030517578125 + 0.1 +
                         Car[iFinishingCarId_1]
                             .fTotalRaceTime)) // Adjust finishing time based on
                                               // track position
        {
          ++iDistanceCounter;
        }
...
double getbankz(float fInput, int iChunkIdx, tData *pData) {
  ...
  dCalcVal = (pGroundPt->pointAy[0].fY + pUseData->pointAy[3].fY) *
                 pUseData->pointAy[1].fY +
             (pGroundPt->pointAy[0].fX + pUseData->pointAy[3].fX) *
                 pUseData->pointAy[0].fY +
             (pGroundPt->pointAy[0].fZ + pUseData->pointAy[3].fZ) *
                 pUseData->pointAy[2].fY;
  ...
  while (1) {
    fCurrCalcVal =
        (pPoint1->fY + pUseData->pointAy[3].fY) * pUseData->pointAy[1].fY +
...
INTERPOLATION_CALC:
  ...
  return ((pGroundPt2->pointAy[iSelectedPointIdx].fY +
           pUseData->pointAy[3].fY) *
              pUseData->pointAy[1].fZ +
          (pGroundPt2->pointAy[iSelectedPointIdx].fX +
           pUseData->pointAy[3].fX) *
              pUseData->pointAy[0].fZ +
          (pGroundPt2->pointAy[iSelectedPointIdx].fZ +
           pUseData->pointAy[3].fZ) *
              pUseData->pointAy[2].fZ) *
             (fInput - fCurrCalcVal)         // weight for first point
             / (fPrevCalcVal - fCurrCalcVal) // total range
         + ((pGroundPt2->pointAy[iSelectedPointIdx + 1].fY +
             pUseData->pointAy[3].fY) *
                pUseData->pointAy[1].fZ +
            (pGroundPt2->pointAy[iSelectedPointIdx + 1].fX +
             pUseData->pointAy[3].fX) *
                pUseData->pointAy[0].fZ +
            (pGroundPt2->pointAy[iSelectedPointIdx + 1].fZ +
             pUseData->pointAy[3].fZ) *
                pUseData->pointAy[2].fZ) *
...
//-------------------------------------------------------------------------------------------------
// 000373A0
void calculate_aheadbehindtime(int iCarIdx, float *pfAheadTime,
                               float *pfBehindTime) {
  int iCurrentCarId;          // eax
  int iCurrChunk;             // esi
  int iAheadCarId;            // edi
  ...
  if ((char)Car[iCurrentCarId].byLives >
      0) // Check if current car is alive, otherwise return -1 for both times
  {
    iCurrChunk = Car[iCurrentCarId]
                     .nCurrChunk; // Calculate current car's absolute position
                                  // on track (chunk + lap distance)
    if (iCurrChunk == -1)
      iCurrChunk = Car[iCurrentCarId].iLastValidChunk;
    fCurrentCarPosition =
        (float)(averagesectionlen * iCurrChunk +
                totaltrackdistance * (char)Car[iCarIdx].byLapNumber);
    if (Car[iCarIdx].nCurrChunk != -1)
      fCurrentCarPosition = fCurrentCarPosition + Car[iCarIdx].pos.fX;
    if (Car[iCarIdx].byRacePosition) // Calculate time to car ahead (if not in
                                     // first position)
    {
      // TODO recheck this offset to ensure it is correct
      iAheadCarId = carorder[Car[iCarIdx].byRacePosition -
                             1]; // Get car ahead from race position order
      if ((char)Car[iAheadCarId].byLives > 0) {
        nCurrChunk = Car[iAheadCarId].nCurrChunk;
        if (nCurrChunk == -1)
          nCurrChunk = Car[iAheadCarId].iLastValidChunk;
        fAheadCarPosition =
            (float)(averagesectionlen * nCurrChunk +
                    totaltrackdistance *
                        (char)Car[iAheadCarId]
                            .byLapNumber); // Calculate ahead car's absolute
                                           // position on track
        if (Car[iAheadCarId].nCurrChunk != -1)
          fAheadCarPosition = fAheadCarPosition + Car[iAheadCarId].pos.fX;
        iAheadCarRef = iAheadCarId;
        fAheadDistance = (float)fabs(
            fCurrentCarPosition -
            fAheadCarPosition); // Calculate absolute distance between cars
        if (Car[iCarIdx].fFinalSpeed <=
            (double)Car[iAheadCarRef]
                .fFinalSpeed) // Use higher speed of the two cars for time
                              // calculation
        ...
        if (fAheadDistance >=
            (double)fTrackDistance) // Convert distance to time: if >1 lap apart
                                    // use large penalty, else distance/speed
    ...
    iRacePosition = Car[iCarIdx].byRacePosition; // Calculate time to car behind
                                                 // (if not in last position)
    if (iRacePosition == numcars - 1) {
      *pfBehindTime = -1.0;
    } else {
      iBehindCarId = carorder[iRacePosition +
                              1]; // Get car behind from race position order
      if ((char)Car[iBehindCarId].byLives > 0) {
        ...
        fBehindCarPosition =
            (float)(averagesectionlen * iLastValidChunk +
                    totaltrackdistance *
                        (char)Car[iBehindCarId]
                            .byLapNumber); // Calculate behind car's absolute
                                           // position and distance/time gap
        ...
        if (Car[iCurrentCarRef].fFinalSpeed <=
            (double)Car[iBehindCarRef].fFinalSpeed)
...
void initnearcars() {
  ...
  memset(
      nearcall, 255,
      sizeof(nearcall)); // Initialize nearcall array with -1 (no car assigned)
  ...
  for (int iGroup = 0; iGroup < 4;
       iGroup++) // Loop through 4 nearcall groups: [0],[1],[2],[3]
  {
    iCarsPerGroup =
        iAICarsRemaining /
        iGroupSize; // Calculate how many cars to assign to current group
    ...
    iAICarsRemaining -=
        iCarsPerGroup; // Subtract assigned cars from remaining count
    iGroupSize--;      // Decrease group size for uneven distribution
  }
  // iCurrentCarIdx = 0;
  // iGroupSize = 4;                               // Start with group size 4,
  // distribute AI cars across 4 groups for (iGroupOffset = 0; iGroupOffset !=
  // 64; iGroupOffset += 16)// Loop through 4 nearcall groups: [0],[1],[2],[3]
  //{
  //   iCarsPerGroup = iAICarsRemaining / iGroupSize;// Calculate how many cars
  //   to assign to current group iGroupCarCount = 0; if (iAICarsRemaining /
  //   iGroupSize > 0) {
  //     iNearCallOffset = iGroupOffset;
  //     do {                                         // Find next AI car (skip
  //     non-competitors and human players)
  //       for (iTempCarIdx = iCurrentCarIdx; non_competitors[iTempCarIdx] ||
  //       human_control[iTempCarIdx]; ++iTempCarIdx)
  //         ++iCurrentCarIdx;
  //       ++iGroupCarCount;
  //       *(int *)((char *)nearcall[0] + iNearCallOffset) = iCurrentCarIdx;//
  //       Assign AI car index to current nearcall group slot iNearCallOffset +=
  //       4;
  //       ++iCurrentCarIdx;
  //     } while (iGroupCarCount < iCarsPerGroup);
  //   }
  //   iAICarsRemaining -= iCarsPerGroup;          // Subtract assigned cars
  //   from remaining count
  //   --iGroupSize;                               // Decrease group size for
  //   uneven distribution
  // }
...
void initpits() {
  ...
  if (TRAK_LEN - 1 >= 0) {
    ...
    do {
      if ((TrakColour[iChunkIdx][TRAK_COLOUR_CENTER] & SURFACE_FLAG_PIT_BOX) !=
          0) {
...
void dodamage(tCar *pCar, float fDamage) {
  ...
  pCarEngine = &CarEngines.engines[pCar->byCarDesignIdx]; // Get car engine data
                                                          // based on car design
  fHealth = pCar->fHealth;
  iDriverIdx = pCar->iDriverIdx;
  if (!invulnerable[iDriverIdx] &&
      !finished_car[iDriverIdx]) // Check if car is vulnerable and not finished
  { // Check if car is not already destroyed (status flag 4)
    if ((pCar->byStatusFlags & 4) == 0) {
      pCarSpray = CarSpray[pCar->iDriverIdx];
      if (fDamage >
          0.55) // Handle visual spray effects for significant damage (> 0.55)
      {
        //_CHP();
        ...
        do {
          ...
          ++pCarSpray;
                 ...
                 32); // Loop through all 32 spray effects for this car
        if (player_type == 2) {
          pPlayerSpray = 0;
    ...
    pCar->fHealth =
        pCar->fHealth -
        fDamage * pCarEngine->fDurability; // Apply damage to car health
                                           // (modified by engine durability)
    if (pCar->fHealth <
        1.0) // Handle car destruction when health drops below 1.0
    {
      ...
      if ((byStatusFlags & 4) == 0) {
        ...
        if (iPlayerType != 2 && (cheat_mode & CHEAT_MODE_CLONES) == 0) {
          ...
          if (player1_car != iDeadDriverIdx &&
              pCar->byCarDesignIdx == Car[player1_car].byCarDesignIdx &&
              !human_control[iDeadDriverIdx] && pCar->byCarDesignIdx <= 7u) {
            // SOUND_SAMPLE_ARIEL
            speechsample(pCar->byCarDesignIdx + SOUND_SAMPLE_ARIEL, 20000, 18,
                         player1_car + 17152);
            // SOUND_SAMPLE_TGONE
        ...
        if (!pCar->byLives && !finished_car[iCurrentDriverIdx]) {
          iRacePosition = pCar->byRacePosition; // Update race positions when
                                                // car is eliminated
          ++Destroyed;
        ...
        if (player1_car == iCheckDriverIdx || player2_car == iCheckDriverIdx) {
          ...
          speechsample(
              SOUND_SAMPLE_FATALITY, 0x8000, 18,
              pCar->iDriverIdx); // Play death sound effects for human players
          if (!pCar->byLives) {
            // SOUND_SAMPLE_0LEFT
        ...
        if (pCar->byDamageSourceTimer && pCar->byAttacker == player1_car) {
          ...
          if ((char)pCar->byLives <= 0)
            sprintf(buffer, "%s %s", &language_buffer[1984],
                    driver_names[pCar->iDriverIdx]);
          else
            sprintf(buffer, "%s %s", &language_buffer[1920],
                    driver_names[pCar->iDriverIdx]);
          small_zoom(buffer);
          if (pCar->byDamageSourceTimer) {
            sprintf(buffer, "%s %s", &language_buffer[2176],
                    driver_names[pCar->byAttacker]);
            subzoom(buffer);
          }
        }
        if (player_type == 2) {
          if (pCar->byDamageSourceTimer && pCar->byAttacker == player2_car) {
            ...
            if (player2_car != iPlayer2DriverIdx) {
              if ((char)pCar->byLives <= 0)
                sprintf(buffer, "%s %s", &language_buffer[1984],
                        driver_names[iPlayer2DriverIdx]);
              else
                sprintf(buffer, "%s %s", &language_buffer[1920],
                        driver_names[iPlayer2DriverIdx]);
              small_zoom(buffer);
    ...
    if (pCar->fHealth < 30.0 &&
        fHealth >= 30.0) // Handle transition from healthy to critical health
                         // (30.0 threshold)
    {
      iPlayerTypeCheck = player_type;
      pCar->nChangeMateCooldown = 1080;
      // CHEAT_MODE_CLONES
      if (iPlayerTypeCheck != 2 && (cheat_mode & CHEAT_MODE_CLONES) == 0 &&
          player1_car != pCar->iDriverIdx) {
        byCarDesignIdx = pCar->byCarDesignIdx;
        if (byCarDesignIdx == Car[player1_car].byCarDesignIdx &&
            !human_control[pCar->iDriverIdx] && byCarDesignIdx <= 7u) {
          // SOUND_SAMPLE_ARIEL
          speechsample(byCarDesignIdx + SOUND_SAMPLE_ARIEL, 20000, 18,
                       player1_car + 17408);
  ...
  if ((player1_car == iFinalDriverIdx || player2_car == iFinalDriverIdx) &&
      pCar->fHealth < 30.0 && fHealth >= 30.0 && pCar->byCarDesignIdx <= 7u)
    // SOUND_SAMPLE_BLOWTIME
    speechsample(SOUND_SAMPLE_BLOWTIME, 20000, 18,
                 pCar->iDriverIdx); // Play critical health warning sound for
                                    // human players
}
...
void doviewtend(tCar *pCar, int iFrameDelta, int iViewIdx) {
  ...
  iSelectedView =
      SelectedView[iViewIdx]; // Get the selected view type for this view index
                              // (a3: 0=player1 view, 1=player2 view)
  if (iSelectedView == 1 ||
      iSelectedView == 3) // Only process chase views (view types 1 and 3)
  {
    iControlType =
        pCar->iControlType; // Get car control type to determine pull behavior
    iPullSpeed = 40 * iFrameDelta; // Calculate pull speed increment (40 units
                                   // per frame) * frame delta
    if (iControlType < 2) { // Control type 0: Autopilot/Default car behavior
      if (!iControlType) {
        dChasePullDefault = viewdata[chaseview[iViewIdx]]
                                .fChasePullDefault; // Get default chase pull
                                                    // distance from view data
        //_CHP();
    ...
    } else if (iControlType <=
               3) { // Check if car is stunned/crashed to use crash pull values
      if (pCar->iStunned) {
        dChasePullCrash =
            viewdata[chaseview[iViewIdx]]
                .fChasePullCrash; // Use crash pull distance for stunned cars
        //_CHP();
        ...
        dChasePullNormal =
            viewdata[chaseview[iViewIdx]]
                .fChasePullNormal; // Use normal pull distance for active cars
        //_CHP();
    ...
    } else if (iControlType == 1002) // Control type 1002: High-speed mode with
                                     // modified pull distances
    {
      iHighSpeedPullIncrement =
          160 * iFrameDelta; // Use faster pull speed for high-speed mode (160
                             // units per frame)
      if (pCar->iStunned) {
        dHighSpeedCrashPull =
            viewdata[chaseview[iViewIdx]].fChasePullCrash +
            -400.0; // High-speed crash: use crash pull distance minus 400 units
        //_CHP();
        ...
        if (PULLZ[iViewIdx] > (double)fTargetPullHighSpeedCrash) {
          dTempHighSpeedCrash =
              PULLZ[iViewIdx] - (double)iHighSpeedPullIncrement;
        ...
        if (PULLZ[iViewIndex4] < (double)fTargetPullHighSpeedCrashLimit) {
          dTempHighSpeedCrash2 =
              (double)(40 * iFrameDelta) + PULLZ[iViewIndex4];
        ...
        dHighSpeedNormalPull =
            viewdata[chaseview[iViewIdx]].fChasePullNormal +
            400.0; // High-speed normal: use normal pull distance plus 400 units
        //_CHP();
        ...
        if (PULLZ[iViewIndex5] < (double)fTargetPullHighSpeedLimit) {
          dTempHighSpeedNormal2 =
              (double)(40 * iFrameDelta) + PULLZ[iViewIndex5];
...
void changemateto(int iCarIndex, int iNewStrategy) {
  ...
  if ((iCarIndex & 1) != 0)
    ...
    iMateCarIndex = iCarIndex + 1;
  if ((char)Car[iMateCarIndex].byLives > 0 &&
      !human_control[iMateCarIndex]) // Check if mate car is alive and
                                     // AI-controlled
  { // Check if mate already has the requested strategy
    if (iNewStrategy ==
        Car[iMateCarIndex]
            .iSelectedStrategy) { // Play acknowledgment speech when mate
                                  // already has requested strategy
      // CHEAT_MODE_CLONES
      if (player_type != 2 && iCarIndex == player1_car &&
          (cheat_mode & CHEAT_MODE_CLONES) == 0)
        speechsample(iNewStrategy + 57, 20000, 18, iCarIndex);
    } else {
      iRandomValue = ROLLERrandRaw(); // Random chance (1/4) or cooldown check -
                                      // mate may refuse strategy change
      if (GetHighOrderRand(4, iRandomValue) == 3 ||
          Car[iMateCarIndex].nChangeMateCooldown) {
        // CHEAT_MODE_CLONES
        if (player_type == 2 || iCarIndex != player1_car ||
            (cheat_mode & CHEAT_MODE_CLONES) != 0) {
          ROLLERrandRaw();
          ...
          iSpeechParam =
              player1_car + ((Car[iMateCarIndex].iSelectedStrategy + 61)
                             << 8); // Play refusal speech sequence when mate
                                    // rejects strategy change
          iRandomSpeech = ROLLERrandRaw();
          speechsample(GetHighOrderRand(2, iRandomSpeech) + 65, 20000, 18,
                       iSpeechParam);
          speechsample(Car[iMateCarIndex].iSelectedStrategy + 61, 20000, 0,
                       player1_car);
        }
        iCooldownCarIndex = iMateCarIndex;
        if (!Car[iCooldownCarIndex].nChangeMateCooldown)
          Car[iCooldownCarIndex].nChangeMateCooldown =
              1080; // Set strategy change cooldown (1080 ticks)
      } else {
        Car[iMateCarIndex].iSelectedStrategy =
            iNewStrategy; // Accept strategy change and play confirmation speech
        // CHEAT_MODE_CLONES
        if (player_type != 2 && iCarIndex == player1_car &&
            (cheat_mode & CHEAT_MODE_CLONES) == 0)
          speechsample(iNewStrategy + 57, 20000, 18, iCarIndex);
...
//-------------------------------------------------------------------------------------------------
// 000384C0
// valid when car movement is not blocked by i.e. fence separating track from
// run-off area top-down view, driving up (= start is down, end is up):
/******************************************************************
...
bool linevalid(int iChunkIdx, float fStartCoord, float fEndCoord) {
  ...
  // Check if invisible
  iNotRendered =
      abs(TrakColour[iChunkIdx][TRAK_COLOUR_CENTER]) & SURFACE_FLAG_SKIP_RENDER;
  ...
  if (!uiStartRegion) // start outside positive side
  {
    ...
    {
      // Line crosses entire track, blocked if any barriers present and track
      // rendered
...
void analysefalloff(tCar *pCar) {
  ...
  if (dCarY <=
      pData->fTrackHalfWidth) // Determine which lane/area the car is in: 0=left
                              // shoulder, 1=on track, 2=right shoulder
  {
  ...
  if (!iLaneType) // Handle left shoulder collision - set wall position and
  ...
  if (iLaneType > 1) {
    ...
    iWallFlag = TrakColour[pCar->nCurrChunk][TRAK_COLOUR_CENTER] &
                SURFACE_FLAG_SKIP_RENDER; // ((HIDWORD(v8) ^ v8) - HIDWORD(v8))
                                          // & 0x20000;
  LABEL_11:
  ...
  iWallFlag =
      TrakColour[pCar->nCurrChunk][TRAK_COLOUR_LEFT_LANE] &
      SURFACE_FLAG_SKIP_RENDER; //((HIDWORD(v7) ^ v7) - HIDWORD(v7)) & 0x20000;
  fTrackHalfWidth = pData->fTrackHalfWidth;
LABEL_12:
  if (iWallFlag &&
      fTrackHalfWidth <
          pCar->fCarHalfWidth * 0.5 +
              pCar->pos.fY) // Check if car is hitting the wall - if wall exists
                            // and car position exceeds wall boundary
  {
    nActualYaw = pCar->nActualYaw;
    dSinVelocity =
        pCar->fFinalSpeed *
        tsin[nActualYaw]; // Calculate lateral velocity component (sin
                          // component) for wall collision handling
    if (dSinVelocity < 30.0) {
      ...
      if (fSinVelocityTemp2 >= 0.0) {
        ...
        pCar->nActualYaw = getangle(
            fCosVelocityTemp3, 40.0); // Adjust car yaw and speed when bouncing
                                      // off left wall - redirect momentum
        pCar->fFinalSpeed =
            (float)sqrt(fCosVelocityTemp3 * fCosVelocityTemp3 + 1600.0);
      }
    }
    dRollMomentum =
        tcos[pCar->nYaw] * 200.0 +
        (double)pCar->iRollMomentum; // Apply roll momentum based on car
                                     // orientation - banking effect on slopes
    //_CHP();
  ...
  if (iLaneType) {
    if (iLaneType <= 1) {
      ...
      iWallFlag2 = TrakColour[pCar->nCurrChunk][TRAK_COLOUR_RIGHT_LANE] &
                   SURFACE_FLAG_SKIP_RENDER; //((HIDWORD(v15) ^ v15) -
                                             // HIDWORD(v15)) & 0x20000;
    } else {
    ...
    iWallFlag2 = TrakColour[pCar->nCurrChunk][TRAK_COLOUR_CENTER] &
                 SURFACE_FLAG_SKIP_RENDER; //((HIDWORD(v14) ^ v14) -
                                           // HIDWORD(v14)) & 0x20000;
    fWallPosition = pData->fTrackHalfWidth;
  }
  if (iWallFlag2 &&
      fWallPosition > pCar->pos.fY - pCar->fCarHalfWidth *
                                         0.5) // Check for right wall collision
                                              // - mirror of left wall logic
  {
    ...
    if (dSinVelocity2 > -30.0) {
      ...
      if (fSinVelocityTemp <= 0.0) {
        ...
        pCar->nActualYaw = getangle(
            fCosVelocity,
            -40.0); // Adjust car yaw and speed when bouncing off right wall
  ...
  iPrevChunk = pCar->nCurrChunk -
               1; // Check previous track chunk for front wall collision
  if (iPrevChunk < 0)
    iPrevChunk = TRAK_LEN - 1;
  // v22 = TrakColour[iPrevChunk][iLaneType];
  // if ((((HIDWORD(v22) ^ (unsigned int)v22) - HIDWORD(v22)) & 0x20000) != 0 &&
  // -pData->fTrackHalfLength > pCar->pos.fX - pCar->fCarWidthBankingProjection
  // * 0.5) {
  if ((TrakColour[iPrevChunk][iLaneType] & SURFACE_FLAG_SKIP_RENDER) != 0 &&
      -pData->fTrackHalfLength >
          pCar->pos.fX - pCar->fCarWidthBankingProjection * 0.5) {
    ...
    if (dCosVelocity > -30.0) {
      ...
      if (fCosVelocityTemp2 <= 0.0) {
        ...
        pCar->nActualYaw = getangle(
            -40.0, fSinVelocity); // Handle front wall collision - adjust yaw by
  ...
  // v26 = TrakColour[pCar->nCurrChunk + 1][iLaneType];// Check next track chunk
  // for rear wall collision if ((((HIDWORD(v26) ^ (unsigned int)v26) -
  // HIDWORD(v26)) & 0x20000) != 0 && pData->fTrackHalfLength <
  // pCar->fCarWidthBankingProjection * 0.5 + pCar->pos.fX) {
  if ((TrakColour[pCar->nCurrChunk + 1][iLaneType] &
    ...
    if (dCosVelocity2 < 30.0) {
      ...
      if (fCosVelocityTemp >= 0.0) {
        ...
        pCar->nActualYaw = getangle(
            40.0, fSinVelocity2); // Handle rear wall collision - adjust yaw by
                                  // redirecting backward momentum
...
void showmap(uint8 *pScrPtr, int iCarIdx) {
  ...
  if ((cheat_mode & CHEAT_MODE_DOUBLE_TRACK) !=
      0) // Double scale if cheat mode bit 0x1000 is set
    fMapScale = cur_mapsize * 2.0f;
  iChunk2 = Car[iCarIdx]
                .nReferenceChunk; // Calculate view angle based on car direction
  ...
  dInvScale =
      1.0 / fMapScale; // Setup coordinate transformation for map display
  dRelX =
      (pCarChunkData->pointAy[3].fX - localdata[0].pointAy[3].fX) * dInvScale;
  dRelY =
      (pCarChunkData->pointAy[3].fY - localdata[0].pointAy[3].fY) * dInvScale;
  pData = pCarChunkData;
  dRotatedX = dRelX * tcos[iViewAngle] -
              dRelY * tsin[iViewAngle]; // line skipped decompiler artifact
  //_CHP();
  dRotatedY_1 = dRelX * tsin[iViewAngle] + dRelY * tcos[iViewAngle];
  //_CHP();
  iCalcResult =
      scr_size * ((int)dRotatedX + 38); // line skipped decompiler artifact
  ...
  while (1) {
    ...
    dSectionRelX =
        (pData->pointAy[3].fX - pSectionData->pointAy[3].fX) * dSectionInvScale;
    dSectionRelY =
        (pData->pointAy[3].fY - pSectionData->pointAy[3].fY) * dSectionInvScale;
    dRotatedX =
        dSectionRelX * tcos[iViewAngle] -
        dSectionRelY * tsin[iViewAngle]; // line skipped decompiler artifact
    //_CHP();
    dSectionRotatedY =
        dSectionRelX * tsin[iViewAngle] + dSectionRelY * tcos[iViewAngle];
    //_CHP();
    iSectionCalc =
        scr_size * ((int)dRotatedX + 38); // line skipped decompiler artifact
    iSectionScreenXTemp = iSectionCalc >> 6;
    iSectionScreenY = (scr_size * (160 - (int)dSectionRotatedY)) >> 6;
    compout(pScrPtr, iNextScreenX, iScreenY, (iSectionCalc >> 6) + 1,
            iSectionScreenY, 0x70u);
  ...
  if (cur_mapsect < TRAK_LEN) // Draw track edges with different colors
  {
    ...
    do {
      ...
      dLoopRelX =
          (pData->pointAy[3].fX - pLoopSectionData->pointAy[3].fX) * fInvScale;
      dLoopRelY =
          (pData->pointAy[3].fY - pLoopSectionData->pointAy[3].fY) * fInvScale;
      dRotatedX =
          dLoopRelX * tcos[iViewAngle] -
          dLoopRelY * tsin[iViewAngle]; // line skipped decompiler artifact
      //_CHP();
      dLoopRotatedY =
          dLoopRelX * tsin[iViewAngle] + dLoopRelY * tcos[iViewAngle];
      //_CHP();
      iLoopCalc =
          scr_size * ((int)dRotatedX + 38); // line skipped decompiler artifact
      iSectionScreenX = iLoopCalc >> 6;
      iY1 = (scr_size * (160 - (int)dLoopRotatedY)) >> 6;
      if ((TrakColour[iMapSectLoop][0] & SURFACE_FLAG_PIT_ZONE) != 0 ||
          (TrakColour[iMapSectLoop][1] & SURFACE_FLAG_PIT_ZONE) != 0 ||
          (TrakColour[iMapSectLoop][2] & SURFACE_FLAG_PIT_ZONE) != 0)
        compout(pScrPtr, iCurrentScreenX, iCurrentScreenY, iSectionScreenX, iY1,
                0xCDu);
      else
        compout(pScrPtr, iCurrentScreenX, iCurrentScreenY, iSectionScreenX, iY1,
                0x83u);
  ...
  if ((TrakColour[iLastSection][0] & SURFACE_FLAG_PIT_ZONE) != 0 ||
      (TrakColour[iLastSection][1] & SURFACE_FLAG_PIT_ZONE) != 0 ||
      (TrakColour[iLastSection][2] & SURFACE_FLAG_PIT_ZONE) != 0)
    compout(pScrPtr, iCurrentScreenX, iCurrentScreenY, iX1, iOrigScreenY,
            0xCDu);
  else
    compout(pScrPtr, iCurrentScreenX, iCurrentScreenY, iX1, iOrigScreenY,
            0x83u);
  ...
  do {
    iCurrentX = iX1 - 1;
    for (i = 0; i < 3; ++i) {
      if (iCurrentX >= 0 && iCurrentX < iWinWidth && iMarkerY >= 0 &&
          iMarkerY < winh)
  ...
  if (numcars > 0) // Draw all cars on the minimap
  {
    ...
    do {
      ...
      if ((char)pCarArray->byLives > 0) {
        ...
        dCarInvScale =
            1.0 / fMapScale; // Transform car position to screen coordinates
        dTransformedX = (fX + pData->pointAy[3].fX) * dCarInvScale;
        dTransformedY = (fY + pData->pointAy[3].fY) * dCarInvScale;
        dRotatedX =
            dTransformedX * tcos[iViewAngle] - dTransformedY * tsin[iViewAngle];
        //_CHP();
        dRotatedY =
            dTransformedX * tsin[iViewAngle] + dTransformedY * tcos[iViewAngle];
        ...
        if (iDriverIdx ==
            iCarIdx) // Special handling for player car (larger marker)
        {
        ...
        } else if (iCarScreenX > 0 && iWinWidth - 1 > iCarScreenX &&
                   iCarScreenY > 0 &&
                   winh - 1 > iCarScreenY) // Draw car marker (cross pattern)
        {
  ...
  if ((char)Car[iCarIdx].byLives > 0 &&
      (Car[iCarIdx].byRacePosition || (frames & 8) != 0)) {
...
void firework_display() {
  ...
  for (updates = 0; readptr != writeptr;
       readptr = ((uint16)readptr + 1) &
                 0x1FF) { // Only advance animation if not paused
    if (!paused)
      ++updates;
    dozoomstuff(0);
        ...
        if (byType) { // Handle ascending rocket (type 1)
          if (byType <= 1u) {
            ...
            if (iLifetimeCount > 0) {
              ...
              dVelocityY = pSprayData->velocity.fY +
                           0.1; // Apply gravity and update rocket position
              pSprayData->velocity.fY = (float)dVelocityY;
              pSprayData->position.fY =
                  (float)dVelocityY + pSprayData->position.fY;
              ...
              sfxsample(
                  SOUND_SAMPLE_EXPLO,
                  0x8000); // Rocket reached peak - create explosion particles
              ...
              iParticleCount = 0;
              iFireworkColor =
                  firework_colours[GetHighOrderRand(12, iRandColor)];
              do {
                pSprayData->iType = 2;
                ...
                // iAngleIndex = ((iRandAngle << 14) - (__CFSHL__(iRandAngle <<
                // 14 >> 31, 15) + (iRandAngle << 14 >> 31 << 15))) >> 15;

                dRandVelocity =
                    (double)ROLLERrandRaw() * 2.0 * 0.000030517578125;
                pSprayData->velocity.fX =
                    (float)dRandVelocity * tcos[iAngleIndex] + fOrigVelX;
                pSprayData->velocity.fY =
                    (float)dRandVelocity * tsin[iAngleIndex] + fOrigVelY;
                iRandParticleLife = ROLLERrandRaw();
                ++pSprayData;
                ++iParticleCount;
                pSprayData[-1].iLifeTime =
                    GetHighOrderRand(36, iRandParticleLife) + 36;
          ...
          } else if (byType == 2) // Handle explosion particles (type 2)
          {
            ...
            for (j = 0; j < 32; ++j) {
              ...
              if (iLifeTime > 0) {
                dParticleVelY =
                    pSprayData->velocity.fY +
                    0.1; // Apply gravity to particle and update position
                pSprayData->velocity.fY = (float)dParticleVelY;
                pSprayData->position.fY =
                    (float)dParticleVelY + pSprayData->position.fY;
                dParticleVelX =
                    pSprayData->velocity.fX + pSprayData->position.fX;
            ...
            if (!iActiveParticles) {
              pCleanupSpray =
                  CarSpray[i]; // All particles expired - reset spray slot
              do {
                pCleanupSpray += 4;
          ...
          if (iTimerCount < 0) {
            iWidthDiv3 =
                winw / 3; // Launch new rocket with random position and velocity
            iRandX = ROLLERrandRaw();
            pSprayData->position.fX =
                (float)(2 * winw / 3 - GetHighOrderRand(iWidthDiv3, iRandX));
            pSprayData->position.fY = (float)winh;
            iRandVelX = ROLLERrandRaw();
            pSprayData->velocity.fX =
                (float)(2 - GetHighOrderRand(4, iRandVelX));
            pSprayData->velocity.fY =
                (float)(-4.8 -
                        (double)ROLLERrandRaw() * 1.2 * 0.000030517578125);
          ...
          pSprayData->position.fZ =
              -1.0; // Initialize new firework with random launch delay
