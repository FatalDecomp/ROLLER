...
void game_render_sw_destroy(GameRendererSoftware *sw) { free(sw); }

// ---------------------------------------------------------------------------
// Frame lifecycle
// ---------------------------------------------------------------------------

void game_render_sw_begin_frame(GameRendererSoftware *sw) { (void)sw; }
...
void game_render_sw_quad_screen(GameRendererSoftware *sw, tPolyParams *poly,
  ...
  if (handle > 0 && handle < GAME_RENDER_MAX_TEXTURE_SLOTS &&
    ...
    POLYTEX(slot->pixels, screen_pointer, poly, slot->tex_idx, slot->gfx_size);
  } else {
