...
char revs_files1[6][13] = // 000A41C0
    {"minitext.bm", "font6.bm", "panel2.bm", "font3.bm", "pancar1.bm", ""};
char revs_files2[6][13] = // 000A420E
    {"minitext.bm", "font6.bm", "panel2.bm", "font3.bm", "pancar2.bm", ""};
...
void plotxyz(float fWorldX, float fWorldY, float fWorldZ, char byColor) {
  ...
  if (fabs(k1) <= 5600.0 && fabs(k2) <= 5600.0 && fabs(k3) <= 5600.0) {
    ...
    if (fTransformedZ >= (double)fViewDistance) {
      ...
      dScreenX =
          fViewDistance * fTransformedX * dPerspectiveScale + (double)xbase;
      //_CHP
      iScreenXInt = (int)dScreenX;
      dScreenY =
          dPerspectiveScale * (fViewDistance * fTransformedY) + (double)ybase;
      //_CHP

      // Clip to screen bounds and plot pixel to 320x200 screen
      if ((int)dScreenX >= 0 && iScreenXInt <= 319 &&
          (unsigned int)(int)dScreenY < 200)
...
void linexyz(uint8 *pScrBuf, float fX1, float fY1, float fZ1, float fX2,
  ...
  fRightProj_1 =
      k1 * vk2 + k2 * vk5 + k3 * vk8;       // Right (X) projection for point 1
  fUpProj = k1 * vk3 + k2 * vk6 + k3 * vk9; // Up (Y) projection

  // Transform point 2 to camera space
  fClippedForwardProj =
      k4 * vk1 + k5 * vk4 + k6 * vk7; // Forward projection for point 2
  fRightProj =
      k4 * vk2 + k5 * vk5 + k6 * vk8;        // Right (X) projection for point 2
  ...
  if (fDepthOffset1 < 0.0 && fDepthOffset2 > 0.0 ||
      fDepthOffset1 > 0.0 && fDepthOffset2 < 0.0) {
    // Line crosses near plane - interpolate to find clipping point
    dClipInterpolationFactor = 1.0 / (fDepthOffset1 - fDepthOffset2);
    fClippedRightProj =
        (float)(dClipInterpolationFactor *
                (fRightProj * fDepthOffset1 - fDepthOffset2 * fRightProj_1));

    if (fDepthOffset1 >= 0.0) {
      // Point 1 is behind near plane, clip it
      fDepthOffset2 = 0.0;
      fRightProj = fClippedRightProj;
      fForwardProj = (float)((fClippedForwardProj * fDepthOffset1 -
                              fForwardProj * fDepthOffset2) *
                             dClipInterpolationFactor);
    } else {
      // Point 2 is behind near plane, clip it
      fDepthOffset1 = 0.0;
      fRightProj_1 = fClippedRightProj;
      fClippedForwardProj = (float)((fClippedForwardProj * fDepthOffset1 -
                                     fForwardProj * fDepthOffset2) *
                                    dClipInterpolationFactor);
    }
  }
  ...
  if (fFinalDepth1 >= (double)fViewDist && fFinalDepth2 >= (double)fViewDist) {
    ...
    iScaledScreenX1 = (int)((fViewDist * fForwardProj * (1.0 / fFinalDepth1) +
                             dScreenCenterX) *
                            dScrScale);
    ...
    // Convert scaled X coordinate to pixel coordinate (divide by 64 for
    // fixed-point)
    iScreenX1 = iScaledScreenX1 >> 6;

    // Y coordinate for point 1
    int iScaledScreenY1 =
        (int)((199.0 - 1.0 / fFinalDepth1 * (fViewDist * fRightProj_1) -
               dScreenCenterY) *
              dScrScale_1);
    ...
    int iScaledScreenX2 =
        (int)((fViewDist * fClippedForwardProj * (1.0 / fFinalDepth2) +
               dScreenCenterX) *
              dScrScale_1);
    ...
    iScaledScreenY2 =
        (int)((199.0 - 1.0 / fFinalDepth2 * (fViewDist * fRightProj) -
               dScreenCenterY) *
              dScrScale_1);
...
//-------------------------------------------------------------------------------------------------
// 000280E0
void compout(uint8 *pScrBuf, int iX0, int iY0, int iX1, int iY1,
             uint8 byColor) {
  int iNewX;         // edi
  int iNewY;         // esi
  int iX0_1;         // ebp
...
void line(uint8 *pScrBuf, int iX0, int iY0, int iX1, int iY1, uint8 byColor) {
  ...
  if (iX0 == iX1 && iY0 == iY1) {
    ...
    } else // negative slope
    {
      ...
      if (iDx <= iY0 - iY1) {
        iError_3 = iDy_1 >> 1;
        for (bAboveTarget = iY0 < iY1; !bAboveTarget;
             bAboveTarget = iY0 < iY1) {
          --iY0;
          iError_3 -= iDx;
          *pDest = byColor;  // draw pixel
...
void InitRemaps() {
  ...
  if (numcars > 0) {
    ...
    do {
      ...
      if (iCarTexsLoaded != -1)
        init_remap(cartex_vga[iCarTexsLoaded + 1],
                   car_texs_loaded[iCartexIdx] - 1,
                   num_textures[iCarTexsLoaded + 1], gfx_size);
      ++iCarIdx;
      ++iCartexIdx;
...
void LoadCarTexture(int iCartexIdx, uint8 byTexSlotIdx) {
  ...
  // Advanced cars
  if ((textures_off & TEX_OFF_ADVANCED_CARS) != 0 && iCartexIdx >= 1 &&
      iCartexIdx <= 8)
    *szTexFile = 'y'; // load Y tex files

  // Check if file exists
  iFileHandle = ROLLERopen(
      szTexFile, O_RDONLY | O_BINARY); // 0x200 is O_BINARY in WATCOM/h/fcntl.h
  if (iFileHandle == -1) {
  ...
  if (gfx_size == 1) {
    ...
    pTexBuf =
        (uint8 *)getbuffer((((int16)iTotalFileLength + 7) & 0xFFF8) << 10);
    ...
    while (iRemainingFileLength > 0) {
      ...
      if (no_mem) {
        ...
        iTexBlockIdx = 0;
        for (pSourcePixel = scrbuf + 40000; iTexBlockIdx < iNumTexsToProcess;
             ++iTexBlockIdx) {
      ...
      } else // normal memory mode
      {
        ...
        iTexBlockIdx_1 = 0;
        for (pSourcePixel_1 = scrbuf + 40000;
             iTexBlockIdx_1 < iNumTexsToProcess; ++iTexBlockIdx_1) {
...
void LoadBldTextures() {
  ...
  if (gfx_size == 1) {
    ...
    building_vga =
        (uint8 *)getbuffer((((int16)iNumTextureBlocks + 7) & 0xFFF8) << 10);
  ...
  } else // 64x64 mode
  {
    ...
    building_vga =
        (uint8 *)getbuffer((((int16)iNumTextureBlocks + 3) & 0xFFFC) << 12);
    iFinalTexCount = iCompressedFileLength / 4096;
...
void LoadTextures() {
  ...
  if (gfx_size == 1) {
    ...
    pTexBuf =
        (uint8 *)getbuffer((((int16)iTotalTextureBlocks + 7) & 0xFFF8) << 10);
    texture_vga = pTexBuf;
    ...
    if (g_pGameRenderer)
      game_render_load_texture(g_pGameRenderer, texture_vga, 256, 0, 0, 1);
  ...
  } else // 64x64 mode
  {
    ...
    if (g_pGameRenderer)
      game_render_load_texture(g_pGameRenderer, texture_vga, 256, 0, 0, 0);
  ...
  if (iNumBlocks > 0) {
    ...
    do {
      ...
      if (iIsLowRes) {
        ...
        pBlockRow =
            &pTextureBaseAddr[0x2000 * (iBlockIdx >> 3)]; // row start addr
        iTexRowBytes_1 = 32 * (iBlockIdx & 7);            // column offset
      } else {
        // 4 blocks per row, 64px wide
        pBlockRow =
            &pTextureBaseAddr[0x4000 * (iBlockIdx >> 2)]; // row start addr
      ...
      for (iDominantColorSearch = 0; iDominantColorSearch < 4;
           ++iDominantColorSearch) {
        iCurrColorIdx = 0;
        iSearchIdx = 0;
        iMaxColorCount = 0;
      ...
      *(int *)((char *)car_remap + iTexRowBytes) =
          nearest_colour(iAvgR, iAvgB, iAvgG);
    STORE_COMPLETE:
      ...
      if (iTexturesInGroup > 0) {
        ...
        for (iRowIdx = 0; iRowIdx < 64; ++iRowIdx) {
          ...
          for (iTexIdx = 0; iTexIdx < iTexturesInGroup; ++iTexIdx) {
            ...
            iSourceRowOffset +=
                4096; // move to next texture (4096 bytes = 64x64 px)
          }
          iTempRowOffset +=
              256; // move to next row in temp buffer (4 tex * 64 px)
        }
      ...
      if (iTexturesInGroup > 0) {
        ...
        for (iRowIdx = 0; iRowIdx < 32; ++iRowIdx) {
          ...
          do {
            ...
            ++iTexIdx;
            iSourceRowOffset +=
                1024; // Move to next texture (1024 bytes = 32x32 px)
