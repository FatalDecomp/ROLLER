...
void InitBuildings() {
  ...
  memset(
      BuildingSect, 255,
      sizeof(
          BuildingSect)); // Initialize building sector map to -1 (no buildings)
  iBuildingIdx = 0;
  if (NumBuildings > 0) {
    iAngleIdx = 0;
    iBuildingIdx_1 = 0;
    ...
    do {
      ...
      if (iBuildingType <= 0x10) // Check if building type is valid (<=16)
      {
        ...
        iNextSect =
            iBuildingSect + 1; // Get next track segment, wrap around if needed
        fHalfWidth =
            (float)(32 * BuildingBase[iBuildingIdx_1][2] /
                    2); // Calculate half width of building from BuildingBase[2]
        ...
        dDeltaY =
            ...
            pNextTrackData->pointAy[3].fY; // Calculate track direction vector
                                           // between current and next segment
        dDeltaX = pTrackData->pointAy[3].fX - pNextTrackData->pointAy[3].fX;
        dDistance = sqrt(
            dDeltaX * dDeltaX +
            dDeltaY *
                dDeltaY); // Calculate distance and normalize direction vector
        // if ((HIDWORD(dDeltaX) & 0x7FFFFFFF) != 0 || LODWORD(dDeltaX))
        if (dDeltaX != 0)
          dDeltaX =
              (pTrackData->pointAy[3].fX - pNextTrackData->pointAy[3].fX) /
              dDistance;
        // if ((HIDWORD(dDeltaY) & 0x7FFFFFFF) != 0 || LODWORD(dDeltaY))
        if (dDeltaY != 0)
          dDeltaY = dDeltaY / dDistance;
        iHalfHeight = 32 * BuildingBase[iBuildingIdx_1][3] / 2;
        BuildingX[iBuildingIdx_2] =
            fNegTrackX -
            (float)dDeltaY * fHalfWidth; // Calculate building position offset
        ...
        while (byNumPols <
               pBuildingPlan->byNumPols) { // Skip polygons with special texture
                                           // flag (0x200)
        ...
        fMinZ = 1073741800.0f; // Initialize min/max values for bounding box
                               // calculation
        fMinY = 1073741800.0f;
        fMinX = 1073741800.0f;
        ...
        fMaxY = -1073741800.0f;
        fMaxX = -1073741800.0f;
        for (i = 0; i < pBuildingPlan->byNumCoords;
             ++i) // Find min/max coordinates for building bounding box
        {
        ...
        if ((cheat_mode & CHEAT_MODE_DOUBLE_TRACK) !=
            0) // Scale building 2x if cheat mode 0x1000 is enabled
        {
          fMinX = fMinX * 2.0f;
          fMinY = fMinY * 2.0f;
          fMinZ = fMinZ * 2.0f;
        ...
        fYawAngle = BuildingAngles[iAngleIdx] *
                    0.0174532925199f; // Convert building rotation angles from
                                      // degrees to radians
        ...
        dCosYaw = cos(fYawAngle); // Calculate sine and cosine values for 3D
                                  // rotation matrix
        ...
        BuildingBox[iBuildingIdx][0].fX =
            fMinX; // Set up 8 corners of building bounding box (cube vertices)
        ...
        int iPoint = 0;
        do {
          fX = BuildingBox[iBuildingIdx][iPoint]
                   .fX; // Apply 3D rotation matrix and translate each bounding
                        // box corner
          fY = BuildingBox[iBuildingIdx][iPoint].fY;
          fZ = BuildingBox[iBuildingIdx][iPoint].fZ;
          fMatrix00 = (float)(dCosYaw * dCosPitch);
          fMatrix02 =
              (float)(-dCosYaw * dSinPitch * dCosRoll - dSinYaw * dSinRoll);
          fMatrix01 =
              (float)(dCosYaw * dSinPitch * dSinRoll - dSinYaw * dCosRoll);
          BuildingBox[iBuildingIdx][iPoint].fX =
              fX * fMatrix00 + fY * fMatrix01 + fZ * fMatrix02 + fBuildingX;
          fMatrix12 =
              (float)(-dSinYaw * dSinPitch * dCosRoll + dCosYaw * dSinRoll);
          fMatrix10 = (float)(dSinYaw * dCosPitch);
          fMatrix11 =
              (float)(dSinYaw * dSinPitch * dSinRoll + dCosYaw * dCosRoll);
          BuildingBox[iBuildingIdx][iPoint].fY =
              fX * fMatrix10 + fY * fMatrix11 + fZ * fMatrix12 + fBuildingY;
          ...
          BuildingBox[iBuildingIdx][iPoint].fZ =
              fX * fMatrix20 + fY * fMatrix21 + fZ * fMatrix22 + fBuildingZ;
          ++iPoint;
          //*(float *)&BuildingBase[255][uiBuildingBoxOffset / 4 + 3] = fX *
          // fMatrix20 + fY * fMatrix21 + fZ * fMatrix22 + fBuildingZ;//
          // reference to BuildingBox
        } while (iPoint <
                 8); // while (uiBuildingBoxOffset != iBuildingBoxStride);//
        BuildingBaseX[iBuildingIdx_2] =
            BuildingX[iBuildingIdx_2]; // Store final building position in base
                                       // arrays
...
void CalcVisibleBuildings() {
  ...
  VisibleBuildings[0].iBuildingIdx =
      -1;                  // Initialize visible buildings array with terminator
  NumVisibleBuildings = 0; // Reset visible building count
  for (iBuildingCounter = 0; iBuildingCounter < NumBuildings;
       ++iBuildingCounter) // Count through all buildings (purpose unclear -
                           // possibly validation)
    ;
  if ((textures_off & 0x200) ==
      0) // Check if building textures are enabled (bit 9 of textures_off)
  {
    while (1) {
      if (iTrackSectIdx < 0)
        goto FUNCTION_EXIT; // Start of main track section loop - iterate
                            // backwards through track
      if (iTrackSectIdx <= first_size ||
          iTrackSectIdx >= gap_size) // Skip sections in track gap (between
                                     // first_size and gap_size)
      {
        iWrappedSectIdx =
            iTrackSectIdx + start_sect; // Calculate wrapped track section index
                                        // with start_sect offset
        ...
        if (iWrappedSectIdx >= iTrackLen) // Handle positive wraparound
          iWrappedSectIdx -= iTrackLen;
        iBuildingIdx = BuildingSect[iWrappedSectIdx]; // Get building index for
                                                      // this track section
        if (iBuildingIdx != -1)
    ...
    uiBuildingType =
        BuildingBase[iBuildingIdx]
                    [0];    // Get building type from BuildingBase array
    if (uiBuildingType < 4) // Check building type - types 0,1 use max depth,
                            // others use min depth
    {
      if (uiBuildingType > 1)
        goto CALC_MIN_DEPTH;
    } else if (uiBuildingType > 7 &&
               uiBuildingType !=
                   14) // Types 8+ except 14 use min depth calculation
    {
    CALC_MIN_DEPTH:
      // Calculate min depth
      ...
      // iPointOffset2 = 96 * iBuildingIdx + 12;   // Calculate min depth for
      // other building types - start with first point fBuildingDepth =
      // (BuildingBox[iBuildingIdx][0].fX - viewx) * vk3 +
      // (BuildingBox[iBuildingIdx][0].fY - viewy) * vk6 +
      // (BuildingBox[iBuildingIdx][0].fZ - viewz) * vk9;// Transform first
      // bounding box point to view space depth do {
      //   dPointDepth2 = (*(float *)((char *)&BuildingBox[0][0].fX +
      //   iPointOffset2) - viewx) * vk3
      //     + (*(float *)((char *)&BuildingBox[0][0].fY + iPointOffset2) -
      //     viewy) * vk6
      //     + (*(float *)((char *)&BuildingBox[0][0].fZ + iPointOffset2) -
      //     viewz) * vk9;// Transform current point to view space depth
      ...
      //   }
      //   iPointOffset2 += 12;
      // } while (iPointOffset2 != 96 * iBuildingIdx + 96);// Loop through all 8
      // bounding box points
    ADD_BUILDING_TO_LIST:
      ++pVisibleBuilding; // Add building to visible list - advance pointer
      pVisibleBuilding[-1].fDepth =
          fBuildingDepth; // Store depth value in array (float)
      iCurrentCount = NumVisibleBuildings;
      pVisibleBuilding[-1].iBuildingIdx =
          iBuildingIdx;                    // Store building index in array
      pVisibleBuilding->iBuildingIdx = -1; // Add array terminator (-1)
      NumVisibleBuildings =
          iCurrentCount + 1; // Increment visible building count
    ...
    // iPointOffset = 96 * iBuildingIdx + 12;      // Calculate max depth for
    // building types 0,1 - start with first point fBuildingDepth =
    // (BuildingBox[iBuildingIdx][0].fX - viewx) * vk3 +
    // (BuildingBox[iBuildingIdx][0].fY - viewy) * vk6 +
    // (BuildingBox[iBuildingIdx][0].fZ - viewz) * vk9;// Transform first
    // bounding box point to view space using view matrix do {
    //   dPointDepth = (*(float *)((char *)&BuildingBox[0][0].fX + iPointOffset)
    //   - viewx) * vk3
    //     + (*(float *)((char *)&BuildingBox[0][0].fY + iPointOffset) - viewy)
    //     * vk6
    //     + (*(float *)((char *)&BuildingBox[0][0].fZ + iPointOffset) - viewz)
    //     * vk9;// Transform current point to view space depth
    ...
    // } while (iPointOffset != 96 * iBuildingIdx + 96);// Loop through all 8
    // bounding box points (96 bytes total, 12 per point)
...
void DrawBuilding(int iBuildingIdx, uint8 *pScrPtr) {
  ...
  uiBuildingType =
      BuildingBase[iBuildingIdx]
                  [0]; // Get building plan data (polygons, coordinates, etc.)
  ...
  if (uiBuildingType >= 9 &&
      (uiBuildingType <= 0xA ||
       uiBuildingType == 15)) // Special rotation for buildings 9, 10, and 15
                              // based on world direction
  {
  ...
  fYawRad = fYawAngle *
            0.0174532925199f; // Convert rotation angles from degrees to radians
  ...
  for (i = 0; byNumCoords > i; ++i) {
    ...
    if ((cheat_mode & 0x1000) !=
        0) // Scale coordinates 2x if cheat mode enabled
    {
      fX = pCoords->fX * 2.0f;
      fCoordY = *p_fY * 2.0f;
      ++pCoords;
    ...
    fMatrix01 = (float)(dCosYaw * dSinPitch * dSinRoll -
                        dSinYaw * dCosRoll); // Build 3x3 rotation matrix
                                             // elements from yaw/pitch/roll
    fMatrix00 = (float)(dCosYaw * dCosPitch);
    fMatrix02 = (float)(-dCosYaw * dSinPitch * dCosRoll - dSinYaw * dSinRoll);
    dTransformedX =
        fX * fMatrix00 + fCoordY * fMatrix01 + v109 * fMatrix02 +
        fBuildingX; // Apply 3D rotation and translation to building coordinates
    ...
    dTransformedY =
        fX * fMatrix10 + fCoordY * fMatrix11 + v109 * fMatrix12 + fBuildingY;
    dTempY = dTransformedY;
    fVert2X = (float)(dCosPitch * dCosRoll);
    fMatrix20 = (float)dSinPitch;
    fMatrix21 = (float)(-dSinRoll * dCosPitch);
    dTransformedZ =
        fX * fMatrix20 + fCoordY * fMatrix21 + v109 * fVert2X + fBuildingZ;
    // Store raw (un-floored, un-translated) world coords for
    // game_render_quad_world. sw_quad_world performs floor(world − viewer) and
    // the view-matrix multiply itself, matching legacy precision.
  ...
  while (iPolygonLoop < byNumPols) {
    BuildingZOrder[iZOrderIdx].iPolygonIndex =
        iPolygonLoop; // Build Z-order sorting list for polygons
    BuildingZOrder[iZOrderIdx].iPolygonLink = pPols->nNextPolIdx;
    if ((pPols->uiTex & 0x8000) == 0) // Calculate polygon Z depth for sorting
    ...
    { // Find maximum Z (farthest) for back-to-front rendering order
      ...
      } else if (sortDepths[pPols->verts[0]] <=
                 (double)sortDepths[pPols->verts[1]]) {
  ...
  qsort(BuildingZOrder, byNumPols, 0xCu,
        bldZcmp); // Sort polygons by Z-depth using qsort
    ...
    if (iFoundPolygonLink < 0) {
      ...
      do {
        ...
        // the legacy (d1 × d2) · V0 test using int-truncated view-space coords.
        {
          ...
          // Sum per-vertex clip flags; skip if entire poly is behind near
          // plane.
          ...
          // back-facing SURFACE_FLAG_FLIP_BACKFACE case so texture mapping
          // stays correct.
          int iOrder[4];
          if (isBackFace) {
            iOrder[0] = iV3;
          ...
          if ((float)iZ2 < fClosestZ)
            fClosestZ = (float)iZ2;
          if ((float)iZ3 < fClosestZ)
            fClosestZ = (float)iZ3;
          ...
          TextureHandle th = ((uiTex & SURFACE_FLAG_APPLY_TEXTURE) != 0)
                                 ? game_render_get_texture_handle(
                                       g_pGameRenderer, TEXTURE_BANK_BUILDING)
      ...
      skip_polygon:;
        ...
        ++iCurrentZOrderIdx;
        ++iBestZOrderIdx;
      } while (byNumPols > iBestZOrderIdx &&
               iFoundPolygonLink ==
                   *(int *)((char *)&BuildingZOrder[0].iPolygonLink +
                            iZOrderOffset));
    }
  }
...
int bldZcmp(const void *pBuilding1, const void *pBuilding2) {
  ...
  const tBuildingZOrderEntry *pEntry1 =
      (const tBuildingZOrderEntry *)pBuilding1;
  const tBuildingZOrderEntry *pEntry2 =
      (const tBuildingZOrderEntry *)pBuilding2;
