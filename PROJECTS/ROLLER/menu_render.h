...
typedef enum { MENU_RENDER_GPU, MENU_RENDER_SOFTWARE } MenuRenderMode;
...
void menu_render_sprite(MenuRenderer *renderer, int slot, int blockIdx, int x,
...
void menu_render_fade_wait(MenuRenderer *renderer, void (*redraw_fn)(void *ctx),
                           void *ctx);

// Text rendering
void menu_render_text(MenuRenderer *renderer, int fontSlot, const char *text,
                      const char *mappingTable, int *charVOffsets, int x, int y,
                      uint8 colorReplace, int alignment, const tColor *palette);
