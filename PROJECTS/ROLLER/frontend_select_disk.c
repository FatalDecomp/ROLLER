...
void select_disk() {
  ...
  iMenuCursor = 2;
  iStatusMessage = 0;
  check_saves(); // Check save file status and scan for existing championship
                 // saves
  do {           // Handle game type switches and updates
    ...
    { // RENDER FRAME (GPU)
      ...
      if (iMenuCursor >= 2) // Draw selection cursor
      {
        ...
        menu_render_text(mr, 2, "~", font2_ascii, font2_offsets,
                         sel_posns[iMenuCursor].x, sel_posns[iMenuCursor].y,
                         0x8Fu, 0, pal_addr);
      }
      menu_render_text(
          mr, 2, &language_buffer[576], font2_ascii, font2_offsets,
          sel_posns[0].x + 132, sel_posns[0].y + 7, 0x8Fu, 2u,
          pal_addr); // Display main menu options: "Load Game" and "Save Game"
      ...
      menu_render_text(mr, 15, &language_buffer[704], font1_ascii,
                       font1_offsets, 400, 270, 0xABu, 1u,
                       pal_addr); // CURRENT GAME INFO: Display current
                                  // championship settings and progress
      menu_render_text(mr, 15, &language_buffer[768], font1_ascii,
                       font1_offsets, 400, 290, 0x8Fu, 2u, pal_addr);
      uiCupIndex = (TrackLoad - 1) / 8;
      // uiCupIndex = (TrackLoad - 1 - (__CFSHL__((TrackLoad - 1) >> 31, 3) + 8
      // * ((TrackLoad - 1) >> 31))) >> 3;// Show current cup name based on
      // track group
      if (uiCupIndex) {
        if (uiCupIndex <= 1) {
    ...
    LABEL_20:
      ...
      menu_render_text(mr, 15, &language_buffer[64 * level + 1472], font1_ascii,
                       font1_offsets, 405, 344, 0x8Fu, 0, pal_addr);
      ...
      iSlotLoop = 0; // SAVE SLOT DISPLAY: Show all 4 championship save slots
                     // with their details
      iSlotYPosition = 56;
      iSaveArrayIndex = 0;
      iY = 74;
      do {
        sprintf(buffer, "%s %i:", &language_buffer[2432],
                iSlotLoop + 1); // Display slot number with highlighting for
        ...
        if (save_status[iSaveArrayIndex]
                .iSlotUsed) // Show save slot contents: cup, track, difficulty,
                            // damage, player type
        {
          uiSaveCupIndex = (save_status[iSaveArrayIndex].iPackedTrack - 1) / 8;
          iSaveTrackNumber =
              ((save_status[iSaveArrayIndex].iPackedTrack - 1) % 8) + 1;
          // uiSaveCupIndex = (save_status[iSaveArrayIndex].iPackedTrack
          //                 - 1
          //                 -
          //                 (__CFSHL__((save_status[iSaveArrayIndex].iPackedTrack
          //                 - 1) >> 31, 3)
          //                    + 8 *
          //                    ((save_status[iSaveArrayIndex].iPackedTrack - 1)
          //                    >> 31))) >> 3;
          // iSaveTrackNumber =
          // ((LOBYTE(save_status[iSaveArrayIndex].iPackedTrack) - 1) & 7) + 1;
          if (uiSaveCupIndex) {
            if (uiSaveCupIndex <= 1) {
              if (iSelectedSlot == iSlotNumber)
                ...
                byCupColor2 = 0x8F;
              menu_render_text(mr, 15, &language_buffer[896], font1_ascii,
                               font1_offsets, 305, iSlotYPosition, byCupColor2,
                               0, pal_addr);
              ...
              if (iSelectedSlot == iSlotNumber)
                ...
                byCupColor3 = 0x8F;
              menu_render_text(mr, 15, &language_buffer[4928], font1_ascii,
                               font1_offsets, 305, iSlotYPosition, byCupColor3,
                               0, pal_addr);
            }
          } else {
            if (iSelectedSlot == iSlotNumber)
              ...
              byCupColor1 = 0x8F;
            menu_render_text(mr, 15, &language_buffer[832], font1_ascii,
                             font1_offsets, 305, iSlotYPosition, byCupColor1, 0,
                             pal_addr);
          }
          ...
          menu_render_text(
              mr, 15,
              &language_buffer[64 * save_status[iSaveArrayIndex].iDifficulty +
                               1472],
              font1_ascii, font1_offsets, 460, iY, byLevelColor, 2u, pal_addr);
          if (iSelectedSlot == iSlotLoop + 1)
            byDamageColor = 0xAB;
          else
            byDamageColor = 0x8F;
          ...
          menu_render_text(
              mr, 15,
              &language_buffer[64 * save_status[iSaveArrayIndex].iPlayerType +
                               2112],
              font1_ascii, font1_offsets, 480, iY, byPlayerTypeColor, 0,
              pal_addr);
          ...
          menu_render_text(mr, 15, &language_buffer[2496], font1_ascii,
                           font1_offsets, 305, iSlotYPosition, byEmptySlotColor,
                           0, pal_addr);
        }
        iSlotYPosition += 40;
        ++iSaveArrayIndex;
      ...
      case 0:
        ...
        break;
      case 1:
        if (iChampResult) // Case 1: Load operation messages
                          // (success/confirmation)
          menu_render_text(mr, 15, &language_buffer[2624], font1_ascii,
                           font1_offsets, 400, 230, 0xE7u, 1u, pal_addr);
        else
          menu_render_text(mr, 15, &language_buffer[2560], font1_ascii,
                           font1_offsets, 400, 230, 0xE7u, 1u, pal_addr);
        break;
      case 2:
        menu_render_text(mr, 15, &language_buffer[2688], font1_ascii,
                         font1_offsets, 400, 230, 0xE7u, 1u,
                         pal_addr); // Case 2: Save operation success message
        break;
      case 4:
        menu_render_text(
            mr, 15, &language_buffer[2752], font1_ascii, font1_offsets, 400,
            230, 0xE7u, 1u,
            pal_addr); // Case 4: Empty slot selected (no save to load)
        break;
      default:
                         ...
                         // player synchronization
    {
      ...
      if (players > 0) {
        ...
        //   ++iCheatSetLoop;
        //   *(int *)((char *)&infinite_laps + iPlayersCarsOffset) = switch_same
        //   - 666;// offset into Players_Cars
      ...
      //     iCheatResetOffset += 4;
      //     ++iCheatResetLoop;
      //     *(int *)((char *)&infinite_laps + iCheatResetOffset) = -1;// offset
      //     into Players_Cars
      //   } while (iCheatResetLoop < players);
      // }
      uiUpdatedCheatFlags = cheat_mode;
      // BYTE1(uiUpdatedCheatFlags) = BYTE1(cheat_mode) & 0xBF;// &=
      // ~CHEAT_MODE_CLONES
      uiUpdatedCheatFlags &= ~CHEAT_MODE_CLONES;
    }
  ...
  LABEL_95:
                       ...
                       // save/load operations
    {
      byInputKey = fatgetch();
      if (byInputKey <
          0xDu) { // Arrow keys: Navigate between menu options and save slots
        if (!byInputKey) {
          byExtendedKey = fatgetch();
          if (byExtendedKey >= 0x48u) {
            if (byExtendedKey <= 0x48u) { // Up arrow: Move up in save slot
                                          // selection or main menu
              if (uiMenuMode) {
                if (iSelectedSlot > 1) {
                  iStatusMessage = 0;
                  --iSelectedSlot;
            ...
            } else if (byExtendedKey == 80) { // Down arrow: Move down in save
                                              // slot selection or main menu
              if (uiMenuMode) {
                if (iSelectedSlot < 4) {
                  iStatusMessage = 0;
                  ++iSelectedSlot;
      ...
      } else if (byInputKey <= 0xDu) { // Enter key: Execute save/load operation
                                       // based on current menu and selection
        if (uiMenuMode) {
          if (uiMenuMode <= 1) {
            save_champ(iSelectedSlot); // SAVE operation: Save current
                                       // championship to selected slot
            uiMenuMode = 0;
            check_saves();
            iStatusMessage = 2;
          } else {
            ...
            if (save_status[iSelectedSlot - 1].iSlotUsed) {
              uiMenuMode =
                  0; // LOAD operation: Load championship from selected slot
          ...
          if ((unsigned int)iMenuCursor <= 1) {
            uiMenuMode =
                2; // \"Save Game\" selected: Enter save slot selection mode
            iSelectedSlot = 1;
          } else if (iMenuCursor == 2) {
            goto LABEL_128;
          }
          ...
          uiMenuMode =
              1; // \"Load Game\" selected: Enter save slot selection mode
          iSelectedSlot = 1;
        }
      } else if (byInputKey ==
                 27) { // Escape key: Cancel operation or exit to main menu
        if (uiMenuMode) {
          uiMenuMode = 0;
          iStatusMessage = 0;
        } else {
    ...
    UpdateSDL();
      ...
      !iExitFlag); // MAIN MENU LOOP - Handle UI rendering and input processing
  // GPU fade-out
  {
    MenuRenderer *mr = GetMenuRenderer();
