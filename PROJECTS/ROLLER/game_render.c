...
TextureHandle game_render_load_texture(GameRenderer *renderer, uint8 *pixelData,
  ...
  TextureHandle swHandle = game_render_sw_load_texture(
      renderer->sw, pixelData, width, height, tex_idx, gfx_size);
  TextureHandle sceneHandle = scene_render_load_texture(
      renderer->scene, pixelData, width, height, tex_idx, gfx_size);
...
TextureHandle game_render_get_texture_handle(GameRenderer *renderer,
  ...
  return handle != TEXTURE_HANDLE_INVALID
             ? handle
...
void game_render_quad_world_subdivide_type(GameRenderer *renderer,
  ...
  scene_render_quad_world_legacy(renderer->scene, verts, handle, surfaceFlags,
                                 options);
}
