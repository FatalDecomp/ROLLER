...
void snapshot_render_menu_select_type(void) {
  ...
  for (int i = 0; i < 5; ++i)
    SnapshotQueueRawKey(
        0x48); // Up arrow: real input path from Exit to Game Type.
  select_type();
}
...
void select_type() {
  ...
  if (game_type == 1 && Race > 0) // Championship mode after first race -
                                  // restrict options to track selection only
    iSkipColor = -1;
  else
    iSkipColor = 0;
  ...
  iBlockIdx = (TrackLoad - 1) / 8; // Calculate cup index from current track
                                   // (TrackLoad): cups are groups of 8 tracks
  front_vga[14] = (tBlockHeader *)load_picture(
      "cupicons.bm"); // Load cup icons graphics and initialize screen
  iMenuSelection = 0;
  fade_palette(0);
  front_fade = 0;
  ...
  do { // Handle game type switches and updates
    ...
    { // RENDER FRAME (GPU)
      ...
      if (iCurrentOption >= 5) // Draw selection cursor or \"Random\" indicator
                               // (option 5 = Random)
      {
        ...
        menu_render_text(mr, 2, "~", font2_ascii, font2_offsets,
                         ...
                         sel_posns[iCurrentOption].y, 0x8Fu, 0, pal_addr);
      }
      if (iSkipColor) // OPTION LABELS: Display main option categories (Game
                      // Type/Difficulty/etc.)
      {
      ...
      menu_render_sprite(mr, 14, iBlockIdx, 500, 300, 0,
                         pal_addr); // Display cup icon corresponding to current
                                    // track selection
      if (iSkipColor) // CHAMPIONSHIP MODE UI: Show current settings and
                      // restrictions
      {
        ...
        if ((cheat_mode & 2) != 0)
          ...
          menu_render_scaled_text(
              mr, 15, &language_buffer[64 * damage_level + 1856], font1_ascii,
              font1_offsets, 405, 118, 143, 0, 200, 640, pal_addr);
            ...
            do {
              if (iNetworkPlayerCount >= 8)
                menu_render_scaled_text(mr, 15, szText, font1_ascii,
                                        font1_offsets, 405, iY, 143, 0, 200,
                                        640, pal_addr);
              else
                menu_render_scaled_text(mr, 15, szText, font1_ascii,
                                        font1_offsets, 400, iNetworkDisplayY,
                                        143, 2u, 200, 640, pal_addr);
              szText += 9;
              iY += 18;
              iNetworkDisplayY += 18;
              ++iNetworkPlayerCount;
      ...
      case 0:
        if (!iSkipColor) // Option 0 - Game Type: Show race modes with
                         // highlighting for current selection
        {
          menu_render_scaled_text(mr, 15, &language_buffer[384], font1_ascii,
                                  ...
                                  pal_addr);
          if (iMenuSelection == 1) {
            menu_render_scaled_text(mr, 15, &language_buffer[3584], font1_ascii,
          ...
          menu_render_scaled_text(mr, 15, &language_buffer[3648], font1_ascii,
                                  font1_offsets, 400, 135, byGameModeColor1, 1u,
                                  200, 640, pal_addr);
          if (game_type == 1)
            byGameModeColor2 = byTextColor;
          else
            byGameModeColor2 = -113;
          menu_render_scaled_text(mr, 15, &language_buffer[3520], font1_ascii,
                                  font1_offsets, 400, 153, byGameModeColor2, 1u,
                                  200, 640, pal_addr);
          if (game_type == 2)
      ...
      case 1:
        menu_render_scaled_text(mr, 15, &language_buffer[3776], font1_ascii,
                                font1_offsets, 400, 75, 143, 1u, 200, 640,
                                pal_addr); // Option 1 - Difficulty: Show skill
                                           // levels with cheat mode override
        if (iMenuSelection == 2) {
        ...
        if ((cheat_mode & 2) == 0) {
          ...
          menu_render_scaled_text(mr, 15, &language_buffer[1792], font1_ascii,
                                  font1_offsets, 400, 135, byDifficultyColor1,
                                  1u, 200, 640, pal_addr);
          if (level == 4)
            byDifficultyColor2 = byDifficultyMenuColor;
          else
            byDifficultyColor2 = -113;
          menu_render_scaled_text(mr, 15, &language_buffer[1728], font1_ascii,
                                  font1_offsets, 400, 153, byDifficultyColor2,
                                  1u, 200, 640, pal_addr);
          if (level == 3)
            byDifficultyColor3 = byDifficultyMenuColor;
          else
            byDifficultyColor3 = -113;
          menu_render_scaled_text(mr, 15, &language_buffer[1664], font1_ascii,
                                  font1_offsets, 400, 171, byDifficultyColor3,
                                  1u, 200, 640, pal_addr);
          if (level == 2)
            byDifficultyColor4 = byDifficultyMenuColor;
          else
            byDifficultyColor4 = -113;
          menu_render_scaled_text(mr, 15, &language_buffer[1600], font1_ascii,
                                  font1_offsets, 400, 189, byDifficultyColor4,
                                  1u, 200, 640, pal_addr);
          if (level == 1)
            byDifficultyColor5 = byDifficultyMenuColor;
          else
            byDifficultyColor5 = -113;
          menu_render_scaled_text(mr, 15, &language_buffer[1536], font1_ascii,
                                  font1_offsets, 400, 207, byDifficultyColor5,
                                  1u, 200, 640, pal_addr);
          if (level)
        ...
        menu_render_scaled_text(mr, 15, &language_buffer[2048], font1_ascii,
                                font1_offsets, 400, 135, byDifficultyMenuColor,
                                1u, 200, 640, pal_addr);
        break;
      case 2:
        menu_render_scaled_text(
            mr, 15, &language_buffer[3904], font1_ascii, font1_offsets, 400, 75,
            143, 1u, 200, 640,
            pal_addr); // Option 2 - Competitors: Show number of opponent cars
                       // (2/8/16 or \"Just Me\")
        if (iMenuSelection == 3) {
        ...
        if (competitors != 1) {
          ...
          menu_render_scaled_text(mr, 15, &language_buffer[1088], font1_ascii,
                                  font1_offsets, 400, 135, byCompetitorColor1,
                                  1u, 200, 640, pal_addr);
          if (competitors == 8)
          ...
          menu_render_scaled_text(mr, 15, &language_buffer[1152], font1_ascii,
                                  font1_offsets, 400, 153, byCompetitorColor2,
                                  1u, 200, 640, pal_addr);
          if (competitors == 16)
        ...
        menu_render_scaled_text(mr, 15, &language_buffer[3968], font1_ascii,
                                font1_offsets, 400, 135, byDamageMenuColor, 1u,
                                200, 640, pal_addr);
        break;
      case 3:
        menu_render_scaled_text(mr, 15, &language_buffer[4032], font1_ascii,
                                font1_offsets, 400, 75, 143, 1u, 200, 640,
                                pal_addr); // Option 3 - Damage: Show car damage
                                           // levels (None/Cosmetic/Realistic)
        if (iMenuSelection == 4) {
          menu_render_scaled_text(mr, 15, &language_buffer[3840], font1_ascii,
                                  font1_offsets, 400, 93, 143, 1u, 200, 640,
                                  pal_addr);
        ...
        if ((cheat_mode & 2) == 0) {
          ...
          menu_render_scaled_text(mr, 15, &language_buffer[1856], font1_ascii,
                                  font1_offsets, 400, 135, byDamageColor1, 1u,
                                  200, 640, pal_addr);
          if (damage_level == 1)
            byDamageColor2 = byTextureMenuColor;
          else
            byDamageColor2 = -113;
          menu_render_scaled_text(mr, 15, &language_buffer[1920], font1_ascii,
                                  font1_offsets, 400, 153, byDamageColor2, 1u,
                                  200, 640, pal_addr);
          if (damage_level == 2)
        ...
        menu_render_scaled_text(mr, 15, &language_buffer[2048], font1_ascii,
                                font1_offsets, 400, 135, byTextureMenuColor, 1u,
                                200, 640, pal_addr);
        break;
      case 4:
        menu_render_scaled_text(mr, 15, &language_buffer[4288], font1_ascii,
                                font1_offsets, 400, 75, 143, 1u, 200, 640,
                                pal_addr); // Option 4 - Textures: Show texture
                                           // quality options (On/Off)
        if (iMenuSelection == 5) {
      ...
      LABEL_130:
        ...
        break;
      default:
        break; // OPTION-SPECIFIC UI: Display details for currently selected
               // option
      }
    ...
    if (switch_same > 0) // CHEAT MODE HANDLING: Process switch_same command for
                         // player synchronization
    {
      ...
      if (players > 0) {
        ...
        //   ++iCheatSetLoop;
        //   *(int *)((char *)&infinite_laps + iCheatArrayOffset2) = switch_same
        //   - 666;
  ...
  LABEL_142:;
    while (fatkbhit()) // KEYBOARD INPUT PROCESSING: Handle navigation and
                       // option changes
    {
      byInputKey = fatgetch();
      if (byInputKey < 0x1Bu) {
        if (byInputKey) { // Enter key: Confirm selection or navigate into
                          // sub-options
          if (byInputKey == 13) {
          ...
          byExtendedKey = fatgetch(); // Arrow keys: Navigate through options
                                      // and change values
          switch (iMenuSelection) {
          case 0:
            if (byExtendedKey >= 0x48u) // Main menu navigation: Up/Down arrows
                                        // move between option categories
            {
            ...
            break;
          case 1:
            if (byExtendedKey >= 0x48u) // Game Type option: Navigate between
                                        // race modes and adjust competitors
            {
              if (byExtendedKey <= 0x48u) {
                if (--game_type < 0)
                  game_type = 0;
          ...
          case 3:
            if (byExtendedKey >= 0x48u) {
              ...
              } else if (byExtendedKey == 80 && game_type < 2 &&
      ...
      } else if (byInputKey <= 0x1Bu) // Escape key: Exit to main menu or back
                                      // to option selection
      {
        if (iMenuSelection)
          iMenuSelection = 0;
        ...
        LABEL_248:
          iExitFlag = -1;
      } else if (byInputKey < 0x59u) { // Space key: Cycle through cups (track
                                       // groups) if not in championship
        if (byInputKey == 32 && !iSkipColor) {
          ...
          while (broadcast_mode)
            UpdateSDL();
                 ...
                 byInputKey == 121) { // Y/y key: \"Yes\" - confirm championship
                                      // exit and reset to race mode
        if (iSkipColor) {
          if (iMenuSelection == 6) {
            game_type = 0;
            iSkipColor = 0;
    ...
    UpdateSDL();
  } while (!iExitFlag); // MAIN SELECTION LOOP - Handle UI rendering and input
                        // processing
  if (!iSkipColor) // CLEANUP: Set final game parameters and track selection
                   // based on options
  {
    network_champ_on = 0;
    iTrackUpperLimit = 8 * iBlockIdx + 8;
    iTrackLowerLimit = 8 * iBlockIdx + 1;
    if (game_type) // Initialize championship mode: reset all statistics and set
                   // starting track
    {
