...
void WriteIOCTL(uint8 bySubCommand, unsigned int uiSize, void *pBuffer) {
  ...
  RMIcd.ebx = 0;
  RMIcd.es = (uint16)(((int64)iobuffer + ((int64)iobuffer < 0 ? 15 : 0)) >>
                      4); // segment_of(iobuffer)
...
void *AllocDOSMemory(int iSizeBytes, int16 *pOutSegment) {
  ...
  // regs.w.bx = ((iSizeBytes - (__CFSHL__(iSizeBytes >> 31, 4) + 16 *
  // (iSizeBytes >> 31))) >> 4) + 1;// compute number of 16-byte paragraphs
  // needed to hold iSizeBytes int386x(0x31, &regs, &regs, &sregs);
  //
...
void GetAudioInfo() {
  ROLLERGetAudioInfo();
  // uint8 buffer[7];
  // uint32 track_sectors[100];  // Temporary storage for track sectors
  // (including lead-out)
  //
  ...
  //     uint32 uiSector = (uiMinutes * 4500) + (uiSeconds * 75) + uiFrames -
  //     150;
  //
  ...
  // uint32 uiLeadOutSector = (uiLeadMin * 4500) + (uiLeadSec * 75) +
  // uiLeadFrame - 150; track_sectors[last_track + 1] = uiLeadOutSector;  //
  // Store after last track
  //
...
void PlayTrack4(int iStartTrack) {
  ...
  //  tracklengths[iStartTrack + 1] + tracklengths[iStartTrack + 2] +
  //  tracklengths[iStartTrack + 3];
  //
  //// Prepare audio control structure
...
void intRM(uint8 byInterruptNumber) {
  /*
  regs.w.ax = 768;                              // DPMI: Simulate Real Mode
  Interrupt regs.w.bx = byInterruptNumber;                // Interrupt number to
  simulate regs.w.cx = 0;                                // Reserved sregs.es =
  __DS__;                            // Segment of RMIcd regs.x.edi = (unsigned
  int)&RMIcd;            // Offset of RMIcd (real mode register set)
  int386x(0x31, &regs, &regs, &sregs);*/
}
