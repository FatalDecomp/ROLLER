...
void select_car() {
  ...
  front_fade = 0;
  front_vga[3] = (tBlockHeader *)load_picture(
      "carnames.bm"); // Load car company name graphics (carnames.bm) and car
                      // selection UI (selcar2.bm)
  front_vga[7] = (tBlockHeader *)load_picture(
      "selcar2.bm"); // Load car selection UI bitmap
  ...
  car_request = 0;
  byMenuExitFlag = 0; // Initialize selection state: byMenuExitFlag=0 (stay in
                      // menu), iOriginalCarSelection=-1 (no backup)
  iOriginalCarSelection =
      -1; // byExitCode = 0 - stay in selection, -1 = exit to menu
  iPlayer1Car = Players_Cars[player1_car]; // iSavedCarIndex = -1 initially,
                                           // stores original car selection
  if (game_type == 1) // Championship mode: restrict car changes except for
                      // first race (Race == 0)
  {
  ...
  if (iPlayer1Car >= CAR_DESIGN_AUTO) {
    ...
    if (iCartexLoaded == -1) {
      LoadCarTexture(
          carType,
          1u); // Car texture not loaded yet - load it now and mark as loaded
      iLoadCarTextures = 2;
  ...
  if (iPlayer1Car <
      CAR_DESIGN_AUTO) // Initialize car statistics pie chart display blocks (7
                       // stats: Speed, Acceleration, etc.)
  {
    for (int i = 0; i < 7; ++i) {
      blockIdxAy[i] = 8;
    }
  ...
  iZoomDistance = 2000; // Setup 3D car zoom animation: start at 2000 units,
                        // zoom out to 40000 (-2000 speed)
  iZoomSpeed = -2000;
  if (iPlayer1Car >= CAR_DESIGN_SUICYCO) // Position car selector cursor
    iCurrentCarSelectorPos = 8;
  else
    iCurrentCarSelectorPos = iPlayer1Car;
  if (iCurrentCarSelectorPos <
      0) // Clamp car selector position to valid range (0-8, where 8 = random)
    iCurrentCarSelectorPos = 8;
  iDelayBeforeRotation =
      36; // 36-frame delay before starting car rotation animation
  iActivePlayer = 0;
  ...
  if (!byMenuExitFlag) {
    ...
    do { // Handle game type switches (race type, championship, etc.)
      ...
      if (SoundCard && front_fade && iFrontendSpeechPending &&
          sfxplaying(SOUND_SAMPLE_CARIN) == 0) {
      ...
      { // RENDER FRAME (GPU)
        ...
        if (iPlayer1Car < CAR_DESIGN_AUTO) {
          ...
          if (iPlayer1Car == CAR_DESIGN_F1WACK) {
            ...
            menu_render_draw_car_preview(mr, 1280.0f, fCarDrawDistance,
                                         Car[0].nYaw, PREVIEW_X, CAR_PREVIEW_Y,
                                         PREVIEW_W, PREVIEW_H);
          }
        ...
        do {
          menu_render_sprite(mr, 7, blockIdxAy[uiNetworkLoop / 4], 568,
                             iPieChartY, 0, pal_addr);
          uiNetworkLoop += 4;
          iPieChartY += 51;
        ...
        if (iCurrentCarSelectorPos >= 8) // Draw car selection cursor
        {
          ...
          menu_render_text(mr, 2, "~", font2_ascii, font2_offsets,
                           ...
                           sel_posns[iCurrentCarSelectorPos].y, 0x8Fu, 0,
                           pal_addr);
        }
        ...
        menu_render_text(mr, 2, "REISE WAGON", font2_ascii, font2_offsets,
                         ...
                         pal_addr);
        if (iCurrentCarSelectorPos < 8 && network_on &&
            (cheat_mode & 0x4000) == 0) // Network mode
        {
          menu_render_text(mr, 15, &language_buffer[4672], font1_ascii,
                           font1_offsets, 380, 380, 0x8Fu, 2u, pal_addr);
          if (allocated_cars[iCurrentCarSelectorPos]) {
            ...
            if (players > 0) {
              ...
              do {
                if (iCurrentCarSelectorPos ==
                    Players_Cars[uiNetworkPlayerIndex / 4]) {
                  if (iPlayerNameX == 385)
                    uiPlayerIndex = 0;
                  else
                    uiPlayerIndex = 2;
                  menu_render_text(mr, 15, szPlayerName, font1_ascii,
                                   font1_offsets, iPlayerNameX, 380, 0x8Fu,
                                   uiPlayerIndex, pal_addr);
                  iPlayerNameX = 620;
                }
            ...
            menu_render_text(mr, 15, &language_buffer[4736], font1_ascii,
                             font1_offsets, 385, 380, 0x83u, 0, pal_addr);
          }
        }
        if (iOriginalCarSelection >=
            0) // Display current player's selected car company name at bottom
        {
          if (iActivePlayer)
            sprintf(buffer, "%s %s", &language_buffer[2880],
                    CompanyNames[Players_Cars[player2_car]]);
          else
            sprintf(buffer, "%s %s", &language_buffer[2816],
                    szCurrentCompanyName);
          menu_render_scaled_text(mr, 15, buffer, font1_ascii, font1_offsets,
                                  375, 316, 231, 1u, 170, 550, pal_addr);
        }
      ...
      iAnimationCounter =
          iDelayBeforeRotation; // ANIMATION UPDATE: Rotate pie chart segments
                                // during delay period
      if (iDelayBeforeRotation) {
        ...
        // do {
        //   int iIncrementedPieValue = *(int *)((char *)blockIdxAy +
        //   iAnimationCounter) + 1;
        //   *(int *)((char *)blockIdxAy + iAnimationCounter) =
        //   iIncrementedPieValue; if (iIncrementedPieValue > 8)
        //     *(int *)((char *)blockIdxAy + iAnimationCounter) = 1;
        //   iAnimationCounter += 4;
        // } while (iAnimationCounter != 28);
      }
      if (!iDelayBeforeRotation) // ZOOM ANIMATION: Handle 3D car zoom in/out
                                 // with car switching logic
      {
        iNewZoomDistance = iStatAnimationFrame * iZoomSpeed + iZoomDistance;
        iZoomDistance = iNewZoomDistance;
        if (iNewZoomDistance <=
            40000) // Handle car zoom in/out animation and car selection
        {
          ...
          iCarAllocationStatus =
              allocated_cars[iSelectedCar]; // Zoom reached maximum - switch
                                            // cars if available, otherwise
                                            // restore previous car
          iZoomDistance = 40000;
          iZoomSpeed = -iZoomSpeed;
          if (iCarAllocationStatus <
              2) { // Free old car textures and load new car's textures
            if (iPlayer1Car >= CAR_DESIGN_AUTO) {
      ...
      if (car_request < 0) // NETWORK CAR REQUESTS: Handle incoming car
                           // selection requests from other players
      {
      ...
      if (switch_same > 0) // CHEAT MODE: Handle "switch_same" command to force
                           // all players to same car
      {
        if (switch_same - 666 != Players_Cars[player1_car]) {
          ...
          if (players > 0) {
            for (iPlayerLoopCounter = 0; iPlayerLoopCounter < players;
                 iPlayerLoopCounter++) {
          ...
          //     ++iPlayerLoopCounter;
          //     *(int *)((char *)&infinite_laps + iPlayerArrayOffset) =
      ...
      if (bStartedFadeIn) // Screen fade-in: Load car voice sample and start
                          // playback
      {
      ...
      while (fatkbhit()) // KEYBOARD INPUT PROCESSING: Handle all user input and
                         // navigation
      {
        byInputKey = fatgetch();
        if (byInputKey < 0x20u) {
          if (byInputKey < 0xDu) {
            if (!byInputKey) {
              ...
              case 0x48u:
              case 0x4Bu:
                goto LABEL_152; // Arrow keys: Handle up/down/left/right
          ...
          } else if (byInputKey <= 0xDu) // Enter key: Confirm car selection or
                                         // trigger car switching animation
          {
            ...
            if (iCurrentCarSelectorPos == iPlayer1Car && SoundCard &&
                frontendspeechhandle != -1 &&
                DIGISampleDone(
                    frontendspeechhandle)) { // DIGISampleDone(*(int
                                             // *)&DIGIHandle,
                                             // frontendspeechhandle)) {
              frontendspeechhandle = -1;
              frontendsample(0x8000);
            }
            if (iCurrentCarSelectorPos == 8) {
              byMenuExitFlag = -1;
                       ...
                       (allocated_cars[iCurrentCarSelectorPos] < 2 ||
                        game_type == 1 && Race > 0)) {
              if (network_on) {
        ...
        } else if (byInputKey <= 0x20u) { // Space key: Switch between Player 1
                                          // and Player 2 in two-player mode
          if (player_type == 2) {
            if (iActivePlayer) {
              iActivePlayer = 0;
              iDelayBeforeRotation = 0;
      ...
      if (uiNavigationDirection) // Apply navigation input to move car selector
                                 // cursor
      {
      ...
      nRotationAngle = Car[0].nYaw +
                       32 * iStatAnimationFrame; // Update 3D car rotation angle
                                                 // for spinning animation
      nRotationAngle &= 0x3FFFu;
      Car[0].nYaw = nRotationAngle;

      UpdateSDL();
    } while (!byMenuExitFlag); // MAIN SELECTION LOOP - Handle UI rendering,
                               // input, and car switching
  }
  // GPU fade-out
  {
    MenuRenderer *mr = GetMenuRenderer();
