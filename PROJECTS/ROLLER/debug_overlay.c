...
static void LogCallback(void *pUserdata, int iCategory,
                        SDL_LogPriority priority, const char *pMessage) {
  ...
  if (pOverlay->pPrevLogFn)
    pOverlay->pPrevLogFn(pOverlay->pPrevLogUserdata, iCategory, priority,
                         pMessage);
  ...
  snprintf(pOverlay->aLogEntries[iIdx].szText, MAX_LOG_LEN, "%s%s",
...
static void OverlayFillRect(uint8_t *pBuf, int iBx, int iBy, int iBw, int iBh,
  ...
  for (int iY = iBy; iY < iBy + iBh; iY++) {
    if (iY < 0 || iY >= OVERLAY_H)
      continue;
    for (int iX = iBx; iX < iBx + iBw; iX++) {
      if (iX < 0 || iX >= OVERLAY_W)
        continue;
...
static void OverlayFillTriangle(uint8_t *pBuf, int iX0, int iY0, int iX1,
                                int iY1, int iX2, int iY2, struct nk_color c) {
...
static void OverlayDrawGlyph(DebugOverlay *pOverlay,
                             const struct nk_font_glyph *pGlyph, int iCx,
                             int iCy, struct nk_color fg) {
  int iGw = (int)(pGlyph->x1 - pGlyph->x0);
  int iGh = (int)(pGlyph->y1 - pGlyph->y0);
  for (int iPy = 0; iPy < iGh; iPy++) {
    for (int iPx = 0; iPx < iGw; iPx++) {
      float fU =
          pGlyph->u0 + (pGlyph->u1 - pGlyph->u0) * ((float)iPx / (float)iGw);
      float fV =
          pGlyph->v0 + (pGlyph->v1 - pGlyph->v0) * ((float)iPy / (float)iGh);
      int iAx = (int)(fU * (float)pOverlay->iAtlasW);
      int iAy = (int)(fV * (float)pOverlay->iAtlasH);
      if (iAx < 0 || iAx >= pOverlay->iAtlasW || iAy < 0 ||
          iAy >= pOverlay->iAtlasH)
        continue;
      int iAlpha =
          pOverlay->pAtlasPixels[(iAy * pOverlay->iAtlasW + iAx) * 4 + 3];
      iAlpha = iAlpha * fg.a / 255;
      int iDx = iCx + (int)pGlyph->x0 + iPx;
      int iDy = iCy + (int)pGlyph->y0 + iPy;
      if (iDx < 0 || iDx >= OVERLAY_W || iDy < 0 || iDy >= OVERLAY_H)
        continue;
...
static void RenderCommands(DebugOverlay *pOverlay) {
  ...
  nk_foreach(pCmd, &pOverlay->nk) {
    ...
    case NK_COMMAND_LINE: {
      ...
      OverlayStrokeLine(pOverlay->pPixels, pL->begin.x, pL->begin.y, pL->end.x,
                        pL->end.y, pL->color);
    } break;
    ...
    case NK_COMMAND_TRIANGLE_FILLED: {
      ...
      OverlayFillTriangle(pOverlay->pPixels, pT->a.x, pT->a.y, pT->b.x, pT->b.y,
                          pT->c.x, pT->c.y, pT->color);
    } break;
...
static void UploadAndBlit(DebugOverlay *pOverlay,
                          SDL_GPUCommandBuffer *pCmdBuf) {
  void *pMapped =
      SDL_MapGPUTransferBuffer(pOverlay->pDevice, pOverlay->pTransfer, true);
  if (!pMapped)
    return;
...
LoadOverlayShader(SDL_GPUDevice *pDevice, SDL_GPUShaderStage stage,
...
DebugOverlay *debug_overlay_create(SDL_GPUDevice *pDevice,
                                   SDL_Window *pWindow) {
  ...
  const void *pBaked =
      nk_font_atlas_bake(&pOverlay->atlas, &pOverlay->iAtlasW,
                         &pOverlay->iAtlasH, NK_FONT_ATLAS_RGBA32);
  ...
  nk_font_atlas_end(&pOverlay->atlas, nk_handle_ptr(pOverlay->pAtlasPixels),
                    &nullTex);
  ...
  pStyle->window.fixed_background =
      nk_style_item_color(nk_rgba(45, 45, 45, 200));
  ...
  pStyle->checkbox.cursor_normal =
      nk_style_item_color(nk_rgba(220, 220, 220, 255));
  pStyle->checkbox.cursor_hover =
      nk_style_item_color(nk_rgba(255, 255, 255, 255));
  ...
  SDL_GPUShader *pVert =
      LoadOverlayShader(pDevice, SDL_GPU_SHADERSTAGE_VERTEX,
                        overlay_vertex_spirv, overlay_vertex_spirv_size,
                        overlay_vertex_msl, overlay_vertex_msl_size, 0, 0);
  SDL_GPUShader *pFrag =
      LoadOverlayShader(pDevice, SDL_GPU_SHADERSTAGE_FRAGMENT,
                        overlay_pixel_spirv, overlay_pixel_spirv_size,
                        overlay_pixel_msl, overlay_pixel_msl_size, 1, 0);
  if (pVert && pFrag) {
  ...
  if (pVert)
    SDL_ReleaseGPUShader(pDevice, pVert);
  if (pFrag)
    SDL_ReleaseGPUShader(pDevice, pFrag);
...
void debug_overlay_toggle(DebugOverlay *pOverlay) {
  ...
  if (pOverlay->bVisible) {
    ...
    pOverlay->iNetIfaceCount =
        ROLLERCommsEnumLocalAddrs(pOverlay->aNetIfaces, ROLLER_MAX_IFACES);
  } else {
...
void debug_overlay_handle_event(DebugOverlay *pOverlay, SDL_Event *pEvent) {
  ...
  if (pEvent->type == SDL_EVENT_MOUSE_MOTION) {
    nk_input_motion(pCtx, (int)(pEvent->motion.x * fScaleX),
    ...
    if (pEvent->button.button == SDL_BUTTON_LEFT)
      nk_input_button(pCtx, NK_BUTTON_LEFT, (int)(pEvent->button.x * fScaleX),
    ...
    case SDLK_BACKSPACE:
      nk_input_key(pCtx, NK_KEY_BACKSPACE, bDown);
      break;
    case SDLK_DELETE:
      nk_input_key(pCtx, NK_KEY_DEL, bDown);
      break;
    case SDLK_RETURN:
    case SDLK_RETURN2:
      nk_input_key(pCtx, NK_KEY_ENTER, bDown);
      break;
    case SDLK_LEFT:
      nk_input_key(pCtx,
                   (mod & SDL_KMOD_CTRL) ? NK_KEY_TEXT_WORD_LEFT : NK_KEY_LEFT,
                   bDown);
      break;
    case SDLK_RIGHT:
      nk_input_key(
          pCtx, (mod & SDL_KMOD_CTRL) ? NK_KEY_TEXT_WORD_RIGHT : NK_KEY_RIGHT,
          bDown);
      break;
    case SDLK_HOME:
      nk_input_key(pCtx, NK_KEY_TEXT_LINE_START, bDown);
      break;
    case SDLK_END:
      nk_input_key(pCtx, NK_KEY_TEXT_LINE_END, bDown);
      break;
    case SDLK_LSHIFT:
    case SDLK_RSHIFT:
      nk_input_key(pCtx, NK_KEY_SHIFT, bDown);
      break;
    case SDLK_A:
      if (mod & SDL_KMOD_CTRL)
        nk_input_key(pCtx, NK_KEY_TEXT_SELECT_ALL, bDown);
      break;
    default:
      break;
    }
...
static void DrawNetworkAdapterCombo(DebugOverlay *pOverlay) {
  ...
  int iNewSel = nk_combo(pCtx, apszDisplay, iTotal, iCurSel, 20,
                         nk_vec2(LEFT_W - 20, 150));
  if (iNewSel != iCurSel) {
    if (iNewSel == 0) {
...
static void DrawDebugPanel(DebugOverlay *pOverlay) {
  ...
  if (nk_begin(pCtx, "Settings",
    ...
    int iNewMusicSel =
        nk_combo(pCtx, apszMusic, 2, iMusicSel, 20, nk_vec2(100, 60));
    ...
    if (pRenderer) {
      ...
      if (nk_checkbox_label(pCtx, "Hardware rendering", &bGPU))
        menu_render_set_mode(pRenderer,
                             bGPU ? MENU_RENDER_GPU : MENU_RENDER_SOFTWARE);
    }
...
static void DrawLogPanel(DebugOverlay *pOverlay) {
  ...
  if (nk_begin(pCtx, "Log", nk_rect(RIGHT_X, PANEL_MARGIN, RIGHT_W, PANEL_H),
               NK_WINDOW_BORDER | NK_WINDOW_TITLE)) {
...
void debug_overlay_render(DebugOverlay *pOverlay, SDL_GPUCommandBuffer *pCmdBuf,
                          SDL_GPUTexture *pSwapchainTex, Uint32 uiSwapchainW,
                          Uint32 uiSwapchainH) {
  if (!pOverlay || !pOverlay->bVisible)
    return;
  ...
  if (!pOverlay->pPipeline || !pOverlay->pSampler)
    return;
  ...
  SDL_GPUTextureSamplerBinding binding = {.texture = pOverlay->pTexture,
                                          .sampler = pOverlay->pSampler};
