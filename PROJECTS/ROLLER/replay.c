...
char *views[8] = {                      // 000A644C
    "IN CAR", "CHASE", "MIRROR", "BEHIND", "CAMERA", "ABOVE", "BACK", "TEAM"};
...
tPoint rrotate[8] = // 000A65AC
    {{194, 11}, {199, 11}, {203, 15}, {203, 20},
     {199, 24}, {194, 24}, {190, 20}, {190, 15}};
...
void setreplaytrack() {
  ...
  if (replaytype == 2) {
    ...
    if (g_bSnapshotMode) {
      ...
      strncpy(replayfilename, g_SnapshotConfig.szReplayName,
              sizeof(replayfilename) - 1);
      replayfilename[sizeof(replayfilename) - 1] = '\0';
    } else if (intro || replayfilename[0] == 73) {
      pszRememberFilename =
          rememberfilename; // Copy current filename to remember buffer
      ...
      iIntroFileNum1 = SelectIntroFile(
          lastintro); // Generate random intro file number (1 to introfiles)
    ...
    if (!pFile && introfiles > 1) {
      iIntroFileNum2 =
          SelectIntroFile(lastintro); // Fallback: try different intro file if
    ...
    if (pFile) {
      ...
      if ((buffer[0] & 0x40) !=
          0) // Process cheat mode flags (bit 6 = 0x40, bit 7 = 0x80)
      {
      ...
      numcars = buffer[0] &
                0x1F; // Extract number of cars from lower 5 bits (0x1F mask)
      fseek(pFile, 605, 0); // Seek to non-competitors data at offset 605
      do {
        pReplayNonCompetitor =
            iNonCompetitor++; // Read 16 non-competitor entries (4 bytes each)
      ...
      if (numcars > 0) {
        ...
        do {
          ...
          ++iCarIndex;
          //*(_DWORD *)&car_texture_names[10][iByteOffset + 252] = iCarDesign;//
          // offset into Drivers_Car
          ++iCarCounter;
...
void startreplay() {
  ...
  if (replaytype) { // RECORDING MODE: Set up replay file for writing
    if ((unsigned int)replaytype <= 1) {
      ...
      pWriteFile =
          ROLLERfopen(replayfilename, "wb"); // Open replay file for writing
      ...
      if (pWriteFile) {
        ...
        buffer[0] = (uint8)
            ViewType[0]; // Write view type and selected view to replay file
        fwrite(buffer, 1, 1, replayfile);
        buffer[0] = SelectedView[0];
        fwrite(buffer, 1, 1, replayfile);
        pNonCompetitorsWrite = non_competitors;
        fwrite(camera, 6, 100,
               replayfile); // Write camera data and cuts to replay file
        buffer[0] = cuts;
        fwrite(buffer, 1, 1, replayfile);
        do {
          pCurrentNonCompetitor =
              pNonCompetitorsWrite++; // Write non-competitors and racers data
        ...
        if (numcars > 0) {
          ...
          do {
            buffer[0] =
                Car[iCarWriteIndex]
                    .byCarDesignIdx; // Write car design indices for each car
            ++iCarWriteCounter;
            fwrite(buffer, 1, 1, replayfile);
            ++iCarWriteIndex;
        ...
        if (numcars > 0) {
          ...
          do {
            ...
            pDriverNamesWrite += 9;
               ...
               2) { // PLAYBACK MODE: Load replay data from GSS file
      if (!intro)
        rev_vga[15] = (tBlockHeader *)load_picture(
            "replaysc.bm"); // Load replay screen image if not in intro mode

      int iCounter = 0; // added by ROLLER
      pDriverNamesSrc =
          driver_names[0]; // Backup current driver names to temp storage
      ...
      // iFileHandle = open(replayfilename, 0x200);// Get replay file size to
      // calculate frame count iFileHandleCopy = iFileHandle; if (iFileHandle !=
      // -1) {
      ...
      pReadFile =
          ROLLERfopen(replayfilename, "rb"); // Open replay file for reading
      replayfile = pReadFile;
      if (pReadFile) {
        fread(buffer, 1u, 1u,
              pReadFile); // Read track number and game flags from replay file
        TrackLoad = buffer[0];
        game_track = buffer[0];
        ...
        if (numcars > 0) {
          ...
          do {
            fread(buffer, 1u, 1u,
                  replayfile); // Initialize each car from replay data
            ...
            if (iNonCompetitorValue) // Position non-competitor cars off-track
            {
              ...
              Car[iCarArrayIndex].nReferenceChunk =
                  Car[iCarArrayIndex].nCurrChunk;
        ...
        if (numcars > 0) {
          pDriverNamesRead =
              driver_names[0]; // Read driver names from replay file
          do {
            fread(pDriverNamesRead, 9u, 1u,
                  replayfile); // Write driver names (9 bytes each)
            ++iDriverNameIndex;
            pDriverNamesRead += 9;
        ...
        if (numcars > 0) {
          do {
            fread(&replayData, 0x1Eu, 1u,
                  replayfile); // Read initial replay frame data (30 bytes per
                               // car)
            ++iReplayDataIndex;
            ...
            newrepsample[iReplayDataIndex - 1] =
                GET_HIBYTE(replayData.iPackedPosX);
        ...
        replayheader = 10 * numcars +
                       673; // Calculate replay header size and frame block size
      ...
      //   do {
      //     iCarByteOffset += sizeof(tCar);       // Initialize car physics
      //     data for replay
      //     *(_DWORD *)((char *)&CarBox.hitboxAy[15][6].fY + iCarByteOffset) =
      //     8;// offset into Car
      //     *(float *)((char *)&CarBox.hitboxAy[14][2].fZ + iCarByteOffset) =
      //     0.0;
      //     *(float *)((char *)&CarBox.hitboxAy[14][3].fY + iCarByteOffset) =
      //     0.0;
      //     *(float *)((char *)&CarBox.hitboxAy[14][1].fY + iCarByteOffset) =
      //     0.0;
      //     *(float *)((char *)&CarBox.hitboxAy[14][1].fZ + iCarByteOffset) =
      //     0.0;
...
void stopreplay() {
  ...
  if (replaytype ==
      2) // If replay type is 2 (recording/playback mode), restore driver names
  {
    int iCounter = 0;
    szTempNamePtr = temp_names[0]; // Copy temp driver names back to original
                                   // driver names array
    ...
    TrackLoad =
        oldtrack; // Restore original track, car count and texture settings
    numcars = oldcars;
    textures_off = oldtextures;
  }
  if (replaytype ==
      1) // If replay type is 1 (recording mode), manage disk space
  {
    // dos_getdiskfree(0, &diskfree);              // Get disk free space
    // information uiAvailBytes = HIWORD(diskfree.total_clusters) *
    // LOWORD(diskfree.avail_clusters) * HIWORD(diskfree.avail_clusters);//
    // Calculate available disk space in bytes if (uiAvailBytes < 0x100000) //
    // Check if available space is less than required threshold
    //{
    //   iFileHandle = open(replayfilename, 0x202);// Open replay file for
    //   trimming iFileHandleCopy = iFileHandle; if (iFileHandle != -1) {
    //     iNewFileSize = filelength(iFileHandle) - (0x100000 - uiAvailBytes);//
    //     Calculate new file size to free up disk space if (iNewFileSize >=
    //     20480)            // If new size >= 20KB, truncate file; otherwise
    //     delete it
    //     {
  ...
  {
    szReplayDstPtr =
        replayfilename; // Copy remembered filename back to replay filename
...
//-------------------------------------------------------------------------------------------------
// 00064880
void DoReplayData() {  // Check replay type: 0=disabled, 1=recording, 2=playback
  int iCarIndex;       // esi
  tCar *pCar;          // edi
  ...
  if (replaytype) {
    if ((unsigned int)replaytype <=
        1) { // Recording mode: write replay data to file
        ...
        if (numcars > 0) {
          ...
          do {
            if (!non_competitors[iCarArrayIndex]) {
              ...
              replayData.iPackedPosX =
                  PACK_24_WITH_TAG((int32)pCar->pos.fX, repsample[iCarIndex]);
              replayData.iPackedPosY =
                  PACK_24_WITH_TAG((int32)pCar->pos.fY, repvolume[iCarIndex]);
              replayData.iPackedPosZ = PACK_24_WITH_TAG(
                  (int32)pCar->pos.fZ, (uint8)pCar->iSteeringInput);
              //__asm { fld     dword ptr[edx]; Load Real }
              //_CHP();
              //__asm { fistp[esp + 134h + var_24]; Store Integer and Pop }
              // iPackedPosX = (repsample[iCarIndex] << 24) | iConversionBuffer
              // & 0xFFFFFF;// Pack X position with sample data in high byte
              //__asm { fld     dword ptr[edx + 4]; Load Real }
              //_CHP();
              //__asm { fistp[esp + 134h + var_24]; Store Integer and Pop }
              // iPackedPosY = ((unsigned __int8)repvolume[iCarIndex] << 24) |
              // iConversionBuffer & 0xFFFFFF;// Pack Y position with volume
              // data in high byte
              //__asm { fld     dword ptr[edx + 8]; Load Real }
              //_CHP();
              //__asm { fistp[esp + 134h + var_24]; Store Integer and Pop }
              // iPackedPosZ = (pCar->iSteeringInput << 24) | iConversionBuffer
              // & 0xFFFFFF;// Pack Z position with steering input in high byte
              ...
              nPitchWithOffsets = (int16)(pCar->iPitchCameraOffset) +
                                  (int16)(pCar->iPitchDynamicOffset) +
                                  pCar->nPitch;
              nPitchWithOffsets &= 0x3FFF;
              // HIBYTE(nPitchWithOffsets) &= 0x3Fu;
              replayData.nPitchPacked = nPitchWithOffsets;
              nRollWithOffsets = (int16)(pCar->iRollCameraOffset) +
                                 (int16)(pCar->iRollDynamicOffset) +
                                 pCar->nRoll;
              nRollWithOffsets &= 0x3FFF;
              // HIBYTE(nRollWithOffsets) &= 0x3Fu;
              replayData.nRollPacked = nRollWithOffsets;

              // Convert car speed (take absolute value) and pack with
              // lives/status data
              //_CHP();
              iConversionBuffer = (int)fabs(pCar->fFinalSpeed);
              replayData.nSpeedAndStatus =
                  iConversionBuffer | (((char)pCar->byLives + 2) << 10);
              ...
              replayData.byHealthAndStunned =
                  iConversionBuffer |
                  ((uint8)(pCar->iStunned) << 7); // TODO: check on this
              //__asm
              //{
              ...
              //_CHP();
              //__asm { fistp[esp + 134h + var_24]; Store Integer and Pop }
              // nSpeedAndStatus = iConversionBuffer | (((char)pCar->byLives +
              // 2) << 10);// Pack speed and lives data into replay format if
              ...
              //_CHP();
              //__asm { fistp[esp + 134h + var_24]; Store Integer and Pop }
              // byHealthAndStunned = iConversionBuffer |
              // (LOBYTE(pCar->iStunned) << 7);

              replayData.byMiscCarData = (pCar->byDamageToggle << 7) +
                                         8 * (uint8)(pCar->iDamageState) +
                                         pCar->byGearAyMax + 2 +
                                         16 * pCar->byWheelAnimationFrame;
      ...
      if (replayfile) {
        ...
        if (replayframes - 1 == currentreplayframe && replayspeed > 0 &&
            replaytype == 2) {
          //_disable();
          replayspeed = 0;
          fraction = 0;
        ...
        if (lastreplayframe == currentreplayframe) {
          ...
          newreplayframe = -1;
          fseek(replayfile, replayblock * currentreplayframe + replayheader,
                0); // Seek to current replay frame in file
          iReplayCarIndex = 0;
          ...
          if (numcars > 0) {
            ...
            do {
              if (!non_competitors[uiCarOffset / 4]) {
                ...
                pReplayCar->pos.fX =
                    (float)SIGN_EXTEND_24(replayData.iPackedPosX);
                pReplayCar->pos.fY =
                    (float)SIGN_EXTEND_24(replayData.iPackedPosY);
                pReplayCar->pos.fZ =
                    (float)SIGN_EXTEND_24(replayData.iPackedPosZ);
                ...
                newrepsample[iReplayCarIndex] =
                    GET_HIBYTE(replayData.iPackedPosX);
                repvolume[iCarIndexForReplay] =
                    GET_HIBYTE(replayData.iPackedPosY);
                ...
                if (pReplayCar->iControlType == 3) {
                  ...
                  if (pReplayCar->pos.fY < pData->fTrackHalfWidth ||
                      isnan(pReplayCar->pos.fY)) {
                ...
                // Extract speed data from packed format (lower 10 bits) and
                // convert to float
                iConversionBuffer =
                    replayData.nSpeedAndStatus &
                    0x2FF; // Extract lower 10 bits containing speed
                ...
                // Unpack RPM data: convert packed byte back to floating point
                // ratio
                pReplayCar->fRPMRatio =
                    (float)replayData.byRPMPacked * 0.00392156862745098f;
                ...
                // Extract health data from packed format (lower 7 bits) and
                // convert to float
                iTempCalcBuffer =
                    replayData.byHealthAndStunned &
                    0x7F; // Extract lower 7 bits containing health
                ...
                pReplayCar->byDamageToggle =
                    (replayData.byMiscCarData & 0x80) >> 7;
                iDamageState_1 = pReplayCar->iDamageState;
                pReplayCar->byDamageIntensity = replayData.byDamageIntensity;
                if (iDamageState_1 != iDamageState &&
                    ViewType[0] == pReplayCar->iDriverIdx) {
                  if (replayData.nCurrChunk < 0) {

                    tData *pData = &localdata[0];
                    fBestDistance = 1.0e10f; // Very large initial value
                                             // (appears to be a float constant)
                    iTrackSegmentIdx = 0;
                    if (TRAK_LEN > 0) {
                      iGroundColorIdx = 0;
                      do {
                        // Calculate relative position (car position + track
                        // segment offset)
                        float fRelativeX = pReplayCar->pos.fX +
                                           pData->pointAy[3].fX; // [edx+24h]
                        float fRelativeY = pReplayCar->pos.fY +
                                           pData->pointAy[3].fY; // [edx+28h]
                        float fRelativeZ = pReplayCar->pos.fZ +
                                           pData->pointAy[3].fZ; // [edx+2Ch]
                        // Calculate dot product with track normal vector
                        float fDotProduct =
                            (pData->pointAy[0].fZ * fRelativeX) + // [edx+8]
                        ...
                        float fDistance = (float)sqrt(fDistanceSquared) -
                                          pData->fTrackHalfLength; // [edx+30h]
                        fDistanceBuffer = fDistance;
                        // Check if this is a valid segment and closer than
                        // previous best
                        if (fDistance >= -400.0 ||
                            GroundColour[iGroundColorIdx][2] >= 0) {
                ...
                if (iDamageState != pReplayCar->iDamageState) {
                  ...
                  // for (iTexIndex = 0; iTexIndex != 352; car_texs_loaded[352 *
                  // pReplayCar->iDriverIdx + 12 + iTexIndex] = 0)
                  //   iTexIndex += 11;
                  pReplayCar->byLastDamageToggle = pReplayCar->byDamageToggle;
                }

                // Initialize damage spray effects when damage state changes
                if (pReplayCar->byDamageToggle !=
                    pReplayCar->byLastDamageToggle) {
                  iSprayIdx = 0;
            ...
            if (iFrameDifference > 0 && numcars > 0) {
              ...
              do {
                if (!non_competitors[iCarLoopIndex / 4u]) {
                  ...
                  if (enginedelay[0]
                              .engineSoundData[uiSoundOffsetCopy / 0x1C]
                              .iEngineVol == -1 ||
                      enginedelay[0]
                              .engineSoundData[uiDelayOldByteOffset / 0x1C +
                                               uiSoundDataOffset / 0x1C]
                              .iEngineVol == -1) {
                    iNextDelaySlot = iNextDelayIndex;
                    if (iDelayWriteNew != iNextDelayIndex) {
                      uiSoundOffset = uiSoundDataOffset;
                      do {
                        *(int *)((char *)&enginedelay[0]
                                     .engineSoundData[iNextDelaySlot]
                                     .iEngineVol +
                                 uiSoundOffset) = -1;
                    ...
                    do {
                      iDelayIdxForEngine =
                          ((uint8)iDelayIdxForEngine + 1) & 0x1F;
                      iConversionBuffer = 28 * iDelayIdxForEngine;
                      *(int *)((char *)&enginedelay[0]
                                   .engineSoundData[iDelayIdxForEngine]
                                   .iEngineVol +
                               uiEngineCalcOffset) =
                          (iEngineVolStep - iEngineVolDelta) /
                          iInterpolationSteps;
                      ++iEngineStepCounter;
                      iEngineVolStep += iEngineVolIncrement;
                    ...
                    iEnginePitchNew = *(
                        int *)((char *)&enginedelay[0]
                                   .engineSoundData[uiDelayNewByteOffset / 0x1C]
                                   .iEnginePitch +
                               uiEngineCalcOffset);
                    ...
                    do {
                      iEnginePitchDelayIdx =
                          ((uint8)iEnginePitchDelayIdx + 1) & 0x1F;
                      iConversionBuffer = 28 * iEnginePitchDelayIdx;
                      *(int *)((char *)&enginedelay[0]
                                   .engineSoundData[iEnginePitchDelayIdx]
                                   .iEnginePitch +
                               uiEnginePitchOffset) =
                          (iEnginePitchNew - iEnginePitchDelta) /
                          iInterpolationSteps;
                  ...
                  // Begin engine2 sound interpolation (secondary engine
                  // channel)
                  if (enginedelay[0]
                              .engineSoundData[uiSoundOffsetCopy / 0x1C]
                              .iEngine2Vol == -1 ||
                    ...
                    if (iDelayWriteNew != iNextDelayIndex) {
                      ...
                      do {
                        *(int *)((char *)&enginedelay[0]
                                     .engineSoundData[iEngine2DelaySlot]
                                     .iEngine2Vol +
                                 uiEngine2SoundOffset) = -1;
                        iEngine2DelaySlot =
                            ((uint8)iEngine2DelaySlot + 1) & 0x1F;
                    ...
                    do {
                      ...
                      *(int *)((char *)&enginedelay[0]
                                   .engineSoundData[iEngine2StepIdx]
                                   .iEngine2Vol +
                               uiEngine2CalcOffset) =
                          (iEngine2VolCurrent - iEngine2VolDelta) /
                          iInterpolationSteps;
                    ...
                    do {
                      iEngine2PitchDelayIdx =
                          ((uint8)iEngine2PitchDelayIdx + 1) & 0x1F;
                      iConversionBuffer = 28 * iEngine2PitchDelayIdx;
                      *(int *)((char *)&enginedelay[0]
                                   .engineSoundData[iEngine2PitchDelayIdx]
                                   .iEngine2Pitch +
                               uiEngine2PitchOffset) =
                          (iEngine2PitchWorking - iEngine2PitchDelta) /
                          iInterpolationSteps;
                      ++iEngine2PitchStep;
                  ...
                  if (enginedelay[0]
                    ...
                    if (iDelayWriteNew != iNextDelayIndex) {
                      ...
                      do {
                        *(int *)((char *)&enginedelay[0]
                                     .engineSoundData[iSkidDelaySlot]
                                     .iSkid1Vol +
                                 uiSkidSoundOffset) = -1;
                    ...
                    do {
                      ...
                      *(int *)((char *)&enginedelay[0]
                                   .engineSoundData[iSkidVolDelayIdx]
                                   .iSkid1Vol +
                               uiSkidCalcOffset) =
                          (iSkidVolCurrent - iSkidVolDelta) /
                          iInterpolationSteps;
                    ...
                    do {
                      iSkidPitchDelayIdx =
                          ((uint8)iSkidPitchDelayIdx + 1) & 0x1F;
                      iConversionBuffer = 28 * iSkidPitchDelayIdx;
                      *(int *)((char *)&enginedelay[0]
                                   .engineSoundData[iSkidPitchDelayIdx]
                                   .iSkid1Pitch +
                               uiSkidPitchOffset) =
                          (iSkidPitchWorking - iSkidPitchDelta) /
                          iInterpolationSteps;
                      ++iSkidPitchStep;
                  ...
                  iPan = enginedelay[0]
                             .engineSoundData[uiDelayOldOffset / 0x1C]
                             .iPan;
                  iPanIncrement = enginedelay[0]
                                      .engineSoundData[uiSoundOffsetCopy / 0x1C]
                                      .iPan;
                  ...
                  do {
                    ...
                    *(int *)((char *)&enginedelay[0]
                                 .engineSoundData[iPanStepIdx]
                                 .iPan +
                             uiPanCalcOffset) =
                        (iPanStep - iPanDelta) / iInterpolationSteps;
                    iPanStep += iPanIncrement;
                    iPanDelta += iPanBase;
                    ++iPanStepCount;
                  ...
                  do {
                    ...
                    *(int *)((char *)&enginedelay[0]
                                 .engineSoundData[iPanStepCounter]
                                 .iPan +
                             uiPanSoundOffset) =
                        (iPanNew - iPanDeltaStep) / iInterpolationSteps;
                    iPanNew += iPanNewCopy;
          ...
          if (!rewinding && !forwarding && replayspeed) {
            ...
            if (iCurrentCut != -1 && iCurrentCut != lastautocut &&
                (camera[iCurrentCut].byView != SelectedView[0] ||
                 camera[iCurrentCut].byCarIdx != ViewType[0]) &&
                autoswitch) {
          ...
          if (lastreplayframe != -1)
            doviewtend(&Car[ViewType[0]],
                       abs(lastreplayframe - currentreplayframe), 0);
          ...
          if (numcars > 0) {
            pSmokeCar = Car;
            do {
              for (iStepCounter = 0; iStepCounter < iUpdateSteps;
                   ++iStepCounter)
...
void Rplay() {
  if (replaytype == 2) {
    ...
    // Initialize sound timing if playing at normal speed (0x100 is 1.0 in 8.8
    // fixed float)
...
unsigned int readdisable(int iFrame) {
  ...
  if (!replayedit || iFrame > 0x1FFFF)
    return 0;
  iArrayIndex =
      iFrame / 32; // Calculate array index: divide by 32 to get DWORD index
  // iArrayIndex = (iFrame - (__CFSHL__(iFrame >> 31, 5) + 32 * (iFrame >> 31)))
  // >> 5;
  return ((unsigned int)disabled[iArrayIndex] >>
          (byBitPosition - 32 * iArrayIndex)) &
         1;
}
...
void cleardisable(int iFrame) {
  ...
  byBitPosition =
      iFrame; // Store lower 8 bits of replay index for bit position calculation
  if (iFrame <
      0x20000) // Check if replay index is within valid range (0-131071)
  {
    iArrayIndex =
        iFrame / 32; // Calculate array index: divide by 32 to get DWORD index
    // iArrayIndex = (iFrame - (__CFSHL__(iFrame >> 31, 5) + 32 * (iFrame >>
    // 31))) >> 5;// Calculate array index: divide by 32 to get DWORD index
    disabled[iArrayIndex] &=
        ~(1 << (byBitPosition -
                32 * iArrayIndex)); // Clear the specific bit using AND with
                                    // inverted bit mask
  }
}
...
void deleteframes(int iStartFrame, int iEndFrame) {
  ...
  { // Ensure start frame <= end frame by swapping if necessary
    ...
    for (iCurrentFrame = iStartFrame; iCurrentFrame <= iEndFrameFixed;
         ++iCurrentFrame) // Loop through all frames in the range to mark as
                          // disabled
    { // Check frame is within valid range (< 128K frames max)
      if (iCurrentFrame < 0x20000) {
        iArrayIndex =
            iCurrentFrame /
            32; // Calculate array index: divide by 32 to get DWORD index
        // iArrayIndex = (iCurrentFrame - (__CFSHL__(iCurrentFrame >> 31, 5) +
        // 32 * (iCurrentFrame >> 31))) >> 5;// Calculate array index:
        // iCurrentFrame / 32
        disabled[iArrayIndex] |=
            1 << (iCurrentFrame -
                  32 * iArrayIndex); // Set bit in disabled[] bitfield to mark
                                     // frame as disabled
      }
    }
  }
...
void undeleteframes(int iStartFrame, int iEndFrame) {
  ...
  { // Ensure start frame <= end frame by swapping if necessary
    ...
    for (iCurrentFrame = iStartFrame; iCurrentFrame <= iEndFrameFixed;
         ++iCurrentFrame) // Loop through all frames in the range to undelete
                          // them
    {
...
void findnextvalid() {
  ...
  iOriginalFrame = currentreplayframe; // Store original frame position for
                                       // potential fallback
  for (iTestFrame = currentreplayframe + 1;
       currentreplayframe + 1 < replayframes;
       iTestFrame = currentreplayframe +
                    1) // Search forward for next valid (non-disabled) frame
  {
    currentreplayframe = iTestFrame;
    uiDisabledStatus =
        readdisable(iTestFrame); // Check if current frame is disabled
    iTestFrame = currentreplayframe;
    if (!uiDisabledStatus)
      break; // If frame is enabled (not disabled), we found our target
  }
  if (iTestFrame == replayframes) // If reached end without finding valid frame,
                                  // search backward
  {
    iBackwardFrame = iOriginalFrame - 1;
    if (iOriginalFrame - 1 >=
        0) // Search backward from original position for valid frame
    {
      do {
        currentreplayframe = iBackwardFrame;
        uiBackwardDisabled =
            readdisable(iBackwardFrame); // Check if backward frame is disabled
    ...
    if (iBackwardFrame <
        0) // If no valid frame found, fallback to original position
      iBackwardFrame = iOriginalFrame;
    if (replaytype == 2) // If in replay mode, stop playback and reset state
    {
      currentreplayframe =
          iBackwardFrame; // Disable interrupts for atomic update
      //_disable();
    ...
    currentreplayframe = iTestFrame;
    //_disable();                                 // Found valid frame forward -
    ...
    iBackwardFrame = currentreplayframe;
    if (iOriginalFrame !=
        currentreplayframe) // If frame changed, trigger view refresh
      pend_view_init = ViewType[0];
  }
  currentreplayframe = iBackwardFrame;
}
...
void findlastvalid() {
  ...
  iOriginalFrame = currentreplayframe; // Store original frame position for
                                       // potential fallback
  for (iTestFrame = currentreplayframe - 1; currentreplayframe - 1 >= 0;
       iTestFrame =
           currentreplayframe -
           1) // Search backward for previous valid (non-disabled) frame
  {
    currentreplayframe = iTestFrame;
    uiDisabledStatus =
        readdisable(iTestFrame); // Check if current frame is disabled
  ...
  {
    currentreplayframe = iTestFrame;
    //_disable();                                 // Found valid frame backward
    //- update timing atomically
    ticks = currentreplayframe;
    fraction = 0;
    //_enable();
    ...
    iForwardFrame =
        iOriginalFrame +
        1; // No valid frame backward, search forward from original position
    if (iOriginalFrame + 1 <
        replayframes) // Search forward from original position for valid frame
    {
      do {
        currentreplayframe = iForwardFrame;
        uiForwardDisabled =
            readdisable(iForwardFrame); // Check if forward frame is disabled
        iForwardFrame = currentreplayframe;
    ...
    if (iForwardFrame ==
        replayframes) // If no valid frame found, fallback to original position
      iForwardFrame = iOriginalFrame;
    if (replaytype == 2) // If in replay mode, stop playback and reset state
    {
      currentreplayframe = iForwardFrame;
      //_disable();                               // Disable interrupts for
      // atomic update
      replayspeed = 0; // Stop replay playback
      fraction = 0;
      replaydirection = 0;
...
void Rassemble() {
  ...
  {
    memset(emptyCameraData, 0,
           sizeof(emptyCameraData)); // Initialize empty camera data buffer and
                                     // setup car processing arrays
    pData = scrbuf;
    iCarIndex = 0;
    ...
    //_disable();                                 // Disable interrupts and
    // reset replay state for assembly
    iInitFlag = -1;
    replayspeed = 0;
    replaydirection = 0;
    fraction = 0;
    ...
    //_enable();
    strcpy(szFilename,
           "../REPLAYS/"); // Build output filename by concatenating path,
                           // selected name, and .GSS extension
    ...
    if (!strcmp(
            szFilename,
            replayfilename)) // Check if trying to overwrite current replay file
    {
      lastfile = 0;
      screenready = 0;
      filingmenu = 9;
    } else {
      pOutputFile = ROLLERfopen(
          szFilename,
          "wb"); // Open output file and copy replay header from source file
      fp = pOutputFile;
      if (pOutputFile) {
        ...
        fseek(fp, 4, 0);
        iCutProcessed = 0;
        if (cuts > 0) // Process each camera cut, counting valid frames and
                      // writing camera data
        {
          pCameraPtr = camera;
          iOffset = -4;
          ...
          do { // Count valid frames up to current camera cut
            ...
            iFrameBackup = iFrame;
            if (iPrevValidFrame == -1 ||
                iValidFrames !=
                    iPrevValidFrame) // Write camera cut data - either new entry
                                     // or update existing duplicate frame
            {
              pCamera = pCameraPtr;
              pWriteFile = fp;
            ...
            //*(int *)((char *)&yp_variable_1 + iCutIndex * 6) = iFrameBackup;//
            // reference into adjacent data camera
            pCameraPtr = pNextCamera;
            iCutProcessed = iCutCounter;
          } while (iCutCounter < iCutsTotal);
        }
        iPaddingCount = 100 - iCutsWritten; // Fill remaining camera slots with
                                            // empty data (max 100 cuts)
        ...
        tempBuffer[0] =
            iCutsWritten; // Write total number of camera cuts to file
        if (fwrite(tempBuffer, 1, 1, fp) != 1) {
          iErrorCode5 = errno;
          iErrorFlag = -1;
          iLastError = iErrorCode5;
        }
        fseek(fp, replayheader, 0); // Main replay data processing - read blocks
                                    // and filter/process car data
        iFrameCounter = 0;
        do {
          szDataBuffer = (uint8 *)pData;
          uiBytesRead = (uint32)fread(pData, replayblock, uiSize, replayfile);
          iBlockCounter = 0;
          for (k = uiBytesRead; iBlockCounter < k;
               ++iBlockCounter) { // Skip disabled frames, process valid frames
                                  // with car data filtering
            if (readdisable(iFrameCounter)) {
              ...
              if (iInitFlag) {
                ...
                if (numcars > 0) {
                  iCarIndex1 = 0;
                  do {
                    aiCarDirectionFlags[iCarIndex1] =
                        (((int)szCarDataPtr[26] >> 3) & 1) ==
                        aiCarDirectionFlags[iCarIndex1 + 16];
              ...
              szCarDataPtr2 =
                  szDataBuffer; // Process car data - flip direction bits and
              ...
              if (numcars > 0) {
                ...
                do {
                  szCarDataPtr2[26] ^=
                      8 * (uint8)(aiCarDirectionFlags[iCarIndex2]);
                  aiCarDirectionFlags[iCarIndex2 + 16] =
                      ((int)szCarDataPtr2[26] >> 3) & 1;
                  ...
                  iCarCount = numcars;
                  *(int *)(szCarDataPtr2 - 30) =
                      ((char)byProcessedData << 24) |
                      *(int *)(szCarDataPtr2 - 30) & 0xFFFFFF;
        ...
        iHasData = iDataWritten; // Close file and handle errors - delete
                                 // incomplete file on failure
        fclose(fp);
        if (!iHasData) {
          errno = -1000;
          iErrorFlag = -1;
...
void storecut() {
  ...
  if (replaytype == 2 && cuts < 100) // Only allow cut creation in edit mode and
                                     // under limit (100 cuts max)
  {
    iInsertIndex =
        -1;   // Initialize insertion index to -1 (no position found yet)
    if (cuts) // Search for correct insertion position if cuts exist
    {
      iCutCounter = 0;
      if (cuts > 0) {
        iCameraIndex =
            0; // Find the last cut that occurs before or at current frame
      ...
      if (camera[iInsertIndex].iFrame != currentreplayframe ||
          iInsertIndex == -1) // Check if cut already exists at this frame
      { // Calculate insertion position (after last matching cut)
        if (++iInsertIndex != cuts) {
          memmove(&camera[iInsertIndex + 1], &camera[iInsertIndex],
                  6 * (cuts - iInsertIndex)); // Shift existing cuts up to make
                                              // room for new cut
          iNumCuts = cuts;
        }
    ...
    camera[iStoreIndex].byCarIdx =
        (uint8)ViewType[0]; // Store current car index for view
  }
    ...
    if (cuts > 0) // Search through existing cuts to find match
    {
      ...
      do { // Check if current replay frame matches this cut frame
        if (currentreplayframe ==
            camera[iCameraIndex].iFrame) { // If not the last cut, shift
                                           // remaining cuts down to fill gap
          if (iCutIndex != iNumCuts - 1) {
            cuts = iNumCuts;
            memmove(&camera[iCameraIndex], &camera[iCameraIndex + 1],
                    6 * (iNumCuts - iCutIndex -
                         1)); // Move remaining cuts down by 6 bytes each
                              // (camera struct size)
            iNumCuts = cuts;
          }
...
int readcut() {
    ...
    if (cuts > 0) // Iterate through all camera cuts to find the active one
    {
      ...
      do { // Check if this cut's frame is at or before current replay frame
        if (camera[iArrayIndex].iFrame <= currentreplayframe)
          iLastValidCut = iCutIndex; // Update to this cut index (keeps the
                                     // latest valid cut)
        ++iCutIndex;
        ++iArrayIndex;
      } while (iCutIndex < cuts);
  ...
  return iLastValidCut; // Return index of most recent valid cut, or -1 if none
                        // found
}

//-------------------------------------------------------------------------------------------------
// 00066CC0
void displayreplay() { // Apply scanline effect for disabled replay frames (type
                       // 2)
  ...
  if (replaytype == 2 && readdisable(currentreplayframe)) {
    pScreenBuffer = scrbuf; // Get screen buffer and window dimensions for
                            // scanline processing
    iWindowWidth = winw;
    for (iHeightLoop = 0; iHeightLoop < winh;
         iHeightLoop += 2) // First pass: Set every other pixel to black on even
                           // rows (scanlines)
    {
      for (iWidthLoop = 0; iWidthLoop < iWindowWidth;
           pPixelPtr[iWindowWidth * iHeightLoop] = 0) {
        pPixelPtr = &pScreenBuffer[iWidthLoop];
        iWidthLoop += 2;
      }
    }
    iRowIndex = 1; // Second pass: Clear entire odd rows to create horizontal
                   // scanline effect
    winw = iWindowWidth;
  ...
  if (filingmenu) // Handle file menu dialogs for replay management
  {
    switch (filingmenu) {
    case 1:
      fileselect(10, 10, 310, 125, 160, 20, &language_buffer[3136],
                 "../REPLAYS/*.GSS", 1); // Load replay dialog
      break;
    case 2:
      fileselect(10, 10, 310, 140, 160, 20, &language_buffer[3200],
                 "../REPLAYS/*.GSS", 2); // Save replay dialog
      break;
    case 3:
      fileselect(10, 10, 310, 125, 160, 20, &language_buffer[3264],
                 "../REPLAYS/*.GSS", 3); // Delete replay dialog
      break;
    case 4:
      fileselect(10, 10, 310, 140, 160, 20, &language_buffer[3392],
                 "../REPLAYS/*.GSS", 4); // Assemble replay dialog
      break;
    case 5:
      lsd(40, 37, 280, 107); // Show LSD (replay statistics) dialog
      break;
    case 6:
      warning(122, 55, 198, 85,
              &language_buffer[3456]); // Show warning dialogs for various
                                       // replay operations
      break;
    case 7:
  ...
  if (replaypanel) // Validate replay panel display based on screen mode and
                   // size
  {
...
void lsd(int iX1, int iY1, int iX2, int iY2) {
  ...
  poly.uiNumVerts = 4;
  game_render_quad_screen(g_pGameRenderer, &poly, TEXTURE_HANDLE_INVALID, NULL);
  prt_centrecol(rev_vga[1], &language_buffer[3072], 160, iOriginalY1 + 10,
                231); // Display menu title text centered
  while (fatkbhit())  // Main input loop - process keyboard input
  {
    ...
    {          // Check for ENTER key (0x0D)
      ...
      {
        if (byExtendedKey <=
            0x48u) { // UP arrow: move selection up if not at top
          if (lsdsel)
            --lsdsel;
        } else if (byExtendedKey == 0x50 &&
                   lsdsel < 2) // Check for DOWN arrow key (0x50)
        {
  ...
  do { // Select appropriate menu text based on option index
    ...
    do {
      byChar1 = *pSourceText; // String copy loop: copy 2 bytes at a time until
                              // null terminator
  ...
  RENDER_MENU_OPTION:
    if (iMenuOption ==
        lsdsel) // Set text color based on selection: highlighted vs normal
      byTextColor = 0x8F; // Selected item color (0x8F = white)
    else
      byTextColor = 0x83; // Normal item color (0x83 = grey)
    prt_centrecol(rev_vga[1], buffer, 160, iTextY,
                  byTextColor); // Draw menu option text centered
...
void scandirectory(const char *szPattern) {
...
#else
  // For Unix-like systems, we need to extract directory and pattern from
  // szPattern
  ...
  const char *szSeparator =
      (szLastSlash > szLastBackslash) ? szLastSlash : szLastBackslash;
...
void fileselect(int iBoxX0, int iBoxY0, int iBoxX1, int iBoxY1, int iTextX,
  ...
  if (filingmenu == 2 ||
      filingmenu == 4) // Check if in save mode (2) or assemble mode (4) to
                       // enable text editing
    iEditMode = -1;
  else
    iEditMode = 0;
  ...
  params.vertices[0].x =
      iBoxX1; // Set up polygon vertices for file selection background rectangle
  ...
  game_render_quad_screen(g_pGameRenderer, &params, TEXTURE_HANDLE_INVALID,
                          NULL);
  prt_centrecol(rev_vga[1], szText, iTextX, iTextY, 231);
  if (iFileIdx != lastfile) // If different directory selected, rescan files and
                            // reset selection
  {
    topfile = 0;
    filefile = 0;
    ...
    if (byKeyCode) {
      if ((uint8)byKeyCode < 0xDu) {
        ...
        LABEL_85:
          ...
          if (iInputStrLen <= 7 && ((uint8)byProcessedKey >= 0x41u &&
                                        (uint8)byProcessedKey <= 0x5Au ||
                                    (uint8)byProcessedKey >= 0x30u &&
                                        (uint8)byProcessedKey <= 0x39u)) {
        ...
        case 4:
          ...
          break;
        default:
          continue; // Execute action based on filing menu mode (1=Load, 2=Save,
                    // 3=Delete, 4=Assemble)
        }
      } else {
        if (byKeyCode != 27)
      ...
      case 'S':
        ...
        if ((int)(uiDelStrLen - 1) > 0)
          selectfilename[strlen(selectfilename) - 1] = 0;
        // filesel_variable_1[uiDelStrLen - 1] = 0;// reference into
        // selectfilename
        break;
      default:
        continue; // Handle arrow key navigation (H=Up, K=Left, M=Right, P=Down,
                  // S=Delete)
      }
    }
  }
  ...
  if (iEditMode) {
    ...
    if (frames % 18 >= 9) // Animate blinking cursor for filename input (shows
                          // underscore every 9 frames)
    {
    ...
    prt_stringcol(rev_vga[1], selectfilename, iLeftEdge + 20, iTextY + 100,
                  255);
  ...
  if (topfile) // Draw scroll up/down arrows based on current position in file
               // list
    replayicon(scrbuf, rev_vga[15], 79, iUpArrowX, iTextY - 3, iScreenWidth,
               255);
  else
    replayicon(scrbuf, rev_vga[15], 78, iUpArrowX, iTextY - 3, iScreenWidth,
               255);
  iDownArrowX = iTextX + 128;
  if (topfile + 18 >= filefiles)
    replayicon(scrbuf, rev_vga[15], 80, iDownArrowX, iTextY + 82, iScreenWidth,
               255);
  else
    replayicon(scrbuf, rev_vga[15], 81, iDownArrowX, iTextY + 82, iScreenWidth,
               255);
...
void previouscut() {
  ...
  iSearchFrame = currentreplayframe; // Start search from current frame or
  ...
  if (cuts)       // Search through camera cuts if any exist
  {
    ...
    if (cuts > 0) {
      iCameraIndex =
          0; // Find the last cut that occurs before or at search frame
  ...
  if (iCutIndex != -1) // If cut found, use the cut frame as target
    iFrame = camera[iCutIndex].iFrame;
  if (iSearchFrame >= iFrame) // Search backwards from current position to find
                              // previous cut boundary
  { // Get disable status of previous and current frames
    ...
    while (iTargetFrame >
           0) // Search backwards until boundary found or target frame reached
    {
...
void nextcut() {
  ...
  if (iCutIndex >= cuts - 1) // Determine next cut boundary or end of replay
    ...
    iNextCutFrame =
        camera[iCutIndex + 1].iFrame; // Otherwise target is next cut frame
  if (currentreplayframe <
      replayframes - 1) // Advance to next frame if not at end of replay
    iSearchFrame = currentreplayframe + 1;
  if (iSearchFrame <= iNextCutFrame) // Search forward from current position to
                                     // find next cut boundary
  { // Get disable status of previous frame for boundary detection
    ...
    for (uiPrevDisabled = 0;;
         uiPrevDisabled = uiCurrDisabled) // Search forward until boundary found
                                          // or target reached
    {
      currentreplayframe = iSearchFrame;
      uiCurrDisabled = readdisable(iSearchFrame);
...
void loadreplay() {
  ...
  if (filefiles) // If files are available for replay loading
  {
    ...
    strcpy(
        replayfilename,
        "../REPLAYS/"); // Start building replay file path with base directory
    szSrc = selectfilename; // Manually copy selected filename to replay path (2
                            // chars at a time)
    ...
    //_disable();                                 // Disable interrupts while
    // setting time-critical replay state
    replayspeed = 0;
    fraction = 0;
    replaydirection = 0;
    ticks = currentreplayframe;
...
void savereplay() {
  ...
  pbyBuffer = scrbuf;
  if (!strcmp(replayfilename,
              "../REPLAYS/REPLAY.TMP")) // Check if currently working with
                                        // temporary replay file
  {
    ftell(replayfile); // Finalize temp replay: close file and rename to
                       // permanent location
    fclose(replayfile);
    strcpy(replayfilename, "../REPLAYS/"); // Build final replay filename: path
                                           // + selected name + extension
    szSrc1 = selectfilename; // Copy selected filename to replay path (2 chars
                             // at a time)
    ...
    ROLLERremove(
        replayfilename); // Remove existing file and rename temp to final name
    rename("../REPLAYS/REPLAY.TMP", replayfilename);
    replayfile = ROLLERfopen(replayfilename, "rb");
    fseek(replayfile, 0, 0);
  } else {
    strcpy(szTempFilename, "../REPLAYS/"); // Handle non-temp replay: copy
                                           // current replay to new filename
    szSrc2 = selectfilename; // Build target filename: path + selected name +
                             // extension
    ...
    if (strcmp(szTempFilename, replayfilename)) // Only copy if target filename
                                                // is different from current
    {
      pFile = ROLLERfopen(
          szTempFilename,
          "wb"); // Open target file for writing and prepare for copy operation
      if (pFile) {
        ...
        do {
          ...
          if (fwrite(pbyBuffer, 1, uiBytesRead, pFile) != uiBytesRead) {
            iError = errno; // *(_DWORD *)_get_errno_ptr();// Handle write error
                            // - set error flag and save errno
            iErrorFlag = -1;
            iErrorNum = iError;
          }
        } while (uiBytesRead && !iErrorFlag);
        iErrorCopy = iErrorFlag;
        fclose(pFile); // Cleanup: close file, remove on error, restore file
                       // position
        if (iErrorCopy)
...
void deletereplay() {
  ...
  strcpy(szTargetFilename, "../REPLAYS/"); // Build target filename: path +
  ...
  if (!strcmp(replayfilename, szTargetFilename)) // Check if deleting the
                                                 // currently loaded replay file
  {
    ROLLERremove("../REPLAYS/REPLAY.TMP"); // Deleting current replay: cleanup
                                           // temp files and swap to temp
    ftell(replayfile); // Close current replay file and rename target to temp
    szTmpDst = replayfilename;
    fclose(replayfile);
    szTmpSrc = "../REPLAYS/REPLAY.TMP"; // Copy temp filename to current
                                        // replayfilename (2 chars at a time)
    ...
    replayfile =
        ROLLERfopen(replayfilename, "rb"); // Reopen temp file as current replay
    fseek(replayfile, 0, 0);
  } else {
...
void updatedirectory() {
  ...
  if (!filefiles)        // If no files available, reset current file selection
    filefile = 0;
  if (filefile <
      topfile) // If selected file is above view window, scroll up 3 positions
    iNewTopFile = topfile - 3;
  if (iNewTopFile + 18 <= filefile) // If selected file is below view window (18
                                    // files visible), scroll down 3 positions
    iNewTopFile += 3;
  szDst = selectfilename; // Copy currently selected filename to selectfilename
                          // buffer (2 chars at a time)
...
void displaycontrolpanel() {
  ...
  if (!keys[WHIP_SCANCODE_RETURN] || paused ||
      (iIconOffset = -1, controlicon == 18) &&
          !replayselect) // Calculate icon offset: -1 if ENTER pressed and
                         // conditions met, 0 otherwise
    iIconOffset = 0;
  replayicon(scrbuf, rev_vga[15], controlicon, ricon[controlicon].nX,
             ricon[controlicon].nY + 150 + iIconOffset, iScreenWidth,
             -1); // Draw current control icon with calculated offset
  if (replayspeed > 0 && !forwarding &&
      !rewinding) // Draw fast forward indicators when replay speed is positive
  {
    if (controlicon == 4)
      replayicon(scrbuf, rev_vga[15], 4, ricon[4].nX, ricon[4].nY + 149,
                 iScreenWidth, -1);
    else
      replayicon(scrbuf, rev_vga[15], 27, ricon[4].nX, ricon[4].nY + 149,
                 iScreenWidth, -1);
  }
  if (replayspeed < 0 && !forwarding &&
      !rewinding) // Draw rewind indicators when replay speed is negative
  {
    if (controlicon == 3)
      replayicon(scrbuf, rev_vga[15], 3, ricon[3].nX, ricon[3].nY + 149,
                 iScreenWidth, -1);
    else
      replayicon(scrbuf, rev_vga[15], 26, ricon[3].nX, ricon[3].nY + 149,
                 iScreenWidth, -1);
  }
  if (replayedit) // Draw edit mode indicator icon
  {
    if (controlicon == 17)
      replayicon(scrbuf, rev_vga[15], 17, ricon[17].nX, ricon[17].nY + 149,
                 iScreenWidth, -1);
    else
      replayicon(scrbuf, rev_vga[15], 28, ricon[17].nX, ricon[17].nY + 149,
                 iScreenWidth, -1);
  }
  if (replayselect) // Draw replay selection mode indicator icon
  {
    if (controlicon == 18)
      replayicon(scrbuf, rev_vga[15], 18, ricon[18].nX, ricon[18].nY + 149,
                 iScreenWidth, -1);
    else
      replayicon(scrbuf, rev_vga[15], 29, ricon[18].nX, ricon[18].nY + 149,
                 iScreenWidth, -1);
  }
  if (disciconpressed) // Draw disc icon when pressed
    replayicon(scrbuf, rev_vga[15], 16, ricon[16].nX, ricon[17].nY + 149,
               iScreenWidth, -1);
  replaypanelstring(replayhelp[controlicon], 255, 165,
                    iScreenWidth); // Display help text for current control icon
  if (replayedit) // EDIT MODE: Display edit mode information panel
  {
    replaypanelstring("CURRENT:", 164, 165,
                      iScreenWidth); // Show current edit info text
    sprintf(buffer, "CAR %d",
            ViewType[0] + 1); // Display current car number being viewed
    replaypanelstring(buffer, 199, 165, iScreenWidth);
    replaypanelstring(views[SelectedView[0]], 225, 165,
                      iScreenWidth); // Display current camera view type
    if (cuts)                        // Process camera cuts if they exist
    {
      ...
      if (cuts > 0) {
        iCutIndex =
            0; // Find the current active camera cut based on replay frame
        do {
      ...
      { // Show different text for exact frame match vs approximate
        ...
        replaypanelstring(views[camera[iCurrentCut].byView], 225, 171,
                          iScreenWidth);
      }
    }
    ...
    {                 // Order start and end frames correctly for display
      ...
      replaypanelstring(
          "BLOCK:", 164, 159,
          iScreenWidth); // Display selection block text and time range
      ...
      displaypaneltime(
          currentreplayframe, 223, 159,
          iScreenWidth); // Just show current time when not selecting
    }
  } else {
    sprintf(buffer, "X %s",
            replayname[replaysetspeed]); // NORMAL MODE: Display speed
                                         // multiplier and status icons
    replaypanelstring(buffer, 229, 165, iScreenWidth);
    displaypaneltime(currentreplayframe, 223, 159,
                     iScreenWidth); // Display current replay time
    if (replayspeed > 0 && !forwarding &&
        !rewinding) // Show fast forward icon when playing forward
      replayicon(scrbuf, rev_vga[15], 73, 164, 161, iScreenWidth, 255);
    if (replayspeed < 0 && !forwarding &&
        !rewinding) // Show rewind icon when playing backward
      replayicon(scrbuf, rev_vga[15], 72, 164, 161, iScreenWidth, 255);
    if (forwarding &&
        replayspeed > 0) // Show fast forward icon when actively fast forwarding
      replayicon(scrbuf, rev_vga[15], 71, 164, 161, iScreenWidth, 255);
    if (rewinding &&
        replayspeed < 0) // Show rewind icon when actively rewinding
    ...
    replayicon(scrbuf, rev_vga[15], 75, 190, 161, iScreenWidth,
               255); // Draw progress bar background
    if (currentreplayframe <=
        rotpoint) // Calculate position for progress bar indicators
    {             // Current frame is before rotation point
      if (rotpoint - currentreplayframe > 24) {
        iCalculatedFrame = rotpoint - 24;
        for (iFrameLoop2 = rotpoint - 152; iFrameLoop2 >= currentreplayframe;
             iCalculatedFrame -= 64)
          iFrameLoop2 -= 64;
      } else {
        iCalculatedFrame = currentreplayframe;
      }
    } else if (currentreplayframe - rotpoint >
               24) // Current frame is after rotation point
    {
      iCalculatedFrame = rotpoint + 24;
      for (iFrameLoop = rotpoint + 152; iFrameLoop <= currentreplayframe;
           iCalculatedFrame += 64)
        iFrameLoop += 64;
    } else {
      iCalculatedFrame = currentreplayframe;
    }
    iRotateIndex = iCalculatedFrame /
                   8; // Calculate rotation index for progress bar markers
    // iRotateIndex = (iCalculatedFrame - (__CFSHL__(iCalculatedFrame >> 31, 3)
    // + 8 * (iCalculatedFrame >> 31))) >> 3;// Calculate rotation index for
    ...
    do {
      ...
      ++iRotateLoop;
      replayicon(scrbuf, rev_vga[15], 76, iRotateX, iRotateY, iScreenWidth,
                 255);
...
//-------------------------------------------------------------------------------------------------
// 00068A20
void rselectblock() { // Check if replay editing mode is active
  int iStartFrame;    // eax
  int iEndFrame;      // ebx
  int iFrameIndex;    // edx
  ...
  if (replayedit) { // Check if replay type is 2 (selection mode)
    ...
    if (!replayselect) // If no selection active, start new selection at current
                       // frame
      replaystart = currentreplayframe;
    ...
    if (replaytype == 2) { // Ensure start frame <= end frame for proper range
      ...
      for (iFrameIndex = iStartFrame; iFrameIndex <= iEndFrame;
           ++iFrameIndex) // Clear disabled status for all frames in selected
                          // range
      {
    ...
    if (replaytype == 2) {
      //_disable();                               // Reset replay state after
      // selection operation
      replayspeed = 0;
      fraction = 0;
      replaydirection = 0;
...
//-------------------------------------------------------------------------------------------------
// 00068AF0
void rdeleteblock() { // Check if replay editing mode is active
  int iStartFrame;    // ecx
  int iEndFrame;      // ebx
  int iFrameIndex;    // edx
  int iCurrentFrame;  // eax
  ...
  if (replayedit) { // Check if replay type is 2 (deletion mode)
    ...
    if (replaytype == 2) { // Ensure start frame <= end frame for proper range
      ...
      for (iFrameIndex = iStartFrame; iFrameIndex <= iEndFrame;
           ++iFrameIndex) // Mark all frames in selected range as
                          // disabled/deleted
      {
        iCurrentFrame = iFrameIndex;
...
void rstartassemble() {
  ...
  if (replaytype == 2 && replayedit) // Only allow assembly in edit mode with
                                     // replay editing enabled
  {
    sfxsample(SOUND_SAMPLE_BUTTON, 0x8000); // SOUND_SAMPLE_BUTTON
  ...
  CHECK_RESULTS:
    if (iEnabledFramesFound) // Set appropriate menu state based on search
                             // results
...
void replayicon(uint8 *pDest, tBlockHeader *pBlockHeader, int iBlockIdx, int iX,
  ...
  pDestPixel =
      &pDest[iScaledDestX +
             iScreenWidth * iScaledDestY]; // Calculate destination pixel
                                           // pointer: buffer + (y * width) + x
  iWidth = pBlockHeader[iBlockIdx]
               .iWidth; // Get sprite width from sprite data header (offset 0)
  pSpriteHeader = &pBlockHeader[iBlockIdx];
  iHeight =
      pSpriteHeader
          ->iHeight; // Get sprite height from sprite data header (offset 4)
  pSpritePixels =
      (uint8 *)pBlockHeader +
      pSpriteHeader->iDataOffset; // Get pointer to sprite pixel data (offset 8)
  ...
  if (SVGA_ON) {
    ...
    SVGA_NEXT_ROW:
      ...
      pDestPixel +=
          2 * (iScreenWidth - iWidth); // SVGA_NEXT_ROW: Advance to next sprite
                                       // row and check completion
    ...
    SVGA_NEXT_PIXEL:
      iSvgaCurrentX += 2; // SVGA_NEXT_PIXEL: Advance to next pixel in current
                          // row (2x scaling)
  ...
  for (iRowLoop = 0; iRowLoop < iHeight; ++iRowLoop) {
    ...
    for (iPixelX = 0; iPixelX < iWidth; ++pDestPixel) {
      ...
      // Check bounds and transparency: skip if out of bounds or matches
      // transparent color
      if (iCurrentX >= 0 && iCurrentX < iScreenWidth &&
          (byTransparentColor < 0 || byTransparentColor != byPixelColor))
...
void replaypanelletter(char c, int *piX, int *piY, int iScreenWidth) {
  ...
  if (c == '?')
    iCharBlockIdx = 69;
  if (iCharBlockIdx != -1) {
    replayicon(scrbuf, rev_vga[15], iCharBlockIdx, *piX, *piY, iScreenWidth,
               255);
...
void displaypaneltime(int iTime, int iX, int iY, int iScreenWidth) {
  sprintf(buffer, "%02d:%02d:%02d", iTime / 36 / 60, iTime / 36 % 60,
          iTime % 36);
