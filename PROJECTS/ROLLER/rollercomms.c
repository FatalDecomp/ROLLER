...
int ROLLERCommsEnumLocalAddrs(tROLLERNetIface *pOut, int iMax) {
  ...
  if (getifaddrs(&pList) == 0) {
    for (struct ifaddrs *pIfa = pList; pIfa && iCount < iMax;
         pIfa = pIfa->ifa_next) {
      if (!pIfa->ifa_addr || pIfa->ifa_addr->sa_family != AF_INET)
        continue;
      struct sockaddr_in *pSin = (struct sockaddr_in *)pIfa->ifa_addr;
      if (pSin->sin_addr.s_addr == htonl(INADDR_LOOPBACK))
        continue;
      inet_ntop(AF_INET, &pSin->sin_addr, pOut[iCount].szIP,
                sizeof(pOut[iCount].szIP));
      strncpy(pOut[iCount].szName, pIfa->ifa_name,
              sizeof(pOut[iCount].szName) - 1);
...
static int BuildPacket(uint8_t *pPacketBuffer, int *pTotalSizeOut,
                       const void *pHeader, int iHeaderSize, const void *pData,
                       int iDataSize) {
  int iTotalSize = iHeaderSize + iDataSize;

  if (!pPacketBuffer || !pTotalSizeOut || !pHeader || iHeaderSize < 0 ||
      iDataSize < 0)
    return 0;
...
static int SendPacketToIPv4(uint32_t uiIPAddress, uint16_t unPort,
                            const uint8_t *pPacket, int iPacketSize) {
  ...
  int iBytesSent =
      sendto(g_commsState.listenSocket, (const char *)pPacket, iPacketSize, 0,
...
void ROLLERCommsSetLocalPort(uint16_t unPort) { s_unLocalPort = unPort; }
...
int ROLLERCommsInitSystem(unsigned int uiMaxPackets) {
  ...
  if (setsockopt(g_commsState.listenSocket, SOL_SOCKET, SO_BROADCAST,
                 (const char *)&iBroadcast,
                 sizeof(iBroadcast)) == SOCKET_ERROR) {
    SDL_Log("[NET-DISCOVERY] SO_BROADCAST setup failed; manual peers may still "
            "work");
  }
  ...
  if (bind(g_commsState.listenSocket, (struct sockaddr *)&bindAddr,
...
int ROLLERCommsSendData(const void *pHeader, int iHeaderSize, const void *pData,
  ...
  if (iDestNode < 0 || iDestNode >= ROLLER_MAX_NODES ||
...
int ROLLERCommsBroadcastData(const void *pHeader, int iHeaderSize,
  ...
  int iSuccess = 0;
  if (SendPacketToIPv4(htonl(INADDR_BROADCAST), unPort, packetBuffer,
                       iTotalSize)) {
    SDL_Log("[NET-DISCOVERY] broadcast init to 255.255.255.255:%u",
            (unsigned)unPort);
    iSuccess = 1;
  }

  if (s_unLocalPort != unPort &&
      SendPacketToIPv4(htonl(INADDR_BROADCAST), s_unLocalPort, packetBuffer,
                       iTotalSize)) {
    SDL_Log("[NET-DISCOVERY] broadcast init to 255.255.255.255:%u",
            (unsigned)s_unLocalPort);
    iSuccess = 1;
...
int ROLLERCommsGetHeader(void *pHeaderOut, int iHeaderSize, void **ppDataOut) {
  ...
  int iBytesReceived = recvfrom(
      g_commsState.listenSocket, (char *)g_commsState.receiveBuffer,
...
int ROLLERCommsGetActiveNodes(void) { return g_commsState.iActiveNodes; }

//-------------------------------------------------------------------------------------------------

int ROLLERCommsGetConsoleNode(void) { return g_commsState.iConsoleNode; }
...
int ROLLERCommsAddNode(const void *pAddress) {
  ...
  for (int i = 0; i < g_commsState.iActiveNodes; i++) {
    if (memcmp(&g_commsState.nodes[i].address, &address,
  ...
  memcpy(&g_commsState.nodes[iNodeIdx].address, &address,
         sizeof(tROLLERNetAddr));
...
void ROLLERCommsSortNodes(void) {
  ...
  // Diagnostic: dump the sorted node table so we can verify iConsoleNode is
  // correct on each machine.
  ...
  for (int i = 0; i < g_commsState.iActiveNodes; i++) {
    ...
    inet_ntop(AF_INET, &g_commsState.nodes[i].address.uiIPAddress, szIP,
              sizeof(szIP));
...
void ROLLERCommsSetType(int iType) { g_commsState.iNetType = iType; }

//-------------------------------------------------------------------------------------------------

int ROLLERCommsGetType(void) { return g_commsState.iNetType; }
...
int ROLLERCommsNetAddrToNode(const int *pAddress) {
  ...
  for (int i = 0; i < g_commsState.iActiveNodes; i++) {
    if (memcmp(&g_commsState.nodes[i].address, pAddress,
               sizeof(tROLLERNetAddr)) == 0) {
      return i;
    }
  }
...
void ROLLERCommsGetNodeAddrStr(int iNode, char *szBuf, int iBufLen) {
  if (!szBuf || iBufLen <= 0)
    return;
  if (!g_commsState.bInitialized || iNode < 0 ||
      iNode >= g_commsState.iActiveNodes) {
  ...
  inet_ntop(AF_INET, &addr, szIP, sizeof(szIP));
  snprintf(szBuf, iBufLen, "%s:%u", szIP,
           (unsigned)g_commsState.nodes[iNode].address.unPort);
}
...
int ROLLERModemInit(const char *szInitString, int p1, int p2, int p3) {
  return 1;
}
int ROLLERModemDial(const char *szPhoneNumber, int iToneMode) { return 1; }
int ROLLERModemAnswer(void) { return 1; }
int ROLLERModemCheckResponse(int p1, int p2, int p3, int p4) {
  return 0; /* Ready */
}
