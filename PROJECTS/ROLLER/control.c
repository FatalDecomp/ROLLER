...
float levels[7] = {100.0f, 97.0f, 94.0f, 90.0f,
                   85.0f,  80.0f, -1.0f}; // 000A4290
float mineff[6] = {1.0f,  0.95999998f, 0.92000002f,
                   0.88f, 0.83999997f, 0.80000001f}; // 000A42AC
...
void humancar(int iCarIdx) {
  ...
  iCarIdx_1 = iCarIdx;
  iInput = (int16)copy_multiple[readptr][iCarIdx]
               .data.unInput; // Extract steering input and control flags from
                              // player input data
  iSteeringInput = iInput / 256;
  // iSteeringInput = (unInput - (__CFSHL__(unInput >> 31, 8) + (unInput >> 31
  // << 8))) >> 8;
  ...
  if (byCarDesignIdx < 8) // Enable cheat mode for special car designs (8-12),
                          // disable for normal cars
    cheat_control = 0;
  else
    cheat_control = unFlags & 0x20;
  if (finished_car[iCarIdx_1] ||
      racers - 1 == finishers && Car[iCarIdx_1].byLap < NoOfLaps &&
          competitors > 1) // Force brake mode if car finished or race is over
                           // for this player
    iControlFlags = 2;
  // LOWORD(iControlFlags) = 2;
  // if (racers - 1 == finishers && Car[iCarIdx_1].byLap < NoOfLaps &&
  // (LODWORD(Car[iCarIdx_1].fFinalSpeed) & 0x7FFFFFFF) == 0 && competitors >
  // 1)// Handle race finish condition: play finish sounds and mark player as
  // finished
  if (racers - 1 == finishers && Car[iCarIdx_1].byLap < NoOfLaps &&
      fabs(Car[iCarIdx_1].fFinalSpeed) == 0 &&
      competitors > 1) // Handle race finish condition: play finish sounds and
                       // mark player as finished
  {
    if (player1_car == iCarIdx_1 || player2_car == iCarIdx_1) {
      if ((char)Car[iCarIdx_1].byLives > 0)
        speechsample(SOUND_SAMPLE_RUBBISH, 0x8000, 18,
                     iCarIdx_1); // SOUND_SAMPLE_RUBBISH
      speechsample(SOUND_SAMPLE_RACEOVER, 0x8000, 18,
                   iCarIdx_1); // SOUND_SAMPLE_RACEOVER
    }
    iTrakLen = TRAK_LEN;
  ...
  if (Car[iCarIdx_1].fHealth <=
      0.0) // Disable controls if car health is zero or below
    iControlFlags = 0;
  // LOWORD(unControlFlags) = 0;
  iCarIndex1 = iCarIdx_1;
  iCarIndex2 = iCarIdx_1;
  Car[iCarIdx_1].iSteeringInput =
      iSteeringInput; // Store steering input and handle cheat power activation
  if (!cheat_control)
    goto PROCESS_VEHICLE_CONTROLS;
  switch (byCarDesignIdx) {
  case 8:                                // SUICYCO (explode opponent)
    if (Car[iCarIndex1].byCheatCooldown) // Car type 8: finds nearest car and
                                         // applies massive damage
      goto PROCESS_VEHICLE_CONTROLS;
    ...
    iTargetCarIdx = findcardistance(
        iCarIndex1, 8000.0); // Find target car within range (8000 units)
    iTargetCar1 = iTargetCarIdx;
    if (iTargetCarIdx >= 0 && Car[iTargetCarIdx].nCurrChunk != -1 &&
        Car[iTargetCarIdx].byCarDesignIdx != 13) {
      Car[iTargetCarIdx].byAttacker =
          iCarIdx_1; // Apply damage: 200 damage, stop target car, consume ammo
  ...
  SET_DAMAGE_COOLDOWN:
    iTrakLen = TRAK_LEN;
    Car[iCarIdx_1].byCheatCooldown =
        36; // Set cheat power cooldown (36 frames) after use
    goto PROCESS_VEHICLE_CONTROLS;
  ...
  case 10: // 2X4B523P (flip opponent)
    ...
    if (Car[iCarIndex2].nCurrChunk == -1) {
      ...
      iJumpTargetIdx = findcardistance(
          iCarIndex1,
          16000.0); // Find target car within jump range (16000 units)
      iJumpTarget = iJumpTargetIdx;
      if (iJumpTargetIdx >= 0) {
        iJumpTargetCar = iJumpTargetIdx;
        if (Car[iJumpTargetCar].nCurrChunk != -1 &&
            Car[iJumpTargetCar].byCarDesignIdx != 13) {
          iTargetCarDesign =
              Car[iJumpTargetCar]
                  .byCarDesignIdx; // Apply jump effect: launch target car into
                                   // air with roll and Z-offset
          Car[iJumpTargetCar].nRoll = 0x2000;
          dNewZ = CarBox.hitboxAy[iTargetCarDesign][4].fZ +
                  Car[iJumpTargetCar].pos.fZ;
  ...
  case 11: // TINKLE (swap places with opponent)
    ...
    iTeleportTargetIdx = findcardistance(
        iCarIdx_1, 16000.0); // Find target car for teleport swap (16000 units)
    iTeleportTargetIndex = iTeleportTargetIdx;
    if (iTeleportTargetIdx < 0 ||
        (iTeleportTarget = iTeleportTargetIdx,
         Car[iTeleportTargetIdx].nCurrChunk == -1) ||
        Car[iTeleportTargetIdx].byCarDesignIdx == 13) {
      if (!cheatsampleok(iCarIdx_1))
        goto SET_TELEPORT_COOLDOWN;
    ...
    getworldangles(Car[iTeleportTargetIdx].nActualYaw, nPitch,
                   Car[iTeleportTargetIdx].nRoll, nReferenceChunk, &iAzimuth,
                   &iElevation, &iBank); // Swap car positions and orientations
                                         // using world angle transformations
    getworldangles(Car[iTeleportTarget].nYaw, Car[iTeleportTarget].nPitch,
                   Car[iTeleportTarget].nRoll,
                   Car[iTeleportTarget].nReferenceChunk, &iAzi2, &iElevation,
                   &iBank);
    ...
    fLocalX = posAy->fY * Car[iTeleportTarget].pos.fY +
              posAy->fX * Car[iTeleportTarget].pos.fX +
              posAy->fZ * Car[iTeleportTarget].pos.fZ - posAy[3].fX;
    fLocalY = posAy[1].fY * Car[iTeleportTarget].pos.fY +
              posAy[1].fX * Car[iTeleportTarget].pos.fX +
              posAy[1].fZ * Car[iTeleportTarget].pos.fZ - posAy[3].fY;
    dLocalZ = posAy[2].fY * Car[iTeleportTarget].pos.fY +
              posAy[2].fX * Car[iTeleportTarget].pos.fX +
              posAy[2].fZ * Car[iTeleportTarget].pos.fZ - posAy[3].fZ;
  ...
  PROCESS_VEHICLE_CONTROLS:
    TRAK_LEN = iTrakLen;
    if (race_started) // Process main vehicle controls: gear changes, material
                      // selection, throttle/brake
    {                 // Handle gear shift controls if race has started
      ...
      if (player_type !=
          2) // Handle material selection controls (paint job changes)
      {
    ...
    pCar =
        &Car[iCarIdx_1]; // Apply vehicle physics based on throttle/brake input
    if ((iControlFlags & 2) !=
        0) // Check control flags: bit 2=brake, bit 1=accelerate, else=freewheel
    {
      Decelerate(pCar);
  ...
  case 12: // LOVEBUN (formula car)
    if (!Car[iCarIndex2].byCheatCooldown) {
      ...
      if (byCheatAmmo_1) {
        ...
        if (iCurrentChunk != -1) {
          iColorIndex =
              -12; // Modify track surface in 16-chunk range around car position
          do {
            if ((TrakColour[iCurrentChunk][1] &
                 (SURFACE_FLAG_NO_SPAWN | SURFACE_FLAG_WALL_22)) == 0) {
              iTrackColor1 = TrakColour[iCurrentChunk][1] & SURFACE_MASK_FLAGS ^
                             SURFACE_FLAG_APPLY_TEXTURE;
              TrakColour[iCurrentChunk][1] =
                  (159 - (iColorIndex & 7)) | iTrackColor1;
              localdata[iCurrentChunk].iCenterGrip = 12;
            }
            if ((TrakColour[iCurrentChunk][0] &
                 (SURFACE_FLAG_NO_SPAWN | SURFACE_FLAG_WALL_22)) == 0) {
              iTrackColor2 = TrakColour[iCurrentChunk][0] & SURFACE_MASK_FLAGS ^
                             SURFACE_FLAG_APPLY_TEXTURE;
              TrakColour[iCurrentChunk][0] =
                  iTrackColor2 | (159 - (iColorIndex & 7));
              localdata[iCurrentChunk].iLeftShoulderGrip = 12;
            }
            if ((TrakColour[iCurrentChunk][2] &
                 (SURFACE_FLAG_NO_SPAWN | SURFACE_FLAG_WALL_22)) == 0) {
              iTrackColor3 = TrakColour[iCurrentChunk][1] & SURFACE_MASK_FLAGS ^
                             SURFACE_FLAG_APPLY_TEXTURE;
              TrakColour[iCurrentChunk][2] =
                  (159 - (iColorIndex & 7)) | iTrackColor3;
    ...
    goto PROCESS_VEHICLE_CONTROLS;
  default:
    goto PROCESS_VEHICLE_CONTROLS; // Cheat power switch: handle different
...
void GoUpGear(tCar *pCar) {
  ...
  if (byGearAyMax < pEngineData->iNumGears -
                        1) // Check if car can upshift (not already in top gear)
  {
    fHealthFactor =
        (pCar->fHealth + 34.0f) * 0.01f; // Calculate health factor (0-1) based
                                         // on car damage, capped at 1.0
    iTargetGear = byGearAyMax + 1;
    if (fHealthFactor > 1.0)
      fHealthFactor = 1.0;
    fMinShiftSpeed =
        -1.0; // Initialize minimum shift speed to -1 (no restriction)
    fAdjustedSpeed = (float)(fabs(pCar->fFinalSpeed) /
                             fHealthFactor); // Calculate current speed adjusted
                                             // by health factor
    if (human_control[pCar->iDriverIdx] != 2 &&
        iTargetGear > 0) // For AI drivers, set minimum speed for upshift (50%
                         // of current gear max speed)
      fMinShiftSpeed = pEngineData->pSpds[iTargetGear - 1] * 0.5f;
    if (fAdjustedSpeed >=
        (double)fMinShiftSpeed) // Check if car is fast enough to upshift
    {
      ...
      if (iTargetGear < 0) // Handle reverse gear upshift (gear < 0)
      {
        ...
        fRPMRatio = fAdjustedSpeed /
                    pSpds[iTargetGear]; // Calculate RPM ratio for target gear
        if (fRPMRatio > 1.0)
          fRPMRatio = 1.0;
      }
      pCar->fRPMRatio = fRPMRatio;
      if (player1_car == pCar->iDriverIdx &&
          !DriveView[0]) // Play gear shift sound for human players (not in
                         // cockpit view)
      ...
      if (iTargetGear < 0) // Handle upshift to reverse gear (set gear 0)
      {
        ...
        fNewPowerForward =
            (float)calc_pow(iCarDesignIdx, iTargetGear,
                            fRPMRatio); // Handle normal forward gear upshift
        pCar->byGearAyMax = iTargetGear;
      ...
      pCar->fSpeedOverflow =
          pCar->fFinalSpeed -
          pCar->fBaseSpeed; // Update speed overflow (difference between current
                            // and base speed)
    }
  }
}
...
void GoDownGear(tCar *pCar, int iUseAutoLogic) {
  ...
  fHealthFactor = (pCar->fHealth + 34.0f) *
                  0.01f; // Calculate health factor (0-1) based on car damage
  iCarDesignIdx = pCar->byCarDesignIdx; // Get car design index and engine data
  pEngineData = &CarEngines.engines[iCarDesignIdx];
  if (fHealthFactor > 1.0)
    fHealthFactor = 1.0;
  ...
  if (iGearAyMax >
      -2) // Check if car can downshift (not already in lowest gear -2)
  {
    ...
    fCalculatedPower =
        (float)change_gear(iCurrentGear, iGearForCalc, pCarCopy,
                           iCarDesignIdx); // Calculate power output for target
                                           // gear using change_gear function
    fDownshiftThreshold = 1000.0; // Set downshift threshold: high value for
                                  // human, gear change value * 1.2 for AI
    if (human_control[pCar->iDriverIdx] != 2 && iUseAutoLogic)
      fDownshiftThreshold = pEngineData->pChgs[2 * iTargetGearIdx] * 1.2f;
    if (fCalculatedPower <
        (double)fDownshiftThreshold) // Check if calculated power is below
                                     // downshift threshold
    {
      pCar->fPower = fCalculatedPower;
      pCar->byGearAyMax = iTargetGearIdx;
      if (iTargetGearIdx ==
          -1) // Handle downshift to reverse gear (-1): zero base speed
      {
        ...
        pSpds = pEngineData->pSpds;
        if (iTargetGearIdx >=
            0) // Handle forward gear downshift: calculate RPM ratio and speeds
        {
          fRPMRatio = (float)calc_revs(
              pEngineData->pRevs, iTargetGearIdx,
              fCalculatedPower); // Forward gear: use target gear curve data and
                                 // positive max speed
          pCar->fRPMRatio = fRPMRatio;
          ...
          fRPMRatio = (float)calc_revs(
              pEngineData->pRevs, 0,
              fCalculatedPower); // Reverse gear: use gear 0 curve data,
                                 // negative max speed
          pCar->fRPMRatio = fRPMRatio;
          dMaxSpeed = -*pSpds;
        }
        pCar->fBaseSpeed = (float)dMaxSpeed * fRPMRatio *
                           fHealthFactor; // Calculate new base speed: max speed
                                          // * RPM ratio * health factor
        dBaseSpeed = pCar->fFinalSpeed -
                     pCar->fBaseSpeed; // Update speed overflow (difference
                                       // between final and base speed)
      }
...
void control() {
  ...
  while (readptr != writeptr) // Main game update loop - process each frame
                              // while data available
  {
    ...
    if (network_on && replaytype != 2 &&
        write_check >= 0) // Network synchronization: calculate checksums for
      ...
      if (numcars > 0) {
        ...
        do {
          if (!non_competitors[iCompetitorIdx]) {
            ...
            iChecksum +=
                abs(Car[iCarIdx].nCurrChunk) +
                *(int *)&Car[iCarIdx].pos.fX // Float bit pattern as int
                + *(int *)&Car[iCarIdx].pos.fY + *(int *)&Car[iCarIdx].pos.fZ +
                abs(Car[iCarIdx].byLives) + abs((int)dHealth);
          }
          ++iCompetitorIdx;
          ++iCarCounter;
          ++iCarIdx;
    ...
    // if (numcars > 0)                          // Initialize car state array
    // and copy car data for processing
    //{
    ...
    //     iByteOffset += 4;
    //     iDataValue = *((char *)&CarBox.hitboxAy[15][5].fY + iStructOffset +
    //     2);// references adjacent data Car
    //     *(float *)((char *)&CarBox.hitboxAy[15][7].fX + iStructOffset)
    //     = 1.0;// references adjacent data Car aiCarStates[iByteOffset / 4u] =
    //     iDataValue;
    ...
    if (!paused)
      ++updates;
    iFatalityCounter =
        Fatality_Count; // Handle fatality timer and camera zoom effects
    if (Fatality_Count > 0) {
      --Fatality_Count;
    ...
    if (player_type == 2)
      dozoomstuff(1);
    if (readptr >= 0) { // Process each car: handle race start, player controls,
                        // and network events
      for (iCar = 0; iCar < numcars;
           ++iCar) { // Race start condition: clear high bit from gear and
                     // enable racing
        ...
        if (human_control[iCar]) { // Network sync request: reset car to neutral
                                   // state
          ...
          if ((copy_multiple[readptr][iCar].uiFullData & 0x4000000) !=
              0) // Network player quit: cleanup and handle disconnection
          {
    ...
    if (replaytype != 2) {
      memset(
          newrepsample, 0,
          sizeof(
              newrepsample)); // Process car controls and AI for non-replay mode
      iCarArrayIdx = 0;
      if (numcars > 0) {
        iCarStructIdx = 0;
        iCarLoopIdx = 0;
        do {
          byStatusFlags =
              Car[iCarStructIdx].byStatusFlags; // Check car status and handle
                                                // death/damage states
          Car[iCarStructIdx].byAccelerating = 0;
          if ((byStatusFlags & 4) != 0 || !Car[iCarStructIdx].byLives) {
            // if ((LODWORD(Car[iCarStructIdx].fFinalSpeed) & 0x7FFFFFFF) == 0
            // || Car[iCarStructIdx].nUnk21 < 126) {
            if (fabs(Car[iCarStructIdx].fFinalSpeed) == 0 ||
                Car[iCarStructIdx].nDeathTimer < 126) {
          ...
          if ((Car[iCarStructIdx].byLives & 0x80u) ==
              0) { // Process car controls: human player or AI based on control
                   // type
            if (human_control[iCarLoopIdx]) {
              if (Car[iCarStructIdx].iControlType != 2 &&
                  !Car[iCarStructIdx].iStunned)
                humancar(iCarArrayIdx);
            } else if (Car[iCarStructIdx].iControlType == 3 &&
                       !Car[iCarStructIdx].iStunned) {
      ...
      if (countdown > -72 &&
          replaytype != 2) // Update dynamic lighting effects for local players
      {
        for (i = 0; i < local_players; ++i) {
          uiSLightOffset = 0x90 * i;
          do {
            fLightDeltaX = SLight[0][uiSLightOffset / 0x30].targetPos.fX -
                           SLight[0][uiSLightOffset / 0x30]
                               .currentPos.fX; // Animate lights toward target
                                               // positions with speed limiting
            fLightSpeed = fLightDeltaX;
            SLight[0][uiSLightOffset / 0x30].uiRotation =
                (SLight[0][uiSLightOffset / 0x30].uiRotation + 128) & 0x3FFF;
            ...
            if (fabs(fLightSpeed) > FLT_EPSILON) {
              ...
              if (fLightDeltaX <= 0.0)
                dLightPosX =
                    SLight[0][uiSLightOffset / 0x30].currentPos.fX - fX;
              else
                dLightPosX =
                    SLight[0][uiSLightOffset / 0x30].currentPos.fX + fX;
              SLight[0][uiSLightOffset / 0x30].currentPos.fX =
                  (float)dLightPosX;
            }
            fLightDeltaY = SLight[0][uiSLightOffset / 0x30].targetPos.fY -
                           SLight[0][uiSLightOffset / 0x30].currentPos.fY;
            ...
            if (fabs(fLightDeltaY) > FLT_EPSILON) {
              ...
              if (fLightDeltaY <= 0.0)
                dLightPosY =
                    SLight[0][uiSLightOffset / 0x30].currentPos.fY - fY;
              else
                dLightPosY =
                    SLight[0][uiSLightOffset / 0x30].currentPos.fY + fY;
              SLight[0][uiSLightOffset / 0x30].currentPos.fY =
            ...
            fLightDeltaZ = SLight[0][uiSLightOffset / 0x30].targetPos.fZ -
                           SLight[0][uiSLightOffset / 0x30].currentPos.fZ;
            ...
            if (fabs(fLightDeltaZ) > FLT_EPSILON) {
              ...
              if (fLightDeltaZ <= 0.0)
                dLightPosZ =
                    SLight[0][uiSLightOffset / 0x30].currentPos.fZ - fZ;
              else
                dLightPosZ =
                    SLight[0][uiSLightOffset / 0x30].currentPos.fZ + fZ;
              SLight[0][uiSLightOffset / 0x30].currentPos.fZ =
    ...
    if (replaytype == 1 && fudge_wait < 0)
      DoReplayData();
    if (replaytype != 2 || newreplayframe) // Calculate car motion effects based
                                           // on health and speed
    {
      iMotionCarIdx = 0;
      if (numcars > 0) {
        pCurrentCar = Car;
        do {
          fHealthFactor = (pCurrentCar->fHealth + 34.0f) *
                          0.01f; // Apply random motion effects: more damage =
          ...
          dRollMotionCalc =
              (double)iRandomValue * fHealthValue /
              (double)CarEngines.engines[pCurrentCar->byCarDesignIdx]
                  .iStabilityFactor;
          //_CHP();
          ...
          dYawMotionCalc =
              ...
              (double)CarEngines.engines[pCurrentCar->byCarDesignIdx]
                  .iStabilityFactor;
          //*(float *)&iRandomValue = fDamageMultiplier *
          // pCurrentCar->fFinalSpeed; LODWORD(fHealthValue) = ROLLERrandRaw() -
          // 0x4000; dYawMotionCalc = (double)SLODWORD(fHealthValue) * *(float
          // *)&iRandomValue /
          // (double)CarEngines.engines[pCurrentCar->byCarDesignIdx].iStabilityFactor;
          //_CHP();
          ++pCurrentCar;
          ++iMotionCarIdx;
    ...
    if (replaytype == 2 &&
        replayspeed == 256) // Handle replay sound effects synchronization
    {
      for (j = 0; j < numcars; ++j) {
    ...
    analysespeechsamples(); // Finalize frame: analyze speech, order cars,
                            // handle audio
    ordercars();
    if (replaytype != 2) {
      if (paused) {
    ...
    if (!intro && human_finishers >= players &&
        (disable_messages ||
         network_on)) // Check race completion conditions and network state
    {
      ...
      if (network_on) {
        if (wConsoleNode == master) {
          send_finished =
              -1; // Network master: check if any human players still racing
          if (numcars > 0) {
            iMasterCarIdx = 0;
            iMasterByteIdx = 0;
            do {
              if ((Car[iMasterCarIdx].byLives & 0x80u) == 0 &&
                  Car[iMasterCarIdx].fFinalSpeed > 0.0 &&
                  human_control[iMasterByteIdx / 4u])
                send_finished = 0;
              iMasterByteIdx += 4;
              ++iMasterCarIdx;
          ...
          if (numcars > 0) {
            iSlaveCarIdx = 0;
            do {
              if ((Car[iSlaveCarIdx].byLives & 0x80u) == 0 &&
                  Car[iSlaveCarIdx].fFinalSpeed > 0.0 &&
                  human_control[iSlaveByteIdx / 4u])
                racing = -1;
              iSlaveByteIdx += 4;
              ++iSlaveCarIdx;
                 ...
                 -72) // Single player: check if any human players still racing
      {
        racing = network_on;
        if (numcars > 0) {
          iRacingByteIdx = 0;
          do {
            if (*((char *)&Car[0].byLives + iCarOffset) >= 0 &&
                *(float *)((char *)&Car[0].fFinalSpeed + iCarOffset) > 0.0 &&
                human_control[iRacingByteIdx / 4u])
              racing = -1;
            iRacingByteIdx += 4;
    ...
    if (++nearcarcheck ==
        4) // Update near car checking counter and advance frame pointer
      nearcarcheck = 0;
    if (replaytype == 2)
      readptr = writeptr;
    else
  ...
  if (replaytype != 2 && numcars > 0) // Final processing: handle special car
                                      // state changes for active cars
  {
    iFinalArraySize = 4 * numcars;
    iFinalCarIdx = 0;
    iFinalByteIdx = 0;
    while (1) {
      if (Car[iFinalCarIdx].fFinalSpeed <= 36.0 ||
          Car[iFinalCarIdx].fFinalSpeed >= 100.0)
        goto LABEL_185; // Toggle special car state (byUnk68) based on speed and
                        // current state
...
double calc_revs(tRevCurve *pRevs, int iGear, float fChg) {
  ...
  pCurrentGearCurve =
      &pRevs[iGear]; // Get pointer to power/RPM curve for specified gear
  ...
  dDelta0 =
      fChg - fPower0; // Calculate delta values for quadratic interpolation
  ...
  if (fChg < 0.0 || fChg >= (double)fPower1) {
    if (fChg >= (double)fPower3) {
      ...
      dHighInterp1 = (fPower1 - fPower2) *
                     (fPower1 - fPower3); // Mid RPM range: quadratic
                                          // interpolation between points 1-3
      dHighInterp2 = (fPower2 - fPower1) * (fPower2 - fPower3);
      dHighInterp3 = (fPower3 - fPower1) * (fPower3 - fPower2);
      fCalculatedResult =
          (float)(dDelta2 * dDelta3 * fRPM1 / dHighInterp1 +
                  dDelta1 * dDelta3 * fRPM2 / dHighInterp2 +
                  dDelta1 * dDelta2 * fRPM3 /
                      dHighInterp3); // Complete quadratic interpolation using
                                     // all three points
    }
  } else {
    dLowInterp1 =
        (fPower0 - fPower1) *
        fPower0; // Low RPM range: quadratic interpolation between points 0-1
    dLowInterp2 = (fPower1 - fPower0) * fPower1;
    fCalculatedResult =
        (float)(dDelta1 * fChg * fRPM0 / dLowInterp1 +
                dDelta0 * fChg * fRPM1 /
                    dLowInterp2); // Calculate interpolated value using
                                  // quadratic formula for low range
  }
...
double calc_pow(int iCarDesignIdx, int iCurrentGear, float fRPMRatio) {
  ...
  pfRevData =
      CarEngines.engines[iCarDesignIdx]
          .pRevs; // Get pointer to power/RPM curve data for this car and gear
  fPower0 = pfRevData[iCurrentGear]
                .points[0]
                .fPower; // Load 4 power values and 4 RPM points for power curve
                         // interpolation
  ...
  if (fRPMRatio < 0.0 || fRPMRatio >= (double)fRPM1) {
    if (fRPMRatio >= (double)fRPM3) {
      ...
      dHighInterp3 = (fPower3 - fPower1) * (fPower3 - fPower2);
      dHighCoeffA =
          fRPM1 / dHighInterp1 + fRPM2 / dHighInterp2 + fRPM3 / dHighInterp3;
      dHighCoeffB = -(fPower2 + fPower3) * fRPM1 / dHighInterp1 -
                    (fPower1 + fPower3) * fRPM2 / dHighInterp2 -
                    (fPower1 + fPower2) * fRPM3 / dHighInterp3;
      fCalculatedPower =
          (float)((sqrt(dHighCoeffB * dHighCoeffB -
                        (fPower2 * fPower3 * fRPM1 / dHighInterp1 +
                         fPower1 * fPower3 * fRPM2 / dHighInterp2 +
                         fPower1 * fPower2 * fRPM3 / dHighInterp3 - fRPMRatio) *
                            dHighCoeffA * 4.0) -
                   dHighCoeffB) /
                  (dHighCoeffA * 2.0)); // Solve complex quadratic equation for
                                        // mid-range power
    }
  } else {
    dLowInterp1 =
    ...
    fCalculatedPower =
        (float)((sqrt(dLowCoeffB * dLowCoeffB - -fRPMRatio * dLowCoeffA * 4.0) -
                 dLowCoeffB) /
                (dLowCoeffA *
                 2.0)); // Solve quadratic equation using quadratic formula
  }
...
//-------------------------------------------------------------------------------------------------
// 0002B260
void Accelerate(tCar *pCar) { // Set car complexity factor: 2.0 for advanced
                              // cars, 1.0 for simple cars
  ...
  pCar->byAccelerating =
      -1; // Initialize acceleration state and gear change flag
  iGearChanged = 0;
  if (pCar->byThrottlePressed &&
      !pCar->byEngineStartTimer) // Check if car can accelerate (throttle
                                 // pressed and not crashed)
  {
    fHealthFactor =
        (pCar->fHealth + 34.0f) * 0.01f; // Calculate health factor (0-1) based
                                         // on car damage, capped at 1.0
    ...
    iGearAyMax = (char)pCar->byGearAyMax;
    if (pCar->fRPMRatio >=
        1.0) // Check for redline condition (RPM ratio >= 1.0)
    {
      pCar->fWheelSpinFactor = 1.0; // Redline: limit acceleration and apply
                                    // engine damage for over-revving
      if (pEngineData->iNumGears - 1 > iGearAyMax && iGearAyMax >= 0 ||
          iGearAyMax == -2 && pCar->fFinalSpeed > 0.0)
        dodamage(pCar, 0.02f);
    } else {
      nCurrChunk = pCar->nCurrChunk;
      if (nCurrChunk ==
          -1) // Off-track: apply high acceleration bonus and reduced wheel spin
      {
        ...
        iNextChunk =
            nCurrChunk +
            1; // On-track: calculate track physics effects (banking, gravity)
        if (iNextChunk == TRAK_LEN)
          iNextChunk ^= TRAK_LEN;
        iChunkIdx = pCar->nCurrChunk;
        pTrackData = &localdata[iChunkIdx];
        fTrackHalfWidth =
            ((pCar->pos.fX + pTrackData->fTrackHalfLength) *
                 localdata[iNextChunk].fTrackHalfWidth -
             (pCar->pos.fX - pTrackData->fTrackHalfLength) *
                 localdata[iChunkIdx].fTrackHalfWidth) /
            (pTrackData->fTrackHalfLength *
             2.0f); // Calculate interpolated track half-width at car position
        pTrackInfo = &TrackInfo[iChunkIdx];
        if (pCar->pos.fY <=
            (double)fTrackHalfWidth) // Select banking angle based on car
                                     // position (left/right/center of track)
        {
          if (-fTrackHalfWidth <= pCar->pos.fY)
            fAdjustedGravity = pTrackData->gravity.fY;
          else
            fAdjustedGravity = pTrackData->gravity.fY *
                                   tcos[pTrackInfo->iRightBankAngle & 0x3FFF] -
                               pTrackData->gravity.fZ *
                                   tsin[pTrackInfo->iRightBankAngle & 0x3FFF];
        } else {
          fAdjustedGravity = pTrackData->gravity.fY *
                                 tcos[pTrackInfo->iLeftBankAngle & 0x3FFF] +
                             pTrackData->gravity.fZ *
                                 tsin[pTrackInfo->iLeftBankAngle & 0x3FFF];
        }
        fGravityFactor = (pTrackData->gravity.fX * tcos[pCar->nYaw] +
                          fAdjustedGravity * tsin[pCar->nYaw]) *
                         2.0f; // Calculate gravity factor based on track
                               // inclination and car orientation
        ...
        if (player1_car == iDriverIdx ||
            player2_car == iDriverIdx) // Apply different wheel spin factors for
                                       // human vs AI players
          pCar->fWheelSpinFactor = (fGravityFactor + 1.0f) * 6.0f;
        else
          pCar->fWheelSpinFactor = 1.0;
        if (cheat_control &&
            iCarDesignIdx == 9) // Special acceleration bonuses: cheat mode,
                                // neutral gear, or race position
        {
          fPower = fPower + 32.0f;
        } else if (iGearAyMax == -1) {
          fPower = fCarComplexityFactor * 2.0f + fPower;
        } else if (pCar->fTotalRaceTime >= 20.0f ||
                   human_control[pCar->iDriverIdx] ||
                   g_bAINoCheatStart) { // Last check added by ROLLER
          fPower =
              (fHealthFactor + fCarComplexityFactor + fGravityFactor) * 0.5f +
              fPower;
        } else {
          fPower = (fHealthFactor + fCarComplexityFactor + fGravityFactor) /
                       (float)(pCar->byRacePosition / 15 + 1) +
                   fPower;
        }
      }
    }
    ...
    if (fabs(pCar->fFinalSpeed / fHealthFactor) < fDownshiftThreshold &&
        human_control[pCar->iDriverIdx] != 2 &&
        iGearAyMax >
            0) // Auto downshift: check if speed is too low for current gear
    {
      dDownshiftPower =
          change_gear(iGearAyMax, iGearAyMax - 1, pCar, iCarDesignIdx);
      --iGearAyMax;
      iGearChanged = -1;
    ...
    if (pEngineData->iNumGears - 1 > iGearAyMax && iGearAyMax >= 0 &&
        ...
        pCar->fRPMRatio >=
            (double)eng_chg_revs[12 * iCarDesignIdx +
                                 2 * iGearAyMax]) // Auto upshift: check power,
                                                  // RPM, and speed thresholds
    {
      fUpshiftSpeed =
          pCar->fRPMRatio * pSpds[iGearAyMax] * fHealthFactor * 0.5f;
    ...
    if (pCar->nCurrChunk != -1 && iCarDesignIdx != 9 &&
        iGearAyMax !=
            -1) // Limit RPM ratio changes to realistic values when on track
    {
    ...
    if (iGearAyMax < 0 &&
        !race_started) // Reset speed overflow in reverse before race starts
    ...
    if ((char)pCar->byGearAyMax !=
        -1) // Update car pitch dynamics based on acceleration
    {
      if (iCarDesignIdx ==
          9) { // Special pitch handling for cheat mode (car design 9)
        if (cheat_control && pCar->fRPMRatio < 1.0) {
          iPitchDelta =
              32 * pEngineData->iPitchAccelRate + pCar->iPitchDynamicOffset;
          ...
          if (iPitchDynamicOffset > iMaxPitchOffset) {
            ...
            if ((double)(48 * pEngineData->iPitchAccelRate) <
                fPitchDecayAmount) // Handle extreme pitch: reset camera offsets
                                   // and apply damage
            {
        ...
        iPitchDeltaNormal =
            pEngineData->iPitchAccelRate +
            pCar->iPitchDynamicOffset; // Normal cars: increase pitch with
                                       // acceleration, clamp to max
...
void Decelerate(tCar *pCar) {
  ...
  fHealthFactor =
      (pCar->fHealth + 34.0f) *
      0.01f; // Calculate health factor (0-1) based on car damage, capped at 1.0
  iCarDesignIdx = pCar->byCarDesignIdx;
  pEngineData = &CarEngines.engines[iCarDesignIdx];
  ...
  pSpds = pEngineData->pSpds;
  if (byGearAyMax <
      0) // Handle reverse gear: calculate base speed using RPM ratio
    fBaseSpeed = pCar->fRPMRatio * *pEngineData->pSpds * fHealthFactor;
  iCurrChunk = pCar->nCurrChunk;
  fDecelerationRate =
      pSpds[pEngineData->iNumGears - 1] /
      (pEngineData->fDragCoefficient *
       36.0f); // Calculate base deceleration rate from top gear max speed
  if (iCurrChunk == -1) // Off-track: apply 3x deceleration penalty
  {
    dInitialDecelerationCalc = fBaseSpeed - fDecelerationRate * 3.0f;
  } else if (human_control[pCar->iDriverIdx]) // Human players: apply track
                                              // banking and gravity effects
  {
    ...
    fTrackHalfWidth =
        ((pCar->pos.fX + pTrackData->fTrackHalfLength) *
             localdata[iNextChunkIdx].fTrackHalfWidth -
         (pCar->pos.fX - pTrackData->fTrackHalfLength) *
             localdata[pCar->nCurrChunk].fTrackHalfWidth) /
        (pTrackData->fTrackHalfLength *
         2.0f); // Calculate interpolated track half-width at car position
    pTrackInfo = &TrackInfo[pCar->nCurrChunk];
    if (pCar->pos.fY <=
        (double)fTrackHalfWidth) // Select banking angle based on car position
                                 // (left/right/center of track)
    {
      if (-fTrackHalfWidth <= pCar->pos.fY)
        fAdjustedGravity = pTrackData->gravity.fY;
      else
        fAdjustedGravity =
            pTrackData->gravity.fY *
                tcos[pTrackInfo->iRightBankAngle & 0x3FFF] -
            pTrackData->gravity.fZ * tsin[pTrackInfo->iRightBankAngle & 0x3FFF];
    } else {
      fAdjustedGravity =
          pTrackData->gravity.fY * tcos[pTrackInfo->iLeftBankAngle & 0x3FFF] +
          pTrackData->gravity.fZ * tsin[pTrackInfo->iLeftBankAngle & 0x3FFF];
    }
    dGravityFactor = (pTrackData->gravity.fX * tcos[pCar->nYaw] +
                      fAdjustedGravity * tsin[pCar->nYaw]) *
                         -0.3333333333333333 +
                     1.0; // Calculate gravity factor based on track inclination
                          // and car orientation
    fGravityMultiplier = (float)dGravityFactor;
    if (dGravityFactor < 0.5)
      fGravityMultiplier = 0.5f;
    dInitialDecelerationCalc =
        fBaseSpeed - fDecelerationRate * fGravityMultiplier;
  } else {
    dInitialDecelerationCalc =
        fBaseSpeed -
        fDecelerationRate; // AI players: apply standard deceleration rate
  }
  ...
  if (byGearAyMax > 0 && human_control[pCar->iDriverIdx] != 2 &&
      (double)pEngineData->pChgs[2 * byGearAyMax - 1] >
          fCurrentPower) // Auto downshift: check if power is below gear change
                         // threshold
  {
    fRPMRatio = fNewBaseSpeed / (pSpds[--byGearAyMax] * fHealthFactor);
    if (fRPMRatio > 1.0)
  ...
  pCar->fRPMRatio = pCar->fRPMRatio + fRPMRatioChange;
  if (byGearAyMax == -1) // Handle special cases: neutral gear (0) and reverse
                         // gear (negate speed)
  {
    fNewBaseSpeed = 0.0;
  ...
  fNewSpeedOverflow = pCar->fSpeedOverflow -
                      fDecelerationRate; // Update speed overflow, reducing it
  ...
  if (byGearAyMax < 0)
    pCar->fSpeedOverflow = 0.0;
  if (pCar->fFinalSpeed <=
      0.0) // Update car pitch dynamics based on movement direction
  {
    if (pCar->iPitchDynamicOffset >= 0) {
      iPitchDeltaPos =
          pCar->iPitchDynamicOffset - pEngineData->iNoseUpRecoveryRate;
      pCar->iPitchDynamicOffset = iPitchDeltaPos;
      if (iPitchDeltaPos >= 0)
        return;
    } else {
      iPitchDeltaNeg =
          pEngineData->iNoseDownRecoveryRate + pCar->iPitchDynamicOffset;
...
void FreeWheel(tCar *pCar) {
  ...
  if (iGearAyMax <
    ...
    fBaseSpeed = pCar->fRPMRatio * *pEngineData->pSpds * fHealthFactor;
  iCurrChunk = pCar->nCurrChunk;
  fDecelerationRate = pSpds[pEngineData->iNumGears - 1] /
                      (pEngineData->fDragCoefficient *
                       256.0f); // Calculate base deceleration rate from top
                                // gear max speed and drag coefficient
  if (iCurrChunk == -1)         // Off-track: apply 16x deceleration penalty
  {
                                 ...
                                 // (left/right/center of track)
    {
      if (-fTrackHalfWidth <= pCar->pos.fY)
        fAdjustedGravity =
            pTrackData->gravity
                .fY; // fY = adjusted gravity Y component based on track banking
      else
        fAdjustedGravity =
            pTrackData->gravity.fY *
                tcos[pTrackInfo->iRightBankAngle & 0x3FFF] -
    ...
    dGravityFactor = (pTrackData->gravity.fX * tcos[pCar->nYaw] +
                      fAdjustedGravity * tsin[pCar->nYaw]) *
                         -0.3333333333333333f +
                     1.0f; // Calculate gravity factor based on track
                           // inclination and car orientation
    fGravityMultiplier = (float)dGravityFactor;
    if (dGravityFactor < 0.5)
      fGravityMultiplier = 0.5f;
    fAdjustedDecelerationRate = fDecelerationRate * fGravityMultiplier;
    if (iGearAyMax == -1) // Apply different deceleration rates: 6x for neutral
                          // gear, 1x for forward gears
  ...
  fCurrentPower = (float)dCalculatedPower;
  if (iGearAyMax > 0 && human_control[pCar->iDriverIdx] != 2 &&
      (double)pEngineData->pChgs[2 * iGearAyMax - 1] >
          fCurrentPower) // Auto downshift: check if power is below gear change
                         // threshold
  {
    fRPMRatio = fNewBaseSpeed / (pSpds[--iGearAyMax] * fHealthFactor);
  ...
  pCar->fRPMRatio = pCar->fRPMRatio + fRPMRatioChange;
  if (iGearAyMax == -1) // Handle special cases: neutral gear (0) and reverse
                        // gear (negate speed)
  {
    fNewBaseSpeed = 0.0;
  ...
  iPitchDynamicOffset =
      pCar->iPitchDynamicOffset; // Update car pitch dynamics: gradual recovery
                                 // to neutral position
  pCar->fBaseSpeed = fNewBaseSpeed;
...
void SetEngine(tCar *pCar, float fThrottle) {
  ...
  fHealthFactor =
      (pCar->fHealth + 34.0f) *
      0.01f; // Calculate health factor (0-1) based on car health, capped at 1.0
  if (fHealthFactor > 1.0)
    fHealthFactor = 1.0;
  ...
  pEngineData =
      &CarEngines
           .engines[byCarDesignIdx]; // Get engine data for this car design
  pSpds = pEngineData->pSpds;
  ...
  if (fThrottle >= 0.5) // Forward throttle - calculate gear and power
  {
    ...
    fLoopExitFlag = 0.0;
    fAdjustedThrottle =
        fThrottle / fHealthFactor; // Adjust throttle by health factor
    do {
      ...
      fLoopExitFlag = -1.0;
    } while (fabs(fLoopExitFlag) ==
             0); // Find appropriate gear for current throttle
    //} while ((LODWORD(fLoopExitFlag) & 0x7FFFFFFF) == 0);// Find appropriate
    // gear for current throttle
    if (iCurrentGear < iNumGears) {
    ...
    fCurrentPower = (float)calc_pow(
        byCarDesignIdx, iCurrentGear,
        fSpeedRatio); // Calculate power for current gear/throttle
    while (iCurrentGear <
           iMaxGearIdx) // Check if car can upshift to higher gears
    {
      ...
      if ((double)pEngineData->pChgs[iGearTableIdx] * 1.2 >= fCurrentPower) {
        ...
        ++pfCurrentGearSpd;
        fCurrentPower =
            (float)calc_pow(byCarDesignIdx, iCurrentGear, fSpeedRatio);
      }
    }
    pCar->fPower = fCurrentPower;
  ...
  } else if (fabs(fThrottle) >=
             0.5) // Reverse throttle - calculate reverse power and speed
  {
    pCar->byGearAyMax = -2;
  ...
  pCar->fFinalSpeed =
      pCar->fBaseSpeed +
      pCar->fSpeedOverflow; // Final speed = base speed + speed overflow
}
...
double change_gear(int iCurrentGear, int iNextGear, tCar *pCar,
  ...
  if (fHealthFactor > 1.0)
    fHealthFactor = 1.0;
  fAdjustedSpeed = (float)fabs(pCar->fFinalSpeed) /
                   fHealthFactor; // Get current speed adjusted by health factor
                                  // (damaged cars perform worse)
  if (iNextGear < 0) // Clamp negative gear to 0 for power curve lookup (reverse
                     // uses gear 0 data)
    iNextGear = 0;
  fSpeedRatio = fAdjustedSpeed /
                CarEngines.engines[iCarDesignIdx]
                    .pSpds[iNextGear]; // Calculate speed ratio: current speed
                                       // vs max speed for target gear
  if (fSpeedRatio >
      1.0) // Clamp speed ratio to 1.0 (can't exceed gear's max RPM)
    fSpeedRatio = 1.0;
  fCalculatedPower =
      (float)calc_pow(iCarDesignIdx, iNextGear,
                      fSpeedRatio); // Calculate power output for target gear at
                                    // current speed ratio
  if (player1_car == pCar->iDriverIdx &&
      !DriveView[0]) // Play gear shift sound for human players (not in cockpit
                     // view)
    sfxsample(SOUND_SAMPLE_GRSHIFT, 12000); // SOUND_SAMPLE_GRSHIFT
...
void updatecar2(tCar *pCar) {
  ...
  pCarEngine = &CarEngines.engines[byCarTypeIndex]; // Get car engine data based
                                                    // on car design index
  ...
  if (numcars > 0) // Loop through all cars to find race leader
  {
    ...
    while (1) {
      ...
      if (iLastValidChunk < 3 || TRAK_LEN - 4 < iLastValidChunk)
        break;
      if (Car[iCarArrayIdx].iLastValidChunk +
              TRAK_LEN * (char)Car[iCarArrayIdx].byLapNumber >
          iLeaderProgress) // Calculate total progress: chunk + lap *
                           // track_length
      {
        iLeaderCarIdx = iCarLoopIdx;
        iLeaderProgress = Car[iCarArrayIdx].iLastValidChunk +
                          TRAK_LEN * (char)Car[iCarArrayIdx].byLapNumber;
      }
      ++iCarLoopIdx;
      ++iCarArrayIdx;
...
LABEL_10:
  if (iLeaderCarIdx != pCar->iDriverIdx &&
      Car[iLeaderCarIdx].iControlType == 3 &&
      (char)Car[iLeaderCarIdx].byLives > 0 && iLeaderValidationFlag) {
    iLeaderChunk = Car[iLeaderCarIdx].nCurrChunk == -1
                       ? Car[iLeaderCarIdx].iLastValidChunk
                       : Car[iLeaderCarIdx].nCurrChunk;
    iLeaderProgressTotal =
        TRAK_LEN * (char)Car[iLeaderCarIdx].byLapNumber + iLeaderChunk;
    if (lastsample < -18) {
      iPlayerProgress =
          TRAK_LEN + pCar->nCurrChunk + TRAK_LEN * (char)pCar->byLapNumber;
      if (pCar->byLappedStatus) {
        ...
        if (iPlayerCarCheck == iDriverIdx)
          speechsample(SOUND_SAMPLE_UNLAPPED, 0x8000, 18,
                       iPlayerCarCheck); // Play "unlapped" sound sample (52)
        ...
        if (iPlayer1Car == iCurrentDriverIdx)
          speechsample(SOUND_SAMPLE_LAPPED, 0x8000, 18,
                       iPlayer1Car); // Play "lapped" sound sample (51)
        iPlayer2Car = player2_car;
        if (player2_car != pCar->iDriverIdx)
          goto LABEL_30;
...
LABEL_30:
  ...
  if (player1_car == iThisDriverIdx || player2_car == iThisDriverIdx) {
    iPitLaneFlag = 0;
    if ((char)pCar->byLapNumber == NoOfLaps &&
        TRAK_LEN - pCar->nCurrChunk < 200)
      iPitLaneFlag = -1;
    if (death_race)
      iPitLaneFlag = -1;
    if (pCar->fHealth < 50.0 && !iPitLaneFlag) {
      ...
      if (iCurrentChunk != -1) {
        ...
        if ((TrakColour[pCar->nCurrChunk][1] & SURFACE_FLAG_PIT_ZONE) != 0 &&
            (TrakColour[iPrevChunk][1] & SURFACE_FLAG_PIT_ZONE) == 0)
          speechonly(SOUND_SAMPLE_PITIN, 0x8000, 18,
                     pCar->iDriverIdx); // Play "pit in" sound sample (46) when
                                        // entering pit lane
      }
    }
  }
LABEL_45:
  byEngineStartTimer =
      pCar->byEngineStartTimer; // Update various car timers - collision, cheat
                                // cooldown, etc.
  ...
  // if (pCar->byThrottlePressed && !pCar->byEngineStartTimer &&
  // (LODWORD(pCar->fRPMRatio) & 0x7FFFFFFF) == 0 && !pCar->byAccelerating)
  if (pCar->byThrottlePressed && !pCar->byEngineStartTimer &&
      fabs(pCar->fRPMRatio) == 0 && !pCar->byAccelerating)
    pCar->byThrottlePressed = 0;
  if (!pCar->byThrottlePressed && !pCar->byEngineStartTimer &&
      pCar->byAccelerating &&
      pCar->iControlType ==
          3) // False start detection and engine start logic for AI cars
  {
    fHorizontalSpeed = pCar->fHealth * 9.0f * 0.0099999998f;
    iRandomValue = ROLLERrandRaw();
    int iTemp =
        GetHighOrderRand(20, iRandomValue); // (20 * iRandomValue) >> 15;
    // LODWORD(fCameraOscillationRange) = (20 * iRandomValue) >> 15;
    if (fHorizontalSpeed + 10.0 <= (double)iTemp && false_starts) {
      if (player1_car == pCar->iDriverIdx)
        sfxsample(
            29,
            0x8000); // SOUND_SAMPLE_FALSE - play false start sound for player 1
      if (player2_car == pCar->iDriverIdx)
        sfxsample(29, 0x8000); // SOUND_SAMPLE_FALSE
      pCar->byEngineStartTimer = 72;
    } else {
      if (player1_car == pCar->iDriverIdx)
        sfxsample(28, 0x8000); // SOUND_SAMPLE_ESTART - play engine start sound
                               // for player 1
      if (player2_car == pCar->iDriverIdx)
  ...
  if (pCar->fRPMRatio < 1.0) {
    ...
    dWheelSpinAccumulator =
        pCar->fWheelSpinFactor + pCar->fWheelSpinAccumulation;
  ...
  // iOscillationValue = ((iRandomValue2) >> 13) + 512 +
  // pCar->iEngineVibrateOffset;
  iOscillationValue =
      GetHighOrderRand(4, iRandomValue2) + 512 + pCar->iEngineVibrateOffset;
  ...
  if (pCar->fFinalSpeed >= 200.0 && nCurrentChunk >= 0) {
    ...
    if (iCurrentYaw3 > 4096 && iCurrentYaw3 < 12288 &&
        !pCar->nReverseWarnCooldown) {
      iSpeechDriverIdx = pCar->iDriverIdx;
      if (player1_car == iSpeechDriverIdx || player2_car == iSpeechDriverIdx)
        speechsample(
            26, 0x8000, 18,
            pCar->iDriverIdx); // SOUND_SAMPLE_REVERSE - warn player about
  ...
  if ((player1_car == iAudioDriverIdx || player2_car == iAudioDriverIdx) &&
      (nCurrentChunk >= 0 && pCar->nActualYaw < 4096 ||
       pCar->nActualYaw > 12288)) {
    // nCurrentChunk check and iAudioSample init added by ROLLER
    iAudioSample = 0;
    if (nCurrentChunk >= 0 &&
        (double)samplespeed[nCurrentChunk] <= pCar->fFinalSpeed) {
  ...
  fCameraOscillationRange =
      CarEngines.engines[byCarTypeIndex].fOscillationMax -
      CarEngines.engines[byCarTypeIndex]
          .fOscillationMin; // Calculate camera pitch and roll offsets based on
                            // car engine oscillation
  // fHorizontalSpeed = COERCE_FLOAT(abs(pCar->iPitchBackup));
  // dOscillationFactor = (double)SLODWORD(fHorizontalSpeed) *
  // fCameraOscillationRange * 0.00024414062 +
  // CarEngines.engines[byCarTypeIndex].fOscillationMin;
  dOscillationFactor = (double)abs(pCar->iPitchBackup) *
                           fCameraOscillationRange * 0.00024414062 +
                       CarEngines.engines[byCarTypeIndex].fOscillationMin;
  fTempFloat1 =
      tcos[((uint16)CarEngines.engines[byCarTypeIndex].iOscillationFreq *
            (uint16)pCar->iCameraOscillationPhase) &
           0x3FFF];
  ...
  if (iChunkCheck != -1 &&
      pCar->byCarDesignIdx != 9) // Speed change rate limiting - prevent
  ...
  if ((cheat_mode & CHEAT_MODE_TINY_CARS) !=
      0) // CHEAT_MODE_TINY_CARS - modified steering response
  {
    if (iSteeringInput < 0) {
      iRollDynamicTemp =
          pCar->iRollDynamicOffset - 8 * pCarEngine->iRollResponseRate;
...
LABEL_171:
  ...
  if (ViewType[1] == pCar->iDriverIdx)
    doviewtend(pCar, 1, 1);
  iControlTypeLocal =
      pCar->iControlType; // Branch on car control type: 0=Free Movement, 1=?,
                          // 2=Ground, 3=AI/Player
  if (pCar->nCurrChunk >= 0 &&
      pCar->nCurrChunk < TRAK_LEN) // check added by ROLLER
  ...
  if (iControlTypeLocal < 2) {
    if (!iControlTypeLocal) {
      pCar->direction.fZ =
          pCar->direction.fZ + -3.0f; // Free movement physics - direct position
      ...
      // if ((LODWORD(fHorizontalSpeedLocal) & 0x7FFFFFFF) != 0 ||
      // (LODWORD(fCarPosZ) & 0x7FFFFFFF) != 0) {
      if (fabs(fHorizontalSpeedLocal) > FLT_EPSILON ||
          fabs(fCarPosZ) > FLT_EPSILON) {
        dAtanResult =
            atan2(fDirectionZ, fHorizontalSpeedCopy) * 16384.0 / 6.28318530718;
        //_CHP();
      ...
      landontrack(pCar);
      // if ((LODWORD(pCar->fHealth) & 0x7FFFFFFF) == 0 && pCar->nDeathTimer <
      // -90)
  ...
  if (iControlTypeLocal > 2 && pCar->nCurrChunk >= 0 &&
      pCar->nCurrChunk < TRAK_LEN) { // curr chunk check added by ROLLER
    if (iControlTypeLocal != 3)
      goto LABEL_502;
    if ((TrakColour[pCar->nCurrChunk][1] & SURFACE_FLAG_PIT_ZONE) == 0)
      pCar->byPitLaneActiveFlag = 0;
    iChunkCurrent = pCar->nCurrChunk;
    iChunkNext = iChunkCurrent + 1; // Track surface calculations - determine
    ...
    fTrackCenterLine = (float)((dPositionCalc * fLeftShoulderTrackWidth -
                                dTrackPosCalc * fCurrentTrackHalfWidth) *
                               dInterpolationFactor);
    ...
    fLeftShoulderBoundary = (float)((dPositionCalc * fLeftShoulderTrackWidth -
                                     dTrackPosCalc * fCurrentTrackHalfWidth) *
                                    dInterpolationFactor);
    ...
    fRightShoulderBoundary = (float)((dPositionCalc * fLeftShoulderTrackWidth -
                                      dTrackPosCalc * fCurrentTrackHalfWidth) *
                                     dInterpolationFactor);
    switch (iSurfaceType) {
    case 0:
    ...
    LABEL_188:
      dLeftBankAngle =
          atan2(fBankingCalc1, fBankingCalc2) * 16384.0 / 6.28318530718;
      //_CHP();
    ...
    LABEL_192:
      dRightBankAngle =
          atan2(fRightBankCalc2, fRightBankCalc) * 16384.0 / 6.28318530718;
      //_CHP();
    ...
    if (finished_car[pCar->iDriverIdx] || death_race) {
      ...
      iTrackColorFlag =
          TrakColour[pCar->nCurrChunk][pCar->iLaneType] & SURFACE_FLAG_PIT;
    ...
    // if ((LODWORD(fAbsoluteSpeed) & 0x7FFFFFFF) != 0 || !iTrackColorFlag ||
    // (pCar->byStatusFlags & 4) != 0) {
    if (fabs(fAbsoluteSpeed) > FLT_EPSILON || !iTrackColorFlag ||
        (pCar->byStatusFlags & 4) != 0) {
      ...
      pCar->byCheatAmmo = 8;
      if ((iPlayer1CarIdx == iCurrentDriverIdx2 ||
           player2_car == iCurrentDriverIdx2) &&
          !pCar->byRepairSpeechPlayed && pCar->fHealth < 100.0) {
        iSpeechDriverIdx2 = pCar->iDriverIdx;
        pCar->byRepairSpeechPlayed = -1;
        speechonly(SOUND_SAMPLE_MENDING, 0x8000, 18,
                   iSpeechDriverIdx2); // SOUND_SAMPLE_MENDING - play repair
                                       // sound when health is low
      }
      pCar->byDebugDamage = -1;
      iCurrentChunk2 = pCar->nCurrChunk;
      pCar->byStatusFlags |= 8u;
      if ((TrakColour[iCurrentChunk2][1] & SURFACE_FLAG_PIT_BOX) !=
          0) // Handle special pit lane targeting for AI cars on repair strips
      {
        iLoopCounter = 0;
        if (numcars > 0) {
          ...
          iArrayIndex = 0;
          do {
            if (iLoopCounter != pCar->iDriverIdx &&
                pCar->nCurrChunk == Car[iArrayIndex].iAITargetCar)
      ...
      if (dHealthIncrease >= 100.0) {
        ...
        pCar->fHealth = 100.0;
        if ((iPlayer1Check == iDriverForHealth ||
             player2_car == iDriverForHealth) &&
            pCar->byRepairSpeechPlayed) {
          speechonly(
              SOUND_SAMPLE_FIXED, 0x8000, 18,
              pCar->iDriverIdx); // SOUND_SAMPLE_FIXED - play repair complete
                                 // sound when health reaches 100%
          pCar->byRepairSpeechPlayed = 0;
        }
        pCar->byDebugDamage = 0;
    ...
    iCenterGrip = localdata[pCar->nCurrChunk]
                      .iCenterGrip; // Calculate grip values based on car
                                    // position (center track vs shoulders)
    if (pCar->pos.fY > (double)fTrackCenterLine)
      iCenterGrip = localdata[pCar->nCurrChunk].iLeftShoulderGrip;
    if (-fTrackCenterLine > pCar->pos.fY)
      iCenterGrip = localdata[pCar->nCurrChunk].iRightShoulderGrip;
    // CHEAT_MODE_ICY_ROAD
    if ((cheat_mode & CHEAT_MODE_ICY_ROAD) !=
        0) // CHEAT_MODE grip hack - force maximum grip value of 10
      iCenterGrip = 10;
    ...
    fCurrentGripMultiplier =
        (float)((dSurfaceValue +
                 CarEngines.engines[byCarTypeIndex].fGripBonus) /
                (2.5 - fHealthAdjustment * 1.5));
    ...
    fTempGrip = (float)(((double)surface[iGripIndex].iSecondaryGrip +
                         CarEngines.engines[byCarTypeIndex].fGripBonus) /
                        (2.5 - fHealthAdjustment * 1.5));
    ...
    if (fY <= fTrackCenterLine) {
      if (-fTrackCenterLine <= pCar->pos.fY)
        ...
        fZ = pPointAy[6].fZ * tcos[pTrackInfo->iRightBankAngle & 0x3FFF] -
             pPointAy[7].fX * tsin[pTrackInfo->iRightBankAngle & 0x3FFF];
    } else {
      fZ = pPointAy[6].fZ * tcos[pTrackInfo->iLeftBankAngle & 0x3FFF] +
           pPointAy[7].fX * tsin[pTrackInfo->iLeftBankAngle & 0x3FFF];
    }
    ...
    fRPMRatio = pCar->fRPMRatio; // Physics calculations - banking effects on
                                 // car movement
    ...
    iSteeringInputProcessed =
        pCar->iSteeringInput; // Process steering input and apply speed-based
                              // steering modifiers
    if (!iSteeringInputProcessed)
      goto LABEL_272;
    if (pCar->fFinalSpeed < 0.0) {
      // LODWORD(fCameraOscillationRange) = -iSteeringInputProcessed;
      dSteeringSpeedCalc = (4500.0 - fAbsoluteSpeed * 23.0) *
                           (double)-iSteeringInputProcessed * 0.00039999999;
    } else {
      ...
      LABEL_272:
        ...
        iYawCurrent = ((uint16)pCar->iJumpMomentum +
                       (int16)iSteeringInputProcessed + (int16)iYawCurrent) &
                      0x3FFF;
        pCar->nYaw = iYawCurrent;
        ...
        if (iYawCurrent >= 0x2000)
          ...
          fTrackBankingY = -CarBaseX;
        fCarHalfWidthProjected = fCarBaseYProjected * tcos[iYawCurrent] -
                                 fTrackBankingY * tsin[iYawCurrent];
        if ((((int16)iYawCurrent + 4096) & 0x3FFFu) < 0x1000 ||
            (((int16)iYawCurrent + 4096) & 0x3FFFu) >= 0x3000)
        ...
        if ((((int16)iYawCurrent + 4096) & 0x3FFFu) >= 0x2000)
          ...
          fCarBaseXProjected = -CarBaseX;
        pCar->fCarWidthBankingProjection =
            fTrackBankingX * tcos[((int16)iYawCurrent + 4096) & 0x3FFF] -
            fCarBaseXProjected * tsin[((int16)iYawCurrent + 4096) & 0x3FFF];
        ...
        if (dCurrentSpeed > 100.0)
          fSpeedLimited = 100.0;
        //_CHP();
        pCar->iBankingSteerOffset =
            (int)(fSpeedLimited * fBankingForceX * 0.16666667);
        if (pCar->iBankingSteerOffset > 16)
          pCar->iBankingSteerOffset = 16;
        if (pCar->iBankingSteerOffset < -16)
          pCar->iBankingSteerOffset = -16;
        if (race_started &&
            !pCar->byDebugDamage) // Update car position and handle track
                                  // collision detection
        {
          if ((fBankingForceY > 0.0 || fAbsoluteSpeed > 2.0) &&
              pCar->fHealth > 0.0)
            pCar->fSpeedOverflow =
                fBankingForceY * 0.125f + pCar->fSpeedOverflow;
        } else {
          pCar->fSpeedOverflow = 0.0;
        }
        fFinalSpeedThisFrame = pCar->fFinalSpeed;
        if (fFinalSpeedThisFrame < 1.0 && (pCar->byGearAyMax & 0x80u) == 0 &&
            pCar->fPower > 0.0)
        ...
        pCar->pos.fY =
            (float)dMovementSpeed * tsin[iYaw3ForMovement] + pCar->pos.fY;
        ...
        if (pCar->fFinalSpeed < 0.0)
          uiJumpFlag = 0;
        if ((iTrackColorValue & SURFACE_FLAG_PREVENT_JUMP) != 0 &&
            pCar->fFinalSpeed > 240.0)
          uiJumpFlag = 0;
        iCollisionFlag = 0;
        if ((TrakColour[pCar->nCurrChunk][1] & SURFACE_FLAG_WALL_22) != 0 &&
            fTrackCenterLine >= fabs(pCar->pos.fY) - fCarHalfWidthProjected) {
          if (pCar->pos.fY < 0.0) {
        ...
        if ((TrakColour[pCar->nCurrChunk][0] & SURFACE_FLAG_WALL_22) != 0 &&
            pCar->pos.fY + fCarHalfWidthProjected >= fTrackCenterLine) {
          pCar->pos.fY = fTrackCenterLine - fCarHalfWidthProjected;
          hitleft(pCar, 12, 0);
          iCollisionFlag = -1;
        }
        if ((TrakColour[pCar->nCurrChunk][2] & SURFACE_FLAG_WALL_22) != 0 &&
            -fTrackCenterLine >= pCar->pos.fY - fCarHalfWidthProjected) {
          pCar->pos.fY = fCarHalfWidthProjected - fTrackCenterLine;
          hitright(pCar, 12, 1);
          iCollisionFlag = -1;
        }
        if (!iCollisionFlag) {
          ...
          if (pCar->iStunned) {
            ...
            fCarPosZBackup =
                fCarPosZBackup - CarBox.hitboxAy[iCollisionFlag][4].fZ;
          }
          if (pCar->pos.fY > 0.0) {
            switch (pTrackInfo->iLeftSurfaceType) {
            case 0:
            case 2:
              if (fTrackCenterLine + fLeftShoulderBoundary >=
                  pCar->pos.fY - fCarHalfWidthProjected * 0.5) {
                if (!uiJumpFlag)
                  goto LABEL_344;
                fCameraOscillationRange = fCarPosZBackup - fSpeedScaled;
                if (getgroundz(pCar->pos.fX, pCar->pos.fY, pCar->nCurrChunk) >=
                    fCameraOscillationRange)
                  goto LABEL_344;
              }
              goto LABEL_403;
            ...
            case 5:
              ...
              if (pCar->pos.fY + fCarHalfWidthProjected > fTrackCenterLine &&
                  pCar->iLaneType) {
                pCar->pos.fY = fTrackCenterLine - fCarHalfWidthProjected;
                hitleft(pCar, 12, 0);
              }
            ...
            case 9:
              if (fTrackCenterLine + fLeftShoulderBoundary <
                  pCar->pos.fY - fCarHalfWidthProjected * 0.5)
                goto LABEL_403;
              if (fCarHalfWidthProjected >
                      fabs(pCar->pos.fY - fTrackCenterLine) &&
                  pCar->pos.fY + fCarHalfWidthProjected > fTrackCenterLine &&
                  pCar->iLaneType) {
                pCar->pos.fY = fTrackCenterLine - fCarHalfWidthProjected;
                hitleft(pCar, 12, 0);
              ...
              goto LABEL_344;
            default:
              goto LABEL_404; // Handle left track surface types - walls, ramps,
                              // barriers, etc.
            }
          }
          ...
          case 0:
          case 2:
            if (fTrackCenterLine + fRightShoulderBoundary >=
                -pCar->pos.fY - fCarHalfWidthProjected * 0.5) {
              if (!uiJumpFlag)
                goto LABEL_344;
              fCameraOscillationRange = fCarPosZBackup - fSpeedScaled;
          ...
          case 8:
            if (fTrackCenterLine + fRightShoulderBoundary >=
                fCarHalfWidthProjected - pCar->pos.fY) {
              if (uiJumpFlag &&
                  (fHorizontalSpeed = fCarPosZBackup - fSpeedScaled,
                   getgroundz(pCar->pos.fX, pCar->pos.fY, pCar->nCurrChunk) <
                       fHorizontalSpeed))
              LABEL_403:
                converttoair(pCar);
              else
              ...
              pCar->pos.fY = -fTrackCenterLine - fRightShoulderBoundary +
                             fCarHalfWidthProjected;
              hitright(pCar, 12, 1);
            }
            break;
          case 5:
            ...
            if (fCarHalfWidthProjected - pCar->pos.fY > fTrackCenterLine &&
                pCar->iLaneType != 2) {
              pCar->pos.fY = fCarHalfWidthProjected - fTrackCenterLine;
              hitright(pCar, 12, 1);
            }
          ...
          case 9:
            ...
            if (fTrackCenterLine + fRightShoulderBoundary <
                fRightCarPosY - fCarHalfWidthProjected * 0.5)
              goto LABEL_403;
            if (fCarHalfWidthProjected >
                    fabs(fRightCarPosY - fTrackCenterLine) &&
                fCarHalfWidthProjected - pCar->pos.fY > fTrackCenterLine &&
                pCar->iLaneType != 2) {
              pCar->pos.fY = fCarHalfWidthProjected - fTrackCenterLine;
              hitright(pCar, 12, 1);
            }
            ...
            goto LABEL_344;
          default:
            break; // Handle right track surface types - walls, ramps, barriers,
                   // etc.
          }
        }
      LABEL_404:
        if (pCar->iControlType == 3 &&
            localdata[pCar->nCurrChunk].fTrackHalfLength * 2.0 > CarBaseX) {
          if (pCar->pos.fY <= (double)fTrackCenterLine) {
            if (-fTrackCenterLine <= pCar->pos.fY)
              iLaneTypeCheck = 1;
            else
          ...
          // if ((((HIDWORD(llTrackColorData) ^ (unsigned int)llTrackColorData)
          // - HIDWORD(llTrackColorData)) & 0x20000) != 0)
          if ((TrakColour[pCar->nCurrChunk][iLaneTypeCheck] &
        ...
        if (pCar->iControlType == 3) {
          ...
          iYawDifference =
              iYawDifferenceCalc + (iYawDifferenceCalc < 0 ? 0x4000 : 0);
          ...
          if (dSpeedAbs <= fSurfaceGripValue) {
            ...
            dGripCalculation = (fCurrentGripMultiplier - fMaxGripLimit) /
                                   fSurfaceGripValue * fCurrentSpeedAbs +
                               fMaxGripLimit;
          } else {
            dGripCalculation = (fCurrentSpeedAbs - fSurfaceGripBase) /
                                   (fSurfaceGripValue - fSurfaceGripBase) *
                                   fCurrentGripMultiplier +
                               (fCurrentSpeedAbs - fSurfaceGripValue) /
                                   (fSurfaceGripBase - fSurfaceGripValue) *
                                   fTempGrip;
          }
          fMaxGripLimit = (float)dGripCalculation;
          fMaxGripLimit =
              (tsin[abs(iYawDifference)] * 4.0f + 1.0f) * fMaxGripLimit;
          ...
          if (!pCar->iStunned)
            pCar->nActualYaw =
                ((int16)iYawDifference + (uint16)pCar->nActualYaw) & 0x3FFF;
          iHumanControlIdx = pCar->iDriverIdx;
          if (human_control[iHumanControlIdx]) {
            if (!iJumpMomentum) {
              // if (!pCar->iSteeringInput
              //   || (iInput = (int16)copy_multiple[((int16)readptr - 1) &
              //   0x1FF][iHumanControlIdx].data.unInput,
              //       llInputCheck = (iInput) >> 8,
              //       (int)((HIDWORD(llInputCheck) ^ llInputCheck) -
              //       HIDWORD(llInputCheck) - abs32(pCar->iSteeringInput)) <
              //       0)) {
              // TODO check this
              if (!pCar->iSteeringInput ||
                  abs((copy_multiple[((int16)readptr - 1) & 0x1FF]
                                    [iHumanControlIdx]
                                        .data.unInput) >>
                      8) < abs(pCar->iSteeringInput)) {
                iCurrentYawForInput = pCar->nYaw;
                iYaw3ForInput = pCar->nActualYaw;
                if (iCurrentYawForInput != iYaw3ForInput) {
                  iHumanYawDiff =
                      iCurrentYawForInput - iYaw3ForInput +
                      (iCurrentYawForInput - iYaw3ForInput < 0 ? 0x4000 : 0);
        ...
        if (pCar->iStunned && pCar->iControlType == 3 ||
            (pCar->byStatusFlags & 4) != 0) // Apply speed reduction when car is
                                            // stunned or has damage status
        {
          pCar->fFinalSpeed = pCar->fFinalSpeed + -2.0f;
          if (pCar->fFinalSpeed < 0.0)
            pCar->fFinalSpeed = 0.0;
          // if (pCar->nTargetChunk == -1 && (LODWORD(pCar->fFinalSpeed) &
          // 0x7FFFFFFF) == 0)
          ...
          // (LODWORD(pCar->fHealth) & 0x7FFFFFFF) == 0 && pCar->nDeathTimer <
          // -54)
          if ((fabs(pCar->fFinalSpeed) == 0 ||
               fabs(pCar->fHealth) == 0 && pCar->nDeathTimer < -54) &&
              !pCar->iPitchCameraOffset && !pCar->iRollCameraOffset &&
              !pCar->nTargetChunk) {
            checkplacement(pCar);
          }
        }
      ...
      dSteeringSpeedCalc = (double)(6 * iSteeringInputProcessed) *
                               (360.0 - fAbsoluteSpeed) * 0.00027777778 +
                           (double)iSteeringInputProcessed;
    }
    //_CHP();
    iSteeringInputProcessed = (int)dSteeringSpeedCalc;
    goto LABEL_272;
  }
  // if (pCar->nTargetChunk == -1 && (LODWORD(fAbsoluteSpeed) & 0x7FFFFFFF) ==
  // 0)
  ...
  calculateseparatedcoordinatesystem(
      pCar->iLastValidChunk,
      &coordinateSystemData); // Transform car coordinates to track coordinate
                              // system
  ...
  fCoordTransformX = (float)(coordinateSystemData.pointAy[1].fX * dCoordY +
                             coordinateSystemData.pointAy[0].fX * dCoordX +
                             coordinateSystemData.pointAy[2].fX * dCoordZ);
  fCoordTransformY = (float)(coordinateSystemData.pointAy[1].fY * dCoordY +
                             coordinateSystemData.pointAy[0].fY * dCoordX +
                             coordinateSystemData.pointAy[2].fY * dCoordZ);
  iChunkPlus2 = pCar->iLastValidChunk + 2;
  iChunkPlus3 = pCar->iLastValidChunk + 3;
  fCoordTransformZ = (float)(dCoordY * coordinateSystemData.pointAy[1].fZ +
                             dCoordX * coordinateSystemData.pointAy[0].fZ +
                             dCoordZ * coordinateSystemData.pointAy[2].fZ);
  if (iChunkPlus3 >= TRAK_LEN)
    iChunkPlus3 -= TRAK_LEN;
  if (iChunkPlus2 >= TRAK_LEN)
    iChunkPlus2 -= TRAK_LEN;
  dGroundHeightForward =
      (GroundPt[iChunkPlus2].pointAy[2].fZ +
       GroundPt[iChunkPlus2].pointAy[3].fZ +
       GroundPt[iChunkPlus3].pointAy[2].fZ +
       GroundPt[iChunkPlus3].pointAy[3].fZ) *
  ...
  if (fCoordTransformY <= 0.0) {
    fGroundAverageZ1 =
        (GroundPt[pCar->iLastValidChunk].pointAy[3].fY +
         coordinateSystemData.pointAy[3].fY) *
            coordinateSystemData.pointAy[1].fY +
        (GroundPt[pCar->iLastValidChunk].pointAy[3].fX +
         coordinateSystemData.pointAy[3].fX) *
            coordinateSystemData.pointAy[0].fY +
        (GroundPt[pCar->iLastValidChunk].pointAy[3].fZ +
         coordinateSystemData.pointAy[3].fZ) *
            coordinateSystemData.pointAy[2]
                .fY; // Calculate track boundary limits for airborne cars
    if (fCoordTransformY - CarBaseX >= fGroundAverageZ1)
      goto LABEL_493;
    dBoundaryLimit = fGroundAverageZ1 + CarBaseX;
  } else {
    fGroundBoundaryY = (GroundPt[pCar->iLastValidChunk].pointAy[2].fY +
                        coordinateSystemData.pointAy[3].fY) *
                           coordinateSystemData.pointAy[1].fY +
                       (GroundPt[pCar->iLastValidChunk].pointAy[2].fX +
                        coordinateSystemData.pointAy[3].fX) *
                           coordinateSystemData.pointAy[0].fY +
                       (GroundPt[pCar->iLastValidChunk].pointAy[2].fZ +
                        coordinateSystemData.pointAy[3].fZ) *
                           coordinateSystemData.pointAy[2].fY;
...
LABEL_493:
  ...
  SetEngine(pCar, pCar->fFinalSpeed);
  // if (((LODWORD(fCurrentSpeedFloat) & 0x7FFFFFFF) == 0 ||
  // (LODWORD(pCar->fHealth) & 0x7FFFFFFF) == 0 && pCar->nDeathTimer < -54)
  if ((fabs(fCurrentSpeedFloat) == 0 ||
       fabs(pCar->fHealth) == 0 && pCar->nDeathTimer < -54) &&
      !pCar->iPitchCameraOffset && !pCar->iRollCameraOffset &&
      !pCar->nTargetChunk) {
  LABEL_501:
...
LABEL_502:
  if (!pCar->iControlType &&
      pCar->pos.fZ < (double)GroundLevel[pCar->iLastValidChunk] &&
      (char)pCar->byLives >
          0) { // Handle fall damage and car landing after being airborne
    if (death_race)
  ...
  if (!finished_car[pCar->iDriverIdx] &&
      pCar->fRunningLapTime <
          1000.0) // Update lap timing - running lap time and total race time
  {
    if (race_started) {
...
void check_crossed_line(tCar *pCar) {
  ...
  if (champ_mode) {
    iDriverIdx = pCar->iDriverIdx;
    if (champ_go[iDriverIdx]) // Check if this driver has championship go flag
                              // set
    {
      nCurrChunk = pCar->nCurrChunk;
      if (nCurrChunk > 16 && nCurrChunk < 64) // Check if car is in early race
                                              // chunk range (after start line)
      {
        iRacers = racers;
  ...
  iCurrentChunk =
      pCar->nCurrChunk; // Check if car has valid current chunk position
  if (iCurrentChunk != -1) {
    iChunk2 = pCar->nReferenceChunk;
    if (iChunk2 != -1) { // Detect forward lap line crossing (crossed finish
                         // line going forward)
      if (iChunk2 - iCurrentChunk > TRAK_LEN / 2 &&
          !finished_car[pCar->iDriverIdx]) { // Check if lap count matches
                                             // expected lap number
        if (pCar->byLap == pCar->byLapNumber) {
          byLap = pCar->byLap;
          if (byLap <=
              NoOfLaps) { // Record lap time for trial mode (game_type == 2)
            if (game_type == 2)
              ...
              if (champ_car) {
                ...
                if (ViewType[0] == iTargetDriverIdx) {
                  ...
                  if (iRacePos < racers - 1) {
                    ...
                    pEngine =
                        &CarEngines.engines[Car[iTargetCar].byCarDesignIdx];
                    ...
                    dPowerCalc = calc_pow(Car[iTargetCar].byCarDesignIdx,
            ...
            iSoundSample5 = -1;
            iSoundSample6 = -1;
            if (pCar->byLap > NoOfLaps &&
                NoOfLaps) // Check if race is finished (exceeded total lap
                          // count)
            {
              finished_car[pCar->iDriverIdx] =
                  -1; // Mark car as finished and increment finisher count
              iHumanControl1 = human_control[pCar->iDriverIdx];
              ++finishers;
              if (iHumanControl1)
                ++human_finishers;
              iDriverIdx3 = pCar->iDriverIdx;
              if (player1_car == iDriverIdx3 ||
                  player2_car == iDriverIdx3) // Handle finish sounds for player
                                              // cars based on position
              {
                  ...
                  if (byRacePosition) {
                    if (byRacePosition >= 3u) {
                      if (byRacePosition >= 0xDu)
                        iSoundSample6 =
                            SOUND_SAMPLE_RUBBISH; // SOUND_SAMPLE_RUBBISH
                      else
                        iSoundSample6 = SOUND_SAMPLE_POOR; // SOUND_SAMPLE_POOR
                    } else {
                      iSoundSample6 =
                          SOUND_SAMPLE_HARDER; // SOUND_SAMPLE_HARDER
                    }
                  } else {
                    iSoundSample6 = SOUND_SAMPLE_WINNER; // SOUND_SAMPLE_WINNER
                  }
                  iSoundSample5 =
                      SOUND_SAMPLE_RACEOVER; // SOUND_SAMPLE_RACEOVER
                }
              } else if (player_type != 2) {
            ...
            if (pCar->fRunningLapTime < (double)pCar->fBestLapTime &&
                pCar->byLap > 1) // Check if this lap is a new personal best
            {
              pCar->fBestLapTime = pCar->fRunningLapTime;
              if (pCar->fBestLapTime >=
                  (double)RecordLaps[game_track]) // Check if lap time beats
                                                  // track record
              {
                iFastestLapFlag = -1;
                if (racers >
                    0) // Check if this is fastest lap among all drivers
                {
                  iCarCounter = 0;
                  do {
                    iCarOrderIdx = carorder[iCarCounter];
                    if (iCarOrderIdx != pCar->iDriverIdx &&
                        Car[iCarOrderIdx].fBestLapTime <=
                            (double)pCar->fBestLapTime)
                      iFastestLapFlag = 0;
                    ++iCarCounter;
                ...
                if (player1_car == iDriverIdx4 || player2_car == iDriverIdx4) {
                  ...
                  sprintf(buffer, "%s %s", &language_buffer[2752],
                          driver_names[iDriverIdx4]);
                  small_zoom(buffer);
                  make_time(buffer, pCar->fBestLapTime);
                  subzoom(buffer);
                }
            ...
            if (pCar->byLap != 1) {
              ...
              if ((player1_car == iDriverIdx6 || player2_car == iDriverIdx6) &&
                  finishers - Destroyed <= 0 &&
                  !finished_car[pCar->iDriverIdx] &&
                  competitors > 1) { // Announce remaining laps to player cars
                  ...
                  if (byRacePos2) {
                    if (byRacePos2 >= 5u) {
                      if (!SamplePtr[16])
                        goto LABEL_102;
                      iSoundSample2 =
                          SOUND_SAMPLE_FINAL1; // SOUND_SAMPLE_FINAL1
                    } else {
                      if (!SamplePtr[17])
                        goto LABEL_102;
                      iSoundSample2 =
                          SOUND_SAMPLE_FINAL2; // SOUND_SAMPLE_FINAL2
                    }
                    break;
                  }
                  if (!SamplePtr[18])
        ...
        if (iFinishersCount1 > 0) {
          ...
          if (!finished_car[iDriverIdx8] && competitors > 1) {
            ...
            if ((player1_car == iDriverIdx9 || player2_car == iDriverIdx9) &&
                (char)pCar->byLives > 0) {
              ...
              speechsample(SOUND_SAMPLE_RACEOVER, 0x8000, 18,
                           pCar->iDriverIdx); // SOUND_SAMPLE_RACEOVER
            }
          }
        }
      }
      if (pCar->nReferenceChunk - pCar->nCurrChunk <
          -TRAK_LEN / 2) // Detect backward lap line crossing (went backwards
                         // across finish line)
      {
        ...
        if (iFinishersCount2 - Destroyed > 0) {
          ...
          if (!finished_car[iDriverIdx10] && competitors > 1) {
            ...
            if ((player1_car == iDriverIdx11 || player2_car == iDriverIdx11) &&
                (char)pCar->byLives > 0) {
      ...
      if ((pCar->byLapNumber & 0x80u) !=
          0) // Prevent lap number underflow (reset to 0 if went negative)
        pCar->byLapNumber = 0;
    }
  }
}
...
void checkplacement(tCar *pCar) {
  ...
  iRandValue =
      ROLLERrandRaw(); // Generate random chunk position near current chunk
  iNewChunk = GetHighOrderRand(10, iRandValue) + pCar->nReferenceChunk;
  if (iNewChunk >= TRAK_LEN)
    iNewChunk = TRAK_LEN - 1;
  if (iNewChunk == -1)
    iNewChunk = 0;
  // if ((LODWORD(pCar->fHealth) & 0x7FFFFFFF) == 0 && !numstops)// Reset health
  // to 100 if car has zero health and there's no pit boxes
  if (fabs(pCar->fHealth) == 0 &&
      !numstops) // Reset health to 100 if car has zero health and there's no
                 // pit boxes
    pCar->fHealth = 100.0;
  // if ((LODWORD(pCar->fHealth) & 0x7FFFFFFF) != 0)// Handle placement for cars
  // with health (alive cars)
  if (fabs(pCar->fHealth) >
      FLT_EPSILON) // Handle placement for cars with health (alive cars)
  {                // Find a valid track chunk
    while ((TrakColour[iNewChunk][0] &
            (SURFACE_FLAG_NO_SPAWN | SURFACE_FLAG_PIT | SURFACE_FLAG_WALL_22 |
             SURFACE_FLAG_SKIP_RENDER)) != 0 &&
           (TrakColour[iNewChunk][1] &
            (SURFACE_FLAG_NO_SPAWN | SURFACE_FLAG_PIT | SURFACE_FLAG_WALL_22 |
             SURFACE_FLAG_SKIP_RENDER)) != 0 &&
           (TrakColour[iNewChunk][2] &
            (SURFACE_FLAG_NO_SPAWN | SURFACE_FLAG_PIT | SURFACE_FLAG_WALL_22 |
             SURFACE_FLAG_SKIP_RENDER)) != 0) {
      if (++iNewChunk == TRAK_LEN)
        iNewChunk ^= TRAK_LEN;
    ...
    } while ((TrakColour[iNewChunk][uiLaneType] &
              (SURFACE_FLAG_NO_SPAWN | SURFACE_FLAG_PIT | SURFACE_FLAG_WALL_22 |
               SURFACE_FLAG_SKIP_RENDER)) != 0);
    iCarCounter = 0;
    if (numcars > 0) // Check for collision with other cars in same lane
    {
      iCarArrayIdx = 0;
      do {
        if (iCarCounter != pCar->iDriverIdx &&
            Car[iCarArrayIdx].iControlType == 3 &&
            uiLaneType == Car[iCarArrayIdx].iLaneType) {
    ...
    if (!iPlacementResult)
      goto LABEL_39;
    if (uiLaneType) // Set Y position based on lane type (0=left shoulder,
                    // 1=center, 2=right shoulder)
    {
      if (uiLaneType <= 1) {
        pCar->pos.fY = 0.0;
      LABEL_38:
      ...
      if (uiLaneType != 2)
        goto LABEL_38;
      dLanePosition = -localdata[iNewChunk].fTrackHalfWidth -
                      TrackInfo[iNewChunk].fRShoulderWidth * 0.5f;
    } else {
      dLanePosition = TrackInfo[iNewChunk].fLShoulderWidth * 0.5f +
                      localdata[iNewChunk].fTrackHalfWidth;
    }
    pCar->pos.fY = (float)dLanePosition;
    goto LABEL_38;
  }
  iPlacementResult = 0;
  if ((network_on || (char)pCar->byLives > 0 || lastsample >= -144 ||
       pCar->nDeathTimer >= -64) &&
      (!network_on || (char)pCar->byLives > 0 ||
       pCar->nDeathTimer >=
           0)) { // Handle respawn for dead cars (health=0, has lives left)
    if (pCar->nDeathTimer < 0 && (char)pCar->byLives > 0) {
      ...
      if (numcars > 0) {
        iCarArrayIdx2 = 0;
        do {
          if (iCarLoopIdx != pCar->iDriverIdx &&
              (char)Car[iCarArrayIdx2].byLives > 0 &&
              Car[iCarArrayIdx2].iControlType == 3) {
            iTempChunk = Car[iCarArrayIdx2].nCurrChunk;
            if ((TrakColour[iTempChunk][Car[iCarArrayIdx2].iLaneType] &
                 SURFACE_FLAG_PIT) != 0) {
      ...
      if (iPlacementResult) {
        ...
        // for (i = 0; i != 352; car_texs_loaded[352 * pCar->iDriverIdx + 12 +
        // i] = 0)
        //   i += 11;
        ...
        if (player_type == 2) {
          ...
          if (player1_car == iDriverIdx) {
            ...
            // do {
            //   iSprayOffset1 += 352;
            //   *(int *)((char *)&CarSpray[15][24].iLifeTime + iSprayOffset1) =
            //   0;
            //   *(int *)((char *)&CarSpray[15][25].iLifeTime + iSprayOffset1) =
            //   0;
            //   *(int *)((char *)&CarSpray[15][26].iLifeTime + iSprayOffset1) =
            //   0;
            //   *(int *)((char *)&CarSpray[15][27].iLifeTime + iSprayOffset1) =
            //   0;
            //   *(int *)((char *)&CarSpray[15][28].iLifeTime + iSprayOffset1) =
            //   0;
            //   *(int *)((char *)&CarSpray[15][29].iLifeTime + iSprayOffset1) =
            //   0;
            //   *(int *)((char *)&CarSpray[15][30].iLifeTime + iSprayOffset1) =
            //   0;
            //   *(int *)((char *)&CarSpray[15][31].iLifeTime + iSprayOffset1) =
            //   0;
          ...
          if (player2_car == iDriverIdx2) {
            ...
            // do {
            //   iSprayOffset2 += 352;
            //   *(int *)((char *)&CarSpray[16][24].iLifeTime + iSprayOffset2) =
            //   0;
            //   *(int *)((char *)&CarSpray[16][25].iLifeTime + iSprayOffset2) =
            //   0;
            //   *(int *)((char *)&CarSpray[16][26].iLifeTime + iSprayOffset2) =
            //   0;
            //   *(int *)((char *)&CarSpray[16][27].iLifeTime + iSprayOffset2) =
            //   0;
            //   *(int *)((char *)&CarSpray[16][28].iLifeTime + iSprayOffset2) =
            //   0;
            //   *(int *)((char *)&CarSpray[16][29].iLifeTime + iSprayOffset2) =
            //   0;
            //   *(int *)((char *)&CarSpray[16][30].iLifeTime + iSprayOffset2) =
            //   0;
            //   *(int *)((char *)&CarSpray[16][31].iLifeTime + iSprayOffset2) =
            //   0;
    ...
    iSelectedView =
        SelectedView[0]; // Handle permanently dead cars (no lives left)
    pCar->byLives = -1;
    if (iSelectedView == 7) {
    ...
    if (dead_humans < players) {
      ...
      if (iCurrentChunk == -1) {
        ...
        pCar->pos.fX = 0.0f * pDataAy->pointAy[0].fY +
                       0.0f * pDataAy->pointAy[0].fX +
                       0.0f * pDataAy->pointAy[0].fZ - pDataAy->pointAy[3].fX;
        pCar->pos.fY = 0.0f * pDataAy->pointAy[1].fX +
                       0.0f * pDataAy->pointAy[1].fY +
                       0.0f * pDataAy->pointAy[1].fZ - pDataAy->pointAy[3].fY;
        dPosZ = 0.0 * pDataAy->pointAy[2].fX + 0.0 * pDataAy->pointAy[2].fY +
                0.0 * pDataAy->pointAy[2].fZ - pDataAy->pointAy[3].fZ + 1000.0;
      ...
      if ((Car[iCurrentViewType].byLives & 0x80u) != 0) {
        for (ViewType[0] = 0; !human_control[ViewType[0]] ||
                              (Car[ViewType[0]].byLives & 0x80u) != 0;
             ++ViewType[0])
          ;
        initcarview(ViewType[0], 0);
        stopallsamples();
      ...
      if ((Car[ViewType[1]].byLives & 0x80u) != 0) {
        for (ViewType[1] = 0; !human_control[ViewType[1]] ||
...
LABEL_113:
  if (iPlacementResult &&
      (char)pCar->byLives >
          0) // Initialize car state after successful placement
  {
    pCar->pos.fX = 0.0;
    pCar->pos.fZ = 0.0;
    pCar->nPitch = 0;
...
//-------------------------------------------------------------------------------------------------
// 00030260
void hitleft(tCar *pCar, int iSampleIdx,
             int iIsRightSide) { // Calculate maximum yaw change based on speed
                                 // (capped at 500.0)
  ...
  iYaw3 = pCar->nActualYaw;
  fVelX = tcos[iYaw3] * pCar->fFinalSpeed; // Calculate velocity components from
                                           // car's current speed and direction
  fVelY = tsin[iYaw3] * pCar->fFinalSpeed;
  if (iIsRightSide) // Select collision angle based on left/right side (a3
                    // parameter)
    iCollisionAngle = rightang;
  else
    iCollisionAngle = leftang;
  fTangentialVel =
      fVelX * tcos[iCollisionAngle] +
      fVelY * tsin[iCollisionAngle]; // Transform velocity vector to collision
                                     // surface coordinate system
  fPerpVel = -fVelX * tsin[iCollisionAngle] + fVelY * tcos[iCollisionAngle];
  if (!pCar->byCollisionTimer) // Check if car is not already in collision state
                               // (byUnk63 flag)
  { // If velocity into wall is positive, play collision sound and calculate
    // damage
    if (fPerpVel > 0.0) {
      //_CHP();
      sfxpend(iSampleIdx, pCar->iDriverIdx,
              (int)(fVelY * 32768.0 * 0.0049999999));
      if (death_race)
    ...
    fPerpVel = fPerpVel * -0.6f; // Apply collision physics - reverse
                                 // perpendicular velocity with damping
    fTangentialVel = fTangentialVel * 0.8f;
  }
  //_CHP();
  fNegPerpVel = (float)-abs((int)fPerpVel);
  dVelX = fTangentialVel; // Transform velocity back to world coordinates after
                          // collision response
  ...
  if (pCar->fHealth <= 0.0 &&
      fFinalVelY == -40.0) // Special case - if car is dead and velocity is
                           // exactly -40, stop completely
  {
    pCar->fFinalSpeed = 0.0;
    ...
    if (pCar->fFinalSpeed < 0.0) {
      ...
      if (fabs(fNegVelX) > FLT_EPSILON || fabs(-fFinalVelY)) {
        ...
        // dReverseAngle = IF_DATAN2(nFpuTemp2, fNegVelY, fNegVelX) * 16384.0
        // / 6.28318530718; _CHP();
      ...
      if (fabs(dVelY * tcos[iAngleIdx] - fNegPerpVel * tsin[iAngleIdx]) ||
          fabs(dVelX * tsin[iAngleIdx] + fNegPerpVel * tcos[iAngleIdx])) {
        dAngleCalc = atan2(fFinalVelY, fFinalVelX) * 16384.0 / 6.28318530718;
        // dAngleCalc = IF_DATAN2(nFpuTemp1, fFinalVelY, fFinalVelX) * 16384.0
        // / 6.28318530718; _CHP();
  ...
  iYawDiff = pCar->nActualYaw - pCar->nYaw; // Apply yaw constraints - limit how
                                            // fast car can turn based on speed
  ...
  SetEngine(pCar, fFinalSpeed); // Apply final changes - update engine, reset
                                // steering, flatten car, set collision timer
  pCar->iSteeringInput = 0;
  putflat(pCar);
...
//-------------------------------------------------------------------------------------------------
// 00030590
void hitright(tCar *pCar, int iSampleIdx,
              int iIsRightSide) { // Calculate maximum yaw change based on speed
                                  // (capped at 500.0)
  double dSpeedScale;             // st7
  int iYaw3;                      // eax
  ...
  fVelX = tcos[iYaw3] * pCar->fFinalSpeed; // Calculate velocity components from
                                           // car's current speed and direction
  fVelY = tsin[iYaw3] * pCar->fFinalSpeed;
  if (iIsRightSide) // Select collision angle based on left/right side
                    // (iIsRightSide parameter)
    iCollisionAngle = rightang;
  else
    iCollisionAngle = leftang;
  fTangentialVel =
  ...
  if (!pCar->byCollisionTimer) // Check if car is not already in collision state
                               // (byCollisionTimer)
  { // If velocity into wall is negative, play collision sound and calculate
    // damage
    if (fVelY < 0.0) {
      dAbsVelY = -fVelY;
      ...
      //_CHP();
      sfxpend(iSampleIdx, pCar->iDriverIdx,
              (int)(dAbsVelY * 32768.0 * 0.0049999999));
      if (death_race)
                          ...
                          // exactly 40, stop completely
  {
    ...
    if (pCar->fFinalSpeed < 0.0) {
      ...
      if (fabs(dVelY * tcos[iAngleIdx] - fAbsPerpVel * tsin[iAngleIdx]) ||
          fabs(dVelX * tsin[iAngleIdx] + fAbsPerpVel * tcos[iAngleIdx])) {
        dAngleCalc = atan2(fFinalVelY, fFinalVelX) * 16384.0 / 6.28318530718;
        //_CHP();
        iNewYaw = (int)dAngleCalc & 0x3FFF;
      } else {
  ...
  if (!pCar->byCollisionTimer) {
    iYawDiff =
        pCar->nActualYaw -
        pCar->nYaw; // Apply yaw constraints only if not in collision state
    iConstrainedYaw = iYawDiff + (iYawDiff < 0 ? 0x4000 : 0);
    if (iConstrainedYaw > 0x2000)
      iConstrainedYaw -= 0x4000;
    if (iConstrainedYaw > iMaxYawChange)
  ...
  SetEngine(pCar,
            pCar->fFinalSpeed); // Apply final changes - update engine, reset
                                // steering, flatten car, set collision timer
  pCar->iSteeringInput = 0;
  putflat(pCar);
  pCar->byCollisionTimer = 18;
...
void scansection(tCar *pCar) {
  ...
  fTransformedX = pData->pointAy[0].fY * fY +
                  pData->pointAy[0].fX * pCar->pos.fX +
                  pData->pointAy[0].fZ * fZ -
                  pData->pointAy[3].fX; // Transform car position relative to
                                        // current track chunk coordinate system
  fTransformedY = pData->pointAy[1].fY * fY +
                  pData->pointAy[1].fX * pCar->pos.fX +
                  pData->pointAy[1].fZ * fZ - pData->pointAy[3].fY;
  fTransformedZ = pData->pointAy[2].fY * fY +
                  pData->pointAy[2].fX * pCar->pos.fX +
                  pData->pointAy[2].fZ * fZ - pData->pointAy[3].fZ;
  iDirection = 0;
  if (pCar->pos.fX <
      (double)pData->fTrackHalfLength) // If car is behind track half-length,
                                       // scan backwards through chunks
  {
    do {
      if (--iChunkIdx == -1)
      ...
      fX = (float)(pData->pointAy[1].fX * dTempY +
                   pData->pointAy[0].fX * dTempX +
                   pData->pointAy[2].fX *
                       dTempZ); // Apply inverse transformation to get position
                                // in new chunk's coordinate system
      fY = (float)(pData->pointAy[0].fY * dTempX +
                   pData->pointAy[1].fY * dTempY +
                   pData->pointAy[2].fY * dTempZ);
      fZ = (float)(dTempY * pData->pointAy[1].fZ +
                   dTempX * pData->pointAy[0].fZ +
                   dTempZ * pData->pointAy[2].fZ);
      iDirection -= pData->iYaw;
    } while (fX < (double)pData->fTrackHalfLength);
  }
  while (
      fX >
      (double)pData->fTrackHalfLength) // If car is ahead of track half-length,
                                       // scan forwards through chunks
  {
    ++iChunkIdx;
    iDirection += pData->iYaw;
    ...
    fX = (float)(pData->pointAy[1].fX * dTempY2 +
                 pData->pointAy[0].fX * dTempX2 +
                 pData->pointAy[2].fX * dTempZ2);
    fY = (float)(pData->pointAy[0].fY * dTempX2 +
                 pData->pointAy[1].fY * dTempY2 +
                 pData->pointAy[2].fY * dTempZ2);
    fZ = (float)(dTempY2 * pData->pointAy[1].fZ +
                 dTempX2 * pData->pointAy[0].fZ +
                 dTempZ2 * pData->pointAy[2].fZ);
  }
  pCar->pos.fX =
      fX; // Store final transformed position and updated chunk/yaw back to car
  pCar->pos.fY = fY;
  pCar->pos.fZ = fZ;
  pCar->nYaw -= iDirection;
  pCar->nYaw &= 0x3FFF;
  // HIBYTE(pCar->nYaw) &= 0x3Fu;                  // Mask yaw values to keep
  // them within valid ranges (14-bit values)
...
double getgroundz(float fXPos, float fYOffset, int iChunkIdx) {
  ...
  dAngleIdx =
      (double)pData->iBankDelta * fXPos /
      (pData->fTrackHalfLength *
       2.0); // Calculate banking angle index based on X position in chunk
  //_CHP();
  pTrackInfo =
      &TrackInfo[iChunkIdx]; // Get track shoulder information for this chunk
  fGroundZ = -fYOffset *
             ptan[(int)dAngleIdx &
                  0x3FFF]; // Calculate base ground height using banking angle
  if (fYOffset >=
      (double)pData->fTrackHalfWidth) // Check if position is on left shoulder
                                      // (positive Y beyond track)
    fGroundZ = (fYOffset - pData->fTrackHalfWidth) *
                   pTrackInfo->fLShoulderHeight / pTrackInfo->fLShoulderWidth +
               fGroundZ; // Add left shoulder height based on distance beyond
                         // track edge
  dAbsYOffset = -fYOffset;
  if (dAbsYOffset >=
      pData->fTrackHalfWidth) // Check if position is on right shoulder
                              // (negative Y beyond track)
  {
    fTempYOffset = (float)dAbsYOffset;
    return (
        float)((fTempYOffset - pData->fTrackHalfWidth) *
                   pTrackInfo->fRShoulderHeight / pTrackInfo->fRShoulderWidth +
               fGroundZ); // Add right shoulder height and return total ground Z
  }
...
double getroadz(float fX, float fY, int iChunkIdx) {
  ...
  dBankAngleIndex = (double)localdata[iChunkIdx].iBankDelta * fX /
                    (localdata[iChunkIdx].fTrackHalfLength *
                     2.0); // Calculate banking angle index based on X position
                           // and track banking
  //_CHP();
  return -fY * ptan[(int)dBankAngleIndex &
                    0x3FFF]; // Apply banking effect: negative Y times tangent
                             // of banking angle
}
...
void putflat(tCar *pCar) {
  ...
  nYaw = pCar->nYaw;
  if (pCar->pos.fX >=
      0.0) // Determine previous and next chunk indices based on car direction
  {
  ...
  if (pCar->pos.fY <=
      (double)pCurrData->fTrackHalfWidth) // Determine which lane car is in and
                                          // get appropriate pitch angle
  {
    if (-pCurrData->fTrackHalfWidth <= pCar->pos.fY) {
      ...
      iPitch =
          pPrevData->iOuterLanePitchAngle; // Outer lane (left side of track)
      iLaneType = 2;
    }
  } else {
    iPitch =
        pPrevData->iInnerLanePitchAngle; // Inner lane (right side of track)
    iLaneType = 0;
  }
  iPitchInt = iPitch;
  if (pCar->pos.fX < 0.0) // Check surface type and apply pitch corrections
                          // based on car position
  {

    if ((TrakColour[nCurrChunk][iLaneType] & SURFACE_FLAG_SKIP_RENDER) != 0)
      // llSurfaceType3 = TrakColour[nCurrChunk][iLaneType];
      // if ((((HIDWORD(llSurfaceType3) ^ (unsigned int)llSurfaceType3) -
      // HIDWORD(llSurfaceType3)) & 0x20000) != 0)// SURFACE_FLAG_SKIP_RENDER
      iPitchInt = 0;
  } else { // Steep downhill: zero pitch if car is fast and AI state allows
    if (iPitch < -512 && pCar->fFinalSpeed > 50.0 &&
        (TrakColour[pCar->nCurrChunk][pCar->iLaneType] &
         SURFACE_FLAG_NON_MAGNETIC) != 0)
      iPitchInt = 0;

    if ((TrakColour[nCurrChunk][iLaneType] & SURFACE_FLAG_SKIP_RENDER) != 0)
      // llSurfaceType1 = TrakColour[nCurrChunk][iLaneType];// Check current
      // chunk surface type and zero pitch if needed if
      // ((((HIDWORD(llSurfaceType1) ^ (unsigned int)llSurfaceType1) -
      // HIDWORD(llSurfaceType1)) & 0x20000) != 0)// SURFACE_FLAG_SKIP_RENDER
      iPitchInt = 0;

    if (pCar->fFinalSpeed > 50.0) {

      if ((TrakColour[iNextChunkIdx][pCar->iLaneType] &
           SURFACE_FLAG_SKIP_RENDER) != 0)
        // llSurfaceType2 = TrakColour[iNextChunkIdx][pCar->iLaneType];// Check
        // next chunk surface type for fast cars if ((((HIDWORD(llSurfaceType2)
        // ^ (unsigned int)llSurfaceType2) - HIDWORD(llSurfaceType2)) & 0x20000)
        // != 0)// SURFACE_FLAG_SKIP_RENDER
        iPitchInt = 0;
    }
  }
  dPitchFloat =
      (double)iPitchInt * pCar->pos.fX /
      (pCurrData->fTrackHalfLength *
       2.0); // Calculate pitch angle based on car position along track
  //_CHP();
  iPitchFinal = (int)dPitchFloat;
  if ((TrakColour[nCurrChunk][1] & SURFACE_FLAG_BOUNCE_30) !=
      0) // SURFACE_FLAG_BOUNCE_30
  {
    if (pCar->pos.fX < 0.0) {
      dInvHalfLength = 1.0 / pCurrData->fTrackHalfLength;
      dBankTerm1 = (pCar->pos.fX + pCurrData->fTrackHalfLength) *
                   (double)pCurrData->iBankDelta * dInvHalfLength;
      dBankTerm2 =
          (double)((pNextData->iBankDelta + pCurrData->iBankDelta) / -2);
    } else {
      iBankDelta = pCurrData->iBankDelta;
      dInvHalfLength = 1.0 / pCurrData->fTrackHalfLength;
      dBankTerm1 = (pCar->pos.fX - pCurrData->fTrackHalfLength) *
                   (double)-iBankDelta * dInvHalfLength;
      dBankTerm2 = (double)((iBankDelta + pNextData->iBankDelta) / 2);
    }
    dBankAngle =
        dInvHalfLength * (dBankTerm2 * pCar->pos.fX) +
        dBankTerm1; // Apply special banking interpolation and atan2 correction
    //_CHP();
    dPitchCorrected = (double)iPitchFinal -
                      atan2(pCar->pos.fY * tsin[(int)dBankAngle & 0x3FFF],
                            pCurrData->fTrackHalfLength * 2.0) *
                          16384.0 / 6.28318530718;
    //_CHP();
    iPitchFinal = (int)dPitchCorrected;
  }
  dBankFloat = (double)pCurrData->iBankDelta * pCar->pos.fX /
               (pCurrData->fTrackHalfLength *
                2.0); // Calculate basic banking angle based on car X position
  //_CHP();
  iBankInt = (int)dBankFloat;
  pCar->pos.fZ = -(pCar->pos.fY *
                   ptan[(int)dBankFloat & 0x3FFF]); // Calculate car Z position
                                                    // using banking angle
  if (pCar->pos.fY <
      0.0) // Apply track banking adjustments based on car Y position
  {
    fNegYPos = -pCar->pos.fY; // Right side banking adjustments
    if (pCurrData->fTrackHalfWidth + pCar->fCarHalfWidth > fNegYPos) {
      if (pCurrData->fTrackHalfWidth - pCar->fCarHalfWidth <= fNegYPos) {
        dRightBankAdj =
            (fNegYPos - pCurrData->fTrackHalfWidth + pCar->fCarHalfWidth) *
                (double)pTrackInfo->iRightBankAngle /
                (pCar->fCarHalfWidth * 2.0) +
            (double)iBankInt;
        //_CHP();
    ...
    dAbsYPos =
        -pCar->pos.fY; // Add shoulder height if car is off track (right side)
    if (dAbsYPos >= pCurrData->fTrackHalfWidth) {
      fAbsYPos = (float)dAbsYPos;
      dShoulderHeight = (fAbsYPos - pCurrData->fTrackHalfWidth) *
                        pTrackInfo->fRShoulderHeight /
                        pTrackInfo->fRShoulderWidth;
      goto LABEL_43;
    }
    ...
    if (pCurrData->fTrackHalfWidth + pCar->fCarHalfWidth > pCar->pos.fY) {
      if (pCurrData->fTrackHalfWidth - pCar->fCarHalfWidth <= pCar->pos.fY) {
        dLeftBankAdj =
            (double)iBankInt -
            (pCar->pos.fY - pCurrData->fTrackHalfWidth + pCar->fCarHalfWidth) *
                (double)pTrackInfo->iLeftBankAngle /
                (pCar->fCarHalfWidth * 2.0);
        //_CHP();
    ...
    if (pCar->pos.fY >=
        (double)pCurrData->fTrackHalfWidth) // Add shoulder height if car is off
                                            // track (left side)
    {
      dShoulderHeight = (pCar->pos.fY - pCurrData->fTrackHalfWidth) *
                        pTrackInfo->fLShoulderHeight /
                        pTrackInfo->fLShoulderWidth;
    LABEL_43:
      pCar->pos.fZ = (float)dShoulderHeight + pCar->pos.fZ;
    }
  }
  dPitchForCalc = (double)
      iPitchFinal; // Transform pitch and bank angles using car yaw rotation
  ...
  ////iPitchFinal_1 = ((int16)(int)dPitchResult >> 8) & 0x3FFF;// Mask pitch
  /// angle to 14-bit range
  // SET_BYTE1(iPitchFinal_1, ((uint16)(int)dPitchResult >> 8) & 0x3F);// Mask
  // pitch result to 14-bit range (0x3FFF) BYTE1(iPitchFinal_1) = ((unsigned
  // __int16)(int)dPitchResult >> 8) & 0x3F;// Mask pitch result to 14-bit range
  // (0x3FFF)
  ...
  if (iStunned) // Apply stunned car effects - add roll offset and height
                // adjustment
  {
  ...
  pCar->nRoll =
      (int16)iRollFinal & 0x3FFF; // Set final roll angle with 14-bit mask
}
...
void findnearcars(tCar *pCar, int *piLeftCarIdx, float *pfLeftTime,
  ...
  iDriverIdx = pCar->iDriverIdx; // Check if this car should perform full
                                 // collision detection this frame
  if (iDriverIdx == nearcall[nearcarcheck][0] ||
      iDriverIdx == nearcall[nearcarcheck][1] ||
      iDriverIdx == nearcall[nearcarcheck][2] ||
      iDriverIdx ==
          nearcall[nearcarcheck][3]) // Check against scheduled cars for this
                                     // frame (load balancing)
  {
    findnearcarsforce(
        pCar, piLeftCarIdx, pfLeftTime, piRightCarIdx, pfRightTime, pfTargetX,
        pfTargetY); // Perform full collision detection and cache results
    pCar->iLeftTargetIdx =
        *piLeftCarIdx; // Cache collision detection results in car structure
    ...
    *piLeftCarIdx =
        pCar->iLeftTargetIdx; // Use cached collision data from previous frame
    ...
    *pfTargetY = pCar->fTargetY;
    if (*piLeftCarIdx !=
        -1) // Update cached collision times by subtracting frame time
    {
      if (fLeftTargetTime < 0.0) {
        fLeftTargetTime =
            fLeftTargetTime +
            -0.02777777777777778f; // Subtract frame time (1/36 second is about
                                   // 0.0278s for 36 FPS)
      } else {
        fLeftTargetTime = fLeftTargetTime + -0.02777777777777778f;
        if (fLeftTargetTime <
            0.0) // Clamp positive times to zero (collision imminent)
          fLeftTargetTime = 0.0;
      }
    }
    if (*piRightCarIdx != -1) {
    ...
    *pfRightTime = fRightTargetTime;
    ...
    if (*piLeftCarIdx >= 0 &&
        Car[*piLeftCarIdx].nCurrChunk ==
            -1) // Validate left target car is still on track
    {
    ...
    if (*piRightCarIdx >= 0 &&
        Car[*piRightCarIdx].nCurrChunk ==
            -1) // Validate right target car is still on track
    {
      *piRightCarIdx =
          -1; // Clear invalid right target (car removed from track)
      *pfRightTime = -10000.0;
...
void findnearcarsforce(tCar *pCar, int *piLeftCarIdx, float *pfLeftTime,
  ...
  iTrakLen = TRAK_LEN;
  fStrategyValue =
      CarStrategy[pCar->iDriverIdx]
          .fAvoidDistance; // Get AI strategy value for car avoidance distance
  dAbsSpeed =
      fabs(pCar->fFinalSpeed); // Calculate absolute speed for distance scaling
  if (dAbsSpeed < 450.0) {
    fSpeedForCalc = (float)dAbsSpeed;
    fStrategyValue =
        (fStrategyValue + -1000.0f) * fSpeedForCalc * 0.0022222223f + 1000.0f;
  }
  fLeftTime = -10000.0;
  fRightTime = -10000.0;
  nCurrChunk = pCar->nCurrChunk;
  ...
  {
    fReversePos =
        pCar->pos.fX - localdata[nCurrChunk].fTrackHalfLength - fStrategyValue;
    while (-2.0 * localdata[nCurrChunk].fTrackHalfLength > fReversePos) {
      dNewTrackPos =
          2.0 * localdata[nCurrChunk--].fTrackHalfLength + fReversePos;
    ...
    fForwardPos =
        fStrategyValue + pCar->pos.fX + localdata[nCurrChunk].fTrackHalfLength;
    while (1) {
  ...
  if (iNextChunk == TRAK_LEN)
    iNextChunk ^= TRAK_LEN;
  dYPos = ((pTrackData->fTrackHalfLength - fTrackPos) *
               *(&pTrackData->fAILine1 + pCar->iAICurrentLine) +
           (fTrackPos + pTrackData->fTrackHalfLength) *
               *(&localdata[iNextChunk].fAILine1 + pCar->iAICurrentLine)) /
          (pTrackData->fTrackHalfLength * 2.0);
  dTempX = pTrackData->pointAy[1].fX * fTrackPos +
           pTrackData->pointAy[1].fY * dYPos - pTrackData->pointAy[3].fY;
  dTempY = pTrackData->pointAy[0].fY * dYPos +
           pTrackData->pointAy[0].fX * fTrackPos - pTrackData->pointAy[3].fX;
  dTempZ = fTrackPos * pTrackData->pointAy[2].fX +
           dYPos * pTrackData->pointAy[2].fY - pTrackData->pointAy[3].fZ;
  pCurrTrackData = &localdata[pCar->nCurrChunk];
  *pfTargetX = (float)((dTempX + pCurrTrackData->pointAy[3].fY) *
                           pCurrTrackData->pointAy[1].fX +
                       (dTempY + pCurrTrackData->pointAy[3].fX) *
                           pCurrTrackData->pointAy[0].fX +
                       (dTempZ + pCurrTrackData->pointAy[3].fZ) *
                           pCurrTrackData->pointAy[2].fX);
  *pfTargetY = (float)((dTempY + pCurrTrackData->pointAy[3].fX) *
                           pCurrTrackData->pointAy[0].fY +
                       (dTempX + pCurrTrackData->pointAy[3].fY) *
                           pCurrTrackData->pointAy[1].fY +
                       (dTempZ + pCurrTrackData->pointAy[3].fZ) *
                           pCurrTrackData->pointAy[2].fY);
  iCarLoopIdx = 0;
  ...
  if (numcars > 0) // Main loop: check all cars for collision threats
  {
    ...
    do {
      ...
      dOtherCarSpeed =
          Car[iCarIdx].fFinalSpeed * tcos[Car[iCarIdx].nActualYaw & 0x3FFF];
      TRAK_LEN = iTrakLen;
      fOtherCarSpeed = (float)dOtherCarSpeed;
      if (Car[iCarIdx].iControlType != 3 ||
          fabs((double)iNormalizedChunkDiff /
               (fOwnSpeedForward - fOtherCarSpeed)) >= 0.2f ||
          !linevalid(pCar->nCurrChunk, pCar->pos.fY, Car[iCarIdx].pos.fY)) {
        goto LABEL_67; // Check if car is AI-controlled and within collision
                       // range
      }
      ...
      if (iNormalizedChunkDiff <= 0) {
        if (iNormalizedChunkDiff >= 0) {
          ...
          fNegDistance = -(localdata[iOtherCarChunk].fTrackHalfLength -
                           Car[iCarIdx].pos.fX);
          dTempDistance =
              fNegDistance -
              (localdata[pCar->nCurrChunk].fTrackHalfLength + pCar->pos.fX);
          for (i = 1;; ++i) {
            ++iOtherCarChunk;
            ...
            dTempDistance =
                fDistance - 2.0 * localdata[iOtherCarChunk].fTrackHalfLength;
          }
          iTrakLenTemp = TRAK_LEN;
          if (fDistance > 0.0)
            fDistance = 0.0;
        ...
        iCounter = 1;
        for (fDistance = localdata[Car[iCarIdx].nCurrChunk].fTrackHalfLength +
                         Car[iCarIdx].pos.fX + fDistFromChunk;
             iCounter < iNormalizedChunkDiff;
             fDistance = (float)dDistanceCalc) {
          if (iChunkIdx >= TRAK_LEN)
            iChunkIdx -= TRAK_LEN;
          dDistanceCalc =
              2.0 * localdata[iChunkIdx].fTrackHalfLength + fDistance;
          ++iCounter;
          ++iChunkIdx;
        }
        if (fDistance < 0.0)
      ...
      if (fabs(dSpeedDiff) >= 0.0001) {
        ...
        fTimeToCollision =
            fAdjustedDistance /
            (36.0f * fSpeedDiffCalc); // Calculate time to collision based on
                                      // relative speed
      } else {
  ...
  if (pCar->iBobMode) // Return results: left/right car indices and collision
                      // times
  {
...
double interpolatesteer(float fSteeringInput, float fSaturationThreshold,
  ...
  if (dAbsInput <=
      fSaturationThreshold) // Check if input exceeds saturation threshold
  {                         // Check if input is in deadzone
    ...
    return (float)(((fAbsInput - fDeadzoneThreshold) * fMaxOutput -
                    (fAbsInput - fSaturationThreshold) * fMinOutput) /
                   (fSaturationThreshold -
                    fDeadzoneThreshold)); // Linear interpolation between
...
double avoid(int iCurrentCarIdx, int iTargetCarIdx, float fSteeringInput,
  ...
  *piOvertakeFlag = 0; // Initialize collision flag to no collision
  if (iTargetCarIdx != -1 &&
      fSteeringInput <
          fabs(fSaturationThreshold)) // Check if target car exists and steering
                                      // input is within saturation threshold
  {
    fUpperBound =
        CarBaseY + Car[iTargetCarIdx].fCarHalfWidth + 200.0f +
        Car[iTargetCarIdx].pos.fY; // Calculate upper avoidance boundary (car
                                   // position + half width + safety margin)
    fLowerBound =
        -CarBaseY - Car[iTargetCarIdx].fCarHalfWidth + -200.0f +
        Car[iTargetCarIdx].pos.fY; // Calculate lower avoidance boundary (car
                                   // position - half width - safety margin)
    if (fMaxOutput < (double)fUpperBound &&
        fMaxOutput > (double)fLowerBound) // Check if our car Y position is
                                          // within avoidance zone
    {
      driverange(&Car[iTargetCarIdx], &fMinRange,
                 &fMaxRange); // Get driving range boundaries for target car
      iDriverOffset =
          iCurrentCarIdx +
          Car[iTargetCarIdx].iDriverIdx; // Calculate combined driver index for
                                         // avoidance direction
      fCenterRange =
          (fMinRange + fMaxRange) * 0.5f; // Calculate center of driving range
      if ((iDriverOffset & 1) !=
          0) // Choose avoidance direction based on driver index parity
        dTargetY =
            Car[iTargetCarIdx].pos.fY +
            -100.0; // Odd driver index: avoid by going down (-100 offset)
      else
        dTargetY = Car[iTargetCarIdx].pos.fY +
                   100.0; // Even driver index: avoid by going up (+100 offset)
      fTargetY = (float)dTargetY;
      if (fTargetY <= (double)fCenterRange) { // Adjust upper boundary to
                                              // minimum range if needed
        if (fUpperBound > (double)fMinRange) {
          fUpperBound = fMinRange;
          if (fSteeringInput > 0.0 && Car[iCurrentCarIdx].fFinalSpeed > 80.0 &&
              Car[iCurrentCarIdx].fFinalSpeed >
                  Car[iTargetCarIdx].fFinalSpeed *
                          tcos[Car[iTargetCarIdx].nActualYaw] +
                      120.0) // Check for aggressive overtaking conditions
                             // (speed advantage)
          {
            *piOvertakeFlag = -1;
          }
        ...
        if (fMaxOutput <= (double)fUpperBound)
          return (float)interpolatesteer(
              fSteeringInput, fSaturationThreshold, fDeadzoneThreshold,
              fMaxOutput, fUpperBound); // Return interpolated steering to avoid
                                        // upper boundary
        ...
        if (fMaxOutput >= (double)fLowerBound)
          return (float)interpolatesteer(
              fSteeringInput, fSaturationThreshold, fDeadzoneThreshold,
              fMaxOutput, fLowerBound); // Return interpolated steering to avoid
                                        // lower boundary
      }
    }
  }
...
double block(int iCarIdx, float fSteeringInput, float fMaxOutput,
  ...
  if (iCarIdx == -1)
    ...
    return (float)interpolatesteer(fSteeringInput, fSaturationThreshold,
                                   fDeadzoneThreshold, fMaxOutput,
                                   Car[iCarIdx].pos.fY);
}
...
void autocar2(tCar *pCar) {
  ...
  iActionFlag = 0;
  pEngine = &CarEngines.engines[pCar->byCarDesignIdx]; // Initialize car engine
                                                       // and strategy pointers
  ...
  iCurrChunk = pCar->nCurrChunk;
  dAIMaxSpeed =
      localdata[iCurrChunk]
          .fAIMaxSpeed; // Calculate AI maximum speed based on track chunk
  //_CHP();
  ...
  if ((TrakColour[iCurrChunk][1] & SURFACE_FLAG_AI_MAX_SPEED) == 0) {
    ...
    if (!winner_mode) {
      fMaxEngineSpeed =
          fMaxEngineSpeed * levels[level] *
          0.01f; // Apply difficulty level scaling to AI maximum speed
    ...
    iCarIdx = 0;
    if (numcars >
        0) // Find human player with best race position for AI targeting
    {
      iLoopIdx = 0;
      iCarArrayIdx = 0;
      do {
    ...
    iPositionDifference =
        (char)pCar->byLapNumber * TRAK_LEN + pCar->nCurrChunk -
        (TRAK_LEN * (char)Car[iHumanCarIdx].byLapNumber +
         Car[iHumanCarIdx].nCurrChunk); // Calculate position difference between
                                        // AI car and leading human player
    if (iPositionDifference < 80) {
      if (iPositionDifference <=
          20) { // Special logic for car design 13 (boss car) - reduce speed
                // when near human player
      LABEL_18:
        if (pCar->byCarDesignIdx == 13) {
          iCurrentPosition = pCar->byRacePosition;
          if (iCurrentPosition < iHumanBestPosition &&
              iCurrentPosition + 4 >= iHumanBestPosition) {
      ...
      dSpeedAdjustment =
          ((double)(iPositionDifference - 80) - dSpeedReduction) *
          -0.016666668 * (double)iAITargetSpeed;
    } else {
...
LABEL_24:
  ...
  findnearcars(
      pCar, &iLeftCarIdx, &fLeftTime, &iRightCarIdx, &fRightTime, &fTargetX,
      &fTargetY); // Find nearby cars for collision avoidance and targeting
  iSelectedStrategy = pCar->iSelectedStrategy;
  if (pCar->fHealth >=
      30.0) // AI strategy selection based on car health and nearby cars
  {
    if (iRightCarIdx == -1 || fRightTime < 0.0) {
      if (iLeftCarIdx != -1 && fLeftTime >= 0.0) {
        if (Car[iLeftCarIdx].fHealth < 25.0 && !finished_car[iLeftCarIdx])
          iSelectedStrategy =
              Car[iLeftCarIdx].fHealth > 0.0 &&
              ((uint8)iRightCarIdx & (pCar->iDriverIdx == 65534) & 0xFE) == 0 &&
              pCar->fHealth >= 40.0;
        if (human_control[iLeftCarIdx] &&
            ((cheat_mode & 2) != 0 || pCar->byCarDesignIdx == 13) &&
            Car[iLeftCarIdx].fHealth > 0.0 && !finished_car[iLeftCarIdx]) {
          iSelectedStrategy = 1;
      ...
      if (Car[iRightCarIdx].fHealth < 25.0 && !finished_car[iRightCarIdx]) {
        if (Car[iRightCarIdx].fHealth <= 0.0 ||
            ((uint8)iRightCarIdx & (pCar->iDriverIdx == 65534) & 0xFE) != 0 ||
            pCar->fHealth < 40.0)
          iSelectedStrategy = 0;
        else
      ...
      if (human_control[iRightCarIdx] &&
          ((cheat_mode & 2) != 0 || pCar->byCarDesignIdx == 13) &&
          Car[iRightCarIdx].fHealth > 0.0 && !finished_car[iRightCarIdx]) {
        iSelectedStrategy = 3;
  ...
  if (iRightCarIdx != -1 && iSelectedStrategy == 3 &&
      Car[iRightCarIdx].byDebugDamage)
    iSelectedStrategy = 0;
  if (iLeftCarIdx != -1 && Car[iLeftCarIdx].byDebugDamage &&
      iSelectedStrategy <= 3)
    iSelectedStrategy = 0;
  if (iLeftCarIdx != -1 && pCar->byRacePosition < 4u &&
      pCar->byRacePosition - Car[iLeftCarIdx].byRacePosition > 2 &&
      !human_control[iLeftCarIdx])
    iSelectedStrategy = 0;
  if (iRightCarIdx != -1) {
    byCurrentRacePos = pCar->byRacePosition;
    // left car check added by ROLLER, todo look at asm and see if correct
    if (byCurrentRacePos < 4u && iLeftCarIdx != -1 &&
        byCurrentRacePos - Car[iLeftCarIdx].byRacePosition > 2 &&
        !human_control[iLeftCarIdx])
      iSelectedStrategy = 0;
  }
  if ((TrakColour[pCar->nCurrChunk][1] & SURFACE_FLAG_AI_FAST_STRAT) ==
      0) // Check if car is in not in go fast zone
  ...
  if (!iPitZoneFlag || pCar->fHealth >= 60.0 || iLastLapFlag) {
    ...
    iRandomPitStop =
        ROLLERrandRaw(); // Select random pit stop target for AI car
    iPitStopTaken = 0;
  ...
  if (iPitZoneFlag && !pCar->iAICurrentLine && !iLastLapFlag) {
    ...
    if (pCar->fHealth <
        90.0) // Force pit stop strategy (5) if health is low and in pit zone
      iSelectedStrategy = 5;
  }
  ...
  if (iSelectedStrategy == 5) {
    ...
    if (iChunkForPitStop != -1) {
      if (iPitZoneFlag) {
        ...
        if (iPrevChunk < 0)
          iPrevChunk = TRAK_LEN - 1;
        if (!pCar->byPitLaneActiveFlag &&
            (TrakColour[iPrevChunk][1] & SURFACE_FLAG_PIT_ZONE) == 0) {
          pCar->byPitLaneActiveFlag = -1;
          iPlayerType = player_type;
          pCar->nChangeMateCooldown = 1080;
          if (iPlayerType != 2 && (cheat_mode & 0x4000) == 0 &&
              player1_car != pCar->iDriverIdx &&
              pCar->byCarDesignIdx == Car[player1_car].byCarDesignIdx &&
              pCar->byCarDesignIdx <= 7u && readsample == writesample &&
              lastsample < -18) {
            speechsample(pCar->byCarDesignIdx + 71, 20000, 18,
                         player1_car + 17920);
            speechsample(SOUND_SAMPLE_TPITS, 20000, 0,
                         player1_car); // SOUND_SAMPLE_TPITS
          }
        }
      }
    }
  ...
  case 1:
    ...
    if (iLeftCarIdx == pCar->iTrackedCarIdx && fLeftTime < 0.0)
      goto LABEL_178;
    if (iLeftCarIdx >= 0 &&
        pCar->byCarDesignIdx == Car[iLeftCarIdx].byCarDesignIdx)
      goto LABEL_154;
    ...
    if (iLeftCarIdx == -1)
      ...
      fCalculatedY =
          (float)interpolatesteer(fLeftTime, fStrategyParam1, fStrategyParam2,
                                  fTargetY, Car[iLeftCarIdx].pos.fY);
  ...
  case 2:
    if (iRightCarIdx == -1 || fRightTime < 0.0) {
      if (iLeftCarIdx == pCar->iTrackedCarIdx && fLeftTime < 0.0) {
      LABEL_178:
        fTargetSteerY = (float)avoid(pCar->iDriverIdx, iLeftCarIdx, fLeftTime,
                                     fTargetY, pStrategy->fAvoidSensitivity,
                                     pStrategy->fAvoidReaction, &iActionFlag);
      } else {
        ...
        if (iLeftCarIdx == -1)
          ...
          fDefensiveY =
              (float)interpolatesteer(fLeftTime, fStrategyA0_3, fStrategyA1,
                                      fTargetY, Car[iLeftCarIdx].pos.fY);
    ...
    LABEL_175:
      dAvoidanceResult = avoid(pCar->iDriverIdx, iRightCarIdx, fRightTime,
                               fTargetY, pStrategy->fAvoidSensitivity,
                               pStrategy->fAvoidReaction, &iActionFlag);
  ...
  case 3:
    pCar->iTrackedCarIdx = -1;
    if (iRightCarIdx == -1 || fRightTime < 0.0 ||
        fRightTime > (double)fLeftTime && fLeftTime >= 0.0 ||
        pCar->byCarDesignIdx == Car[iRightCarIdx].byCarDesignIdx) {
      if (iLeftCarIdx == -1 || fLeftTime < 0.0 ||
          fLeftTime > (double)fRightTime && fRightTime >= 0.0 ||
          pCar->byCarDesignIdx == Car[iLeftCarIdx].byCarDesignIdx) {
      LABEL_155:
        fInterpolatedY = fTargetY;
      } else {
        fSteerSensitivity = pStrategy->fSteerSensitivity;
        fLeftInterpolated = (float)interpolatesteer(
            fLeftTime, fSteerSensitivity, pStrategy->fSteerDamping, fTargetY,
            Car[iLeftCarIdx].pos.fY);
    ...
    LABEL_156:
      ...
      fRightInterpolated = (float)interpolatesteer(
          fRightTime, fSteerSensitivity_1, pStrategy->fSteerDamping, fTargetY,
          Car[iRightCarIdx].pos.fY);
  ...
  case 5:
    if (iRightCarIdx == -1 || fRightTime < 0.0) {
      ...
      dAvoidanceY = avoid(pCar->iDriverIdx, iRightCarIdx, fRightTime, fTargetY,
                          pStrategy->fAvoidSensitivity,
                          pStrategy->fAvoidReaction, &iActionFlag);
      pCar->iTrackedCarIdx = iRightCarIdx;
    ...
    if (iAITargetCar != -1) {
      ...
      fTrackDistance = localdata[pCar->nCurrChunk].fTrackHalfLength * 2.0f *
                       (float)iDistanceToTarget;
      if (iDistanceToTarget < 0 ||
          fabs(fTrackDistance / fForwardSpeed) <
              fDragCoefficient * 36.0 * fForwardSpeed / fMaxEngineSpeed)
        iActionFlag = -1;
    }
    // if ((LODWORD(pCar->fFinalSpeed) & 0x7FFFFFFF) == 0)
    if (fabs(pCar->fFinalSpeed) == 0)
      pCar->byDebugDamage = -1;
    if ((TrakColour[pCar->nCurrChunk][pCar->iLaneType] & SURFACE_FLAG_PIT) ==
            0 &&
        pCar->fFinalSpeed < 40.0)
      iActionFlag = 0;
    break;
  default:
    break; // AI strategy selection: 0=Normal, 1=Aggressive, 2=Defensive,
           // 3=Attack, 4=Fast, 5=Pits
  }
  if (pCar->byDebugDamage)
    iActionFlag = -1;
  if (iActionFlag)
  ...
  fDeltaXCopy = fDeltaX;
  fDeltaYCopy = fDeltaY;
  // if ((LODWORD(fDeltaX) & 0x7FFFFFFF) != 0 || (LODWORD(fDeltaY) & 0x7FFFFFFF)
  // != 0) {
  if (fabs(fDeltaX) > FLT_EPSILON || fabs(fDeltaY) > FLT_EPSILON) {
    dCalculatedAngle =
        atan2(fDeltaYCopy, fDeltaXCopy) * 16384.0 /
        6.28318530718; // Calculate steering angle to target position
    //_CHP();
  ...
  iSteeringSensitivity =
      ((int16)iTargetAngle - pCar->nYaw) &
      0x3FFF; // Apply steering input with engine sensitivity limits
  ...
  if ((char)pCar->byGearAyMax ==
      -1) // AI gear shifting and RPM control based on engine state
  {
  ...
  if (champ_mode && !champ_go[pCar->iDriverIdx])
    iAIThrottleControl = -1;
  if (iAIThrottleControl <= 0) // Apply acceleration, deceleration, or
                               // freewheeling based on AI decision
  {
...
void changestrategy(tCar *pCar) {
  ...
  pStrategyData =
      &CarStrategy[pCar->iDriverIdx]; // Get strategy data for this driver (40
                                      // bytes per driver)
  if (pCar->nChangeMateCooldown)      // Skip strategy change if cooldown timer
                                      // active
    return;
  iOldStrategy = pCar->iSelectedStrategy;
  if (pCar->fTotalRaceTime >
      40.0) // Only allow strategy changes after 40 seconds of race time
  {
    ...
    if (GetHighOrderRand(800, iRandStrategy) == 100) // 1 in 800 chance
    {
      ...
      iSelectedStrategy = 0;
      iProbabilityRemaining = GetHighOrderRand(
          100, iRandProbability); // Select strategy based on probability table
                                  // (0-99 random)
      for (piStrategyIterator = (int *)pStrategyData;
           iProbabilityRemaining >= *piStrategyIterator; ++iSelectedStrategy) {
        iCurrentProbability = *piStrategyIterator++;
        iProbabilityRemaining -= iCurrentProbability;
      }
      if (pCar->byRacePosition < 4u &&
          iSelectedStrategy == 2) // Front runners (positions 0-3) cannot use
                                  // strategy 2, downgrade to 1
        iSelectedStrategy = 1;
      pCar->iSelectedStrategy = iSelectedStrategy;
      if (iSelectedStrategy) {
        iRandFlip = ROLLERrandRaw();
        if (GetHighOrderRand(100, iRandFlip) <
            flipst[level]) // Chance to flip aggressive strategy back to normal
                           // based on level
          pCar->iSelectedStrategy = 0;
      }
      // CHEAT_MODE_DEATH_MODE
      if ((cheat_mode & CHEAT_MODE_DEATH_MODE) !=
          0) // Death mode forces normal strategy (no aggression)
        pCar->iSelectedStrategy = 0;
    }
    ...
    if ((cheat_mode & CHEAT_MODE_DEATH_MODE) != 0 || level < 4) {
      ...
      if (uiRacePosition >= 7) {
        ...
        if ((cheat_mode & CHEAT_MODE_DEATH_MODE) != 0)
          iRevengeProbability =
              50 * uiRacePosition -
              250; // Death mode: simple position-based revenge (50 * pos - 250)
        else
          iRevengeProbability =
              (int)(pStrategyData->strategyAy[4] * (uiRacePosition - level)) /
                  2 +
              uiRacePosition - level; // Normal mode: complex revenge based on
                                      // strategy data and position
      } else {
        iRevengeProbability = -1;
      }
      iRandRevengeCheck = ROLLERrandRaw();
      if (GetHighOrderRand(1000, iRandRevengeCheck) <
          iRevengeProbability) { // Trigger revenge speech if same car design as
                                 // player (not in clones mode)
        // CHEAT_MODE_CLONES
        if (player_type != 2 && (cheat_mode & CHEAT_MODE_CLONES) == 0 &&
            !pCar->iBobMode &&
            pCar->byCarDesignIdx == Car[player1_car].byCarDesignIdx) {
          byCarDesignIdx = pCar->byCarDesignIdx;
          if (byCarDesignIdx <= 7u) {
            speechsample(byCarDesignIdx + 71, 20000, 18, player1_car + 17664);
            speechsample(SOUND_SAMPLE_REVERST, 20000, 0,
                         player1_car); // SOUND_SAMPLE_REVERST
          }
        }
      ...
      if (GetHighOrderRand(1000, iRandRevengeAlt) <
          pStrategyData->strategyAy[4]) // Advanced levels (4+): use strategy
                                        // data for revenge probability
      {
        // CHEAT_MODE_CLONES
        if (player_type != 2 && (cheat_mode & CHEAT_MODE_CLONES) == 0 &&
            !pCar->iBobMode &&
  ...
  if (iOldStrategy != pCar->iSelectedStrategy) // If strategy changed, play
                                               // speech and set cooldown timer
  {
    // CHEAT_MODE_CLONES
    if (player_type != 2 && (cheat_mode & 0x4000) == 0 &&
        pCar->byCarDesignIdx == Car[player1_car].byCarDesignIdx) {
      byCarDesignCopy = pCar->byCarDesignIdx;
      if (byCarDesignCopy <= 7u) {
        speechsample(byCarDesignCopy + 71, 20000, 18,
                     ((pCar->iSelectedStrategy + 61) << 8) + player1_car);
        speechsample(pCar->iSelectedStrategy + 61, 20000, 0, player1_car);
      }
    }
    pCar->nChangeMateCooldown =
        1080; // Set 1080 frame cooldown (18 seconds at 60 FPS)
  }
}
...
int getangle(float fX, float fY) {
  ...
  // return value is used as an index into tsin/tcos/ptan lookup tables each
  // with 16384 elements masking off all but the 14 least significant bits
  // ensures this value maxes out at 16383
...
void landontrack(tCar *pCar) {
  ...
  fX = pCar->posLastFrame.fX; // Store car's current and previous positions for
  ...
  findnearsection(
      pCar,
      &piNearestChunk); // Find nearest track section for collision detection
  if (piNearestChunk == -1)
  ...
  if (GroundColour[iChunk][2] >=
      0) // Handle separated coordinate system for underground sections
  ...
  dTransformedZ =
      fCarPosZ +
      pCurrentData->pointAy[3].fZ; // Transform car position and direction to
                                   // track local coordinates
  fLocalCoordX = (float)(pCurrentData->pointAy[1].fX * dTransformedY +
                         pCurrentData->pointAy[0].fX * dTransformedX +
                         pCurrentData->pointAy[2].fX * dTransformedZ);
  fLocalCoordY = (float)(pCurrentData->pointAy[0].fY * dTransformedX +
                         pCurrentData->pointAy[1].fY * dTransformedY +
                         pCurrentData->pointAy[2].fY * dTransformedZ);
  fLocalCoordZ = (float)(dTransformedY * pCurrentData->pointAy[1].fZ +
                         dTransformedX * pCurrentData->pointAy[0].fZ +
                         dTransformedZ * pCurrentData->pointAy[2].fZ);
  fPrevZPosition =
      (fX + pCurrentData->pointAy[3].fX) * pCurrentData->pointAy[0].fZ +
      (fY + pCurrentData->pointAy[3].fY) * pCurrentData->pointAy[1].fZ +
      (fZ + pCurrentData->pointAy[3].fZ) * pCurrentData->pointAy[2].fZ;
  fDirectionXLocal = pCurrentData->pointAy[1].fX * pCar->direction.fY +
                     pCurrentData->pointAy[0].fX * pCar->direction.fX +
                     pCurrentData->pointAy[2].fX * pCar->direction.fZ;
  fDirectionYLocal = pCurrentData->pointAy[0].fY * pCar->direction.fX +
                     pCurrentData->pointAy[1].fY * pCar->direction.fY +
                     pCurrentData->pointAy[2].fY * pCar->direction.fZ;
  fDirectionZLocal = pCurrentData->pointAy[0].fZ * pCar->direction.fX +
                     pCurrentData->pointAy[1].fZ * pCar->direction.fY +
                     pCurrentData->pointAy[2].fZ * pCar->direction.fZ;
  if (fLocalCoordY < 0.0) {
    if (TrakColour[iChunk][4]) {
      fRoofHeight =
          pTrackInfo->fRoofHeight; // Handle collision with left wall/barrier
      ...
      if (bRoofCheck && fRoofHeight > 0.0) {
        dWallPosition =
            TrakColour[iChunk][4] < 0
                ? -pCurrentData->fTrackHalfWidth
                : -pCurrentData->fTrackHalfWidth - pTrackInfo->fRShoulderWidth;
        fWallBoundary = (float)dWallPosition;
        if (fLocalCoordY - CarBaseY <= fWallBoundary) {
          if (fDirectionYLocal < 0.0) {
            if (death_race)
              dDamageLeft =
                  -fDirectionYLocal * 0.006 *
                  4.0; // Apply wall collision damage (4x in death race mode)
            else
          ...
          if (fDirectionYLocal > 40.0 && !pCar->bySfxCooldown) {
            ...
            sfxpend(SOUND_SAMPLE_WALL1, pCar->iDriverIdx,
                    (int)dVolumeCalc); // SOUND_SAMPLE_WALL1
          ...
          pCar->direction.fX =
              (float)(pCurrentData->pointAy[0].fY * fDirectionYLocal +
                      pCurrentData->pointAy[0].fX * fDirectionXLocal +
                      pCurrentData->pointAy[0].fZ * fDirectionZLocal);
          pCar->direction.fY =
              (float)(pCurrentData->pointAy[1].fX * dVelocityX +
                      pCurrentData->pointAy[1].fY * dVelocityY +
                      pCurrentData->pointAy[1].fZ * dVelocityZ);
          pCar->direction.fZ =
              (float)(dVelocityY * pCurrentData->pointAy[2].fY +
                      dVelocityX * pCurrentData->pointAy[2].fX +
                      dVelocityZ * pCurrentData->pointAy[2].fZ);
          ...
          // if ((LODWORD(fDirX) & 0x7FFFFFFF) != 0 ||
          // (LODWORD(fDirectionYTemp2) & 0x7FFFFFFF) != 0) {
          if (fabs(fDirX) > FLT_EPSILON ||
              fabs(fDirectionYTemp2) > FLT_EPSILON) {
            dAngle2 = atan2(fDirectionTemp3, fDirectionTemp1) * 16384.0 /
                      6.28318530718;
            //_CHP();
  ...
  } else if (TrakColour[iChunk][3]) // Handle collision with right wall/barrier
                                    // (negative Y direction)
  {
    fRoofHeightCheck = pTrackInfo->fRoofHeight;
    if (fRoofHeightCheck < 0.0)
      bRoofCollision =
          fLocalCoordZ >= (double)fRoofHeightCheck && fLocalCoordZ <= 0.0;
    else
      bRoofCollision =
          fLocalCoordZ <= (double)fRoofHeightCheck && fLocalCoordZ >= 0.0;
    if (bRoofCollision && fRoofHeightCheck > 0.0) {
      if (TrakColour[iChunk][3] < 0) {
        ...
        fTrackHalfWidth =
            pCurrentData->fTrackHalfWidth + pTrackInfo->fLShoulderWidth;
        fTrackHalfWidthNext =
            pNextData->fTrackHalfWidth + pNextTrackInfo->fLShoulderWidth;
      }
      fInterpolatedBoundary =
          ((fLocalCoordX + pCurrentData->fTrackHalfLength) *
               fTrackHalfWidthNext -
           (fLocalCoordX - pCurrentData->fTrackHalfLength) * fTrackHalfWidth) /
          (pCurrentData->fTrackHalfLength * 2.0f);
      if (fLocalCoordY + CarBaseY >= fInterpolatedBoundary) {
        ...
        if (dDamageAmount > 40.0 && !pCar->bySfxCooldown) {
          ...
          sfxpend(SOUND_SAMPLE_WALL1, pCar->iDriverIdx,
                  (int)dSoundVolume); // SOUND_SAMPLE_WALL1
          pCar->bySfxCooldown = 9;
        }
        ...
        pCar->direction.fX =
            (float)(pCurrentData->pointAy[0].fY * fDirectionYLocal +
                    pCurrentData->pointAy[0].fX * fDirectionXLocal +
                    pCurrentData->pointAy[0].fZ * fDirectionZLocal);
        pCar->direction.fY = (float)(pCurrentData->pointAy[1].fX * dLocalX +
                                     pCurrentData->pointAy[1].fY * dLocalY +
                                     pCurrentData->pointAy[1].fZ * dLocalZ);
        pCar->direction.fZ = (float)(dLocalY * pCurrentData->pointAy[2].fY +
                                     dLocalX * pCurrentData->pointAy[2].fX +
                                     dLocalZ * pCurrentData->pointAy[2].fZ);
        ...
        pCar->pos.fX = pCurrentData->pointAy[0].fY * fLocalCoordY +
                       ...
                       pCurrentData->pointAy[0].fZ * fLocalCoordZ -
                       pCurrentData->pointAy[3].fX;
        pCar->pos.fY = (float)(pCurrentData->pointAy[1].fX * dTempX +
                               pCurrentData->pointAy[1].fY * fLocalCoordY +
                               pCurrentData->pointAy[1].fZ * dTempZ -
                               pCurrentData->pointAy[3].fY);
        dPositionZ = dTempZ * pCurrentData->pointAy[2].fZ +
                     dTempX * pCurrentData->pointAy[2].fX +
                     pCurrentData->pointAy[2].fY * fLocalCoordY -
                     pCurrentData->pointAy[3].fZ;
        fTempCalc = pCar->direction.fX;
        ...
        // if ((LODWORD(fDirectionX) & 0x7FFFFFFF) != 0 ||
        // (LODWORD(fDirectionYTemp1) & 0x7FFFFFFF) != 0) {
        if (fabs(fDirectionX) > FLT_EPSILON ||
            fabs(fDirectionYTemp1) > FLT_EPSILON) {
          dAngle1 =
              atan2(fAngleCalcTemp6, fAngleCalcTemp5) * 16384.0 / 6.28318530718;
          //_CHP();
      ...
      LABEL_69:
        dHorizontalSpeed = pCar->direction.fX * pCar->direction.fX +
                           pCar->direction.fY * pCar->direction.fY;
        pCar->nActualYaw = iChunkIdx;
        pCar->fHorizontalSpeed = (float)sqrt(dHorizontalSpeed);
      }
    }
  }
  if (pCurrentData->fTrackHalfWidth + pTrackInfo->fLShoulderWidth >=
      fLocalCoordY) {
    dNegTrackWidth =
        -pCurrentData->fTrackHalfWidth; // Check if car is within track
                                        // boundaries for ground collision
    ...
    if (dNegTrackWidth - pTrackInfo->fRShoulderWidth <= fLocalCoordY) {
      ...
      // if ((((HIDWORD(llTrackColor) ^ (unsigned int)llTrackColor) -
      // HIDWORD(llTrackColor)) & 0x20000) == 0
      if ((TrakColour[iChunk][iGroundType] & SURFACE_FLAG_SKIP_RENDER) == 0 ||
        ...
        iChunkIndex = iChunk;
        fGroundHeight = (float)getgroundz(
            fLocalCoordX, fLocalCoordY,
            iChunk); // Handle normal ground collision - calculate ground height
                     // and landing physics
        if (fLocalCoordZ <= (double)fGroundHeight &&
            fDirectionZLocal * 2.0 + fGroundHeight + -250.0 <= fLocalCoordZ) {
          pCar->pos.fX = fLocalCoordX;
          pCar->pos.fY = fLocalCoordY;
          getlocalangles(pCar->nYaw, pCar->nPitch, pCar->nRoll, iChunkIndex,
                         &iAzimuth, &iElevation, &iBank);
          pCar->nYaw = iAzimuth;
          ...
          // if ((LODWORD(fTransformX) & 0x7FFFFFFF) != 0 ||
          // (LODWORD(fDirectionYLocal) & 0x7FFFFFFF) != 0) {
          if (fabs(fTransformX) > FLT_EPSILON ||
              fabs(fDirectionYLocal) > FLT_EPSILON) {
            dAngle3 = atan2(fDirectionYLocal, fDirectionXLocal) * 16384.0 /
                      6.28318530718;
            //_CHP();
          ...
          if (iStunned) {
            ...
            sfxpend(SOUND_SAMPLE_ROOF, pCar->iDriverIdx,
                    (int)dStunnedVolume); // SOUND_SAMPLE_ROOF
            ...
            if (fHeightDiffCalc <= 40.0) {
              ...
              //_CHP();                           // Play appropriate landing
              // sound effect based on impact severity LODWORD(fTempCalc) =
              // (int)dImpactVolume;
          ...
          fTempDamage3 = (float)fabs((fPrevZPosition - fLocalCoordZ) *
                                     tsin[iElevation & 0x3FFF] *
                                     0.05); // Calculate landing damage based on
                                            // impact angle and velocity
          dodamage(pCar, fTempDamage3);
          fThrottle = (float)sqrt(fDirectionXLocal * fDirectionXLocal +
                                  fDirectionYLocal * fDirectionYLocal);
  ...
  if (pCar->nCurrChunk == -1) {
    iGroundColorType =
        GroundColour[iChunk][2]; // Handle underground section collision
                                 // detection when car is below track level
    if (iGroundColorType != -1) {
      if (iGroundColorType >= 0) {
        ...
        fLocalCoordX = (float)(data.pointAy[1].fX * dSeparatedY +
                               data.pointAy[0].fX * dSeparatedX +
                               data.pointAy[2].fX * dSeparatedZ);
        fLocalCoordY = (float)(data.pointAy[0].fY * dSeparatedX +
                               data.pointAy[1].fY * dSeparatedY +
                               data.pointAy[2].fY * dSeparatedZ);
        pCurrentData = &data;
        fLocalCoordZ = (float)(dSeparatedY * data.pointAy[1].fZ +
                               dSeparatedX * data.pointAy[0].fZ +
                               dSeparatedZ * data.pointAy[2].fZ);
      }
      if ((GroundColour[iChunk][2] < 0 || fLocalCoordY < 0.0) &&
          (GroundColour[iChunk][2] >= 0 || pCurrentData->fTrackHalfWidth +
                                                   pTrackInfo->fLShoulderWidth -
                                                   CarBaseY >
                                               fLocalCoordY)) {
        if (GroundColour[iChunk][2] >= 0) {
          ...
          fLocalCoordX = (float)(data.pointAy[1].fX * dSeparatedY2 +
                                 data.pointAy[0].fX * dSeparatedX2 +
                                 data.pointAy[2].fX * dSeparatedZ2);
          fLocalCoordY = (float)(data.pointAy[0].fY * dSeparatedX2 +
                                 data.pointAy[1].fY * dSeparatedY2 +
                                 data.pointAy[2].fY * dSeparatedZ2);
          pCurrentData = &data;
          fLocalCoordZ = (float)(dSeparatedY2 * data.pointAy[1].fZ +
                                 dSeparatedX2 * data.pointAy[0].fZ +
                                 dSeparatedZ2 * data.pointAy[2].fZ);
        }
        if (GroundColour[iChunk][2] >= 0 && fLocalCoordY < 0.0 ||
            GroundColour[iChunk][2] < 0 && -pCurrentData->fTrackHalfWidth -
                                                   pTrackInfo->fRShoulderWidth +
          ...
          fGroundYInterp1 = (float)(pCurrentData->pointAy[1].fY * dGroundY4 +
                                    pCurrentData->pointAy[0].fY * dGroundX4 +
                                    pCurrentData->pointAy[2].fY * dGroundZ4);
          fGroundZInterp5 = (float)(dGroundY4 * pCurrentData->pointAy[1].fZ +
                                    dGroundX4 * pCurrentData->pointAy[0].fZ +
                                    dGroundZ4 * pCurrentData->pointAy[2].fZ);
          fHeightDiffUnder = fLocalCoordZ - fGroundZInterp5;
          if (fLocalCoordZ <= (double)fGroundZInterp5) {
            dGroundY6 =
                pGroundPts2->pointAy[3].fY + pCurrentData->pointAy[3].fY;
            dGroundX6 =
                pGroundPts2->pointAy[3].fX + pCurrentData->pointAy[3].fX;
            dGroundZ6 =
                pGroundPts2->pointAy[3].fZ + pCurrentData->pointAy[3].fZ;
            fGroundYCalc4 = (float)(pCurrentData->pointAy[1].fY * dGroundY6 +
                                    pCurrentData->pointAy[0].fY * dGroundX6 +
                                    pCurrentData->pointAy[2].fY * dGroundZ6);
            fGroundZInterp1 = (float)(dGroundY6 * pCurrentData->pointAy[1].fZ +
                                      dGroundX6 * pCurrentData->pointAy[0].fZ +
                                      dGroundZ6 * pCurrentData->pointAy[2].fZ);
            ...
            if (fabs(dSlopeDiff6) <= 10.0)
              ...
              fInterpolatedHeight1 = fHeightDiffUnder * fGroundYCalc4 /
                                         (fGroundZInterp1 - fGroundZInterp5) +
                                     (fLocalCoordZ - fGroundZInterp1) *
                                         fGroundYInterp1 / fSlopeDiffCalc4;
            if (fLocalCoordY - CarBaseY < fInterpolatedHeight1) {
              fDirectionXTemp4 =
                  pCurrentData->pointAy[1].fX * pCar->direction.fY +
                  pCurrentData->pointAy[0].fX * pCar->direction.fX +
                  pCurrentData->pointAy[2].fX * pCar->direction.fZ;
              fDirectionYLocal4 =
                  pCurrentData->pointAy[0].fY * pCar->direction.fX +
                  pCurrentData->pointAy[1].fY * pCar->direction.fY +
                  pCurrentData->pointAy[2].fY * pCar->direction.fZ;
              fDirectionZTemp2 =
                  pCurrentData->pointAy[0].fZ * pCar->direction.fX +
                  pCurrentData->pointAy[1].fZ * pCar->direction.fY +
                  pCurrentData->pointAy[2].fZ * pCar->direction.fZ;
              ...
              if (fDirectionYLocal4 > 40.0 && !pCar->bySfxCooldown) {
                ...
                sfxpend(SOUND_SAMPLE_BANK, pCar->iDriverIdx,
                        (int)dBankSoundVolume6); // SOUND_SAMPLE_BANK
                pCar->bySfxCooldown = 9;
              }
              ...
              pCar->pos.fX =
                  (float)(pCurrentData->pointAy[0].fY * dInterpolatedY3 +
                          pCurrentData->pointAy[0].fX * fLocalCoordX +
                          pCurrentData->pointAy[0].fZ * fLocalCoordZ -
                          pCurrentData->pointAy[3].fX);
              pCar->pos.fY =
                  (float)(pCurrentData->pointAy[1].fX * dInterpolatedX3 +
                          pCurrentData->pointAy[1].fY * dInterpolatedY3 +
                          pCurrentData->pointAy[1].fZ * dInterpolatedZ3 -
                          pCurrentData->pointAy[3].fY);
              pCar->pos.fZ =
                  (float)(dInterpolatedY3 * pCurrentData->pointAy[2].fY +
              ...
              pCar->direction.fX =
                  pCurrentData->pointAy[0].fY * fDirectionYLocal4 +
                  pCurrentData->pointAy[0].fX * fDirectionXTemp4 +
                  pCurrentData->pointAy[0].fZ * fDirectionZTemp2;
              pCar->direction.fY =
                  (float)(pCurrentData->pointAy[1].fX * dDirectionX5 +
                          pCurrentData->pointAy[1].fY * dDirectionY5 +
                          pCurrentData->pointAy[1].fZ * dDirectionZ5);
              dFinalDirectionZ5 = dDirectionY5 * pCurrentData->pointAy[2].fY +
                                  dDirectionX5 * pCurrentData->pointAy[2].fX +
                                  dDirectionZ5 * pCurrentData->pointAy[2].fZ;
              fTempCalc = pCar->direction.fX;
              ...
              // if ((LODWORD(fDirXFinal6) & 0x7FFFFFFF) != 0 ||
              // (LODWORD(fDirectionYTemp4) & 0x7FFFFFFF) != 0) {
              if (fabs(fDirXFinal6) > FLT_EPSILON ||
                  fabs(fDirectionYTemp4) > FLT_EPSILON) {
                dAngle7 = atan2(fDirectionYFinal, fAngleCalcTemp1) * 16384.0 /
                          6.28318530718;
                //_CHP();
            ...
            dGroundY5 =
                pGroundPts2->pointAy[5].fY + pCurrentData->pointAy[3].fY;
            dGroundX5 =
                pGroundPts2->pointAy[5].fX + pCurrentData->pointAy[3].fX;
            dGroundZ5 =
                pGroundPts2->pointAy[5].fZ + pCurrentData->pointAy[3].fZ;
            fGroundYCalc1 = (float)(pCurrentData->pointAy[1].fY * dGroundY5 +
                                    pCurrentData->pointAy[0].fY * dGroundX5 +
                                    pCurrentData->pointAy[2].fY * dGroundZ5);
            fGroundZInterp3 = (float)(dGroundY5 * pCurrentData->pointAy[1].fZ +
                                      dGroundX5 * pCurrentData->pointAy[0].fZ +
                                      dGroundZ5 * pCurrentData->pointAy[2].fZ);
            dSlopeDiff5 = fGroundZInterp5 - fGroundZInterp3;
            fSlopeDiffCalc3 = (float)dSlopeDiff5;
            fInterpolatedHeight2 =
                fabs(dSlopeDiff5) <= 10.0
                    ? -1000000.0f
                    : fHeightDiffUnder * fGroundYCalc1 /
                              (fGroundZInterp3 - fGroundZInterp5) +
                          (fLocalCoordZ - fGroundZInterp3) * fGroundYInterp1 /
                              fSlopeDiffCalc3;
            if (fLocalCoordY - CarBaseY < fInterpolatedHeight2) {
              ...
              if (fDirectionYLocal3 > 40.0 && !pCar->bySfxCooldown) {
                ...
                sfxpend(SOUND_SAMPLE_BANK, pCar->iDriverIdx,
                        (int)dBankSoundVolume5); // SOUND_SAMPLE_BANK
                pCar->bySfxCooldown = 9;
              }
              ...
              pCar->pos.fX =
                  (float)(pCurrentData->pointAy[0].fY * dGroundY3 +
                          pCurrentData->pointAy[0].fX * fLocalCoordX +
                          pCurrentData->pointAy[0].fZ * fLocalCoordZ -
                          pCurrentData->pointAy[3].fX);
              ...
              pCar->direction.fX =
                  pCurrentData->pointAy[0].fY * fDirectionYLocal3 +
                  pCurrentData->pointAy[0].fX * fDirectionXTemp3 +
                  pCurrentData->pointAy[0].fZ * fDirectionZTemp4;
              pCar->direction.fY =
                  (float)(pCurrentData->pointAy[1].fX * dDirectionX4 +
                          pCurrentData->pointAy[1].fY * dDirectionY4 +
                          pCurrentData->pointAy[1].fZ * dDirectionZ4);
              dFinalDirectionZ4 = dDirectionY4 * pCurrentData->pointAy[2].fY +
                                  dDirectionX4 * pCurrentData->pointAy[2].fX +
                                  dDirectionZ4 * pCurrentData->pointAy[2].fZ;
              fTempCalc = pCar->direction.fX;
              ...
              // if ((LODWORD(fDirXFinal5) & 0x7FFFFFFF) != 0 ||
              // (LODWORD(fDirectionYTemp6) & 0x7FFFFFFF) != 0) {
              if (fabs(fDirXFinal5) > FLT_EPSILON ||
                  fabs(fDirectionYTemp6) > FLT_EPSILON) {
                dAngle6 = atan2(fDirectionTemp2, fAngleCalcTemp3) * 16384.0 /
                          6.28318530718;
                //_CHP();
        ...
        fGroundYInterp2 = (float)(pCurrentData->pointAy[1].fY * dGroundY1 +
                                  pCurrentData->pointAy[0].fY * dGroundX1 +
                                  pCurrentData->pointAy[2].fY * dGroundZ1);
        fGroundYInterp3 = (float)(dGroundY1 * pCurrentData->pointAy[1].fZ +
                                  dGroundX1 * pCurrentData->pointAy[0].fZ +
                                  dGroundZ1 * pCurrentData->pointAy[2].fZ);
        ...
        if (fLocalCoordZ <= (double)fGroundYInterp3) {
          ...
          fGroundYCalc3 = (float)(pCurrentData->pointAy[1].fY * dGroundY2 +
                                  pCurrentData->pointAy[0].fY * dGroundX2 +
                                  pCurrentData->pointAy[2].fY * dGroundZ2);
          fGroundZInterp2 = (float)(dGroundY2 * pCurrentData->pointAy[1].fZ +
                                    dGroundX2 * pCurrentData->pointAy[0].fZ +
                                    dGroundZ2 * pCurrentData->pointAy[2].fZ);
          ...
          if (fabs(dSlopeDiff2) <= 10.0)
            ...
            fInterpolatedHeight4 = fHeightDiffGround * fGroundYCalc3 /
                                       (fGroundZInterp2 - fGroundYInterp3) +
                                   (fLocalCoordZ - fGroundZInterp2) *
                                       fGroundYInterp2 / fSlopeDiffCalc1;
          if (fLocalCoordY + CarBaseY > fInterpolatedHeight4) {
            ...
            if (dNegSpeed2 > 40.0 && !pCar->bySfxCooldown) {
              ...
              sfxpend(SOUND_SAMPLE_BANK, pCar->iDriverIdx,
                      (int)dBankSoundVolume2); // SOUND_SAMPLE_BANK
              pCar->bySfxCooldown = 9;
            }
            ...
            pCar->pos.fX =
                (float)(pCurrentData->pointAy[0].fY * dInterpolatedY2 +
                        pCurrentData->pointAy[0].fX * fLocalCoordX +
                        pCurrentData->pointAy[0].fZ * fLocalCoordZ -
                        pCurrentData->pointAy[3].fX);
            pCar->pos.fY =
                (float)(pCurrentData->pointAy[1].fX * dInterpolatedX2 +
                        pCurrentData->pointAy[1].fY * dInterpolatedY2 +
                        pCurrentData->pointAy[1].fZ * dInterpolatedZ2 -
                        pCurrentData->pointAy[3].fY);
            pCar->pos.fZ =
                (float)(dInterpolatedY2 * pCurrentData->pointAy[2].fY +
            ...
            pCar->direction.fX =
                pCurrentData->pointAy[0].fY * fDirectionYLocal1 +
                pCurrentData->pointAy[0].fX * fDirectionXTemp2 +
                pCurrentData->pointAy[0].fZ * fDirectionZTemp3;
            pCar->direction.fY =
                (float)(pCurrentData->pointAy[1].fX * dDirectionX3 +
                        pCurrentData->pointAy[1].fY * dDirectionY3 +
                        pCurrentData->pointAy[1].fZ * dDirectionZ3);
            dFinalDirectionZ2 = dDirectionY3 * pCurrentData->pointAy[2].fY +
                                dDirectionX3 * pCurrentData->pointAy[2].fX +
                                dDirectionZ3 * pCurrentData->pointAy[2].fZ;
            fTempCalc = pCar->direction.fX;
            ...
            // if ((LODWORD(fDirXFinal2) & 0x7FFFFFFF) != 0 ||
            // (LODWORD(fDirectionYTemp5) & 0x7FFFFFFF) != 0) {
            if (fabs(fDirXFinal2) > FLT_EPSILON ||
                fabs(fDirectionYTemp5) > FLT_EPSILON) {
              dAngle5 = atan2(fAngleCalcTemp7, fAngleCalcTemp2) * 16384.0 /
                        6.28318530718;
              //_CHP();
          ...
          fGroundYCalc2 = (float)(pCurrentData->pointAy[1].fY * dGroundY0 +
                                  pCurrentData->pointAy[0].fY * dGroundX0 +
                                  pCurrentData->pointAy[2].fY * dGroundZ0);
          fGroundZInterp4 = (float)(dGroundY0 * pCurrentData->pointAy[1].fZ +
                                    dGroundX0 * pCurrentData->pointAy[0].fZ +
                                    dGroundZ0 * pCurrentData->pointAy[2].fZ);
          dSlopeDiff = fGroundYInterp3 - fGroundZInterp4;
          fSlopeDiffCalc2 = (float)dSlopeDiff;
          fInterpolatedHeight3 =
              fabs(dSlopeDiff) <= 10.0
                  ? 1000000.0f
                  : fHeightDiffGround * fGroundYCalc2 /
                            (fGroundZInterp4 - fGroundYInterp3) +
                        (fLocalCoordZ - fGroundZInterp4) * fGroundYInterp2 /
                            fSlopeDiffCalc2;
          if (fLocalCoordY + CarBaseY > fInterpolatedHeight3) {
            ...
            if (dNegSpeed > 40.0 && !pCar->bySfxCooldown) {
              ...
              sfxpend(SOUND_SAMPLE_BANK, pCar->iDriverIdx,
                      (int)dBankSoundVolume); // SOUND_SAMPLE_BANK
              pCar->bySfxCooldown = 9;
            }
            ...
            pCar->pos.fX =
                (float)(pCurrentData->pointAy[0].fY * dInterpolatedY +
                        pCurrentData->pointAy[0].fX * fLocalCoordX +
                        pCurrentData->pointAy[0].fZ * fLocalCoordZ -
                        pCurrentData->pointAy[3].fX);
            pCar->pos.fY =
                (float)(pCurrentData->pointAy[1].fX * dInterpolatedX +
                        pCurrentData->pointAy[1].fY * dInterpolatedY +
                        pCurrentData->pointAy[1].fZ * dInterpolatedZ -
                        pCurrentData->pointAy[3].fY);
            pCar->pos.fZ =
                (float)(dInterpolatedY * pCurrentData->pointAy[2].fY +
            ...
            pCar->direction.fX =
                pCurrentData->pointAy[0].fY * fDirectionYLocal2 +
                pCurrentData->pointAy[0].fX * fDirectionXTemp1 +
                pCurrentData->pointAy[0].fZ * fDirectionZTemp1;
            pCar->direction.fY =
                (float)(pCurrentData->pointAy[1].fX * dDirectionX2 +
                        pCurrentData->pointAy[1].fY * dDirectionY2 +
                        pCurrentData->pointAy[1].fZ * dDirectionZ2);
            dFinalDirectionZ = dDirectionY2 * pCurrentData->pointAy[2].fY +
                               dDirectionX2 * pCurrentData->pointAy[2].fX +
                               dDirectionZ2 * pCurrentData->pointAy[2].fZ;
            fTempCalc = pCar->direction.fX;
            ...
            // if ((LODWORD(fDirXFinal) & 0x7FFFFFFF) != 0 ||
            // (LODWORD(fDirectionYTemp3) & 0x7FFFFFFF) != 0) {
            if (fabs(fDirXFinal) > FLT_EPSILON ||
                fabs(fDirectionYTemp3) > FLT_EPSILON) {
              dAngle4 = atan2(fDirectionYFinal2, fAngleCalcTemp4) * 16384.0 /
                        6.28318530718;
              //_CHP();
          ...
          LABEL_192:
            dFinalHorizSpeed = pCar->direction.fX * pCar->direction.fX +
                               pCar->direction.fY * pCar->direction.fY;
...
void converttoair(tCar *pCar) {
  ...
  if (iCurrChunk < 0)
    iCurrChunk += TRAK_LEN;
  dRollSpeedCalc =
      (double)localdata[iCurrChunk].iRoll * pCar->fFinalSpeed * 0.0013888889 +
      (double)pCar->iRollMomentum; // Calculate banking/roll effect based on
                                   // speed (0.0013888889 = 1/720)
  //_CHP();
  pCar->iRollMomentum = (int)dRollSpeedCalc;
  iChunkBackup = pCar->nCurrChunk;
  getworldangles(pCar->nActualYaw, pCar->nPitch, pCar->nRoll,
                 pCar->nReferenceChunk, &iWorldYaw1, &iWorldPitch,
                 &iWorldRoll); // Convert car yaw3/pitch/roll to world angles
                               // using current chunk
  getworldangles(pCar->nYaw, pCar->nPitch, pCar->nRoll, pCar->nReferenceChunk,
                 &iWorldYaw2, &iWorldPitch,
                 &iWorldRoll); // Convert car yaw/pitch/roll to world angles
  pCar->nYaw = iWorldYaw2;
  pCar->nActualYaw = iWorldYaw2;
  pCar->nPitch = iWorldPitch;
  pData = &localdata[iChunkBackup];
  pCar->nRoll = iWorldRoll;
  fWorldX = pData->pointAy[0].fY * pCar->pos.fY +
            pData->pointAy[0].fX * pCar->pos.fX +
            pData->pointAy[0].fZ * pCar->pos.fZ -
            pData->pointAy[3].fX; // Transform car position from chunk-relative
                                  // to world coordinates
  fWorldY = pData->pointAy[1].fY * pCar->pos.fY +
            pData->pointAy[1].fX * pCar->pos.fX +
            pData->pointAy[1].fZ * pCar->pos.fZ - pData->pointAy[3].fY;
  fWorldZ = pData->pointAy[2].fY * pCar->pos.fY +
            pData->pointAy[2].fX * pCar->pos.fX +
            pData->pointAy[2].fZ * pCar->pos.fZ - pData->pointAy[3].fZ;
  dFinalSpeed = pCar->fFinalSpeed;
  pCar->pos.fX = fWorldX;
  pCar->pos.fY = fWorldY;
  pCar->pos.fZ = fWorldZ;
  pCar->direction.fX =
      (float)(dFinalSpeed * tcos[iWorldYaw1] *
              tcos[iWorldPitch]); // Calculate 3D velocity vector from speed and
                                  // world angles
  ...
  pCar->fSpeedOverflow =
      0.0; // Reset speed overflow and prepare for airborne physics
  nTempChunk = pCar->nCurrChunk;
  pCar->nCurrChunk =
      -1; // Set car as airborne (-1) and backup original chunk position
...
void ordercars() {
  ...
  iTotalCars =
      numcars; // Bubble sort implementation to order cars by race position
  uiSortIndex = 0;
  iSortLimit =
      4 *
      (numcars - 1); // Set up loop limit for bubble sort (numcars-1) iterations
  iSortCounter = 0;
  if (iSortLimit > 0) {
    do {
      pCurrentCar = &Car[carorder[uiSortIndex / 4]]; // Get current car from
                                                     // sorted order array
      if (!finished_car[pCurrentCar->iDriverIdx]) // Skip comparison if current
                                                  // car has finished the race
      {
        pNextCar = &Car[carorder[uiSortIndex / 4 + 1]]; // Get next car in order
                                                        // for comparison
        if (pCurrentCar->nCurrChunk != -1 && pNextCar->nCurrChunk != -1 &&
            !finished_car[pNextCar->iDriverIdx]) // Only compare active cars
                                                 // that are on track and not
                                                 // finished
        {
          byLapNumber = pNextCar->byLapNumber;
          byCurrentProgress = pCurrentCar->byLapNumber;
          if (byLapNumber <=
              byCurrentProgress) // Compare race progress (byUnk24 - likely
                                 // lap/sector progress)
          { // If same progress, compare by track position (chunk and X
            // coordinate)
            if (byLapNumber == byCurrentProgress) {
              nCurrChunk = pNextCar->nCurrChunk;
              if (nCurrChunk > pCurrentCar->nCurrChunk ||
                  nCurrChunk == pCurrentCar->nCurrChunk &&
                      pNextCar->pos.fX > (double)pCurrentCar->pos.fX) {
                iSwapCarIndex =
                    carorder[uiSortIndex / 4]; // Swap car positions: next car
                                               // is ahead by position
            ...
            iTempCarIndex =
                carorder[uiSortIndex / 4]; // Swap car positions: next car is
                                           // ahead in progress
            carorder[uiSortIndex / 4] = carorder[uiSortIndex / 4 + 1];
  ...
  byPosition =
      0; // Assign race positions based on sorted order (0=1st, 1=2nd, etc.)
  if (iTotalCars > 0) {
    iCarIndex = 0;
    do
...
void changeline(tCar *pCar) {
  ...
  if (pCar->iAITargetLine ==
      -1) // Check if car needs to select a new AI driving line
  {       // Check if center line marker is disabled (bit 24)
    if ((TrakColour[iCurrChunk][1] & SURFACE_FLAG_PIT_ZONE) ==
        0) { // Cars in 6th place or lower have reduced aggression
      if (pCar->byRacePosition >= 6u) {
        iRandForPos = ROLLERrandRaw();
        if (GetHighOrderRand(720, iRandForPos) != 100)
          goto LABEL_13; // 1 in 720 chance for line change when in poor
                         // position
        goto LABEL_7;
      }
    LABEL_12:
      pCar->iAITargetLine =
          3; // Set target line to 3 (conservative choice for front runners)
      goto LABEL_13;
    }
    if (pCar->iAITargetCar != -1 &&
        pCar->fHealth <
            60.0) // If targeting another car and low health, stay defensive
    {
    ...
    if (!pCar->iAICurrentLine) // Check if car is not currently on an AI line
    {
      ...
      LABEL_7:
        iRandValue =
            ROLLERrandRaw(); // Select random AI line (0-3 modulo operation)
        pCar->iAITargetLine =
            GetHighOrderRand(4, iRandValue); // (iRandValue) >> 13;
        goto LABEL_13;
      }
      goto LABEL_12;
    }
...
LABEL_13:
  ...
  if (iTargetLine != -1 &&
      linevalid(
          pCar->nCurrChunk, pCar->pos.fY,
          *(&pData->fAILine1 +
            iTargetLine))) // Check if target line is valid at current position
  {
    iPrevTargetLine = pCar->iAITargetLine;
    pCar->iAITargetLine =
        -1; // Successfully switch to target line, clear target
    iChangeSuccess = -1;
    pCar->iAICurrentLine = iPrevTargetLine;
  }
  if (!iChangeSuccess &&
      !linevalid(pCar->nCurrChunk, pCar->pos.fY,
                 *(&pData->fAILine1 +
                   pCar->iAICurrentLine))) // If current line invalid and no
                                           // change made, pick new random line
  {
    iRandNewLine = ROLLERrandRaw();
    pCar->iAITargetLine =
        GetHighOrderRand(4, iRandNewLine); // (iRandNewLine) >> 13;
  }
}
...
void driverange(tCar *pCar, float *pfLeftLimit, float *pfRightLimit) {
  ...
  if (iLeftSurfaceType == 5 || iLeftSurfaceType == 6 ||
      iLeftSurfaceType ==
          9) // Check if left surface is wall/barrier (types 5,6,9)
    iLeftSurfaceFlag = -1;
  else
    iLeftSurfaceFlag = 0;
  iRightSurfaceType = pTrackInfo->iRightSurfaceType;
  if (iRightSurfaceType == 5 || iRightSurfaceType == 6 ||
      iRightSurfaceType ==
          9) // Check if right surface is wall/barrier (types 5,6,9)
    iRightSurfaceFlag = -1;
  else
    iRightSurfaceFlag = 0;
  uiLeftLaneMarker =
      abs(TrakColour[pCar->nCurrChunk][0]) & SURFACE_FLAG_SKIP_RENDER;
  uiCenterLaneMarker =
      abs(TrakColour[pCar->nCurrChunk][1]) & SURFACE_FLAG_SKIP_RENDER;
  uiRightLaneMarker =
      abs(TrakColour[pCar->nCurrChunk][2]) & SURFACE_FLAG_SKIP_RENDER;
  iLaneType = pCar->iLaneType;
  if (!iLaneType) // Lane type 0: Full track width driving (normal racing)
  {
    *pfLeftLimit = pTrackData->fTrackHalfWidth + pTrackInfo->fLShoulderWidth -
                   pCar->fCarHalfWidth; // Set left boundary: track edge + left
                                        // shoulder - car width
    if (iLeftSurfaceFlag ||
        uiCenterLaneMarker) // If left wall/barrier or center lane marker, limit
                            // right boundary
    {
    ...
    goto LABEL_21; // If right wall/barrier or right lane marker, limit left
                   // boundary
  }
  ...
  { // Left lane: adjust left boundary based on wall/lane marker
    if (iLeftSurfaceFlag || uiLeftLaneMarker)
      ...
      dTrackHalfWidth =
          pTrackData->fTrackHalfWidth + pTrackInfo->fLShoulderWidth;
  ...
  LABEL_21:
    *pfRightLimit = -pTrackData->fTrackHalfWidth - pTrackInfo->fRShoulderWidth +
                    pCar->fCarHalfWidth; // Default right boundary: track edge +
                                         // right shoulder - car width
    return;
  }
  if (iLaneType == 2) // Lane type 2: Right lane driving only
  {
    *pfRightLimit = -pTrackData->fTrackHalfWidth - pTrackInfo->fRShoulderWidth +
                    pCar->fCarHalfWidth; // Right lane: set right boundary to
                                         // track edge + shoulder
    if (iRightSurfaceFlag ||
        uiCenterLaneMarker) // If right wall/barrier or center marker, use track
                            // center as left boundary
    {
...
void updatesmokeandflames(tCar *pCar) {
  ...
  if (player_type != 2 && ViewType[0] == pCar->iDriverIdx &&
      (!SelectedView[0] || SelectedView[0] == 2 || SelectedView[0] == 7))
    iCinematicMode = -1;
  else
    iCinematicMode = 0;
...
void dospray(tCar *pCar, int iCinematicMode, tCarSpray *pCarSpray) {
  ...
  fHealthFactor =
      (pCar->fHealth + 34.0f) *
      0.01f; // Calculate health factor (0.0-1.0) based on car damage
  iCarDesignIdx = pCar->byCarDesignIdx;
  pPlaces =
      CarDesigns[iCarDesignIdx]
          .pPlaces; // Get car design data for spray particle placement points
  if (fHealthFactor > 1.0)
    fHealthFactor = 1.0;
  iSprayIdx = 0;
  ...
  do {
    ...
    if (byType) { // Spray type 1: Active damage/smoke particles
      if (byType <= 1u) {
        iRandomColor1 = ROLLERrandRaw(); // Generate random color values for
                                         // active smoke particles
        iRandomColor2 = GetHighOrderRand(
            4, iRandomColor1); // same as (4 * iRandomColor1) / 32768
        // iRandomColor2 = (4 * iRandomColor1 - (__CFSHL__((4 * iRandomColor1)
        // >> 31, 15) + ((4 * iRandomColor1) >> 31 << 15))) >> 15;
        ...
        if (iCinematicMode) // Different particle behavior for menu/replay mode
                            // vs gameplay
        {
          iRandomSize4 = ROLLERrandRaw();
          iRandomSize5 = (GetHighOrderRand(60, iRandomSize4) + 20) * scr_size;
          // iRandomSize5 = (((60 * iRandomSize4 - (__CFSHL__((60 *
          // iRandomSize4) >> 31, 15) + ((60 * iRandomSize4) >> 31 << 15))) >>
          // 15) + 20) * scr_size;
          pCarSpray->fSize = (float)(iRandomSize5 >> 6);
          // pCarSpray->fSize = (float)((iRandomSize5 - (__CFSHL__(iRandomSize5
          // >> 31, 6) + (iRandomSize5 >> 31 << 6))) >> 6);
          iRandomVelY3 = ROLLERrandRaw();
          if (iTimer == 1)
            iRandomVelX1 = (GetHighOrderRand(50, iRandomVelY3) + 40) * scr_size;
          // iRandomVelX1 = (((50 * iRandomVelY3 - (__CFSHL__((50 *
          // iRandomVelY3) >> 31, 15) + ((50 * iRandomVelY3) >> 31 << 15))) >>
          // 15) + 40) * scr_size;
          else
            iRandomVelX1 =
                scr_size * (-40 - GetHighOrderRand(50, iRandomVelY3));
          // iRandomVelX1 = scr_size * (-40 - ((50 * iRandomVelY3 -
          // (__CFSHL__((50 * iRandomVelY3) >> 31, 15) + ((50 * iRandomVelY3) >>
          // 31 << 15))) >> 15));
          pCarSpray->velocity.fX = (float)(iRandomVelX1 >> 6);
          // pCarSpray->velocity.fX = (float)((iRandomVelX1 -
          // (__CFSHL__(iRandomVelX1 >> 31, 6) + (iRandomVelX1 >> 31 << 6))) >>
          // 6);
          iRandomVelY4 = ROLLERrandRaw();
          iRandomVelY5 = (GetHighOrderRand(100, iRandomVelY4) + 40) * scr_size;
          // iRandomVelY5 = (((100 * iRandomVelY4 - (__CFSHL__((100 *
          // iRandomVelY4) >> 31, 15) + ((100 * iRandomVelY4) >> 31 << 15))) >>
          // 15) + 40) * scr_size;
          pCarSpray->velocity.fY = (float)(iRandomVelY5 >> 6);
          // pCarSpray->velocity.fY = (float)((iRandomVelY5 -
          // (__CFSHL__(iRandomVelY5 >> 31, 6) + (iRandomVelY5 >> 31 << 6))) >>
          // 6);
          ROLLERrandRaw();
        } else {
          iRandomSize6 = ROLLERrandRaw(); // Generate larger particle sizes and
                                          // velocities for normal gameplay mode
          pCarSpray->fSize = (float)(GetHighOrderRand(100, iRandomSize6) + 100);
          // pCarSpray->fSize = (float)(((100 * iRandomSize6 - (__CFSHL__((100 *
          // iRandomSize6) >> 31, 15) + ((100 * iRandomSize6) >> 31 << 15))) >>
          // 15) + 100);
          if (iTimer) {
            iRandomVelX3 = ROLLERrandRaw();
            if (iTimer == 1) {
              pCarSpray->velocity.fX =
                  (float)(-100 - GetHighOrderRand(300, iRandomVelX3));
              // pCarSpray->velocity.fX = (float)(-100 - ((300 * iRandomVelX3 -
              // (__CFSHL__((300 * iRandomVelX3) >> 31, 15) + ((300 *
              // iRandomVelX3) >> 31 << 15))) >> 15));
              iRandomVelY7 = ROLLERrandRaw();
              pCarSpray->velocity.fY =
                  (float)(-100 - GetHighOrderRand(200, iRandomVelY7));
              // pCarSpray->velocity.fY = (float)(-100 - ((200 * iRandomVelY7 -
              // (__CFSHL__((200 * iRandomVelY7) >> 31, 15) + ((200 *
              // iRandomVelY7) >> 31 << 15))) >> 15));
              iRandomUnk5_2 = ROLLERrandRaw();
              iCalculatedUnk5 = GetHighOrderRand(200, iRandomUnk5_2) + 200;
              // iCalculatedUnk5 = ((200 * iRandomUnk5_2 - (__CFSHL__((200 *
              // iRandomUnk5_2) >> 31, 15) + ((200 * iRandomUnk5_2) >> 31 <<
              // 15))) >> 15) + 200;
            } else {
              pCarSpray->velocity.fX =
                  (float)(-105 - GetHighOrderRand(150, iRandomVelX3));
              // pCarSpray->velocity.fX = (float)(-105 - ((150 * iRandomVelX3 -
              // (__CFSHL__((150 * iRandomVelX3) >> 31, 15) + ((150 *
              // iRandomVelX3) >> 31 << 15))) >> 15));
              iRandomVelY8 = ROLLERrandRaw();
              pCarSpray->velocity.fY =
                  (float)GetHighOrderRand(400, iRandomVelY8);
              // pCarSpray->velocity.fY = (float)(((400 * iRandomVelY8 -
              // (__CFSHL__((400 * iRandomVelY8) >> 31, 15) + ((400 *
              // iRandomVelY8) >> 31 << 15))) >> 15) - 200);
              iRandomUnk5_3 = ROLLERrandRaw();
              iCalculatedUnk5 = GetHighOrderRand(30, iRandomUnk5_3) - 20;
              // iCalculatedUnk5 = ((30 * iRandomUnk5_3 - (__CFSHL__((30 *
              // iRandomUnk5_3) >> 31, 15) + ((30 * iRandomUnk5_3) >> 31 <<
              // 15))) >> 15) - 20;
            }
          } else {
            iRandomVelX2 = ROLLERrandRaw();
            pCarSpray->velocity.fX =
                (float)(-100 - GetHighOrderRand(300, iRandomVelX2));
            // pCarSpray->velocity.fX = (float)(-100 - ((300 * iRandomVelX2 -
            // (__CFSHL__((300 * iRandomVelX2) >> 31, 15) + ((300 *
            // iRandomVelX2) >> 31 << 15))) >> 15));
            iRandomVelY6 = ROLLERrandRaw();
            pCarSpray->velocity.fY =
                (float)(GetHighOrderRand(200, iRandomVelY6) + 100);
            // pCarSpray->velocity.fY = (float)(((200 * iRandomVelY6 -
            // (__CFSHL__((200 * iRandomVelY6) >> 31, 15) + ((200 *
            // iRandomVelY6) >> 31 << 15))) >> 15) + 100);
            iRandomUnk5_1 = ROLLERrandRaw();
            iCalculatedUnk5 = GetHighOrderRand(200, iRandomUnk5_1) + 200;
            // iCalculatedUnk5 = ((200 * iRandomUnk5_1 - (__CFSHL__((200 *
            // iRandomUnk5_1) >> 31, 15) + ((200 * iRandomUnk5_1) >> 31 << 15)))
            // >> 15) + 200;
          }
        ...
        if (iLifetimeDecrement < 0) {
          pCarSpray->iType =
              pCarSpray->iType & 0xFFFFFF00; // Clear lowest byte only
      ...
      {
        if (pCar->fHealth <=
            0.0) { // Car is dead - generate explosion/death particles
          if (pCarSpray->iLifeTime <= 0) {
            if (pCar->nExplosionSoundTimer < 0) {
              if (replaytype != 2)
                sfxpend(SOUND_SAMPLE_EXPLO, pCar->iDriverIdx,
                        0x8000); // SOUND_SAMPLE_EXPLO
              iRandomValue3 = ROLLERrandRaw();
              pCar->nExplosionSoundTimer =
                  GetHighOrderRand(4, iRandomValue3) + 4;
              // pCar->nExplosionSoundTimer = ((4 * iRandomValue3 -
              // (__CFSHL__((4 * iRandomValue3) >> 31, 15) + ((4 *
              // iRandomValue3) >> 31 << 15))) >> 15) + 4;
            }
            ...
            if ((iSprayIdx & 7) != 0) {
              pCarSpray->velocity.fX =
                  (float)(50 - GetHighOrderRand(100, iRandomValue4));
              // pCarSpray->velocity.fX = (float)(50 - ((100 * iRandomValue4 -
              // (__CFSHL__((100 * iRandomValue4) >> 31, 15) + ((100 *
              // iRandomValue4) >> 31 << 15))) >> 15));
              iRandomVelY2 = ROLLERrandRaw();
              pCarSpray->velocity.fY =
                  (float)(30 - GetHighOrderRand(60, iRandomVelY2));
              iRandomSize3 = ROLLERrandRaw();
              iRandomSize2 = GetHighOrderRand(64, iRandomSize3) + 128;
            } else {
              pCarSpray->velocity.fX =
                  (float)(20 - GetHighOrderRand(40, iRandomValue4));
              iRandomVelY1 = ROLLERrandRaw();
              pCarSpray->velocity.fY =
                  (float)(10 - GetHighOrderRand(20, iRandomVelY1));
            ...
            if (pCarSpray->position.fX * pCarSpray->position.fX +
                        pCarSpray->position.fY + pCarSpray->position.fY <
                    1125000.0 &&
                pCarSpray->position.fZ < 450.0 &&
                pCarSpray->position.fZ > -100.0) {
              iRandomValue1 = ROLLERrandRaw();
              pCarSpray->fSize =
                  (float)(GetHighOrderRand(12, iRandomValue1) + 4) +
                  pCarSpray->fSize;
              pCarSpray->position.fX =
                  pCarSpray->velocity.fX + pCarSpray->position.fX;
              pCarSpray->position.fY =
                  pCarSpray->velocity.fY + pCarSpray->position.fY;
              if (pCarSpray->position.fZ < 450.0 &&
                  pCarSpray->position.fZ > -100.0)
                pCarSpray->position.fZ =
                    pCarSpray->velocity.fZ + pCarSpray->position.fZ;
            }
            --pCarSpray->iLifeTime;
    ...
    if (pCarSpray->iLifeTime <=
        0) // Spray type 0: Initialize new particles based on car health
    {
      if (pCar->fHealth > 0.0) {
        ...
        if (v50 <= 0) {
          if (fHealthFactor >= 1.0) {
          ...
          } else { // Generate damage particles based on health factor - more
                   // damage = more particles
            if ((double)ROLLERrand() * fHealthFactor < 8192.0 &&
                fHealthFactor < 0.66) {
              ...
              if (pPlaces == (int *)-1) {
                ...
                if (iCinematicMode) {
                  ...
                  if (iPlacementIdx == 1)
                    iTempPosX =
                        winw / 2 + GetHighOrderRand(scr_size, iRandomPosX1);
                  else
                    iTempPosX =
                        winw / 2 - GetHighOrderRand(scr_size, iRandomPosX1);
                  ...
                  pCarSpray->position.fY =
                      (float)(((iRandomPosY1 * scr_size) >> 16) + winh);
                } else {
                  iRandomPosY2 = ROLLERrandRaw();
                  pCarSpray->position.fY =
                      (float)(GetHighOrderRand(32, iRandomPosY2) + winh);
                  iRandomPlacement2 = ROLLERrandRaw();
                  iPlacementIdx = GetHighOrderRand(4, iRandomPlacement2);
                  if (iPlacementIdx == 4)
                    iPlacementIdx = 3;
                  pCoordinates1 =
                      &CarDesigns[iStoredCarDesignIdx].pCoords
                           [pPlaces[iPlacementIdx]]; // Select random attachment
                                                     // point from car design
                                                     // for particle placement
                  // CHEAT_MODE_TINY_CARS
                  if ((cheat_mode & CHEAT_MODE_TINY_CARS) ==
                      0) // Handle CHEAT_MODE_TINY_CARS - scale particle
            ...
            if ((uint8)(pCarSpray->iType) != 1) {
              ...
              if (pPlaces == (int *)-1) {
                ...
                if (iPlacementIdx2 == 1)
                  iTempPosX2 =
                      winw / 2 + GetHighOrderRand(scr_size, iRandomPosX2);
                else
                  iTempPosX2 =
                      winw / 2 - GetHighOrderRand(scr_size, iRandomPosX2);
                ...
                pCarSpray->position.fY =
                    (float)(((iRandomPosY3 * scr_size) >> 16) + winh);
              } else { // Health-based placement logic: high health uses fewer
                       // placements
                ...
                pCoordinates2 = &CarDesigns[iStoredCarDesignIdx]
                                     .pCoords[pPlaces[iPlacementIdx2]];
                // CHEAT_MODE_TINY_CARS
              ...
              pCarSpray->iColor = 1302;
              pCarSpray->iLifeTime =
                  GetHighOrderRand(24, iTempRandomValue) + 24;
              ...
              if (iCinematicMode) {
                iRandomVelY9 =
                    ROLLERrandRaw(); // Calculate menu/replay mode velocities
                                     // with screen scaling
                iVelYCalc1 =
                    (-2 - GetHighOrderRand(6, iRandomVelY9)) * scr_size;
                ...
                pCarSpray->fSize =
                    (float)(((double)ROLLERrand() *
                                 ((1.0 - fHealthFactor) * 32.0 + 32.0) *
                                 0.000030517578125 +
                             32.0) *
                            (double)scr_size * 0.015625);
                fHealthMultiplier4 = fHealthFactor * 512.0f;
                dLifetimeCalc1 =
                    (double)ROLLERrand() * fHealthMultiplier4 * 0.000030517578 +
                    64.0;
                //_CHP();
                ...
                iRandomVelX5 =
                    ROLLERrandRaw(); // Calculate gameplay mode velocities based
                                     // on placement position
                pCarSpray->velocity.fX =
                    (float)(-15 - GetHighOrderRand(20, iRandomVelX5));
                if (iPlacementIdx2) {
                ...
                pCarSpray->velocity.fZ =
                    (float)GetHighOrderRand(10, iRandomUnk5_4);
                iRandomSize7 = ROLLERrandRaw();
                pCarSpray->fSize =
                    (float)(GetHighOrderRand(10, iRandomSize7) + 10);
                if (fHealthFactor <= 0.6) {
                  if (pCar->fHealth <= 0.0) {
                    fHealthMultiplier2 = fHealthFactor * 64.0f;
                    dLifetimeCalc3 = (double)ROLLERrand() * fHealthMultiplier2 *
                                         0.000030517578 +
                                     18.0;
                  } else {
                    fHealthMultiplier3 = fHealthFactor * 256.0f;
                    dLifetimeCalc3 = (double)ROLLERrand() * fHealthMultiplier3 *
                                         0.000030517578 +
                                     24.0;
                  }
                  //_CHP();
                  ...
                  dLifetimeCalc2 = (double)ROLLERrand() * fHealthMultiplier1 *
                                       0.000030517578 +
                                   36.0;
                  //_CHP();
    ...
    if ((pCarSpray->iLifeTime & 3) == 0) // Animate particle colors by cycling
                                         // through color palette every 4 frames
    {
  ...
  CONTINUE_COLOR_ANIMATION:
    if ((uint8)pCarSpray->iColor && (uint8)pCarSpray->iColor < 0x15u ||
        (uint8)pCarSpray->iColor > 0x17u)
      pCarSpray->iColor = 1302;
    pCarSpray->position.fX =
        pCarSpray->velocity.fX +
        pCarSpray->position.fX; // Update particle physics: position +=
                                // velocity, apply gravity/forces
    ...
    if (iCinematicMode) {
      ...
      if (pCarSpray->iLifeTime <= 8) {
        ...
        iSizeShrinkCalc1 =
            (GetHighOrderRand(24, iRandomSizeShrink1) + 10) * scr_size;
  ...
  NEXT_PARTICLE:
    ...
    ++pCarSpray;
           ...
           32); // Main loop: process all 32 spray particles for this car
}
...
void calculateseparatedcoordinatesystem(int iChunk, tData *pChunkData) {
  ...
  if (iChunk + 1 >=
      TRAK_LEN) // Wrap around if we've reached the end of the track
    iNextChunk = 0;
  iCurrentChunk = iChunk;
  pLocalData =
      &localdata[iChunk]; // Get pointer to local data for current chunk
  dInverseTrackLength =
      1.0 / (pLocalData->fTrackHalfLength *
             2.0); // Calculate inverse track length for normalization
  fTangentX =
      ((TrakPt[iNextChunk].pointAy[2].fX + TrakPt[iNextChunk].pointAy[3].fX) *
           0.5f -
       (TrakPt[iCurrentChunk].pointAy[2].fX +
        TrakPt[iCurrentChunk].pointAy[3].fX) *
           0.5f) *
      (float)dInverseTrackLength; // Calculate X component of track tangent
                                  // vector (normalized)
  dCenterDeltaY =
      (TrakPt[iNextChunk].pointAy[2].fY + TrakPt[iNextChunk].pointAy[3].fY) *
          0.5 -
      (TrakPt[iCurrentChunk].pointAy[2].fY +
       TrakPt[iCurrentChunk].pointAy[3].fY) *
          0.5; // Calculate Y difference between track centers
  fTangentY = (float)(dInverseTrackLength * dCenterDeltaY);
  pChunkData->pointAy[0].fY = (float)(-(
      dInverseTrackLength * dCenterDeltaY)); // Set up local coordinate system -
                                             // pointAy[0] is right vector
  pChunkData->pointAy[0].fZ = 0.0;
  pChunkData->pointAy[1].fZ = 0.0;
  pChunkData->pointAy[2].fX =
      0.0; // Set up local coordinate system - pointAy[2] is up vector (0,0,1)
  pChunkData->pointAy[2].fY = 0.0;
  pChunkData->pointAy[2].fZ = 1.0;
  pChunkData->pointAy[0].fX = fTangentX; // Set up local coordinate system -
                                         // pointAy[1] is forward vector
  pChunkData->pointAy[1].fX = fTangentY;
  pChunkData->pointAy[1].fY = fTangentX;
  pChunkData->pointAy[3] =
      pLocalData
          ->pointAy[3]; // Copy track position and dimensions from local data
...
void findnearsection(tCar *pCar, int *piNearestChunk) {
  ...
  iTrakLen =
      TRAK_LEN; // Store track length and initialize minimum distance trackers
  fMinValidDistance = 1.0e10;
  fMinCurrentDistance = 1.0e10;
  iLastValidChunk = pCar->iLastValidChunk; // Get car's last known valid track
                                           // chunk and setup search parameters
  iNearestChunk = -1;
  ...
  if (iLastValidChunk <
      iForwardMainEnd) // Search forward main chunks for nearest track section
  {
    iForwardGroundIdx = iLastValidChunk;
    do {
      dForwardCarX =
          pCar->pos.fX +
          pForwardData->pointAy[3]
              .fX; // Calculate car position relative to current track chunk
      ...
      fForwardPosition = (float)(dForwardCarY * pForwardData->pointAy[1].fZ +
                                 pForwardData->pointAy[0].fZ * fTempCarX3 +
                                 dForwardCarZ * pForwardData->pointAy[2].fZ);
      dForwardAbsX = fabs(dForwardCarX);
      fForwardDistance =
          (float)sqrt(dForwardAbsX * dForwardAbsX +
                      dForwardAbsY * dForwardAbsY +
                      dForwardAbsZ * dForwardAbsZ) -
          pForwardData->fTrackHalfLength; // Calculate 3D distance from car to
                                          // track centerline
      if (iForwardGroundIdx >= 0 &&
          iForwardGroundIdx < TRAK_LEN && // bounds check added by ROLLER
          (fForwardPosition >= -400.0 ||
           GroundColour[iForwardGroundIdx][2] >= 0) &&
          fForwardDistance <
              (double)fMinValidDistance) // Check position validity and update
                                         // closest valid chunk if closer
      {
      ...
      if (iForwardGroundIdx >= 0 &&
          iForwardGroundIdx < TRAK_LEN && // bounds check added by ROLLER
          (fForwardDotProduct * 4.0 + -500.0 <= fForwardPosition ||
           GroundColour[iForwardGroundIdx][2] >= 0) &&
          fForwardDistance <
              (double)fMinCurrentDistance) // Check forward position and update
                                           // nearest current chunk if closer
      {
      ...
      if (iForwardChunkIdx ==
          iTrakLen -
              1) // Handle track wrapping: loop back to start when reaching end
  ...
  if (iLastValidChunk >
      iBackwardMainEnd) // Search backward main chunks for nearest track section
  {
    ...
    do {
      ...
      fBackwardPosition = (float)(dBackwardCarY * pBackwardData->pointAy[1].fZ +
                                  pBackwardData->pointAy[0].fZ * fTempCarX +
                                  dBackwardCarZ * pBackwardData->pointAy[2].fZ);
      dBackwardAbsX = fabs(dBackwardCarX);
      fBackwardDistance = (float)sqrt(dBackwardAbsX * dBackwardAbsX +
                                      dBackwardAbsY * dBackwardAbsY +
                                      dBackwardAbsZ * dBackwardAbsZ) -
                          pBackwardData->fTrackHalfLength;
      if (iBackwardGroundIdx >= 0 &&
          iBackwardGroundIdx < TRAK_LEN && // bounds check added by ROLLER
          (fBackwardPosition >= -400.0 ||
           GroundColour[iBackwardGroundIdx][2] >= 0) &&
          fBackwardDistance < (double)fMinValidDistance) {
      ...
      if (iBackwardGroundIdx >= 0 &&
          iBackwardGroundIdx < TRAK_LEN && // bounds check added by ROLLER
          (fBackwardDotProduct * 4.0 + -500.0 <= fBackwardPosition ||
           GroundColour[iBackwardGroundIdx][2] >= 0) &&
          fBackwardDistance < (double)fMinCurrentDistance) {
      ...
      if (iBackwardChunkIdx) // Handle track wrapping: loop to end when reaching
                             // start
        --pBackwardData;
      else
  ...
  if ((TrakColour[iLastValidChunk][1] & SURFACE_FLAG_NO_EXTRAS) ==
      0) // Check if not on special track section - if so, search extra chunks
  {
    ...
    if (iForwardExtraStart >= 0 &&
        iForwardExtraStart < TRAK_LEN) // check added by ROLLER
      pForwardExtraData = &localdata[iForwardExtraStart];
    else
      pForwardExtraData = NULL;

    if (iForwardExtraStart < iForwardExtraEnd &&
        pForwardExtraData) // Search forward extra chunks for nearest track
                           // section
    {
      ...
      do {
        ...
        fForwardExtraDotProduct =
            pForwardExtraData->pointAy[0].fZ * pCar->direction.fX +
        ...
        fForwardExtraPosition =
            (float)(dForwardExtraCarY * pForwardExtraData->pointAy[1].fZ +
        ...
        fForwardExtraDistance =
            (float)sqrt(dForwardExtraAbsX * dForwardExtraAbsX +
                        dForwardExtraAbsY * dForwardExtraAbsY +
                        dForwardExtraAbsZ * dForwardExtraAbsZ) -
            pForwardExtraData->fTrackHalfLength;
        if ((fForwardExtraPosition >= -400.0 ||
             GroundColour[iForwardExtraGroundIdx][2] >= 0) &&
            fForwardExtraDistance < (double)fMinValidDistance) {
        ...
        if ((fForwardExtraDotProduct * 4.0 + -500.0 <= fForwardExtraPosition ||
             GroundColour[iForwardExtraGroundIdx][2] >= 0) &&
    ...
    if (pTrakView->nBackwardExtraStart >= 0 &&
        pTrakView->nBackwardExtraStart <
            TRAK_LEN) { // bounds check added by ROLLER
      iBackwardExtraStart = pTrakView->nBackwardExtraStart;
      iBackwardExtraEnd =
          iBackwardExtraStart - pTrakView->byBackwardExtraChunks;
      pBackwardExtraData = &localdata[iBackwardExtraStart];
      iBackwardExtraIdx = iBackwardExtraStart;
      if (iBackwardExtraStart >
          iBackwardExtraEnd) // Search backward extra chunks for nearest track
                             // section
      {
        ...
        do {
          ...
          fBackwardExtraDotProduct =
              pBackwardExtraData->pointAy[0].fZ * pCar->direction.fX +
          ...
          fBackwardExtraPosition =
              (float)(dBackwardExtraCarY * pBackwardExtraData->pointAy[1].fZ +
          ...
          fBackwardExtraDistance =
              (float)sqrt(dBackwardExtraAbsX * dBackwardExtraAbsX +
                          dBackwardExtraAbsY * dBackwardExtraAbsY +
                          dBackwardExtraAbsZ * dBackwardExtraAbsZ) -
              pBackwardExtraData->fTrackHalfLength;
          if ((fBackwardExtraPosition >= -400.0 ||
               GroundColour[iBackwardExtraGroundIdx][2] >= 0) &&
              fBackwardExtraDistance < (double)fMinValidDistance) {
          ...
          if ((fBackwardExtraDotProduct * 4.0 + -500.0 <=
                   fBackwardExtraPosition ||
               GroundColour[iBackwardExtraGroundIdx][2] >= 0) &&
...
void dozoomstuff(int iPlayerIdx) {
  ...
  iZoomCountdown =
      game_count[iPlayerIdx]; // Get zoom countdown timer for this player
  if (iZoomCountdown <= 0) {
    if (iZoomCountdown >= 0) {
      if (champ_mode)
        dZoomOutScale = game_scale[iPlayerIdx] *
                        0.5; // Zoom out phase: apply zoom factor based on
                             // championship vs regular mode
      else
        dZoomOutScale = game_scale[iPlayerIdx] * 0.7692307692307693;
      game_scale[iPlayerIdx] = (float)dZoomOutScale;
      iPlayerIdx3 = iPlayerIdx;
      if (game_scale[iPlayerIdx3] <
          64.0) // Check if zoom has reached minimum scale limit
      {
        game_scale[iPlayerIdx3] = 64.0; // Clamp to minimum zoom (64.0) and set
                                        // appropriate countdown timer
        if (iPrevChampMode) {
      ...
      dZoomInScale = champ_mode ? game_scale[iPlayerIdx] * 2.0
                                : game_scale[iPlayerIdx] * 1.3;
      game_scale[iPlayerIdx] = (float)dZoomInScale;
      iPlayerIdx2 = iPlayerIdx;
      if (game_scale[iPlayerIdx2] >
          32768.0) // Check if zoom has reached maximum scale limit
      {
        game_scale[iPlayerIdx2] =
            32768.0; // Clamp to maximum zoom and disable further zooming
    ...
    game_count[iPlayerIdx_1] =
        iZoomCountdown - 1; // Countdown is positive - decrement timer
    if (iZoomCountdown == 1 &&
        game_scale[iPlayerIdx_1] ==
            64.0) // Special case: if countdown reaches 1 and scale is at
                  // minimum (64.0), start zoom in
...
int findcardistance(int iCarIdx, float fMaxDistance) {
  ...
  fReferenceCarPos =
      (float)((double)(averagesectionlen * Car[iCarIdx].nCurrChunk) +
              Car[iCarIdx].pos.fX); // Calculate absolute track position of
                                    // reference car
  if (numcars > 0) // Check if there are cars to iterate through
  {
    iCurrentCarIdx = 0;
    do { // Skip self, dead cars, non-AI cars, and stunned cars
      if (iCarCounter != iCarIdx && (char)Car[iCurrentCarIdx].byLives > 0 &&
          Car[iCurrentCarIdx].iControlType == 3 &&
          !Car[iCurrentCarIdx].iStunned) {
        dCurrentCarPos =
            (double)(averagesectionlen * Car[iCurrentCarIdx].nCurrChunk) +
            Car[iCurrentCarIdx]
                .pos.fX; // Calculate absolute track position of current car
        fAdjustedCarPos = (float)dCurrentCarPos;
        if (dCurrentCarPos < fReferenceCarPos)
          fAdjustedCarPos =
              (float)((double)totaltrackdistance +
                      fAdjustedCarPos); // Handle track wrapping - add total
                                        // track distance if car is behind
        dDistance =
            fAdjustedCarPos - fReferenceCarPos; // Calculate distance from
                                                // reference car to current car
        if (dDistance < fMaxDistance) // If this car is closer than current
                                      // closest, update closest car
        {
  ...
  return iClosestCarIdx; // Return index of closest car ahead, or -1 if none
                         // found
}
